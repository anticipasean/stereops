...
import com.oath.cyclops.internal.stream.spliterators.push.CollectAllOperator;
import com.oath.cyclops.internal.stream.spliterators.push.CombineOperator;
import com.oath.cyclops.internal.stream.spliterators.push.CompleteOperator;
...
import com.oath.cyclops.internal.stream.spliterators.push.FlatMapPublisher;
import com.oath.cyclops.internal.stream.spliterators.push.GroupedByTimeAndSizeOperator;
import com.oath.cyclops.internal.stream.spliterators.push.GroupedByTimeOperator;
import com.oath.cyclops.internal.stream.spliterators.push.GroupedStatefullyOperator;
import com.oath.cyclops.internal.stream.spliterators.push.GroupedWhileOperator;
import com.oath.cyclops.internal.stream.spliterators.push.GroupingOperator;
import com.oath.cyclops.internal.stream.spliterators.push.IterableSourceOperator;
import com.oath.cyclops.internal.stream.spliterators.push.LazyFilterOperator;
...
import com.oath.cyclops.internal.stream.spliterators.push.LimitLastOneOperator;
import com.oath.cyclops.internal.stream.spliterators.push.LimitLastOperator;
import com.oath.cyclops.internal.stream.spliterators.push.LimitOperator;
import com.oath.cyclops.internal.stream.spliterators.push.LimitWhileClosedOperator;
import com.oath.cyclops.internal.stream.spliterators.push.LimitWhileOperator;
import com.oath.cyclops.internal.stream.spliterators.push.LimitWhileTimeOperator;
import com.oath.cyclops.internal.stream.spliterators.push.MapOperator;
import com.oath.cyclops.internal.stream.spliterators.push.OnEmptyErrorOperator;
import com.oath.cyclops.internal.stream.spliterators.push.OnEmptyOperator;
import com.oath.cyclops.internal.stream.spliterators.push.OnErrorBreakWithPublisherOperator;
...
import com.oath.cyclops.internal.stream.spliterators.push.RecoverOperator;
import com.oath.cyclops.internal.stream.spliterators.push.ReduceAllOperator;
import com.oath.cyclops.internal.stream.spliterators.push.SingleValueOperator;
import com.oath.cyclops.internal.stream.spliterators.push.SkipLastOneOperator;
import com.oath.cyclops.internal.stream.spliterators.push.SkipLastOperator;
import com.oath.cyclops.internal.stream.spliterators.push.SkipOperator;
import com.oath.cyclops.internal.stream.spliterators.push.SkipWhileClosedOperator;
import com.oath.cyclops.internal.stream.spliterators.push.SkipWhileOperator;
import com.oath.cyclops.internal.stream.spliterators.push.SkipWhileTimeOperator;
...
import com.oath.cyclops.internal.stream.spliterators.push.StreamSubscription;
import com.oath.cyclops.internal.stream.spliterators.push.ZippingLatestOperator;
import com.oath.cyclops.internal.stream.spliterators.push.ZippingOperator;
...
public class ReactiveStreamX<T> extends BaseExtendedStream<T> {
    ...
    public ReactiveSeq<T> reverse() {
        return coflatMap(s -> ReactiveSeq.reversedListOf(s.toList())).flatMap(i -> i);
    }

    @Override
    public ReactiveSeq<T> combine(BiPredicate<? super T, ? super T> predicate,
                                  BinaryOperator<T> op) {
        return createSeq(new CombineOperator<>(source,
                                               predicate,
                                               op)).flatMap(i -> i);
    }

    @Override
    public <R> ReactiveSeq<R> reduceAll(R identity,
                                        BiFunction<R, ? super T, R> accumulator) {
        return createSeq(new ReduceAllOperator<>(source,
                                                 identity,
                                                 accumulator));
    }
    ...
    public <R, A> ReactiveSeq<R> collectAll(Collector<? super T, A, R> collector) {
        return createSeq(new CollectAllOperator<T, A, R>(source,
                                                         collector));
    }
    ...
    public Iterator<T> iterator() {
        ...
        return new OperatorToIterable<>(source,
    ...
    public <R> ReactiveSeq<R> coflatMap(Function<? super ReactiveSeq<T>, ? extends R> fn) {
        return createSeq(new LazySingleValueOperator<ReactiveSeq<T>, R>(createSeq(source),
                                                                        fn));
    ...
    @Override
    public final ReactiveSeq<Seq<T>> sliding(final int windowSize,
                                             final int increment) {
        return createSeq(new SlidingOperator<>(source,
                                               Function.identity(),
                                               windowSize,
                                               increment));
    }
    ...
    public ReactiveSeq<Vector<T>> grouped(final int groupSize) {
        return createSeq(new GroupingOperator<T, Vector<T>, Vector<T>>(source,
                                                                       () -> Vector.empty(),
                                                                       c -> c,
                                                                       groupSize));
    ...
    public ReactiveSeq<Vector<T>> groupedWhile(final BiPredicate<Vector<? super T>, ? super T> predicate) {
        return createSeq(new GroupedStatefullyOperator<>(source,
                                                         () -> Vector.empty(),
                                                         Function.identity(),
                                                         predicate.negate()));
    }
    ...
    public final <R> ReactiveSeq<R> map(final Function<? super T, ? extends R> fn) {
        ...
        return createSeq(new MapOperator<T, R>(this.source,
                                               fn));
    }
    ...
    public final <R> ReactiveSeq<R> flatMap(final Function<? super T, ? extends Stream<? extends R>> fn) {
        ...
        return createSeq(new FlatMapOperator<>(source,
                                               fn));
    ...
    public final <R> ReactiveSeq<R> concatMap(final Function<? super T, ? extends Iterable<? extends R>> fn) {
        ...
        return createSeq(new FlatMapOperator<>(source,
                                               i -> ReactiveSeq.fromIterable(fn.apply(i))));
    ...
    @Override
    public final <R> ReactiveSeq<R> mergeMap(int maxConcurency,
                                             final Function<? super T, ? extends Publisher<? extends R>> fn) {
        FlatMapPublisher<T, R> pub = new FlatMapPublisher<>(source,
                                                            fn,
                                                            maxConcurency);
    ...
    public final ReactiveSeq<T> filter(final Predicate<? super T> fn) {
        return createSeq(new FilterOperator<T>(source,
                                               fn));
    ...
    public Stream<T> unwrapStream() {
        ...
        return StreamSupport.stream(new OperatorToIterable<>(source,
                                                             this.defaultErrorHandler,
    ...
    public <R> ReactiveSeq<R> mapLazyFn(Supplier<Function<? super T, ? extends R>> fn) {
        return createSeq(new LazyMapOperator<>(source,
                                               fn));
    }

    public final ReactiveSeq<T> filterLazyPredicate(final Supplier<Predicate<? super T>> fn) {
        return createSeq(new LazyFilterOperator<T>(source,
                                                   fn));
    ...
    public ReactiveSeq<T> changes() {
        return Spouts.defer(() -> {
            if (async == Type.NO_BACKPRESSURE) {
                Queue<T> discrete = QueueFactories.<T>unboundedNonBlockingQueue().build()
                                                                                 .withTimeout(1);

                Signal<T> signal = new Signal<T>(null,
                                                 discrete);
                ...
                return signal.getDiscrete()
                             .stream();
            } else {
                Queue<T> queue = QueueFactories.<T>unboundedNonBlockingQueue().build();
                Signal<T> signal = new Signal<T>(null,
                                                 queue);
                Subscription sub = source.subscribe(signal::set,
                                                    i -> {
                                                        signal.close();
    ...
    @Override
    public <U, R> ReactiveSeq<R> zipLatest(final Publisher<? extends U> other,
        ...
        return createSeq(new ZippingLatestOperator<>(source,
                                                     right,
                                                     zipper),
                         BACKPRESSURE);
    }
    ...
    public void subscribe(final Subscriber<? super T> sub) {
        ...
        sub.onSubscribe(source.subscribe(sub::onNext,
                                         sub::onError,
                                         sub::onComplete));
    }
    ...
    public <X extends Throwable> ReactiveSeq<T> onEmptyError(final Supplier<? extends X> supplier) {
        return createSeq(new OnEmptyErrorOperator<T, X>(source,
                                                        supplier));
    }
    ...
    public ReactiveSeq<T> prependStream(final Stream<? extends T> other) {
        return Spouts.concat(other,
                             this);
    }

    public ReactiveSeq<T> prependAll(final Iterable<? extends T> other) {
        return Spouts.concat((Spouts.fromIterable(other)),
                             this);
    }

    @Override
    public ReactiveSeq<T> prepend(final T other) {
        return Spouts.concat(Spouts.of(other),
                             this);
    }
    ...
    @Override
    public ReactiveSeq<Vector<T>> groupedBySizeAndTime(final int size,
                                                       final long time,
                                                       final TimeUnit t) {
    ...
    @Override
    public ReactiveSeq<Vector<T>> groupedByTime(final long time,
                                                final TimeUnit t) {
    ...
    @Override
    public ReactiveSeq<T> drop(final long time,
                               final TimeUnit unit) {
        return createSeq(new SkipWhileTimeOperator<>(source,
                                                     time,
                                                     unit));
    }

    @Override
    public ReactiveSeq<T> take(final long time,
                               final TimeUnit unit) {
        return createSeq(new LimitWhileTimeOperator<>(source,
                                                      time,
                                                      unit));
    ...
    public ReactiveSeq<Vector<T>> groupedWhile(final Predicate<? super T> predicate) {
        return createSeq(new GroupedWhileOperator<>(source,
                                                    () -> Vector.empty(),
                                                    Function.identity(),
                                                    predicate));
    ...
    @Override
    public <C extends PersistentCollection<? super T>> ReactiveSeq<C> groupedWhile(final Predicate<? super T> predicate,
                                                                                   final Supplier<C> factory) {
        return createSeq(new GroupedWhileOperator<>(source,
                                                    factory,
                                                    Function.identity(),
    ...
    @Override
    public <C extends PersistentCollection<? super T>> ReactiveSeq<C> groupedBySizeAndTime(final int size,
                                                                                           final long time,
                                                                                           final TimeUnit unit,
                                                                                           final Supplier<C> factory) {
        return createSeq(new GroupedByTimeAndSizeOperator(this.source,
    ...
    @Override
    public <C extends PersistentCollection<? super T>, R> ReactiveSeq<R> groupedBySizeAndTime(final int size,
                                                                                              final long time,
                                                                                              final TimeUnit unit,
                                                                                              final Supplier<C> factory,
                                                                                              Function<? super C, ? extends R> finalizer) {
    ...
    @Override
    public <C extends PersistentCollection<? super T>, R> ReactiveSeq<R> groupedByTime(final long time,
                                                                                       final TimeUnit unit,
                                                                                       final Supplier<C> factory,
                                                                                       Function<? super C, ? extends R> finalizer) {
    ...
    @Override
    public <C extends PersistentCollection<? super T>> ReactiveSeq<C> groupedByTime(final long time,
                                                                                    final TimeUnit unit,
                                                                                    final Supplier<C> factory) {
        return createSeq(new GroupedByTimeOperator(this.source,
                                                   factory,
    ...
    @Override
    public <C extends PersistentCollection<? super T>> ReactiveSeq<C> grouped(final int size,
                                                                              final Supplier<C> factory) {
        return createSeq(new GroupingOperator<>(source,
                                                factory,
                                                Function.identity(),
                                                size));
    ...
    public ReactiveSeq<T> dropRight(final int num) {
        ...
        return createSeq(new SkipLastOperator<>(source,
                                                num < 0 ? 0 : num));
    }
    ...
    public <X extends Throwable> Subscription forEachSubscribe(final Consumer<? super T> consumer) {
        StreamSubscription sub = source.subscribe(consumer,
                                                  this.defaultErrorHandler,
                                                  () -> {
                                                  });
        return sub;
    ...
    @Override
    public <X extends Throwable> Subscription forEach(final long numberOfElements,
    ...
    @Override
    public <X extends Throwable> void forEach(final Consumer<? super T> consumerElement,
                                              final Consumer<? super Throwable> consumerError) {
        if (async != Type.NO_BACKPRESSURE) {
            this.source.subscribe(consumerElement,
                                  consumerError,
                                  () -> {
                                  })
                       .request(Long.MAX_VALUE);
        } else {
            source.subscribeAll(consumerElement,
                                consumerError,
                                () -> {
                                });
        }
    ...
    @Override
    public ReactiveSeq<T> mergeP(final QueueFactory<T> factory,
                                 final Publisher<T>... publishers) {
        Publisher<T>[] pubs = new Publisher[publishers.length + 1];
        pubs[0] = this;
        System.arraycopy(publishers,
                         0,
                         pubs,
                         1,
                         publishers.length);
    ...
    public Topic<T> broadcast() {
        if (async == Type.NO_BACKPRESSURE) {
            Queue<T> queue = QueueFactories.<T>boundedNonBlockingQueue(1000).build()
                                                                            .withTimeout(1);
            ...
            Continuation cont = new Continuation(() -> {
                ...
                if (wip.compareAndSet(false,
                                      true)) {
                    try {
                        source.subscribeAll(topic::offer,
                                            e -> topic.close(),
                                            () -> topic.close());
                    } finally {
                        wip.set(false);
    ...
    @Override
    public <U, R> ReactiveSeq<R> zipWithStream(Stream<? extends U> other,
        ...
        return createSeq(new ZippingOperator<>(source,
                                               right,
                                               zipper),
                         async);
    }
    ...
    @Override
    public final <S, U, R> ReactiveSeq<R> zip3(final Iterable<? extends S> second,
                                               final Iterable<? extends U> third,
                                               final Function3<? super T, ? super S, ? super U, ? extends R> fn3) {
        return this.zip(second)
                   .zip(third,
                        (a, b) -> fn3.apply(a._1(),
                                            a._2(),
                                            b));
    ...
    @Override
    public final <T2, T3, T4, R> ReactiveSeq<R> zip4(final Iterable<? extends T2> second,
    ...
    public ReactiveSeq<T> limit(long num) {
        return createSeq(new LimitOperator<>(source,
                                             num));
    }

    @Override
    public ReactiveSeq<T> skip(long num) {
        return createSeq(new SkipOperator<>(source,
                                            num));
    }
    ...
    public ReactiveSeq<T> cycle() {
        ...
        ReactiveSeq<T> cycling = collectAll(Collectors.toList()).map(s -> Spouts.fromIterable(s)
    ...
    public Tuple2<ReactiveSeq<T>, ReactiveSeq<T>> duplicate() {
        ...
        Seq<Iterable<T>> copy = Streams.toBufferingCopier(() -> iterator(),
                                                          2);
        return Tuple.tuple(createSeq(new IterableSourceOperator<>(copy.getOrElseGet(0,
                                                                                    () -> Arrays.asList())),
                                     SYNC),
                           createSeq(new IterableSourceOperator<>(copy.getOrElseGet(1,
                                                                                    () -> Arrays.asList())),
                                     SYNC));
    ...
    public Tuple3<ReactiveSeq<T>, ReactiveSeq<T>, ReactiveSeq<T>> triplicate() {
        ...
        return Tuple.tuple(createSeq(new IterableSourceOperator<>(copy.getOrElseGet(0,
                           ...
                           createSeq(new IterableSourceOperator<>(copy.getOrElseGet(1,
                                                                                    () -> Arrays.asList())),
                                     SYNC),
                           createSeq(new IterableSourceOperator<>(copy.getOrElseGet(2,
                                                                                    () -> Arrays.asList())),
                                     SYNC));
    ...
    public Seq<ReactiveSeq<T>> multicast(int num) {
                ...
                ReactiveSeq<T> seq = Spouts.<T>async(s1 -> {
                    ...
                    if (subs.size() == num) {
                        this.forEach(e -> subs.forEach(s -> s.onNext(e)),
                                     ex -> subs.forEach(s -> s.onError(ex)),
                                     () -> subs.forEach(s -> s.onComplete()));
                    }
                });
        ...
        if (this.async == BACKPRESSURE) {
            ConcurrentLinkedQueue<Subscriber> subs = new ConcurrentLinkedQueue<>();
            Seq<ReactiveSeq<T>> result = Seq.empty();
            Subscription sub = forEachSubscribe(e -> subs.forEach(s -> s.onNext(e)),
                                                ex -> subs.forEach(s -> s.onError(ex)),
                                                () -> subs.forEach(s -> s.onComplete()));
            for (int i = 0; i < num; i++) {
    ...
    public Tuple4<ReactiveSeq<T>, ReactiveSeq<T>, ReactiveSeq<T>, ReactiveSeq<T>> quadruplicate() {
        ...
        return Tuple.tuple(createSeq(new IterableSourceOperator<>(copy.getOrElseGet(0,
                           ...
                           createSeq(new IterableSourceOperator<>(copy.getOrElseGet(2,
                                                                                    () -> Arrays.asList())),
                                     SYNC),
                           createSeq(new IterableSourceOperator<>(copy.getOrElseGet(3,
                                                                                    () -> Arrays.asList())),
                                     SYNC));
    ...
    public <U> ReactiveSeq<Tuple2<T, U>> zipWithStream(Stream<? extends U> other) {
        ...
        ReactiveStreamX<Tuple2<T, U>> res = createSeq(new ZippingOperator<>(source,
                                                                            right,
                                                                            Tuple::tuple));
        if (this.async == SYNC) {
    ...
    @Override
    public <S, U> ReactiveSeq<Tuple3<T, S, U>> zip3(Iterable<? extends S> second,
    ...
    @Override
    public <T2, T3, T4> ReactiveSeq<Tuple4<T, T2, T3, T4>> zip4(Iterable<? extends T2> second,
    ...
    public ReactiveSeq<T> cycle(long times) {
        return grouped(Integer.MAX_VALUE,
                       () -> Vector.empty()).map(ReactiveSeq::fromIterable)
    ...
    public T reduce(T identity,
        ...
        return findFirstCallAll((ReactiveStreamX<T>) reduceAll(identity,
                                                               accumulator)).orElse(identity);
    }

    @Override
    public final <R, A> R collect(final Collector<? super T, A, R> collector) {

        return findFirstCallAll((ReactiveStreamX<R>) collectAll(collector)).orElseGet(() -> collector.finisher()
                                                                                                     .apply(collector.supplier()
                                                                                                                     .get()));
    ...
    public final ReactiveSeq<Tuple2<T, Integer>> occurances() {
        ...
        return Spouts.defer(() -> {
            ReactiveSeq<Map<T, Integer>> map = collectAll(Collectors.toMap(k -> k,
                                                                           v -> 1,
                                                                           (a, b) -> a + b));
            return map.flatMap(m -> m.entrySet()
                                     .stream());
        })
    ...
    public <R> R collect(final Supplier<R> supplier,
        ...
        forEach(e -> accumulator.accept(container,
                                        e),
                this.defaultErrorHandler,
    ...
    public Optional<T> reduce(BinaryOperator<T> accumulator) {
        ...
        Object[] current = {null};
        forEach(e -> current[0] = current[0] != null ? accumulator.apply((T) current[0],
                                                                         e) : e,
                this.defaultErrorHandler,
                () -> future.complete((T) current[0]));
    ...
    public ReactiveSeq<T> insertStreamAt(int pos,
                                         Stream<T> stream) {
        return insertAt(pos,
                        Spouts.fromSpliterator(stream.spliterator()));
    }
