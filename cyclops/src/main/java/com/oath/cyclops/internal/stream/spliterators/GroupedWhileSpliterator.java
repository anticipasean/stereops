 ...
 */
public class GroupedWhileSpliterator<T, C extends PersistentCollection<? super T>, R> extends
                                                                                      Spliterators.AbstractSpliterator<R> implements
                                                                                                                          CopyableSpliterator<R>,
                                                                                                                          ComposableFunction<R, T, GroupedWhileSpliterator<T, C, ?>> {
    ...
    public <R2> GroupedWhileSpliterator<T, C, ?> compose(Function<? super R, ? extends R2> fn) {
        return new GroupedWhileSpliterator<T, C, R2>(CopyableSpliterator.copy(source),
                                                     factory,
    ...
    public Spliterator<R> copy() {
        return new GroupedWhileSpliterator<T, C, R>(CopyableSpliterator.copy(source),
                                                    factory,
                                                    finalizer,
                                                    predicate);
    }
