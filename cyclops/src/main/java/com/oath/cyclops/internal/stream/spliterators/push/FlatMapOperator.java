    ...
    public StreamSubscription subscribe(Consumer<? super R> onNext,
        ...
        s[0] = source.subscribe(e -> {
                                    try {
                                        ...
                                        Spliterator<? extends R> split = mapper.apply(e)
                                                                               .spliterator();
                                        ...
                                        } while (!status.compareAndSet(statusLocal,
                                                                       statusLocal | (1 << 1))); //set inner active
                                                ...
                                                try {
                                                    while (res.isActive()) {
                                                        ...
                                                        if (canAdvance) {
                                                            ...
                                                            } while (!status.compareAndSet(thunkStatusLocal,
                                                                                           thunkStatusLocal & ~(1 << 1))); //unset inner active

                                                            if (status.compareAndSet(1,
                                                                                     100)) {
                                                                onComplete.run();
                                                                return true;
                                                            }
