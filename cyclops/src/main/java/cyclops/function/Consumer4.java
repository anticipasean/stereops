...
/**
 * A FunctionalInterface for side-effecting statements that accept 4 inputs (with no result). The four-arity specialization of
 * {@link Consumer}.
 *
...
public interface Consumer4<T1, T2, T3, T4> {
     ...
     * @return A curried function that eventually resolves to a Consumer
     */
    default Function<T3, Consumer<T4>> apply(final T1 s,
                                             final T2 s2) {
        return CurryConsumer.curryC4(this)
                            .apply(s)
                            .apply(s2);
     ...
     */
    default Consumer<T4> apply(final T1 s,
                               final T2 s2,
                               final T3 s3) {
        return CurryConsumer.curryC4(this)
                            .apply(s)
