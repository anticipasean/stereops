...
public interface Effect extends Runnable {
    ...
    default Future<Void> future(Executor ex) {
        return Future.of(CompletableFuture.runAsync(this,
                                                    ex));
    }
    ...
    default <R> R fold(Supplier<R> success,
                       Function<Throwable, ? extends R> failure) {
        try {
            run();
        } catch (Throwable t) {
    ...
    default <R> Function0<R> supplyOr(R success,
                                      R failure) {
        return () -> {
            try {
                run();
    ...
    default Effect cycleAsync(long times,
                              Executor ex) {
        return () -> {
     ...
     */
    default Effect retry(final int retries,
                         final long delay,
    ...
    default <R1, R4> Effect forEach4(Supplier<Effect> value2,
        ...
        return this.flatMap(() -> value2.get()
                                        .flatMap(() -> value3.get()
                                                             .flatMap(() -> value4.get()
                                                                                  .andThen(yieldingFunction))));
