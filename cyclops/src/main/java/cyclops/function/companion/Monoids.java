...
public interface Monoids {
     ...
     */
    static <T, A extends Zippable<T>> Monoid<A> combineScalarFunctors(Function<T, A> zeroFn,
                                                                      Monoid<T> monoid) {

        return Monoid.of(zeroFn.apply(monoid.zero()),
                         Semigroups.combineScalarFunctors(monoid));
    }
     ...
     */
    static <T, A extends Zippable<T>> Monoid<A> combineZippables(Function<T, A> zeroFn,
                                                                 Monoid<T> monoid) {

        return Monoid.of(zeroFn.apply(monoid.zero()),
                         Semigroups.combineZippables(monoid));
    }
    ...
    static <T> Monoid<ReactiveSeq<T>> combineReactiveSeq() {
        return Monoid.of(ReactiveSeq.empty(),
                         Semigroups.combineReactiveSeq());
    }

    static <T> Monoid<ReactiveSeq<T>> firstNonEmptyReactiveSeq() {
        return Monoid.of(ReactiveSeq.empty(),
                         Semigroups.firstNonEmptyReactiveSeq());
    }

    static <T> Monoid<ReactiveSeq<T>> mergeLatestReactiveSeq() {
        return Monoid.of(Spouts.empty(),
                         Semigroups.mergeLatestReactiveSeq());
    }

    static <T> Monoid<Publisher<T>> mergeLatest() {
        return Monoid.of(Spouts.empty(),
                         Semigroups.mergeLatest());
    }
    ...
    static <T> Monoid<ReactiveSeq<T>> ambReactiveSeq() {
        return Monoid.of(Spouts.empty(),
                         Semigroups.ambReactiveSeq());
    }

    static <T> Monoid<ReactiveSeq<T>> zipReactiveSeq(Monoid<T> m) {
        return Monoid.of(Spouts.of(m.zero()),
                         Semigroups.zipReactiveSeq(m));
    }
    ...
    static <T> Monoid<Stream<T>> combineStream() {
        return Monoid.of(Stream.empty(),
                         Semigroups.combineStream());
    }
    ...
    static <T> Monoid<CompletableFuture<T>> firstCompleteCompletableFuture() {
        return Monoid.of(new CompletableFuture<T>(),
                         Semigroups.firstCompleteCompletableFuture());
    }
    ...
    static <T> Monoid<Future<T>> firstCompleteFuture() {
        return Monoid.of(Future.future(),
                         Semigroups.firstCompleteFuture());
    }

    static <T> Monoid<Future<T>> zipFutures(Monoid<T> m) {
        return Monoid.of(Future.ofResult(m.zero()),
                         Semigroups.zippedFutures(m));
    }
    ...
    static <T> Monoid<Future<T>> firstSuccessfulFuture() {
        return Monoid.of(Future.future(),
                         Semigroups.firstSuccessfulFuture());
    }
    ...
    static <ST, PT> Monoid<Either<ST, PT>> firstRightEither(ST zero) {
        return Monoid.of(Either.left(zero),
                         Semigroups.firstRightEither());
    }
    ...
    static <ST, PT> Monoid<Either<ST, PT>> firstLeftEither(PT zero) {
        return Monoid.of(Either.right(zero),
                         Semigroups.firstLeftEither());
    }
    ...
    static <T, X extends Throwable> Monoid<Try<T, X>> firstTrySuccess(X zero) {
        return Monoid.of(Try.failure(zero),
                         Semigroups.firstTrySuccess());
    }
    ...
    static <T, X extends Throwable> Monoid<Try<T, X>> firstTryFailure(T zero) {
        return Monoid.of(Try.success(zero),
                         Semigroups.firstTryFailure());
    }
    ...
    static <ST, PT> Monoid<Ior<ST, PT>> firstPrimaryIor(ST zero) {
        return Monoid.of(Ior.left(zero),
                         Semigroups.firstPrimaryIor());
    }
    ...
    static <ST, PT> Monoid<Ior<ST, PT>> firstSecondaryIor(PT zero) {
        return Monoid.of(Ior.right(zero),
                         Semigroups.firstSecondaryIor());
    }
    ...
    static <T> Monoid<Maybe<T>> firstPresentMaybe() {
        return Monoid.of(Maybe.nothing(),
                         Semigroups.firstPresentMaybe());
    }
    ...
    static <T> Monoid<Optional<T>> firstPresentOptional() {
        return Monoid.of(Optional.empty(),
                         Semigroups.firstPresentOptional());
    }
    ...
    static <T> Monoid<Optional<T>> lastPresentOptional() {
        return Monoid.of(Optional.empty(),
                         Semigroups.lastPresentOptional());
    }
    ...
    static <T, T2 extends Comparable<T>> Monoid<T2> minComparable(T2 max) {
        return Monoid.of(max,
                         Semigroups.minComparable());
    }
    ...
    static <T, T2 extends Comparable<T>> Monoid<T2> maxComparable(T2 min) {
        return Monoid.of(min,
                         Semigroups.maxComparable());
    }

    static <A> Monoid<NaturalTransformation<A, A>> naturalTransformationComposition() {
        return Monoid.of(NaturalTransformation.identity(),
                         Semigroups.naturalTransformationComposition());
    }
    ...
    static <A> Monoid<Function<A, A>> functionComposition() {
        return Monoid.of(Function.identity(),
                         Semigroups.functionComposition());
    }
