...
/**
 * A FunctionalInterface for side-effecting statements that accept 5 inputs (with no result). The five-arity specialization of
 * {@link Consumer}.
 *
...
public interface Consumer5<T1, T2, T3, T4, T5> {
     ...
     * @return A curried function that eventually resolves to a Consumer
     */
    default Function<T3, Function<T4, Consumer<T5>>> apply(final T1 s,
                                                           final T2 s2) {
        return CurryConsumer.curryC5(this)
                            .apply(s)
                            .apply(s2);
     ...
     * @return A curried function that eventually resolves to a Consumer
     */
    default Function<T4, Consumer<T5>> apply(final T1 s,
                                             final T2 s2,
                                             final T3 s3) {
        return CurryConsumer.curryC5(this)
                            .apply(s)
     ...
     */
    default Consumer<T5> apply(final T1 s,
                               final T2 s2,
                               final T3 s3,
                               final T4 s4) {
        return CurryConsumer.curryC5(this)
