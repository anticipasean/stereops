 ...
 * <p>
 * Practically the method signature to reduce matches the Monoid interface Monoids could regrarded as immutable equivalents to JDK
...
public interface Monoid<T> extends Semigroup<T> {
    ...
    /**
     * An element that when provided as a parameter to the combiner with another value, results in the other value being returned
     * e.g.
     * <pre>
     *  0  + 1  = 1
     *
    ...
    default <R> R fold(BiFunction<? super Semigroup<T>, ? super T, ? extends R> visitFn) {
        return visitFn.apply(this,
                             zero());
    }
    ...
    default T fold(final T toFold) {
        return foldLeft(Arrays.asList(zero(),
                                      toFold));
    }
    ...
    default ReactiveSeq<T> foldRightAsync(final Publisher<T> toReduce) {
        return Spouts.from(toReduce)
                     .reduceAll(zero(),
                                this);
    }

    default <A> T foldMap(final Publisher<A> toFoldMap,
                          Function<? super A, ? extends T> mapFn) {
        ReactiveSeq<T> toReduce = Spouts.from(toFoldMap)
                                        .map(mapFn);
        return toReduce.reduce(zero(),
                               this);
    }

    default <A> T foldMap(final Stream<A> toFoldMap,
                          Function<? super A, ? extends T> mapFn) {
    ...
    default <A> T foldMap(final Iterable<A> toFoldMap,
                          Function<? super A, ? extends T> mapFn) {
        ReactiveSeq<T> toReduce = ReactiveSeq.fromIterable(toFoldMap)
                                             .map(mapFn);
        return toReduce.reduce(zero(),
                               this);
    }

    default <A> T foldMap(final ReactiveSeq<A> toFoldMap,
                          Function<? super A, ? extends T> mapFn) {
