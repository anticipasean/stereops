...
import cyclops.function.hkt.DataWitness.managed;
...
public abstract class Managed<T> implements Higher<managed, T>, To<Managed<T>>, Publisher<T> {
    ...
    public static <T> Managed<T> managed(T acq,
                                         Consumer<T> cleanup) {
        return of(IO.of(() -> acq),
                  cleanup);
    }
    ...
    public static <T> Managed<T> of(Publisher<T> acq,
                                    Consumer<T> cleanup) {
        return of(IO.fromPublisher(acq),
                  cleanup);
    }

    public static <T> Managed<T> of(Supplier<? extends T> acq,
                                    Consumer<T> cleanup) {
        return of(IO.of(acq),
                  cleanup);
    }
        ...
        return new Managed<T>() {
            public <R> IO<R> apply(Function<? super T, ? extends IO<R>> fn) {
                IO<R> y = IO.Comprehensions.forEach(acquire,
                                                    t1 -> {
                                                        IO<? extends Try<? extends IO<R>, Throwable>> res1 = IO.ReactiveSeqIO.withCatch(() -> fn.apply(t1),
                                                                                                                                        Throwable.class);
                                                        return res1;
                                                    },
                                                    t2 -> {
                                                        ...
                                                        IO<R> res = tr.fold(r -> r,
                                                                            e -> IO.fromPublisher(Future.ofError(e)));
    ...
    public static <T, R> Managed<Seq<R>> traverse(Iterable<T> stream,
                                                  Function<? super T, Managed<? extends R>> fn) {
        Seq<Managed<R>> s = Seq.fromIterable(stream)
                               .map(fn.andThen(Managed::narrow));
        return sequence(s);
    }

    public static <T, R> Managed<Seq<R>> traverse(Function<? super T, ? extends R> fn,
                                                  Iterable<Managed<T>> stream) {
        Seq<Managed<R>> s = Seq.fromIterable(stream)
                               .map(j -> j.map(fn));
        return sequence(s);
    }
    ...
    public static <T> Semigroup<Managed<T>> semigroup(Semigroup<T> s) {
        return (a, b) -> a.flatMap(t1 -> b.map(t2 -> s.apply(t1,
                                                             t2)));
    }

    public static <T> Monoid<Managed<T>> monoid(Monoid<T> s) {
        return Monoid.of(managed(s.zero(),
                                 __ -> {
                                 }),
                         semigroup(s));
    }
    ...
    public <R2> Tupled<T, R2> with(Function<? super T, ? extends R2> fn) {
        return new Tupled<>(this,
                            i -> Tuple.tuple(i,
                                             fn.apply(i)));
    }

    public final <T2, R> Managed<R> zip(Managed<T2> b,
                                        BiFunction<? super T, ? super T2, ? extends R> fn) {
        return flatMap(t1 -> b.map(t2 -> fn.apply(t1,
                                                  t2)));
    }

    public abstract <R> IO<R> apply(Function<? super T, ? extends IO<R>> fn);

    public final void forEach(Consumer<? super T> onNext,
                              Consumer<Throwable> errorHandler) {
        stream().forEach(onNext,
                         errorHandler);
    ...
    public static class Tupled<T1, T2> {
        ...
        public <R> Tupled3<T1, T2, R> with(BiFunction<? super T1, ? super T2, ? extends R> fn) {
            return new Tupled3<T1, T2, R>(managed,
                                          t1 -> {
                                              Tuple2<T1, T2> t2 = fn2.apply(t1);
                                              return Tuple.tuple(t2._1(),
                                                                 t2._2(),
                                                                 fn.apply(t2._1(),
                                                                          t2._2()));
                                          });
        ...
        public static <T, F, R1, R2, R3, R4, R5, R6, R7> Managed<R7> forEach(Managed<T> io,
            ...
            return io.flatMap(in -> {
                ...
                return a.flatMap(ina -> {
                    Managed<R2> b = value3.apply(Tuple.tuple(in,
                                                             ina));
                    return b.flatMap(inb -> {

                        Managed<R3> c = value4.apply(Tuple.tuple(in,
                                                                 ina,
                                                                 inb));

                        return c.flatMap(inc -> {
                            Managed<R4> d = value5.apply(Tuple.tuple(in,
                                                                     ina,
                                                                     inb,
                                                                     inc));
                            return d.flatMap(ind -> {
                                Managed<R5> e = value6.apply(Tuple.tuple(in,
                                                                         ina,
                                                                         inb,
                                                                         inc,
                                                                         ind));
                                return e.flatMap(ine -> {
                                    Managed<R6> f = value7.apply(Tuple.tuple(in,
                                                                             ina,
                                                                             inb,
                                                                             inc,
                                                                             ind,
                                    ...
                                    return f.flatMap(inf -> {
                                        Managed<R7> g = value8.apply(Tuple.tuple(in,
                                                                                 ina,
                                                                                 inb,
                                                                                 inc,
                                                                                 ind,
