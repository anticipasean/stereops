...
import cyclops.stream.spliterator.push.BufferingSinkOperator;
import cyclops.stream.spliterator.push.GenerateOperator;
import cyclops.stream.spliterator.push.IterableSourceOperator;
import cyclops.stream.spliterator.push.IterateOperator;
import cyclops.stream.spliterator.push.IteratePredicateOperator;
import cyclops.stream.spliterator.push.LazyArrayConcatonatingOperator;
import cyclops.stream.spliterator.push.MergeLatestOperator;
import cyclops.stream.spliterator.push.Operator;
import cyclops.stream.spliterator.push.PublisherToOperator;
import cyclops.stream.spliterator.push.RangeIntOperator;
...
/**
 * reactive : is used to denote creational methods for reactive-streams that support non-blocking backpressure async : is used to
 * denote creational methods for asynchronous streams that do not support backpressure
 */
...
public interface Spouts {
    ...
    /**
     * Create an Subscriber for Observable style asynchronous push based Streams. Streams generated from AsyncSubscribers are not
     * backpressure aware (in cases where backpressue is not needed they may perform better). For backpressure aware Streams see
     * {@link Spouts#reactiveSubscriber}
     *
    ...
    /**
     * Create a Stream that accepts data via the Subsriber passed into the supplied Consumer. reactive-streams susbscription is
     * ignored (i.e. this Stream is backpressure free)
     *
     * <pre>
     *     {@code
    ...
    /**
     * Create a push based Stream with <b>no backpressure</b> fromm the provided Stream. The provided Stream will be executed on
     * the provided executor and pushed to the returned Stream
     *
     ...
     */
    @Deprecated
    static <T> ReactiveSeq<T> async(Stream<T> seq,
                                    Executor exec) {
        return async(s -> {

            ReactiveSeq.fromStream(seq)
                       .foldFuture(exec,
                                   t -> {

                                       PushSubscriber<T> local = s;
                                       t.forEach(local::onNext,
                                                 local::onError,
                                                 local::onComplete);
                                       return null;
                                   });
    ...
    /**
     * Create a buffering reactive-streams source. Your subscriber can respect or ignore reactive-streams backpressure. This
     * operator will buffer incoming data before sending on when downstream streams are ready. This operator drops values once
     ...
     * @return
     */
    static <T> ReactiveSeq<T> reactiveBuffer(int buffer,
                                             Consumer<? super Subscriber<T>> onNext) {
        return Spouts.reactiveStream(new BufferingSinkOperator<T>(new ManyToManyConcurrentArrayQueue<T>(buffer),
                                                                  onNext,
                                                                  BufferOverflowPolicy.DROP));
    }

    static <T> ReactiveSeq<T> reactiveBufferBlock(int buffer,
                                                  Consumer<? super Subscriber<T>> onNext) {
        return Spouts.reactiveStream(new BufferingSinkOperator<T>(new ManyToManyConcurrentArrayQueue<T>(buffer),
                                                                  onNext,
    ...
    static <T> ReactiveSeq<T> reactiveBuffer(Queue<T> buffer,
                                             BufferOverflowPolicy policy,
                                             Consumer<? super Subscriber<T>> onNext) {
        return Spouts.reactiveStream(new BufferingSinkOperator<T>(buffer,
                                                                  onNext,
                                                                  policy));
    }

    @Deprecated
    static <T> ReactiveSeq<T> asyncBuffer(int buffer,
                                          Consumer<? super PushSubscriber<T>> onNext) {
        return Spouts.asyncStream(new BufferingSinkOperator<T>(new ManyToManyConcurrentArrayQueue<T>(buffer),
                                                               c -> onNext.accept(PushSubscriber.of(c)),
                                                               BufferOverflowPolicy.DROP));
    }

    @Deprecated
    static <T> ReactiveSeq<T> asyncBufferBlock(int buffer,
                                               Consumer<? super PushSubscriber<T>> onNext) {
        return Spouts.asyncStream(new BufferingSinkOperator<T>(new ManyToManyConcurrentArrayQueue<T>(buffer),
                                                               c -> onNext.accept(PushSubscriber.of(c)),
                                                               BufferOverflowPolicy.BLOCK));
    ...
    @Deprecated
    static <T> ReactiveSeq<T> asyncBuffer(Queue<T> buffer,
                                          BufferOverflowPolicy policy,
                                          Consumer<? super PushSubscriber<T>> onNext) {
        return Spouts.asyncStream(new BufferingSinkOperator<T>(buffer,
                                                               c -> onNext.accept(PushSubscriber.of(c)),
                                                               policy));
    }

    static <T> ReactiveSeq<T> reactive(Stream<T> seq,
                                       Executor exec) {
        ...
        ReactiveSeq.fromStream(seq)
                   .foldFuture(exec,
                               t -> {
                                   Subscriber<T> local = subscriber.getFuture()
                                                                   .join();
                                   Subscription streamSub = t.forEach(0,
                                                                      local::onNext,
                                                                      local::onError,
                                                                      () -> {
                                                                          complete.set(true);
                                   ...
                                   while (!complete.get()) {
                                       ...
                                       if (next == 0) {
                                           ...
                                           while (!requested.compareAndSet(next,
                                                                           0)) {
                                               next = requested.get();
                                           }
    ...
    /**
     * The recommended way to connect a Spout to a Publisher is via Spouts#from Create an Subscriber for Observable style
     * asynchronous push based Streams, that implements backpressure internally via the reactive-streams spec.
     * <p>
     * Subscribers signal demand via their subscription and publishers push data to subscribers synchronously or asynchronously,
     * never exceeding signalled demand
     *
     * @param <T> Stream data type
    ...
    static <T> ReactiveSeq<T> iterate(final T seed,
        ...
        return syncStream(new IterateOperator<T>(seed,
                                                 f));
    }
    ...
    static <T> ReactiveSeq<T> iterate(final T seed,
                                      ...
                                      final UnaryOperator<T> f) {
        return syncStream(new IteratePredicateOperator<T>(seed,
                                                          f,
                                                          pred));
    ...
    public static ReactiveSeq<Integer> range(int start,
        ...
        if (start < end) {
            ...
            return syncStream(new RangeIntOperator(end,
                                                   start));
        }
    }
    ...
    public static ReactiveSeq<Long> rangeLong(long start,
        ...
        if (start < end) {
            ...
            return syncStream(new RangeLongOperator(end,
                                                    start));
        }
    }
    ...
    static <T> ReactiveSeq<T> mergeLatest(Publisher<? extends Publisher<T>> publisher) {
        return Spouts.from(publisher)
                     .mergeMap(Integer.MAX_VALUE,
                               i -> i);
    }

    static <T> ReactiveSeq<T> mergeLatest(int maxConcurrency,
                                          Publisher<T>... array) {
    ...
    static ReactiveSeq<Integer> interval(String cron,
    ...
    static ReactiveSeq<Integer> interval(final long millis,
                                         ScheduledExecutorService exec) {
        ReactiveSubscriber<Integer> sub = reactiveSubscriber();
        AtomicBoolean isOpen = new AtomicBoolean(true);
        Subscription[] s = {null};
    ...
    static <T> ReactiveSeq<T> schedule(final Stream<T> stream,
                                       final String cron,
    ...
    static <U, T> ReactiveSeq<T> unfold(final U seed,
        ...
        return reactiveStream(new SpliteratorToOperator<T>(new UnfoldSpliterator<>(seed,
                                                                                   unfolder)));
    }
