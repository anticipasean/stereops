 ...
 * <p>
 * A Queue that takes data from one or more input Streams and provides them to one or more emitted Streams
 * <p>
 * Interface specifies a BlockingQueue, but non-BlockingQueues (such as ConcurrentLinkedQueue can be used in conjunction with an
 * implementation of the Continuation interface
 *
...
public class Queue<T> implements Adapter<T> {
    ...
    Queue(final BlockingQueue<T> queue,
    ...
    public Queue(final BlockingQueue<T> queue) {
        this(queue,
             new DirectWaitStrategy<T>(),
             new DirectWaitStrategy<T>());
    }

    Queue(final BlockingQueue<T> queue,
          final Signal<Integer> sizeSignal) {
        this(queue,
             new DirectWaitStrategy<T>(),
             new DirectWaitStrategy<T>());
    }
    ...
    public ReactiveSeq<T> stream() {
        ...
        return ReactiveSeq.fromStream(closingStream(this::get,
                                                    new AlwaysContinue()));
    }
     ...
     * </pre>
     *
     * @param closeScalingFactor Scaling factor for Queue closed messages to propagate to connected parallel Streams. Scaling
     *                           Factor may need to be high to reach all connect parallel threads.
    ...
    public Stream<T> jdkStream(int closeScalingFactor) {
        ...
        } while (!listeningStreams.compareAndSet(listeningStreams.get(),
                                                 update));

        return closingStream(this::get,
                             new AlwaysContinue());
    }
    ...
    public ReactiveSeq<Collection<T>> streamBatchNoTimeout(final Continueable s,
                                                           final Function<Supplier<T>, Supplier<Collection<T>>> batcher) {
        this.sub = s;
        listeningStreams.incrementAndGet(); //assumes all Streams that ever connected, remain connected
        return ReactiveSeq.fromStream(closingStreamBatch(batcher.apply(() -> ensureOpen(this.timeout,
                                                                                        this.timeUnit)),
                                                         s));
    }
    ...
    public ReactiveSeq<Collection<T>> streamBatch(final Continueable s,
        ...
        this.sub = s;
        listeningStreams.incrementAndGet(); //assumes all Streams that ever connected, remain connected
        return ReactiveSeq.fromStream(closingStreamBatch(batcher.apply((timeout, timeUnit) -> ensureOpen(timeout,
                                                                                                         timeUnit)),
                                                         s));
    }

    public ReactiveSeq<Seq<T>> streamGroupedByTime(long time,
                                                   TimeUnit t) {
        return streamGroupedBySizeAndTime(Integer.MAX_VALUE,
                                          time,
                                          t);
    ...
    public ReactiveSeq<Seq<T>> streamGroupedBySizeAndTime(int size,
                                                          long time,
                                                          TimeUnit t) {
        long toRun = t.toNanos(time);
        return streamBatch(new Subscription(),
                           source -> {
                               ...
                               return () -> {
                                   ...
                                   try {
                                       while (result.size() < size && checkTime(System.nanoTime(),
                                                                                start,
                                                                                toRun)) {
                                           try {
                                               T next = source.apply(100l,
    ...
    public ReactiveSeq<T> streamControl(final Continueable s,
                                        final Function<Supplier<T>, Supplier<T>> batcher) {

        listeningStreams.incrementAndGet(); //assumes all Streams that ever connected, remain connected
        return ReactiveSeq.fromStream(closingStream(batcher.apply(() -> ensureOpen(this.timeout,
                                                                                   this.timeUnit)),
                                                    s));
    }

    public ReactiveSeq<CompletableFuture<T>> streamControlFutures(final Continueable s,
                                                                  final Function<Supplier<T>, CompletableFuture<T>> batcher) {
        this.sub = s;
        listeningStreams.incrementAndGet(); //assumes all Streams that ever connected, remain connected
        return ReactiveSeq.fromStream(closingStreamFutures(() -> batcher.apply(() -> ensureOpen(this.timeout,
                                                                                                this.timeUnit)),
                                                           s));
    }
    ...
    /**
     * @return Infinite (until Queue is closed) Stream of CompletableFutures that can be used as input into a SimpleReact
     * concurrent dataflow
     * <p>
     * This Stream itself is Sequential, SimpleReact will applyHKT concurrency / parralellism via the constituent
     * CompletableFutures
     */
    @Override
    ...
    private T ensureOpen(final long timeout,
            ...
            try {
                ...
                if (timeout == -1) {
                    if (this.sub != null && this.sub.timeLimit() > -1) {
                        data = ensureClear(consumerWait.take(() -> queue.poll(sub.timeLimit(),
                    ...
                    data = ensureClear(consumerWait.take(() -> queue.poll(timeout,
                                                                          timeUnit)));
                    if (data == null) {
                        throw new QueueTimeoutException();
                    }
    ...
    private void handleTimeout(final SimpleTimer timer,
                               final long timeout) {
    ...
    public T poll(final long time,
                  final TimeUnit unit) throws QueueTimeoutException {
        return this.ensureOpen(time,
                               unit);
    }

    public T get() {

        T res = ensureOpen(this.timeout,
                           this.timeUnit);
     ...
     * <p>
     * If the queue is a bounded queue and is full it will block until space comes available or until offer time out is reached
     * (default is Integer.MAX_VALUE DAYS).
     *
     * @param data data to add
     * @return self
     */
    ...
    /**
     * Close this Queue Poison Pills are used to communicate closure to connected Streams A Poison Pill is added per connected
     * Stream to the Queue If a BlockingQueue is backing this async.Queue it will block until able to add to the Queue.
     *
    ...
    public static class QueueReader<T> {
        ...
        public T next() {
            ...
            last = queue.ensureOpen(queue.timeout,
                                    queue.timeUnit);
        ...
        public Collection<T> drainToOrBlock() {
            ...
            if (size() > 0) {
                ...
                try {
                    ...
                    result.add(queue.ensureOpen(queue.timeout,
                                                queue.timeUnit));
