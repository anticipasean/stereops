...
public interface Foldable<T> extends Iterable<T> {
    ...
    default TreeSet<T> treeSet(Comparator<? super T> comp) {
        return TreeSet.fromIterable(this,
                                    comp);
    }
    ...
    default NonEmptyList<T> nonEmptyList(Supplier<T> s) {
        ...
        return NonEmptyList.cons(it.hasNext() ? it.next() : s.get(),
                                 LazySeq.fromIterator(it));
    }


    default <K, V> HashMap<K, V> toHashMap(Function<? super T, ? extends K> keyMapper,
                                           Function<? super T, ? extends V> valueMapper) {
        return HashMap.fromStream(ReactiveSeq.fromIterable(this)
                                             .map(a -> Tuple.tuple(keyMapper.apply(a),
                                                                   valueMapper.apply(a))));
    }
    ...
    default <R> R collect(Supplier<R> supplier,
        ...
        return stream().collect(supplier,
                                accumulator,
                                combiner);
    }
    ...
    default <R1, R2, A1, A2> Tuple2<R1, R2> collect(Collector<? super T, A1, R1> c1,
                                                    Collector<? super T, A2, R2> c2) {
        return stream().collect(Collector.of(() -> Tuple.tuple(c1.supplier()
                                                                 .get(),
                                                               c2.supplier()
                                                                 .get()),
                                             (t2, next) -> {
                                             ...
                                             (t2, t2b) -> Tuple.tuple(c1.combiner()
                                                                        .apply(t2._1(),
                                                                               t2b._1()),
                                                                      c2.combiner()
                                                                        .apply(t2._2(),
                                                                               t2b._2())),
                                             t2 -> Tuple.tuple(c1.finisher()
                                                                 .apply(t2._1()),
                                                               c2.finisher()
                                                                 .apply(t2._2()))));
    }

    default <R1, R2, R3, A1, A2, A3> Tuple3<R1, R2, R3> collect(Collector<? super T, A1, R1> c1,
                                                                Collector<? super T, A2, R2> c2,
                                                                Collector<? super T, A3, R3> c3) {
        return stream().collect(Collector.of(() -> Tuple.tuple(c1.supplier()
                                                                 .get(),
                                                               c2.supplier()
                                                                 .get(),
                                             ...
                                             (t3, t3b) -> Tuple.tuple(c1.combiner()
                                                                        .apply(t3._1(),
                                                                               t3b._1()),
                                                                      c2.combiner()
                                                                        .apply(t3._2(),
    ...
    default <U> Option<T> maxBy(Function<? super T, ? extends U> function,
                                Comparator<? super U> comparator) {
        return foldLeft(BinaryOperator.maxBy(new Comparator<T>() {
            @Override
            public int compare(T o1,
                               T o2) {
                return comparator.compare(function.apply(o1),
                                          function.apply(o2));
            }
        }));
    }
    ...
    default Option<T> mode() {
        Map<T, Integer> map = stream().collect(Collectors.toMap(k -> k,
                                                                v -> 1,
                                                                (a, b) -> a + b));
    ...
    default T foldLeft(final T identity,
     ...
     * @return Reduced value
     */
    default <U> U foldRight(final U identity,
     ...
     * </pre>
     *
     * @return A Maybe with single value if this Stream has exactly one element, otherwise Optional Empty
     */
    ...
    default ReactiveSeq<T> scheduleStream(final String cron,
                                          final ScheduledExecutorService ex) {
        return Spouts.schedule(stream(),
                               cron,
                               ex);
    }
     ...
     * </pre>
     * <p>
     * Alternatively @see {@link Foldable#atLeast} and {@link Foldable#atMost}
     */
    default boolean xMatch(final int num,
                           final Predicate<? super T> c) {
        return stream().xMatch(num,
                               c);
    }
     ...
     */
    default boolean atLeast(final int num,
                            final Predicate<? super T> c) {
        int[] matched = {0};
        return stream().filter(n -> {
            ...
            return false;
                       ...
                       .takeWhileInclusive(n -> matched[0] < num)
                       .count() >= num;
    }
     ...
     */
    default boolean atMost(final int num,
                           final Predicate<? super T> c) {
        int[] matched = {0};
        return stream().filter(n -> {
            if (c.test(n)) {
            ...
            return false;
                       ...
                       .takeWhileInclusive(n -> matched[0] < num + 1)
                       .count() < num + 1;
    }
}
