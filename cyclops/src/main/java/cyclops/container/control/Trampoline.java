...
public interface Trampoline<T> extends Value<T>, Function0<T>, To<Trampoline<T>> {
    ...
    default <R> R fold(Function<? super Trampoline<T>, ? extends R> more,
                       Function<? super T, ? extends R> done) {
    ...
    default <B, R> Trampoline<R> zip(Trampoline<B> b,
        ...
        if (first.isLeft() && second.isLeft()) {
            return Trampoline.more(() -> first.leftOrElse(null)
                                              .zip(second.leftOrElse(null),
                                                   zipper));
        }
        if (first.isRight() && second.isRight()) {
            return Trampoline.done(zipper.apply(first.orElse(null),
                                                second.orElse(null)));
        }
        ...
        if (first.isRight() && second.isLeft()) {
            return Trampoline.more(() -> this.zip(second.leftOrElse(null),
                                                  zipper));
        }
        //unreachable
        return null;
    ...
    default <B, C> Trampoline<Tuple3<T, B, C>> zip(Trampoline<B> b,
                                                   Trampoline<C> c) {
        return zip(b,
                   c,
                   (x, y, z) -> Tuple.tuple(x,
    ...
    default <B, C, R> Trampoline<R> zip(Trampoline<B> b,
                                        Trampoline<C> c,
        ...
        if (first.isLeft() && second.isLeft() && third.isLeft()) {
            return Trampoline.more(() -> first.leftOrElse(null)
                                              .zip(second.leftOrElse(null),
                                                   third.leftOrElse(null),
                                                   fn));
        }
        if (first.isRight() && second.isRight() && third.isRight()) {
            return Trampoline.done(fn.apply(first.orElse(null),
                                            second.orElse(null),
                                            third.orElse(null)));
        }
        ...
        if (first.isRight() && second.isRight() && third.isLeft()) {
            return Trampoline.more(() -> this.zip(b,
                                                  third.leftOrElse(null),
                                                  fn));
        }
    ...
    @Override
    default <R> R fold(Function<? super T, ? extends R> present,
