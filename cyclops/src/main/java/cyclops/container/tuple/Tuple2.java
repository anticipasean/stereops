...
public class Tuple2<T1, T2> implements To<Tuple2<T1, T2>>, Serializable, EqualTo<Higher<tuple2, T1>, T2, Tuple2<T1, T2>>,
    ...
    public static <T1, T2> Tuple2<T1, T2> of(T1 value1,
                                             T2 value2) {
        return new Tuple2<T1, T2>(value1,
                                  value2);
    }

    public static <T1, T2> Tuple2<T1, T2> lazy(Supplier<? extends T1> supplier1,
                                               Supplier<? extends T2> supplier2) {
        return new Tuple2<T1, T2>(null,
                                  null) {
            @Override
            public T1 _1() {
    ...
    public static <T1, T2, R> Tuple2<T1, R> tailRec(Monoid<T1> op,
        ...
        do {
            ...
            cont = next[0].fold((a, p) -> p.fold(s -> {
                                                     next[0] = narrowK(fn.apply(s)).map1(t1 -> op.apply(next[0]._1(),
                                                                                                        t1));
                                                     return true;
                                                 },
                                                 __ -> false));
    ...
    public <R> Tuple2<T1, R> flatMap(Monoid<T1> m,
                                     Function<? super T2, ? extends Tuple2<T1, R>> fn) {
        return fn.apply(_2())
                 .map1(t1 -> m.apply(t1,
                                     _1()));
    }
    ...
    public <R1, R2> Tuple2<R1, R2> bimap(Function<? super T1, ? extends R1> fn1,
                                         Function<? super T2, ? extends R2> fn2) {
        return of((fn1.apply(_1())),
                  fn2.apply(_2()));
    }

    public <R1, R2> Tuple2<R1, R2> lazyBimap(Function<? super T1, ? extends R1> fn1,
                                             Function<? super T2, ? extends R2> fn2) {
