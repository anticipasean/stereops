...
public interface HashedPatriciaTrie<K, V> {
    ...
    static <K, V> Node<K, V>[] emptyArray() {
        return Arrays.copyOf((Node<K, V>[]) EMPTY_ARRAY,
                             BUCKET_SIZE);
    }
    ...
    interface Node<K, V> extends Sealed4<EmptyNode<K, V>, SingleNode<K, V>, CollisionNode<K, V>, ArrayNode<K, V>>, Serializable {
        ...
        Node<K, V> put(int hash,
                       K key,
                       V value);

        Option<V> get(int hash,
                      K key);

        V getOrElse(int hash,
                    K key,
                    V alt);

        V getOrElseGet(int hash,
                       K key,
                       Supplier<? extends V> alt);

        Node<K, V> minus(int hash,
                         K key);
    ...
    static final class EmptyNode<K, V> implements Node<K, V> {
        ...
        @Override
        public Node<K, V> put(int hash,
                              K key,
                              V value) {
            if (hash == 0) {
                return new SingleNode<>(key,
                                        value);
            } else {
                ...
                if (newHash == 0) {
                    nodes[0] = this;
                    nodes[index] = new SingleNode<>(key,
                                                    value);
                } else {
                    nodes[index] = Instance.put(newHash,
                                                key,
                                                value);
                    if (index != 0) {
                        nodes[0] = this;
        ...
        @Override
        public Option<V> get(int hash,
                             K key) {
            return Option.none();
        }

        @Override
        public V getOrElse(int hash,
                           K key,
                           V alt) {
            return alt;
        }
        ...
        @Override
        public V getOrElseGet(int hash,
                              K key,
                              Supplier<? extends V> alt) {
            return alt.get();
        }
        ...
        @Override
        public Node<K, V> minus(int hash,
                                K key) {
            return this;
        }
        ...
        @Override
        public <R> R fold(Function<? super EmptyNode<K, V>, ? extends R> fn1,
                          Function<? super SingleNode<K, V>, ? extends R> fn2,
                          Function<? super CollisionNode<K, V>, ? extends R> fn3,
                          Function<? super ArrayNode<K, V>, ? extends R> fn4) {
    ...
    static final class SingleNode<K, V> implements Node<K, V>, Deconstruct2<K, V> {
        ...
        LazySeq<Tuple2<K, V>> bucket() {
            return LazySeq.of(Tuple.tuple(key,
                                          value));
        }
        ...
        public Node<K, V> put(int hash,
                              ...
                              V value) {
            if (hash == 0) {
                return new CollisionNode<>(bucket()).put(hash,
                                                         key,
                                                         value);
            } else {
                int newHash = hash >>> BITS;
                ...
                if (newHash == 0) {
                    ...
                    nodes[index] = EmptyNode.Instance.put(newHash,
                                                          key,
                                                          value);
                    if (index != 0) {
                        nodes[0] = this;
                    } else {
                        nodes[0] = nodes[0].put(0,
                                                this.key,
                                                this.value);
                    }
                }
    ...
    static final class CollisionNode<K, V> implements Node<K, V>, Deconstruct1<LazySeq<Tuple2<K, V>>> {
        ...
        public Node<K, V> put(int hash,
                              ...
                              V value) {
            if (hash == 0) {
                return new CollisionNode<>(bucket.filter(p -> !p._1()
                                                                .equals(key))
                                                 .prepend(Tuple.tuple(key,
                                                                      value)));
            } else {
                int newHash = hash >>> BITS;
                int index = hash & MASK;
                ...
                if (newHash == 0) {
                    ...
                    if (index != 0) {
                        ...
                        bucket.forEach(t2 -> nodes[0] = nodes[0].put(0,
                                                                     t2._1(),
                                                                     t2._2()));
                    }
                }
        ...
        public Option<V> get(int hash,
                             K key) {
            return (hash == 0) ? bucket.filter(t2 -> t2._1()
                                                       .equals(key))
                                       .get(0)
                                       .map(Tuple2::_2) : Option.none();
        }
        ...
        public Node<K, V> minus(int hash,
            ...
            LazySeq<Tuple2<K, V>> newBucket = bucket.filter(t2 -> !t2._1()
                                                                     .equals(key));
            //@TODO safety
            return newBucket.fold(c -> c.size() > 1 ? new CollisionNode<K, V>(newBucket) : new SingleNode<>(newBucket.get(0)
                                                                                                                     .orElse(null)),
                                  nil -> HashedPatriciaTrie.empty());
        }
    ...
    static final class ArrayNode<K, V> implements Node<K, V>, Deconstruct1<Node<K, V>[]> {
        ...
        public Node<K, V> put(int hash,
            ...
            int newHash = hash >>> BITS;
            int index = hash & MASK;
            Node<K, V>[] newNodes = Arrays.copyOf(nodes,
                                                  nodes.length);
            newNodes[index] = nodes[index].put(newHash,
                                               key,
                                               value);
        ...
        public Option<V> get(int hash,
            ...
            int newHash = hash >>> BITS;
            int index = hash & MASK;
            return nodes[index].get(newHash,
                                    key);
        }
        ...
        public V getOrElse(int hash,
            ...
            int newHash = hash >>> BITS;
            int index = hash & MASK;
            return nodes[index].getOrElse(newHash,
                                          key,
                                          alt);
        }
        ...
        public V getOrElseGet(int hash,
            ...
            int newHash = hash >>> BITS;
            int index = hash & MASK;
            return nodes[index].getOrElseGet(newHash,
                                             key,
                                             alt);
        }
        ...
        public Node<K, V> minus(int hash,
            ...
            if (node.isEmpty()) {
                ...
                Node<K, V> newNode = node.minus(newHash,
                                                key);
                if (newNode == node) {
                    return this;
                } else {
