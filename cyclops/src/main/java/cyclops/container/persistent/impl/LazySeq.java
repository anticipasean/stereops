package cyclops.container.persistent.impl;
...
public interface LazySeq<T> extends ImmutableList<T>, Folds<T>, Filters<T>, Transformable<T>, Serializable, Higher<lazySeq, T> {


    static <T> Collector<T, List<T>, LazySeq<T>> collector() {
        Collector<T, ?, List<T>> c = Collectors.toList();
        return Collectors.<T, List<T>, Iterable<T>, LazySeq<T>>collectingAndThen((Collector) c,
                                                                                 LazySeq::fromIterable);
    }
    ...
    static <T, R> LazySeq<R> tailRec(T initial,
        ...
        ReactiveSeq<R> x = Either.sequenceRight(next.stream())
    ...
    static <T> LazySeq<T> deferred(Supplier<? extends Iterable<? extends T>> lazy) {
        return fromStream(ReactiveSeq.of(1)
    ...
    static LazySeq<Integer> range(final int start,
                                  final int end) {
        return LazySeq.fromStream(ReactiveSeq.range(start,
                                                    end));
    ...
    static LazySeq<Long> rangeLong(final long start,
                                   final long step,
                                   final long end) {
        return LazySeq.fromStream(ReactiveSeq.rangeLong(start,
                                                        step,
                                                        end));
    }
     ...
     * @return Stream over enum
     */
    static <E extends Enum<E>> LazySeq<E> enumsFromTo(Class<E> c,
                                                      E start,
                                                      E end) {
        return LazySeq.fromStream(ReactiveSeq.enumsFromTo(c,
                                                          start,
                                                          end));
    }
    ...
    static <T> LazySeq<T> fromStream(Stream<T> stream) {
        ...
        return new Lazy<>(Eval.later(() -> t.hasNext() ? cons(Eval.later(() -> t.next()),
                                                              head -> {
                                                                  head.get();
    ...
    static <T> LazySeq<T> lazy(T head,
                               Supplier<ImmutableList<T>> tail) {
        return Cons.cons(Eval.now(head),
                         () -> tail.get()
                                   .lazySeq());
    }
    ...
    static <T> LazySeq<T> cons(T head,
                               Supplier<LazySeq<T>> tail) {
        return Cons.cons(Eval.now(head),
                         tail);
    }

    static <T> LazySeq<T> cons(Eval<T> head,
                               Supplier<LazySeq<T>> tail) {
    ...
    static <T> LazySeq<T> cons(Eval<T> head,
                               Function<? super Eval<T>, ? extends Supplier<LazySeq<T>>> tail) {
        return Cons.cons(head,
                         tail.apply(head));
    }
    ...
    default LazySeq<T> insertAt(int pos,
        ...
        return (LazySeq<T>) ImmutableList.super.insertAt(pos,
                                                         list);
    }
    ...
    default LazySeq<T> insertAt(final int i,
                                final T e) {
        return (LazySeq<T>) ImmutableList.super.insertAt(i,
                                                         e);
    }
    ...
    default Tuple2<ImmutableList<T>, ImmutableList<T>> span(Predicate<? super T> pred) {
        return Tuple.tuple(takeWhile(pred),
                           dropWhile(pred));
    }
    ...
    default LazySeq<LazySeq<T>> split(Predicate<? super T> test) {
        return defer(() -> {
            ...
            Tuple2<LazySeq<T>, LazySeq<T>> split = next.splitBy(test)
                                                       .bimap(ImmutableList::lazySeq,
                                                              ImmutableList::lazySeq);
            return next.foldLazySeq(c -> cons(split._1(),
                                              () -> split._2()
                                                         .split(test)),
                                    n -> n);
        });
    ...
    default LazySeq<T> take(final long n) {
        ...
        return fromStream(ReactiveSeq.fromIterable(this)
                                     .take(n));
    ...
    default LazySeq<T> drop(final long num) {
        return defer(() -> {
            ...
            while (pos-- > 0 && !current.isEmpty()) {
                current = current.foldLazySeq(c -> c.tail.get(),
                                              nil -> nil);
            }
            return current;
        });
    ...
    default <R1, R2> Tuple2<LazySeq<R1>, LazySeq<R2>> unzip(Function<? super T, Tuple2<? extends R1, ? extends R2>> fn) {
        Tuple2<LazySeq<R1>, LazySeq<Tuple2<? extends R1, ? extends R2>>> x = map(fn).duplicate()
    ...
    default LazySeq<T> replaceFirst(T currentElement,
        ...
        boolean[] found = {false};
        return map(i -> {
            if (Objects.equals(currentElement,
                               i) && !found[0]) {
                found[0] = true;
                return newElement;
            }
    ...
    default Option<T> get(int pos) {
        ...
        return Option.ofNullable(l.fold(c -> c.head(),
                                        n -> null));
    }
    ...
    default T getOrElse(int pos,
        ...
        LazySeq<T> l = this;
        for (int i = 0; i < pos; i++) {
            l = l.foldLazySeq(c -> c.tail.get(),
                              n -> n);
            if (l instanceof Nil) { //short circuit
                return alt;
            }
        }
        return l.foldLazySeq(c -> c.head.get(),
                             n -> alt);
    }
    ...
    default LazySeq<T> prependAll(Iterable<? extends T> it) {
        return defer(() -> {
            ...
            return value.fold(cons -> cons.foldRight(this,
                                                     (a, b) -> b.prepend(a)),
                              nil -> this);
        });
    }
    ...
    default LazySeq<T> subList(int start,
                               int end) {
        return (LazySeq<T>) ImmutableList.super.subList(start,
                                                        end);
    }
    ...
    @Override
    default <T2, R> LazySeq<R> zip(BiFunction<? super T, ? super T2, ? extends R> fn,
                                   Publisher<? extends T2> publisher) {
        return (LazySeq<R>) ImmutableList.super.zip(fn,
                                                    publisher);
    }

    @Override
    default <U, R> LazySeq<R> zipWithStream(Stream<? extends U> other,
                                            BiFunction<? super T, ? super U, ? extends R> zipper) {
        return (LazySeq<R>) ImmutableList.super.zipWithStream(other,
                                                              zipper);
    }
    ...
    @Override
    default <S, U, R> LazySeq<R> zip3(Iterable<? extends S> second,
                                      Iterable<? extends U> third,
                                      Function3<? super T, ? super S, ? super U, ? extends R> fn3) {
        return (LazySeq<R>) ImmutableList.super.zip3(second,
                                                     third,
                                                     fn3);
    }

    @Override
    default <T2, T3, T4, R> LazySeq<R> zip4(Iterable<? extends T2> second,
                                            Iterable<? extends T3> third,
                                            Iterable<? extends T4> fourth,
                                            Function4<? super T, ? super T2, ? super T3, ? super T4, ? extends R> fn) {
        return (LazySeq<R>) ImmutableList.super.zip4(second,
                                                     third,
                                                     fourth,
                                                     fn);
    }
    ...
    @Override
    default LazySeq<T> combine(BiPredicate<? super T, ? super T> predicate,
                               BinaryOperator<T> op) {
        return (LazySeq<T>) ImmutableList.super.combine(predicate,
                                                        op);
    }
    ...
    default LazySeq<T> combine(Monoid<T> op,
        ...
        return (LazySeq<T>) ImmutableList.super.combine(op,
                                                        predicate);
    }
    ...
    default LazySeq<T> cycle(Monoid<T> m,
                             long times) {
        return (LazySeq<T>) ImmutableList.super.cycle(m,
                                                      times);
    }
    ...
    @Override
    default <U, R> LazySeq<R> zip(Iterable<? extends U> other,
                                  BiFunction<? super T, ? super U, ? extends R> zipper) {
        return (LazySeq<R>) ImmutableList.super.zip(other,
                                                    zipper);
    }

    @Override
    default <S, U> LazySeq<Tuple3<T, S, U>> zip3(Iterable<? extends S> second,
                                                 Iterable<? extends U> third) {
        return (LazySeq) ImmutableList.super.zip3(second,
                                                  third);
    }

    @Override
    default <T2, T3, T4> LazySeq<Tuple4<T, T2, T3, T4>> zip4(Iterable<? extends T2> second,
                                                             Iterable<? extends T3> third,
                                                             Iterable<? extends T4> fourth) {
        return (LazySeq) ImmutableList.super.zip4(second,
                                                  third,
                                                  fourth);
    }
    ...
    default <C extends PersistentCollection<? super T>> LazySeq<C> grouped(int size,
                                                                           Supplier<C> supplier) {
        return (LazySeq<C>) ImmutableList.super.grouped(size,
                                                        supplier);
    }
    ...
    @Override
    default <C extends PersistentCollection<? super T>> LazySeq<C> groupedWhile(Predicate<? super T> predicate,
                                                                                Supplier<C> factory) {
        return (LazySeq<C>) ImmutableList.super.groupedWhile(predicate,
                                                             factory);
    }

    @Override
    default <C extends PersistentCollection<? super T>> LazySeq<C> groupedUntil(Predicate<? super T> predicate,
                                                                                Supplier<C> factory) {
        return (LazySeq<C>) ImmutableList.super.groupedUntil(predicate,
                                                             factory);
    }
    ...
    default <U> LazySeq<U> scanLeft(U seed,
        ...
        return (LazySeq<U>) ImmutableList.super.scanLeft(seed,
                                                         function);
    }
    ...
    default <U> LazySeq<U> scanRight(U identity,
        ...
        return (LazySeq<U>) ImmutableList.super.scanRight(identity,
                                                          combiner);
    }
    ...
    default LazySeq<T> slice(long from,
                             long to) {
        return (LazySeq<T>) ImmutableList.super.slice(from,
                                                      to);
    }
    ...
    default LazySeq<T> deleteBetween(int start,
                                     int end) {
        return (LazySeq<T>) ImmutableList.super.deleteBetween(start,
                                                              end);
    }
    ...
    default LazySeq<T> insertStreamAt(int pos,
                                      Stream<T> stream) {
        return (LazySeq<T>) ImmutableList.super.insertStreamAt(pos,
                                                               stream);
    }
    ...
    <R> R foldLazySeq(Function<? super Cons<T>, ? extends R> fn1,
                      Function<? super Nil, ? extends R> fn2);
    ...
    @Override
    default <R1, R2, R3, R> LazySeq<R> forEach4(Function<? super T, ? extends Iterable<R1>> iterable1,
        ...
        return (LazySeq<R>) ImmutableList.super.forEach4(iterable1,
                                                         iterable2,
                                                         iterable3,
                                                         yieldingFunction);
    }
    ...
    @Override
    default <R1, R2, R> LazySeq<R> forEach3(Function<? super T, ? extends Iterable<R1>> iterable1,
                                            BiFunction<? super T, ? super R1, ? extends Iterable<R2>> iterable2,
                                            Function3<? super T, ? super R1, ? super R2, ? extends R> yieldingFunction) {
        return (LazySeq<R>) ImmutableList.super.forEach3(iterable1,
                                                         iterable2,
                                                         yieldingFunction);
    }
    ...
    @Override
    default <R1, R> LazySeq<R> forEach2(Function<? super T, ? extends Iterable<R1>> iterable1,
                                        BiFunction<? super T, ? super R1, ? extends R> yieldingFunction) {
        return (LazySeq<R>) ImmutableList.super.forEach2(iterable1,
                                                         yieldingFunction);
    }
    ...
    public static class Cons<T> implements LazySeq<T>, ImmutableList.Some<T> {
        ...
        public static <T> Cons<T> cons(Eval<T> value,
                                       Supplier<LazySeq<T>> tail) {
            return new Cons<>(value,
                              Memoize.memoizeSupplier(tail));
        }


        @Override
        public Tuple2<T, ImmutableList<T>> unapply() {
            return Tuple.tuple(head.get(),
                               tail.get());
        }
        ...
        public T foldRight(Monoid<T> reducer) {
            return foldRight(reducer.zero(),
                             reducer);
        }


        public <R> R lazyFoldRight(R zero,
                                   BiFunction<? super T, Supplier<R>, ? extends R> f) {
            ...
            Tuple2<Boolean, R> value = Tuple.tuple(true,
                                                   zero);
            while (!next.isEmpty() && value._1()) {

                Cons<T> cons = next.foldLazySeq(c -> c,
                                                n -> null);
                value = cons.lazyFoldRightImpl(value._2(),
                                               f);
                next = host.drop(count)
                           .take(inc);
                count = count + inc;
            }
            return value._2();
        ...
        private <R> Tuple2<Boolean, R> lazyFoldRightImpl(R zero,
                                                         BiFunction<? super T, Supplier<R>, ? extends R> f) {
            ...
            R x = f.apply(head.get(),
                          () -> {
                              ...
                              return ls.foldLazySeq(s -> {
                                                        Tuple2<Boolean, R> t2 = s.lazyFoldRightImpl(zero,
                                                                                                    f);
                                                        if (t2._1()) {
                                                            called[0] = true;
                                                        }
        ...
        public <R> LazySeq<R> map(Function<? super T, ? extends R> fn) {
            ...
            Supplier<LazySeq<R>> s = () -> tail.get()
                                               .map(fn);
            return LazySeq.cons(newHead,
                                s);
        }
        ...
        @Override
        public <R> LazySeq<R> mergeMap(int maxConcurecy,
                                       Function<? super T, ? extends Publisher<? extends R>> fn) {
            return fromIterable(stream().mergeMap(maxConcurecy,
                                                  fn));
        }
        ...
        public LazySeq<T> removeValue(T e) {
            ...
            return Objects.equals(head,
                                  e) ? tail() : LazySeq.cons(this.head,
                                                             () -> tail().removeValue(e));
        ...
        public LazySeq<T> removeFirst(Predicate<? super T> e) {
            T head = this.head.get();
            return e.test(head) ? tail() : LazySeq.cons(this.head,
                                                        () -> tail().removeFirst(e));
        ...
        public <R> R foldLazySeq(Function<? super Cons<T>, ? extends R> fn1,
                                 Function<? super Nil, ? extends R> fn2) {
            return fn1.apply(this);
        }
        ...
        @Override
        public LazySeq<T> updateAt(int i,
                                   T value) {
            if (i < 0) {
                return this;
            }
        ...
        public LazySeq<T> insertAt(final int i,
                                   final T value) {
            return defer(() -> {
                if (i == 0) {
                    return prepend(value);
                ...
                return LazySeq.super.insertAt(i,
                                              value);
            });
        }

        @Override
        public LazySeq<T> insertAt(int i,
                                   Iterable<? extends T> list) {
            return defer(() -> {
                if (i == 0) {
    ...
    public static class Nil<T> implements LazySeq<T>, ImmutableList.None<T> {
        ...
        public <R> LazySeq<R> lazyScanRight(R zero,
                                            BiFunction<? super T, Supplier<R>, ? extends R> f) {
            return LazySeq.of(zero);
        }

        @Override
        public <R> R foldRight(R zero,
                               BiFunction<? super T, ? super R, ? extends R> f) {
            return zero;
        }
