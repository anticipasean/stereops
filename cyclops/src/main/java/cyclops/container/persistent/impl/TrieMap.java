...
import cyclops.function.hkt.DataWitness.trieMap;
import cyclops.function.hkt.Higher2;
import cyclops.container.persistent.PersistentMap;
import cyclops.control.Option;
import cyclops.container.persistent.impl.base.HashedPatriciaTrie;
...
public final class TrieMap<K, V> implements ImmutableMap<K, V>, Higher2<trieMap, K, V>, Serializable {
    ...
    public static <K, V> TrieMap<K, V> of(K k,
                                          V v) {
    ...
    public static <K, V> TrieMap<K, V> of(K k1,
                                          V v1,
                                          K k2,
                                          V v2) {
        TrieMap<K, V> res = empty();
    ...
    @Override
    public TrieMap<K, V> put(K key,
                             V value) {
        return new TrieMap<>(map.put(key.hashCode(),
                                     key,
                                     value));
    }
    ...
    public TrieMap<K, V> remove(K key) {
        return new TrieMap<>(map.minus(key.hashCode(),
                                       key));
    }
    ...
    public V getOrElse(K key,
                       V alt) {
        return map.getOrElse(key.hashCode(),
                             key,
                             alt);
    }
    ...
    public V getOrElseGet(K key,
        ...
        return map.getOrElseGet(key.hashCode(),
                                key,
                                alt);
    }
    ...
    @Override
    public <R1, R2> TrieMap<R1, R2> bimap(Function<? super K, ? extends R1> fn1,
                                          Function<? super V, ? extends R2> fn2) {
        return fromStream(stream().map(t -> t.bimap(fn1,
                                                    fn2)));
    }
