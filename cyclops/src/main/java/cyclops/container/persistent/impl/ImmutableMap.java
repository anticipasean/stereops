...
public interface ImmutableMap<K, V> extends Iterable<Tuple2<K, V>>, Folds<Tuple2<K, V>>, Filters<Tuple2<K, V>>, Transformable<V>,
    ...
    default boolean containsValue(V value) {
        return stream().anyMatch(t -> Objects.equals(value,
                                                     t._2()));
    }
    ...
    default Map<K, V> javaMap() {
        return new AbstractMap<K, V>() {
            ...
            public V get(Object key) {
                return ImmutableMap.this.get((K) key)
                                        .orElse(null);
            }
            ...
            @Override
            public Set<K> keySet() {
                return ImmutableMap.this.stream()
                                        .map(t -> t._1())
                                        .toSet();
            }
            ...
            public Set<Entry<K, V>> entrySet() {
                return ImmutableMap.this.stream()
                                        .map(t -> (Entry<K, V>) new AbstractMap.SimpleEntry<>(t._1(),
                                                                                              t._2()))
                                        .toSet();
            }
        };
    ...
    @Override
    <R1, R2> ImmutableMap<R1, R2> bimap(Function<? super K, ? extends R1> fn1,
                                        Function<? super V, ? extends R2> fn2);

    @Override
    default ImmutableMap<K, V> bipeek(Consumer<? super K> c1,
                                      Consumer<? super V> c2) {
        return (ImmutableMap<K, V>) BiTransformable.super.bipeek(c1,
                                                                 c2);
    }
    ...
    default <K1, K2, K3, K4, R1, R2, R3, R> ImmutableMap<K4, R> forEach4(Function<? super Tuple2<K, V>, ? extends Iterable<Tuple2<K1, R1>>> iterable1,
        ...
        return this.concatMap((a1, b1) -> {
            Tuple2<K, V> in = Tuple.tuple(a1,
                                          b1);
            ReactiveSeq<Tuple2<K1, R1>> a = ReactiveSeq.fromIterable(iterable1.apply(in));
            return a.flatMap(ina -> {
                ReactiveSeq<Tuple2<K2, R2>> b = ReactiveSeq.fromIterable(iterable2.apply(in,
                                                                                         ina));
                return b.flatMap(inb -> {
                    ReactiveSeq<Tuple2<K3, R3>> c = ReactiveSeq.fromIterable(iterable3.apply(in,
                                                                                             ina,
                                                                                             inb));
    ...
    default <T> LazySeq<T> toLazySeq(final Function<? super Tuple2<? super K, ? super V>, ? extends T> fn) {
        return LazySeq.narrow(stream().map(fn)
                                      .to()
                                      .lazySeq());
