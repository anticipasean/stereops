...
public class ConvertableSequence<T> implements ToStream<T> {
    ...
    public <K, V> HashMap<K, V> hashMap(Function<? super T, ? extends K> keyMapper,
                                        Function<? super T, ? extends V> valueMapper) {
        return HashMap.fromStream(stream().map(t -> Tuple.tuple(keyMapper.apply(t),
                                                                valueMapper.apply(t))));
    }

    public TreeSet<T> treeSet(Comparator<T> comp) {
        return TreeSet.fromIterable(iterable,
                                    comp);
    }
    ...
    /**
     * Lazily converts this ReactiveSeq into a Collection. This does not trigger the Stream. E.g. Collection is not thread safe on
     * the first iteration.
     *
     * <pre>
     * {@code
