...
public interface ImmutableQueue<T> extends Sealed2<ImmutableQueue.Some<T>, ImmutableQueue.None<T>>,
                                           ...
                                           OnEmptyError<T, ImmutableQueue<T>>, Contains<T>, IterableX<T>, To<ImmutableQueue<T>>,
                                           PersistentQueue<T> {
    ...
    default ImmutableQueue<T> replace(T currentElement,
                                      T newElement) {
    ...
    default ImmutableQueue<T> removeFirst(Predicate<? super T> pred) {
        ...
        ImmutableQueue<T> ar = rem.fold(s -> s.fold((h, t) -> t),
                                        n -> n);
        return res[0].foldLeft(ar,
                               (a, b) -> a.prepend(b));

    }


    default ImmutableQueue<T> subList(int start,
                                      int end) {
    ...
    default LazySeq<T> lazySeq() {
        ...
        return fold(c -> LazySeq.lazy(c.head(),
                                      () -> c.tail()
                                             .lazySeq()),
                    nil -> LazySeq.empty());
    }
    ...
    @Override
    <R> ImmutableQueue<R> mergeMap(int maxConcurecy,
    ...
    @Override
    default <T2, R> ImmutableQueue<R> zip(BiFunction<? super T, ? super T2, ? extends R> fn,
                                          Publisher<? extends T2> publisher) {
        return unitStream(stream().zip(fn,
                                       publisher));
    }

    default <U, R> ImmutableQueue<R> zipWithStream(Stream<? extends U> other,
    ...
    @Override
    default <S, U, R> ImmutableQueue<R> zip3(Iterable<? extends S> second,
    ...
    @Override
    default <T2, T3, T4, R> ImmutableQueue<R> zip4(Iterable<? extends T2> second,
    ...
    @Override
    default ImmutableQueue<T> combine(BiPredicate<? super T, ? super T> predicate,
                                      BinaryOperator<T> op) {
    ...
    @Override
    default ImmutableQueue<T> combine(Monoid<T> op,
    ...
    @Override
    default ImmutableQueue<T> cycle(Monoid<T> m,
                                    long times) {
    ...
    @Override
    default <U, R> ImmutableQueue<R> zip(Iterable<? extends U> other,
                                         BiFunction<? super T, ? super U, ? extends R> zipper) {
        return unitStream(stream().zip(other,
                                       zipper));
    }
    ...
    @Override
    default <S, U> ImmutableQueue<Tuple3<T, S, U>> zip3(Iterable<? extends S> second,
                                                        Iterable<? extends U> third) {
        return unitStream(stream().zip3(second,
                                        third));
    }
    ...
    @Override
    default <T2, T3, T4> ImmutableQueue<Tuple4<T, T2, T3, T4>> zip4(Iterable<? extends T2> second,
                                                                    Iterable<? extends T3> third,
                                                                    Iterable<? extends T4> fourth) {
        return unitStream(stream().zip4(second,
                                        third,
    ...
    @Override
    default <C extends PersistentCollection<? super T>> ImmutableQueue<C> grouped(int size,
                                                                                  Supplier<C> supplier) {
    ...
    @Override
    default <C extends PersistentCollection<? super T>> ImmutableQueue<C> groupedWhile(Predicate<? super T> predicate,
                                                                                       Supplier<C> factory) {
        return unitStream(stream().groupedWhile(predicate,
                                                factory));
    }
    ...
    @Override
    default <C extends PersistentCollection<? super T>> ImmutableQueue<C> groupedUntil(Predicate<? super T> predicate,
                                                                                       Supplier<C> factory) {
        return unitStream(stream().groupedUntil(predicate,
                                                factory));
    }
    ...
    @Override
    default <U> ImmutableQueue<U> scanLeft(U seed,
    ...
    @Override
    default <U> ImmutableQueue<U> scanRight(U identity,
    ...
    @Override
    default ImmutableQueue<T> slice(long from,
                                    long to) {
    ...
    @Override
    default ImmutableQueue<T> insertAt(int pos,
                                       T... values) {
        if (pos == 0) {
        ...
        return unitStream(stream().insertAt(pos,
                                            values));
    }
    ...
    @Override
    default ImmutableQueue<T> deleteBetween(int start,
                                            int end) {
    ...
    @Override
    default ImmutableQueue<T> insertStreamAt(int pos,
                                             Stream<T> stream) {
    ...
    @Override
    default ImmutableQueue<T> updateAt(int pos,
                                       T value) {
