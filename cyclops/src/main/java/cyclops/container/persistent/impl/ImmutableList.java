...
import cyclops.matching.Deconstruct.Deconstruct2;
...
public interface ImmutableList<T> extends Sealed2<ImmutableList.Some<T>, ImmutableList.None<T>>, IterableX<T>, Contains<T>,
                                          Comparable<T>, PersistentList<T>, OnEmptySwitch<ImmutableList<T>, ImmutableList<T>>,
                                          OnEmptyError<T, ImmutableList<T>>, To<ImmutableList<T>> {
    ...
    default int compareTo(final T o) {
        if (o instanceof ImmutableList) {
            ...
            if (this.size() == l.size()) {
                ...
                if (i1.hasNext()) {
                    if (i2.hasNext()) {
                        final int comp = Comparator.<Comparable>naturalOrder().compare((Comparable) i1.next(),
                                                                                       (Comparable) i2.next());
                        if (comp != 0) {
                            return comp;
                        }
    ...
    default boolean equalToDirectAccess(Iterable<T> iterable) {
        ...
        for (int i = 0; i < size; i++) {
            T at = this.getOrElse(i,
                                  null);
            if (!it.hasNext()) {
                return false;
            }
            if (!Objects.equals(at,
                                it.next())) {
                return false;
            }
        }
    ...
    default ImmutableList<T> replaceFirst(T currentElement,
                                          T newElement) {
        ...
        while (!tail.isEmpty()) {
            ...
            Tuple3<ImmutableList<T>, ImmutableList<T>, Boolean> t3 = tail.fold(c -> {
                                                                                   if (Objects.equals(c.head(),
                                                                                                      currentElement)) {
                                                                                       return Tuple.tuple(ref,
                                                                                                          tailRef,
                                                                                                          true);
                                                                                   }
                                                                                   return Tuple.tuple(ref.prepend(c.head()),
                                                                                                      c.tail(),
                                                                                                      false);
                                                                               },
                                                                               nil -> Tuple.tuple(ref,
        ...
        return tail.fold(cons -> cons.tail()
                                     .prepend(newElement)
                                     .prependAll(start),
                         nil -> this);
    }
    ...
    default ImmutableList<T> subList(int start,
                                     int end) {
        if (start > 0) {
    ...
    default Option<NonEmptyList<T>> nonEmptyList() {
        return Option.ofNullable(fold(c -> NonEmptyList.cons(c.head(),
                                                             c.tail()),
                                      nil -> null));
    }
    ...
    default Zipper<T> focusAt(int pos,
                              T alt) {
        Tuple2<ImmutableList<T>, ImmutableList<T>> t2 = splitAt(pos);
        T value = t2._2()
                    .fold(c -> c.head(),
                          n -> alt);
        ImmutableList<T> right = t2._2()
                                   .fold(c -> c.tail(),
                                         n -> null);
        return Zipper.of(t2._1(),
                         value,
                         right);
    }
    ...
    default Option<Zipper<T>> focusAt(int pos) {
        Tuple2<ImmutableList<T>, ImmutableList<T>> t2 = splitAt(pos);
        Option<T> value = t2._2()
                            .fold(c -> Option.some(c.head()),
                                  n -> Option.none());
        return value.map(l -> {
            ImmutableList<T> right = t2._2()
                                       .fold(c -> c.tail(),
    ...
    default boolean containsValue(T value) {
        return stream().filter(o -> Objects.equals(value,
                                                   o))
                       .findFirst()
                       .isPresent();
    }
    ...
    @Override
    <R> R fold(Function<? super Some<T>, ? extends R> fn1,
    ...
    default <R> R fold(BiFunction<? super T, ? super ImmutableList<T>, ? extends R> fn,
                       Supplier<? extends R> alt) {
        return fold(s -> fn.apply(s.head(),
                                  s.tail()),
                    n -> alt.get());
    }
    ...
    default Tuple2<ImmutableList<T>, ImmutableList<T>> partition(final Predicate<? super T> splitter) {
        ...
        return Tuple.tuple(filter(splitter),
    ...
    @Override
    default <T2, R> ImmutableList<R> zip(BiFunction<? super T, ? super T2, ? extends R> fn,
                                         Publisher<? extends T2> publisher) {
        return unitStream(stream().zip(fn,
                                       publisher));
    }

    default <U, R> ImmutableList<R> zipWithStream(Stream<? extends U> other,
    ...
    @Override
    default <S, U, R> ImmutableList<R> zip3(Iterable<? extends S> second,
    ...
    @Override
    default <T2, T3, T4, R> ImmutableList<R> zip4(Iterable<? extends T2> second,
    ...
    @Override
    default ImmutableList<T> combine(BiPredicate<? super T, ? super T> predicate,
                                     BinaryOperator<T> op) {
    ...
    @Override
    default ImmutableList<T> combine(Monoid<T> op,
    ...
    @Override
    default ImmutableList<T> cycle(Monoid<T> m,
                                   long times) {
    ...
    @Override
    default <U, R> ImmutableList<R> zip(Iterable<? extends U> other,
                                        BiFunction<? super T, ? super U, ? extends R> zipper) {
    ...
    @Override
    default <S, U> ImmutableList<Tuple3<T, S, U>> zip3(Iterable<? extends S> second,
                                                       Iterable<? extends U> third) {
        return unitStream(stream().zip3(second,
                                        third));
    }
    ...
    @Override
    default <T2, T3, T4> ImmutableList<Tuple4<T, T2, T3, T4>> zip4(Iterable<? extends T2> second,
                                                                   Iterable<? extends T3> third,
                                                                   Iterable<? extends T4> fourth) {
        return unitStream(stream().zip4(second,
                                        third,
    ...
    @Override
    default <C extends PersistentCollection<? super T>> ImmutableList<C> grouped(int size,
                                                                                 Supplier<C> supplier) {
    ...
    @Override
    default <C extends PersistentCollection<? super T>> ImmutableList<C> groupedWhile(Predicate<? super T> predicate,
                                                                                      Supplier<C> factory) {
        return unitStream(stream().groupedWhile(predicate,
                                                factory));
    }
    ...
    @Override
    default <C extends PersistentCollection<? super T>> ImmutableList<C> groupedUntil(Predicate<? super T> predicate,
                                                                                      Supplier<C> factory) {
        return unitStream(stream().groupedUntil(predicate,
                                                factory));
    }
    ...
    @Override
    default <U> ImmutableList<U> scanLeft(U seed,
    ...
    default ImmutableList<T> scanRight(Monoid<T> monoid) {
        return scanRight(monoid.zero(),
                         monoid);
    }

    @Override
    default <U> ImmutableList<U> scanRight(U identity,
    ...
    @Override
    default ImmutableList<T> slice(long from,
                                   long to) {
    ...
    @Override
    <R> ImmutableList<R> mergeMap(int maxConcurecy,
    ...
    @Override
    default ImmutableList<T> insertAt(int pos,
                                      T... values) {
        if (pos == 0) {
        ...
        return unitIterable(IterableX.super.insertAt(pos,
                                                     values));
    }

    @Override
    default ImmutableList<T> deleteBetween(int start,
                                           int end) {
    ...
    @Override
    default ImmutableList<T> insertStreamAt(int pos,
                                            Stream<T> stream) {
    ...
    @Override
    default ImmutableList<T> updateAt(int pos,
                                      T value) {
        return unitIterable(IterableX.super.updateAt(pos,
                                                     value));
    }
    ...
    @Override
    default ImmutableList<T> insertAt(int i,
                                      T value) {
        return unitIterable(IterableX.super.insertAt(i,
                                                     value));
    }
