...
public interface Seq<T> extends ImmutableList<T>, Folds<T>, Filters<T>, Transformable<T>, PersistentList<T>, Serializable,
                                Higher<seq, T> {

    static <T> Collector<T, List<T>, Seq<T>> collector() {
        Collector<T, ?, List<T>> c = Collectors.toList();
        return Collectors.<T, List<T>, Iterable<T>, Seq<T>>collectingAndThen((Collector) c,
                                                                             Seq::fromIterable);
    }
    ...
    static <T, R> Seq<R> tailRec(T initial,
        ...
        return Seq.fromStream(Either.sequenceRight(next)
                                    .orElse(ReactiveSeq.empty()));
    }

    static <T> Seq<T> fill(T t,
                           int max) {
        return Seq.fromStream(ReactiveSeq.fill(t)
                                         .take(max));
    }

    static <U, T> Seq<T> unfold(final U seed,
                                final Function<? super U, Option<Tuple2<T, U>>> unfolder) {
        return fromStream(ReactiveSeq.unfold(seed,
                                             unfolder));
    }

    static <T> Seq<T> iterate(final T seed,
    ...
    static Seq<Integer> range(final int start,
                              final int end) {
        return Seq.fromStream(ReactiveSeq.range(start,
                                                end));
    ...
    static Seq<Long> rangeLong(final long start,
                               final long step,
                               final long end) {
        return Seq.fromStream(ReactiveSeq.rangeLong(start,
                                                    step,
                                                    end));
    }
    ...
    static <T> Seq<T> fromStream(Stream<T> stream) {
        ...
        return t.hasNext() ? cons(t.next(),
                                  fromIterator(t)) : empty();
    }
    ...
    default Seq<T> insertAt(final int i,
                            final T e) {
        return (Seq<T>) ImmutableList.super.insertAt(i,
                                                     e);
    }

    default Seq<T> insertAt(int i,
    ...
    @Override
    default Seq<T> insertAt(int pos,
                            T... values) {
        return (Seq<T>) ImmutableList.super.insertAt(pos,
                                                     values);
    }
    ...
    default Option<T> get(final int pos) {
        ...
        Seq<T> l = this;
        for (int i = 0; i < pos; i++) {
            l = l.foldSeq(c -> c.tail,
                          n -> n);
            if (l instanceof Nil) { //short circuit
                return Option.none();
            }
        }
        return Option.ofNullable(l.foldSeq(c -> c.head,
                                           n -> null));
    }
    ...
    default Seq<T> drop(final long num) {
        ...
        while (pos-- > 0 && !current.isEmpty()) {
            current = current.foldSeq(c -> c.tail,
                                      nil -> nil);
        }
        return current;
    }
    ...
    @Override
    default Seq<T> replaceFirst(T currentElement,
                                T newElement) {
        return (Seq<T>) ImmutableList.super.replaceFirst(currentElement,
                                                         newElement);
    }

    @Override
    default Seq<T> subList(int start,
                           int end) {
        return (Seq<T>) ImmutableList.super.subList(start,
                                                    end);
    }
    ...
    @Override
    default <T2, R> Seq<R> zip(BiFunction<? super T, ? super T2, ? extends R> fn,
                               Publisher<? extends T2> publisher) {
        return (Seq<R>) ImmutableList.super.zip(fn,
                                                publisher);
    }

    @Override
    default <U, R> Seq<R> zipWithStream(Stream<? extends U> other,
                                        BiFunction<? super T, ? super U, ? extends R> zipper) {
        return (Seq<R>) ImmutableList.super.zipWithStream(other,
                                                          zipper);
    }
    ...
    @Override
    default <S, U, R> Seq<R> zip3(Iterable<? extends S> second,
                                  Iterable<? extends U> third,
                                  Function3<? super T, ? super S, ? super U, ? extends R> fn3) {
        return (Seq<R>) ImmutableList.super.zip3(second,
                                                 third,
                                                 fn3);
    }

    @Override
    default <T2, T3, T4, R> Seq<R> zip4(Iterable<? extends T2> second,
                                        Iterable<? extends T3> third,
                                        Iterable<? extends T4> fourth,
                                        Function4<? super T, ? super T2, ? super T3, ? super T4, ? extends R> fn) {
        return (Seq<R>) ImmutableList.super.zip4(second,
                                                 third,
                                                 fourth,
                                                 fn);
    }
    ...
    @Override
    default Seq<T> combine(BiPredicate<? super T, ? super T> predicate,
                           BinaryOperator<T> op) {
        return (Seq<T>) ImmutableList.super.combine(predicate,
                                                    op);
    }

    @Override
    default Seq<T> combine(Monoid<T> op,
                           BiPredicate<? super T, ? super T> predicate) {
        return (Seq<T>) ImmutableList.super.combine(op,
                                                    predicate);
    }
    ...
    @Override
    default Seq<T> cycle(Monoid<T> m,
                         long times) {
        return (Seq<T>) ImmutableList.super.cycle(m,
                                                  times);
    }
    ...
    @Override
    default <U, R> Seq<R> zip(Iterable<? extends U> other,
                              BiFunction<? super T, ? super U, ? extends R> zipper) {
        return (Seq<R>) ImmutableList.super.zip(other,
                                                zipper);
    }

    @Override
    default <S, U> Seq<Tuple3<T, S, U>> zip3(Iterable<? extends S> second,
                                             Iterable<? extends U> third) {
        return (Seq) ImmutableList.super.zip3(second,
                                              third);
    }

    @Override
    default <T2, T3, T4> Seq<Tuple4<T, T2, T3, T4>> zip4(Iterable<? extends T2> second,
                                                         Iterable<? extends T3> third,
                                                         Iterable<? extends T4> fourth) {
        return (Seq) ImmutableList.super.zip4(second,
                                              third,
                                              fourth);
    }
    ...
    @Override
    default <C extends PersistentCollection<? super T>> Seq<C> grouped(int size,
                                                                       Supplier<C> supplier) {
        return (Seq<C>) ImmutableList.super.grouped(size,
                                                    supplier);
    }
    ...
    @Override
    default <C extends PersistentCollection<? super T>> Seq<C> groupedWhile(Predicate<? super T> predicate,
                                                                            Supplier<C> factory) {
        return (Seq<C>) ImmutableList.super.groupedWhile(predicate,
                                                         factory);
    }

    @Override
    default <C extends PersistentCollection<? super T>> Seq<C> groupedUntil(Predicate<? super T> predicate,
                                                                            Supplier<C> factory) {
        return (Seq<C>) ImmutableList.super.groupedUntil(predicate,
                                                         factory);
    }
    ...
    default <U> Seq<U> scanLeft(U seed,
        ...
        return (Seq<U>) ImmutableList.super.scanLeft(seed,
                                                     function);
    }
    ...
    @Override
    default <U> Seq<U> scanRight(U identity,
                                 BiFunction<? super T, ? super U, ? extends U> combiner) {
        return (Seq<U>) ImmutableList.super.scanRight(identity,
                                                      combiner);
    }
    ...
    default Seq<T> slice(long from,
                         long to) {
        return (Seq<T>) ImmutableList.super.slice(from,
                                                  to);
    }
    ...
    @Override
    <R> Seq<R> mergeMap(int maxConcurecy,
    ...
    @Override
    default Seq<T> deleteBetween(int start,
                                 int end) {
        return (Seq<T>) ImmutableList.super.deleteBetween(start,
                                                          end);
    }

    @Override
    default Seq<T> insertStreamAt(int pos,
                                  Stream<T> stream) {
        return (Seq<T>) ImmutableList.super.insertStreamAt(pos,
                                                           stream);
    }
    ...
    default T foldRight(Monoid<T> m) {
        return foldRight(m.zero(),
                         (T a, T b) -> m.apply(a,
                                               b));
    }

    default T foldLeft(Monoid<T> m) {
        return foldLeft(m.zero(),
                        (T a, T b) -> m.apply(a,
                                              b));
    }
    ...
    <R> R foldSeq(Function<? super Cons<T>, ? extends R> fn1,
    ...
    @Override
    default <R1, R2, R3, R> Seq<R> forEach4(Function<? super T, ? extends Iterable<R1>> iterable1,
        ...
        return (Seq<R>) ImmutableList.super.forEach4(iterable1,
                                                     iterable2,
                                                     iterable3,
                                                     yieldingFunction);
    }
    ...
    @Override
    default <R1, R2, R> Seq<R> forEach3(Function<? super T, ? extends Iterable<R1>> iterable1,
                                        BiFunction<? super T, ? super R1, ? extends Iterable<R2>> iterable2,
                                        Function3<? super T, ? super R1, ? super R2, ? extends R> yieldingFunction) {
        return (Seq<R>) ImmutableList.super.forEach3(iterable1,
                                                     iterable2,
                                                     yieldingFunction);
    }
    ...
    @Override
    default <R1, R> Seq<R> forEach2(Function<? super T, ? extends Iterable<R1>> iterable1,
                                    BiFunction<? super T, ? super R1, ? extends R> yieldingFunction) {
        return (Seq<R>) ImmutableList.super.forEach2(iterable1,
                                                     yieldingFunction);
    }
    ...
    default Either<Integer, Seq<T>> set(int pos,
                                        T value) {
        if (pos < 0 || pos >= size()) {
    ...
    public static final class Cons<T> implements ImmutableList.Some<T>, Seq<T>, Serializable {
        ...
        public static <T> Cons<T> cons(T value,
                                       Seq<T> tail) {
        ...
        public Seq<T> updateAt(int i,
            ...
            Seq<T> replaced = tail.updateAt(i - 1,
                                            value);
        ...
        public Seq<T> insertAt(final int i,
                               final T value) {
            if (i == 0) {
                return prepend(value);
            }
            ...
            return Seq.super.insertAt(i,
                                      value);

        }

        @Override
        public Seq<T> insertAt(int i,
                               Iterable<? extends T> list) {
            if (i == 0) {
        ...
        @Override
        public <R> R foldSeq(Function<? super Cons<T>, ? extends R> fn1,
                             Function<? super Nil, ? extends R> fn2) {
        ...
        @Override
        public <R> Seq<R> mergeMap(int maxConcurecy,
                                   Function<? super T, ? extends Publisher<? extends R>> fn) {
