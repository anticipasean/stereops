...
import cyclops.function.hkt.DataWitness.diet;
import cyclops.function.hkt.Higher;
import cyclops.matching.Deconstruct.Deconstruct3;
...
public interface DIET<T> extends Sealed2<DIET.Node<T>, DIET.Nil<T>>, Iterable<T>, Higher<diet, T>, Folds<T> {
    ...
    default DIET<T> add(T value,
                        Enumeration<T> enm,
                        Comparator<? super T> comp) {
        return enm.succ(value)
                  .fold(s -> {
    ...
    <R> DIET<R> map(Function<? super T, ? extends R> fn,
                    Enumeration<R> enm,
    ...
    public static final class Node<T> implements DIET<T>, Deconstruct3<DIET<T>, Range<T>, DIET<T>>, Serializable {
        ...
        private static <T> DIET<T> merge(DIET<T> l,
                                         DIET<T> r) {
            if (r.isEmpty()) {
                return l;
            }
            ...
            Node<T> left = l.fold(i -> i,
                                  n -> null);
            Node<T> right = r.fold(i -> i,
                                   n -> null);
        ...
        private Tuple2<Range<T>, DIET<T>> max() {
            return right.fold(s -> s.max()
                                    .transform((r, d) -> tuple(r,
                                                               cons(left,
                                                                    focus,
                                                                    d))),
                              n -> tuple(focus,
        ...
        private Trampoline<Tuple2<DIET<T>, T>> findRightAndEndingPoint(T value) {
            ...
            if (focus.ordering()
                     .isLessThanOrEqual(value,
                                        focus.end)) {
                return done(tuple(right,
                                  focus.end));
            }
            //             value
            //                                  end
            //                    start              (split rightwad diet recursively)
            return left.fold(p -> p.findRightAndEndingPoint(value),
                             leftNil -> done(tuple(leftNil,
                                                   value)));
        ...
        private Trampoline<Tuple2<DIET<T>, T>> findLeftAndStartingPoint(T value) {
            ...
            if (focus.ordering()
                     .isGreaterThanOrEqual(value,
                                           focus.start)) {
                return done(tuple(left,
                                  focus.start));
            }
            //             value
            //                                  end
            //   <--     left         --> start               (split leftward diet recursively)
            return left.fold(p -> p.findLeftAndStartingPoint(value),
                             leftNil -> done(tuple(leftNil,
                                                   value)));
        ...
        public DIET<T> add(Range<T> range) {
            ...
            return t._2()
                    .fold(s -> t._1() == focus ? cons(left,
                                                      focus,
                                                      right.add(s)) : cons(left.add(s),
                                                                           focus,
                                                                           right),
                          () -> {

                              //create new expanded range and rebalance the trees
                              Tuple2<DIET<T>, T> leftAndStart = left.fold(l -> l.findLeftAndStartingPoint(t._1().start)
                                                                                .get(),
                                                                          n -> tuple(n,
                                                                                     t._1().start));
                              Tuple2<DIET<T>, T> rightAndEnd = right.fold(l -> l.findRightAndEndingPoint(t._1().end)
                                                                                .get(),
                                                                          n -> tuple(n,
                                                                                     t._1().start));

                              return cons(leftAndStart._1(),
                                          Range.range(leftAndStart._2(),
                                                      rightAndEnd._2(),
                                                      focus.enumeration(),
                                                      focus.ordering()),
                                          rightAndEnd._1());
        ...
        public DIET<T> remove(T value) {
            return focus.enumeration()
                        .succ(value)
                        .fold(s -> {
                                  return remove(Range.range(value,
                                                            s,
                                                            focus.enumeration(),
                                                            focus.ordering()));
                              },
        ...
        public DIET<T> remove(Range<T> range) {
            ...
            return x.fold(s -> s.transform((r, mr) -> mr.fold(sr -> {
                                                              ...
                                                              () -> cons(range.startsBefore(focus) ? left.remove(range) : left,
                                                                         r,
                                                                         range.endsAfter(focus) ? right.remove(range) : right))),
                          //none
                          () -> merge(left,
                                      right));
            //
        ...
        @Override
        public <R> DIET<R> map(Function<? super T, ? extends R> fn,
                               Enumeration<R> enm,
                               Comparator<? super R> comp) {
            Range<R> r = focus.map(fn,
                                   enm,
                                   comp);
            DIET<R> l2 = left.map(fn,
                                  enm,
                                  comp);
            return merge(l2.add(r),
                         right.map(fn,
                                   enm,
                                   comp));
        ...
        public LazySeq<T> lazySeq() {
            return left.lazySeq()
                       .append(() -> focus.lazySeq())
                       .append(() -> right.lazySeq());
        }

        @Override
        public ReactiveSeq<T> stream() {
            return left.stream()
                       .appendStream(focus.stream())
                       .appendStream(right.stream());
        }

        @Override
        public ReactiveSeq<Range<T>> streamRanges() {
            return left.streamRanges()
                       .append(focus)
                       .appendStream(right.streamRanges());
        }
        ...
        @Override
        public <R> R fold(Function<? super Node<T>, ? extends R> fn1,
