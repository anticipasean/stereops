...
import cyclops.function.hkt.DataWitness.nonEmptyList;
...
public class NonEmptyList<T> implements Deconstruct2<T, ImmutableList<T>>, ImmutableList<T>, ImmutableList.Some<T>,
    ...
    public static <T> NonEmptyList<T> cons(T value,
    ...
    @Override
    public NonEmptyList<T> replaceFirst(T currentElement,
                                        T newElement) {
        return ImmutableList.Some.super.replaceFirst(currentElement,
                                                     newElement)
                                       .fold(cons -> cons(cons.head(),
                                                          cons.tail()),
                                             nil -> this);
    }

    @Override
    public NonEmptyList<T> insertAt(int pos,
                                    T... values) {
        return (NonEmptyList<T>) ImmutableList.Some.super.insertAt(pos,
                                                                   values);
    }
    ...
    @Override
    public NonEmptyList<T> updateAt(int pos,
                                    T value) {
        return (NonEmptyList<T>) ImmutableList.Some.super.updateAt(pos,
                                                                   value);
    }
    ...
    @Override
    public NonEmptyList<T> insertAt(int i,
                                    T value) {
        return (NonEmptyList<T>) ImmutableList.Some.super.insertAt(i,
                                                                   value);
    }
    ...
    public NonEmptyList<T> prependAll(Iterable<? extends T> value) {
        ...
        return cons(list.fold(c -> c.head(),
                              nil -> null),
                    list.drop(1));
    }
    ...
    public <R> NonEmptyList<R> map(Function<? super T, ? extends R> fn) {
        return NonEmptyList.of(fn.apply(head),
                               tail.map(fn));
    }
    ...
    @Override
    public <C extends PersistentCollection<? super T>> NonEmptyList<C> grouped(int size,
                                                                               Supplier<C> supplier) {
        return (NonEmptyList<C>) ImmutableList.Some.super.grouped(size,
                                                                  supplier);
    }
    ...
    @Override
    public <C extends PersistentCollection<? super T>> NonEmptyList<C> groupedWhile(Predicate<? super T> predicate,
                                                                                    Supplier<C> factory) {
        return (NonEmptyList<C>) ImmutableList.Some.super.groupedWhile(predicate,
                                                                       factory);
    }

    @Override
    public <C extends PersistentCollection<? super T>> NonEmptyList<C> groupedUntil(Predicate<? super T> predicate,
                                                                                    Supplier<C> factory) {
        return (NonEmptyList<C>) ImmutableList.Some.super.groupedUntil(predicate,
                                                                       factory);
    }
    ...
    @Override
    public <U> NonEmptyList<U> scanLeft(U seed,
                                        BiFunction<? super U, ? super T, ? extends U> function) {
        return (NonEmptyList<U>) ImmutableList.Some.super.scanLeft(seed,
                                                                   function);
    }
    ...
    @Override
    public <U> NonEmptyList<U> scanRight(U identity,
                                         BiFunction<? super T, ? super U, ? extends U> combiner) {
        return (NonEmptyList<U>) ImmutableList.Some.super.scanRight(identity,
                                                                    combiner);
    }
    ...
    @Override
    public NonEmptyList<T> insertStreamAt(int pos,
                                          Stream<T> stream) {
        return (NonEmptyList<T>) ImmutableList.Some.super.insertStreamAt(pos,
                                                                         stream);
    }
