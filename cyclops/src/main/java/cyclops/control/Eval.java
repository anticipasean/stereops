...
/**
 * Represents a computation that can be deferred (always), cached (later) or immediate(now). Supports tail recursion via transform
 * / flatMap. Eval are always Lazy even when performed against a Now instance. Heavily inspired by Cats Eval @link
 ...
 * @author johnmcclean
 */
public interface Eval<T> extends To<Eval<T>>, Function0<T>, Deconstruct1<T>, MonadicValue<T>, Higher<eval, T> {
    ...
    static <T> Eval<T> async(final Executor ex,
                             final Supplier<T> s) {
    ...
    static <T> CompletableEval<T, T> eval() {
        ...
        return new CompletableEval<T, T>(c,
    ...
    /**
     * Lazily create an Eval from the specified Supplier. Supplier#getValue will only be called once. Return values of Eval
     * operations will also be cached (later indicates maybe and caching - characteristics can be changed using flatMap).
     *
     * <pre>
     * {@code
    ...
    /**
     * Lazily create an Eval from the specified Supplier. Supplier#getValue will only be every time getValue is called on the
     * resulting Eval.
     *
     * <pre>
     * {@code
    ...
    public static <T> Eval<ReactiveSeq<T>> sequence(ReactiveSeq<? extends Eval<T>> stream) {
        ...
        BiFunction<Eval<ReactiveSeq<T>>, Eval<T>, Eval<ReactiveSeq<T>>> combineToStream = (acc, next) -> acc.zipWith(next,
                                                                                                                     (a, b) -> a.append(b));

        BinaryOperator<Eval<ReactiveSeq<T>>> combineStreams = (a, b) -> a.zipWith(b,
    ...
    default <B, R> Eval<R> zip(Supplier<B> b,
                               BiFunction<? super T, ? super B, ? extends R> zipper) {
        Trampoline<B> tb = b instanceof Trampoline ? (Trampoline<B>) b : eval(b).toTrampoline();
        return Eval.later(toTrampoline().zip(tb,
                                             zipper));
    }

    default <B, C, R> Eval<R> zip(Supplier<B> b,
                                  Supplier<C> c,
                                  Function3<? super T, ? super B, ? super C, ? extends R> zipper) {
        Trampoline<B> tb = b instanceof Trampoline ? (Trampoline<B>) b : eval(b).toTrampoline();
    ...
    default ReactiveSeq<T> streamUntil(long time,
                                       TimeUnit unit) {
        return ReactiveSeq.generate(this)
                          .take(time,
                                unit);
    ...
    @Deprecated
    default Eval<T> combineEager(final Monoid<T> monoid,
                                 final MonadicValue<? extends T> v2) {
        return unit(this.forEach2(t1 -> v2,
                                  (t1, t2) -> monoid.apply(t1,
                                                           t2))
    ...
    default Eval<T> onErrorRestart(long retries) {
        return recoverWith(Throwable.class,
                           t -> {
                               if (retries > 0) {
    ...
    default void forEach(Consumer<? super T> onNext,
    ...
    default Eval<T> recover(Function<Throwable, ? extends T> value) {
        return recover(Throwable.class,
                       value);

    }

    default <C extends Throwable> Eval<T> recover(Class<C> type,
                                                  Function<? super C, ? extends T> value) {
    ...
    @Override
    default <R> R fold(final Function<? super T, ? extends R> present,
    ...
    default <T2, R> Eval<R> zipWith(final Iterable<? extends T2> app,
                                    final BiFunction<? super T, ? super T2, ? extends R> fn) {
        return fromIterable(ReactiveSeq.fromIterable(this)
                                       .zip(app,
                                            fn));
    }

    default <T2, R> Eval<R> zipWith(final BiFunction<? super T, ? super T2, ? extends R> fn,
                                    final Publisher<? extends T2> app) {
        return mergeMap(a -> Eval.fromPublisher(app)
                                 .map(b -> fn.apply(a,
                                                    b)));
    }
    ...
    default <T2, R1, R2, R3, R> Eval<R> forEach4(Function<? super T, ? extends MonadicValue<R1>> value1,
        ...
        return (Eval<R>) MonadicValue.super.forEach4(value1,
                                                     value2,
                                                     value3,
                                                     yieldingFunction);
    }
    ...
    default <T2, R1, R2, R> Eval<R> forEach3(Function<? super T, ? extends MonadicValue<R1>> value1,
        ...
        return (Eval<R>) MonadicValue.super.forEach3(value1,
                                                     value2,
                                                     yieldingFunction);
    }
    ...
    default <R1, R> Eval<R> forEach2(Function<? super T, ? extends MonadicValue<R1>> value1,
        ...
        return (Eval<R>) MonadicValue.super.forEach2(value1,
                                                     yieldingFunction);
    }
    ...
    static class CompletableEval<ORG, T2> implements Eval<T2>, Completable<ORG> {
        ...
        public ReactiveSeq<T2> streamUntil(long time,
                                           TimeUnit unit) {
            return Spouts.generate(this)
                         .take(time,
                               unit);
    ...
    static class Module {
        ...
        public static class Later<T> implements Eval<T> {
            ...
            Later(Rec<?, T> in) {
                memo = Memoize.memoizeSupplier(() -> in.toTrampoline()
                                                       .get());
            ...
            public <R> Eval<R> flatMap(final Function<? super T, ? extends MonadicValue<? extends R>> mapper) {
                return new Later<R>(new Rec<T, R>(this,
                                                  Memoize.memoizeFunction(mapper)));
            }
        ...
        public static class Always<T> implements Eval<T> {
            ...
            @Override
            public <R> Eval<R> flatMap(final Function<? super T, ? extends MonadicValue<? extends R>> mapper) {
                Rec<T, R> rec = new Rec<T, R>(this,
                                              mapper);
                return new Always<R>(rec);
            }
        ...
        public static class FutureAlways<T> implements Eval<T> {
            ...
            public Eval<T> recover(Function<Throwable, ? extends T> value) {
                ...
                toFuture().forEach(t -> res.complete(t),
            ...
            public <C extends Throwable> Eval<T> recover(Class<C> type,
                                                         Function<? super C, ? extends T> value) {
                CompletableEval<T, T> res = eval();
                toFuture().forEach(t -> res.complete(t),
                                   e -> {
            ...
            public <C extends Throwable> Eval<T> recoverWith(Class<C> type,
                                                             Function<? super C, ? extends Eval<T>> value) {
                CompletableEval<Eval<T>, Eval<T>> res = eval();
                toFuture().forEach(t -> res.complete(Eval.now(t)),
                                   e -> {
                                       if (type.isAssignableFrom(e.getClass())) {
                                           res.complete(value.apply((C) e));
                                       }
            ...
            public ReactiveSeq<T> streamUntil(long time,
                                              TimeUnit unit) {
                return Spouts.generate(this)
                             .take(time,
                                   unit);
            ...
            public ReactiveSeq<T> iterate(UnaryOperator<T> fn) {
                return Spouts.from(input)
                             .map(Eval::get)
                             .flatMap(i -> Spouts.iterate(i,
                                                          fn));
            }
            ...
            public T get() {
                ...
                Eval<T> eval = input.fold(i -> i,
                                          e -> {
                                              throw ExceptionSoftener.throwSoftenedException(e.getCause());
                                          });
                return eval.get();
            }
        ...
        private static class Rec<T, R> {
            ...
            public Trampoline<R> toTrampoline() {
                Trampoline<? extends R> x = Trampoline.more(() -> {
                    Trampoline<? extends R> t = eval.toTrampoline()
                                                    .flatMap(v -> {
                                                        Trampoline<? extends R> t2 = Eval.fromIterable(fn.apply(v))
                                                                                         .toTrampoline();
                                                        return t2;
                                                    });
                    return t;
        ...
        public static <T, F, R1, R2, R3, R4, R5, R6, R7> Eval<R7> forEach(Eval<T> eval,
            ...
            return eval.flatMap(in -> {
                ...
                return a.flatMap(ina -> {
                    Eval<R2> b = value3.apply(Tuple.tuple(in,
                                                          ina));
                    return b.flatMap(inb -> {

                        Eval<R3> c = value4.apply(Tuple.tuple(in,
                                                              ina,
                                                              inb));

                        return c.flatMap(inc -> {
                            Eval<R4> d = value5.apply(Tuple.tuple(in,
                                                                  ina,
                                                                  inb,
                                                                  inc));
                            return d.flatMap(ind -> {
                                Eval<R5> e = value6.apply(Tuple.tuple(in,
                                                                      ina,
                                                                      inb,
                                                                      inc,
                                                                      ind));
                                return e.flatMap(ine -> {
                                    Eval<R6> f = value7.apply(Tuple.tuple(in,
                                                                          ina,
                                                                          inb,
                                                                          inc,
                                                                          ind,
                                    ...
                                    return f.flatMap(inf -> {
                                        Eval<R7> g = value8.apply(Tuple.tuple(in,
                                                                              ina,
                                                                              inb,
                                                                              inc,
                                                                              ind,
