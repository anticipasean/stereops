...
import cyclops.stream.spliterator.DistinctKeySpliterator;
import cyclops.stream.spliterator.DistinctSpliterator;
import cyclops.stream.spliterator.FilteringSpliterator;
import cyclops.stream.spliterator.ForEachWithError;
import cyclops.stream.spliterator.FunctionSpliterator;
...
import cyclops.stream.spliterator.IterableFlatMappingSpliterator;
import cyclops.stream.spliterator.LazyFilteringSpliterator;
import cyclops.stream.spliterator.LazyMappingSpliterator;
import cyclops.stream.spliterator.LazySingleSpliterator;
import cyclops.stream.spliterator.LimitLastSpliterator;
import cyclops.stream.spliterator.LimitSpliterator;
import cyclops.stream.spliterator.LimitWhileClosedSpliterator;
import cyclops.stream.spliterator.LimitWhileSpliterator;
import cyclops.stream.spliterator.LimitWhileTimeSpliterator;
import cyclops.stream.spliterator.MappingSpliterator;
import cyclops.stream.spliterator.OnEmptyGetSpliterator;
import cyclops.stream.spliterator.OnEmptySpliterator;
import cyclops.stream.spliterator.OnEmptyThrowSpliterator;
import cyclops.stream.spliterator.OnErrorBreakWithPublisherSpliterator;
import cyclops.stream.spliterator.RecoverSpliterator;
import cyclops.stream.spliterator.ReversableSpliterator;
import cyclops.stream.spliterator.ScanLeftSpliterator;
import cyclops.stream.spliterator.SingleSpliterator;
import cyclops.stream.spliterator.SkipLastSpliterator;
import cyclops.stream.spliterator.SkipSpliterator;
import cyclops.stream.spliterator.SkipWhileClosedSpliterator;
...
import cyclops.stream.spliterator.StreamFlatMappingSpliterator;
import cyclops.stream.spliterator.Zipping3Spliterator;
import cyclops.stream.spliterator.Zipping4Spliterator;
...
public abstract class SpliteratorBasedStream<T> extends BaseExtendedStream<T> {
    ...
    public SpliteratorBasedStream(final Spliterator<T> stream,
    ...
    public SpliteratorBasedStream<T> onComplete(final Runnable fn) {
        return (SpliteratorBasedStream<T>) this.createSeq(new CompleteSpliterator<>(stream,
                                                                                    fn));
    }
    ...
    public <R> ReactiveSeq<R> coflatMap(Function<? super ReactiveSeq<T>, ? extends R> fn) {
        return ReactiveSeq.fromSpliterator(new LazySingleSpliterator<T, ReactiveSeq<T>, R>(createSeq(copy()),
                                                                                           fn));
    ...
    public <A, R> ReactiveSeq<R> collectSeq(Collector<? super T, A, R> c) {
        ...
        CollectingSinkSpliterator<T, A, R> fs = new CollectingSinkSpliterator<T, A, R>(s.estimateSize(),
                                                                                       s.characteristics(),
                                                                                       s,
                                                                                       c);

        return createSeq(new ValueEmittingSpliterator<R>(1,
                                                         s.characteristics(),
                                                         createSeq(fs)));
    ...
    public Stream<T> unwrapStream() {
        ...
        return StreamSupport.stream(copy(),
                                    false);
    ...
    public final <S> ReactiveSeq<Tuple2<T, S>> zipWithStream(final Stream<? extends S> second) {
        return createSeq(new ZippingSpliterator<>(get(),
                                                  second.spliterator(),
                                                  (a, b) -> new Tuple2<>(a,
                                                                         b)));
    }
    ...
    @Override
    public final <U, R> ReactiveSeq<R> zipWithStream(final Stream<? extends U> other,
    ...
    public final <S, U, R> ReactiveSeq<R> zip3(final Iterable<? extends S> second,
        ...
        return createSeq(new Zipping3Spliterator<>(get(),
                                                   second.spliterator(),
                                                   third.spliterator(),
                                                   fn3));
    }
    ...
    @Override
    public final <T2, T3, T4> ReactiveSeq<Tuple4<T, T2, T3, T4>> zip4(final Iterable<? extends T2> second,
    ...
    public final ReactiveSeq<Seq<T>> sliding(final int windowSize,
                                             final int increment) {
        return createSeq(new SlidingSpliterator<>(get(),
                                                  Function.identity(),
                                                  windowSize,
                                                  increment),
                         reversible);
    ...
    public ReactiveSeq<Vector<T>> grouped(final int groupSize) {
        return createSeq(new GroupingSpliterator<T, Vector<T>, Vector<T>>(get(),
                                                                          () -> Vector.empty(),
                                                                          c -> c,
                                                                          groupSize),
                         this.reversible);
    ...
    public ReactiveSeq<Vector<T>> groupedWhile(final BiPredicate<Vector<? super T>, ? super T> predicate) {
        return createSeq(new GroupedStatefullySpliterator<>(get(),
                                                            () -> Vector.empty(),
                                                            Function.identity(),
                                                            predicate),
                         this.reversible);
    ...
    public final ReactiveSeq<T> distinct() {
        return createSeq(new DistinctSpliterator<T, T>(get()),
                         reversible);
    }
    ...
    public ReactiveSeq<T> skip(final long num) {
        return createSeq(new SkipSpliterator<>(get(),
                                               num),
                         reversible);
    }
    ...
    public final ReactiveSeq<T> dropWhile(final Predicate<? super T> p) {
        return createSeq(new SkipWhileSpliterator<T>(get(),
                                                     p),
                         reversible);
    }
    ...
    public final ReactiveSeq<T> dropWhileInclusive(final Predicate<? super T> p) {
        return createSeq(new SkipWhileClosedSpliterator<T>(get(),
                                                           p),
                         reversible);
    }
    ...
    public ReactiveSeq<T> limit(final long num) {
        ...
        return createSeq(new LimitSpliterator<T>(get(),
                                                 num),
                         reversible);
    }
    ...
    public final ReactiveSeq<T> takeWhile(final Predicate<? super T> p) {
        return createSeq(new LimitWhileSpliterator<T>(get(),
                                                      p),
                         reversible);
    }
    ...
    public final ReactiveSeq<T> takeWhileInclusive(final Predicate<? super T> p) {
        return createSeq(new LimitWhileClosedSpliterator<T>(get(),
                                                            p),
                         reversible);
    }
    ...
    public final Optional<T> reduce(final BinaryOperator<T> accumulator) {
        ...
        stream.forEachRemaining(e -> {
            if (result[0] == null) {
                ...
                result[0] = accumulator.apply((T) result[0],
                                              e);
            }
        });
    ...
    @Override
    public final T reduce(final T identity,
    ...
    public final <R> ReactiveSeq<R> map(final Function<? super T, ? extends R> fn) {
        ...
        if (this.stream instanceof ComposableFunction) {
            ...
            return createSeq(f.compose(fn),
                             reversible);
        }
        return createSeq(new MappingSpliterator<T, R>(this.get(),
                                                      fn),
                         reversible);
    }
    ...
    public final <R> ReactiveSeq<R> flatMap(final Function<? super T, ? extends Stream<? extends R>> fn) {
        if (this.stream instanceof FunctionSpliterator) {
            ...
            return createSeq(StreamFlatMappingSpliterator.compose(f,
                                                                  fn),
                             reversible);
        }
        return createSeq(new StreamFlatMappingSpliterator<>(get(),
                                                            fn),
                         Optional.empty());
    ...
    public final <R> ReactiveSeq<R> concatMap(final Function<? super T, ? extends Iterable<? extends R>> fn) {
        if (this.stream instanceof FunctionSpliterator) {
            FunctionSpliterator f = (FunctionSpliterator) stream;
            return createSeq(IterableFlatMappingSpliterator.compose(f,
                                                                    fn),
                             reversible);
        }

        return createSeq(new IterableFlatMappingSpliterator<>(get(),
                                                              fn),
                         Optional.empty());
    ...
    public <R> ReactiveSeq<R> mergeMap(final int maxConcurrency,
        ...
        return Spouts.fromIterable(this)
                     .mergeMap(maxConcurrency,
                               mapper);
    }


    abstract <X> ReactiveSeq<X> createSeq(Stream<X> stream,
                                          Optional<ReversableSpliterator> reversible);

    abstract <X> ReactiveSeq<X> createSeq(Spliterator<X> stream,
                                          Optional<ReversableSpliterator> reversible);

    <X> ReactiveSeq<X> createSeq(Spliterator<X> stream) {

        return createSeq(stream,
                         Optional.empty());
    ...
    public final ReactiveSeq<T> filter(final Predicate<? super T> fn) {
        return createSeq(new FilteringSpliterator<T>(get(),
                                                     fn).compose(),
                         reversible);
    ...
    public final ReactiveSeq<T> filterLazyPredicate(final Supplier<Predicate<? super T>> fn) {
        return createSeq(new LazyFilteringSpliterator<T>(get(),
                                                         fn),
                         reversible);
    ...
    public ReactiveSeq<T> onEmpty(final T value) {
        return createSeq(new OnEmptySpliterator<>(stream,
                                                  value));
    ...
    public ReactiveSeq<T> onEmptyGet(final Supplier<? extends T> supplier) {
        return createSeq(new OnEmptyGetSpliterator<>(stream,
                                                     supplier));
    }

    @Override
    public <X extends Throwable> ReactiveSeq<T> onEmptyError(final Supplier<? extends X> supplier) {
        return createSeq(new OnEmptyThrowSpliterator<>(stream,
                                                       supplier));
    }
    ...
    public ReactiveSeq<T> prependStream(final Stream<? extends T> other) {
        return ReactiveSeq.concat(avoidCopy(other),
                                  get());
    }
    ...
    public ReactiveSeq<T> prepend(final T other) {
        return ReactiveSeq.concat(new SingleSpliterator<T>(other),
                                  get());
    }

    @Override
    public ReactiveSeq<T> prependAll(final T... other) {
        return ReactiveSeq.concat(ReactiveSeq.of(other)
                                             .spliterator(),
                                  get());
    }
    ...
    public <U> ReactiveSeq<T> distinct(final Function<? super T, ? extends U> keyExtractor) {
        return createSeq(new DistinctKeySpliterator<>(keyExtractor,
                                                      stream),
                         reversible);
    }
    ...
    public <R> ReactiveSeq<R> mapLazyFn(Supplier<Function<? super T, ? extends R>> fn) {
        ...
        return createSeq(new LazyMappingSpliterator<T, R>(this.get(),
                                                          fn),
                         reversible);
    ...
    public ReactiveSeq<T> drop(final long time,
                               final TimeUnit unit) {
        return createSeq(new SkipWhileTimeSpliterator<T>(get(),
                                                         time,
                                                         unit),
                         this.reversible);
    }
    ...
    public ReactiveSeq<T> take(final long time,
                               final TimeUnit unit) {
        return createSeq(new LimitWhileTimeSpliterator<T>(get(),
                                                          time,
                                                          unit),
                         reversible);
    ...
    @Override
    public <C extends PersistentCollection<? super T>> ReactiveSeq<C> groupedUntil(final Predicate<? super T> predicate,
                                                                                   final Supplier<C> factory) {
        return groupedWhile(predicate.negate(),
                            factory);
    }
    ...
    public <C extends PersistentCollection<? super T>, R> ReactiveSeq<R> groupedByTime(final long time,
                                                                                       ...
                                                                                       final Supplier<C> factory,
                                                                                       Function<? super C, ? extends R> finalizer) {
        return createSeq(new GroupedByTimeSpliterator(this.get(),
                                                      factory,
                                                      finalizer,
                                                      time,
                                                      unit),
    ...
    public <C extends PersistentCollection<? super T>> ReactiveSeq<C> grouped(final int size,
                                                                              final Supplier<C> factory) {
        return createSeq(new GroupingSpliterator<>(get(),
                                                   factory,
                                                   Function.identity(),
                                                   size),
                         this.reversible);
    ...
    public ReactiveSeq<T> dropRight(final int num) {
        return createSeq(SkipLastSpliterator.dropRight(get(),
                                                       num < 0 ? 0 : num),
                         this.reversible);
    }
    ...
    public ReactiveSeq<T> takeRight(final int num) {
        return createSeq(LimitLastSpliterator.takeRight(get(),
                                                        num < 0 ? 0 : num),
                         this.reversible);
    }
    ...
    public ReactiveSeq<T> recover(final Function<? super Throwable, ? extends T> fn) {
        return createSeq(new RecoverSpliterator<T, Throwable>(get(),
                                                              fn,
                                                              Throwable.class),
                         this.reversible);
    }
    ...
    public ReactiveSeq<T> recoverWith(final Function<Throwable, ? extends Publisher<? extends T>> fn) {
        return createSeq(new OnErrorBreakWithPublisherSpliterator<>(get(),
                                                                    fn),
                         this.reversible).flatMap(ReactiveSeq::fromPublisher);
    }

    @Override
    public <EX extends Throwable> ReactiveSeq<T> recover(final Class<EX> exceptionClass,
                                                         final Function<? super EX, ? extends T> fn) {
        return createSeq(new RecoverSpliterator<T, EX>(get(),
                                                       fn,
                                                       exceptionClass),
                         this.reversible);
    }
    ...
    public <X extends Throwable> void forEach(final Consumer<? super T> consumerElement,
        ...
        new ForEachWithError<T>(this.copy(),
                                consumerError).forEachRemaining(consumerElement);
    ...
    public <X extends Throwable> void forEach(final Consumer<? super T> consumerElement,
        ...
        new ForEachWithError<T>(this.copy(),
                                consumerError,
                                onComplete).forEachRemaining(consumerElement);
    }
