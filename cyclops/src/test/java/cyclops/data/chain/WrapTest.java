...
public class WrapTest extends BaseImmutableListTest {
    ...
    public IterableX<Integer> range(int start,
                                    int end) {
        return Chain.wrap(LazySeq.range(start,
                                        end));
    }
    ...
    public IterableX<Long> rangeLong(long start,
                                     long end) {
        return Chain.wrap(ReactiveSeq.rangeLong(start,
                                                end));
    }
    ...
    public <T> IterableX<T> iterate(int times,
                                    T seed,
                                    UnaryOperator<T> fn) {
        return Chain.wrap(ReactiveSeq.iterate(seed,
                                              fn)
                                     .take(times));
    }
    ...
    public <T> IterableX<T> generate(int times,
                                     Supplier<T> fn) {
        return Chain.wrap(ReactiveSeq.generate(fn)
                                     .take(times));
    }
    ...
    public <U, T> IterableX<T> unfold(U seed,
        ...
        return Chain.wrap(ReactiveSeq.unfold(seed,
                                             unfolder));
    }
    ...
    public void removeValue2() {
        ...
        System.out.println("FS" + fromStream(vec.stream()
