...
public class OptionalMatchingTest {
    ...
    public void shouldMatchOptionalPresent() {
        Assert.assertEquals((Long) 1L,
                            new Matching.OptionalMatching<>(of("present")).of(new Case.CaseOptional<>(() -> 1L,
                                                                                                      () -> 2L)));
    }
    ...
    public void shouldMatchOptionalAbsent() {
        Assert.assertEquals((Long) 2L,
                            new Matching.OptionalMatching<>(empty()).of(new Case.CaseOptional<>(() -> 1L,
                                                                                                () -> 2L)));
    }
    ...
    public void matchOption() {
        ...
        int res = MatchType(empty).with(Case(value -> value),
                                        Case(nil -> defaultValue));

        //res is -1

        int res2 = empty.fold(v -> v,
                              n -> -1);
