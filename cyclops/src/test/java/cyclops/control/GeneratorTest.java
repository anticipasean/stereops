...
import static cyclops.reactive.Generator.GeneratorFunction;
import static cyclops.reactive.Generator.Suspended;
import static cyclops.reactive.Generator.suspend;
import static cyclops.reactive.Generator.suspendRef;
...
public class GeneratorTest {
    ...
    public void suspendLoop() {
        assertThat(ReactiveSeq.fromIterable(suspend(times(100),
                                                    s -> i < 5 ? s.yield(i++) : s.yieldAndStop(i)))
                              .toList(),
    ...
    public void sequenceWithFunction() {
        assertThat(ReactiveSeq.fromIterable(suspend((Integer i) -> i != 4,
                                                    s -> {
    ...
    public void sequenceWithMaybe() {
        assertThat(ReactiveSeq.generate(suspend((Integer i) -> i != 4,
                                                s -> {
                                                    System.out.println("Top level - repeat 2 times after sequence!");
                                                    return s.yield(1,
                                                                   () -> s.yield(s.maybe()
    ...
    public void suspendRefTest() {
        Generator<Integer> generator = suspendRef(times(10),
                                                  this::next);

        assertThat(generator.stream()
                            .toList(),
                   equalTo(Arrays.asList(0,
                                         1,
                                         2,
    ...
    @Test
    public void stream() {
        assertThat(ReactiveSeq.generate(suspend(times(2),
                                                s2 -> s2.yield(i++)))
                              .toList(),
                   equalTo(Arrays.asList(0,
                                         1)));
    ...
    public void nested() {
        ...
        assertThat(ReactiveSeq.generate(suspend((Integer i) -> i != 4,
                                                s -> {
                                                    System.out.println("Top level - repeat 2 times after sequence!");
                                                    Generator<Integer> gen = suspend(times(2).before(() -> i = i * 2),
                                                                                     s2 -> s2.yield(i++));
                                                    return s.yieldAll(1,
                                                                      gen.stream());
                                                }))
    ...
    public void nestedInPeek() {
        assertThat(ReactiveSeq.fromIterable(suspend(times(10),
                                                    s -> s.yield(i++)))
                              .peek(i -> {
