...
public class AsyncRSFlatMapPTest {
    ...
    protected ReactiveSeq<Integer> range(int start,
                                         int end) {
        return Spouts.from(Flux.range(start,
                                      end)
                               .subscribeOn(Schedulers.fromExecutor(ForkJoinPool.commonPool())));
    ...
    protected ReactiveSeq<Integer> empty() {
        return Spouts.from(Flux.from(Spouts.<Integer>empty())
                               .subscribeOn(Schedulers.fromExecutor(ForkJoinPool.commonPool())));
    ...
    public void fluxTest() {
        List<String> list = flux(1,
                                 2,
                                 3,
                                 4).flatMap(i -> flux(10,
                                                      20,
                                                      30).flatMap(j -> flux("hello" + j,
                                                                            "world"),
                                                                  4),
                                            3)
                                                 ...
                                                 i -> of2(i,
                                                          ...
                                                          9).mergeMap(4,
                                                                      x -> of2(5,
                                                                               ...
                                                                               9)))
                                       .iterator();
            assertThat("Iteration " + l,
                       ReactiveSeq.fromIterator(it)
                                  .size(),
                       equalTo(480));
        }
    }
    ...
    public void rangeMaxConcurrencyM32() {
        List<Integer> list = range(0,
                                   1_000_000).mergeMap(32,
                                                       Flux::just)
                                             .toList();

        assertThat(list.size(),
                   CoreMatchers.equalTo(1_000_000));
    }

    @Test
    public void rangeMaxConcurrencyM64() {
        List<Integer> list = range(0,
                                   1_000_000).mergeMap(64,
                                                       Flux::just)
                                             .toList();
    ...
    public void nullReactiveStreamsPublisher() {
        ...
        range(0,
              1000).mergeMap(v -> Flux.just((Integer) null))
                   .forEach(n -> {
                                data.set(true);
                            },
                            e -> {
    ...
    public void nullReactiveSeq() {
        ...
        range(0,
              1000).mergeMap(v -> (v % 2 == 0 ? Spouts.of((Integer) null) : Spouts.of(v)))
                   .forEach(n -> {
                                data.set(true);
                            },
                            e -> {
    ...
    public void emptyTest() {
        ...
        this.<Integer>empty().mergeMap(v -> Flux.just(v))
                             .forEach(n -> {
                                          data.set(true);
                                      },
                                      e -> {
    ...
    public void innerEmpty() {
        ...
        range(0,
              1000).mergeMap(v -> Flux.<Integer>empty())
                   .forEach(n -> {
                                data.set(true);
                            },
                            e -> {
    ...
    public void mixedMergeMap() {
        ...
        range(0,
              1000).mergeMap(v -> v % 2 == 0 ? Flux.just(v) : Flux.fromIterable(Arrays.asList(v)))
                   .forEach(n -> {
                                data.incrementAndGet();
                            },
                            e -> {
    ...
    public void mergeMapMixedIncremental() {
        ...
        Subscription c = range(0,
                               1000).mergeMap(v -> v % 2 == 0 ? Spouts.of(v) : ReactiveSeq.of(v))
                                    .forEach(0,
                                             n -> {
                                                 data.incrementAndGet();
                                             },
    ...
    public void mergeMapMixedOversubscribed() {
        ...
        Subscription c = range(0,
                               1000).mergeMap(v -> v % 2 == 0 ? Spouts.of(v) : Seq.of(v))
                                    .forEach(0,
                                             n -> {
                                                 data.incrementAndGet();
                                             },
