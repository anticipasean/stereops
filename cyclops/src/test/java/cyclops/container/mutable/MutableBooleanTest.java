...
import cyclops.container.mutable.MutableBoolean;
...
public class MutableBooleanTest {
    ...
    public void testMutate() {
        ...
        Stream.of(1,
                  ...
                  4)
              .map(i -> i * 10)
              .peek(i -> num.mutate(n -> !n))
              .forEach(System.out::println);

        assertThat(num.getAsBoolean(),
                   is(false));
    }
    ...
    @Test
    public void testSet() {
        assertThat(new MutableBoolean().set(falseB)
                                       .getAsBoolean(),
                   is(falseB));
    }
    ...
    public void testClosedVar() {
        assertThat(new MutableBoolean(trueB).getAsBoolean(),
                   equalTo(trueB));
    }

    @Test
    public void testClosedVarEquals() {
        assertThat(new MutableBoolean(trueB),
                   equalTo(new MutableBoolean(trueB)));
    }

    @Test
    public void testClosedVarEqualsFalse() {
        assertThat(new MutableBoolean(trueB),
                   not(equalTo(new MutableBoolean(falseB))));
    }

    @Test
    public void testClosedVarHashCode() {
        assertThat(new MutableBoolean(trueB).hashCode(),
                   equalTo(new MutableBoolean(trueB).hashCode()));
    }

    @Test
    public void testClosedVarHashCodeFalse() {
        assertThat(new MutableBoolean(trueB).hashCode(),
                   not(equalTo(new MutableBoolean(falseB).hashCode())));
    }

    @Test
    public void externalSet() {
        value = false;
        MutableBoolean ext = MutableBoolean.fromExternal(() -> value,
                                                         v -> this.value = v);
        ext.set(false);
        assertThat(value,
                   equalTo(false));
    ...
    public void externalMapInputObj() {
        value = false;
        Mutable<Boolean> ext = MutableBoolean.fromExternal(() -> value,
                                                           v -> this.value = v)
                                             .mapInputToObj(s -> !s);
        ext.set(true);
        assertThat(value,
