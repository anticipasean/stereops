...
import cyclops.container.box.MutableChar;
...
public class MutableCharTest {
    ...
    public void testMutate() {
        ...
        Stream.of(1,
                  ...
                  4)
              .map(i -> i * 10)
              .peek(i -> num.mutate(n -> new Character((char) (n + i))))
              .forEach(System.out::println);

        assertThat(num.getAsChar(),
                   is((char) 120));
    }
    ...
    public void inClosure() {
        ...
        Function<Integer, Function<Integer, MutableChar>> fn = ((Integer i) -> (Integer j) -> myInt.set(new Character((char) (i
            * j))));
        fn.apply(10)
          .apply(20);
    ...
    @Test
    public void testSet() {
        assertThat(new MutableChar().set(twenty)
                                    .getAsChar(),
                   is(twenty));
    }
    ...
    public void testClosedVar() {
        assertThat(new MutableChar(ten).getAsChar(),
                   equalTo(ten));
    }

    @Test
    public void testClosedVarEquals() {
        assertThat(new MutableChar(ten),
                   equalTo(new MutableChar(ten)));
    }

    @Test
    public void testClosedVarEqualsFalse() {
        assertThat(new MutableChar(ten),
                   not(equalTo(new MutableChar(twenty))));
    }

    @Test
    public void testClosedVarHashCode() {
        assertThat(new MutableChar(ten).hashCode(),
                   equalTo(new MutableChar(ten).hashCode()));
    }

    @Test
    public void testClosedVarHashCodeFalse() {
        assertThat(new MutableChar(ten).hashCode(),
                   not(equalTo(new MutableChar(twenty).hashCode())));
    }

    @Test
    public void externalSet() {
        value = 0;
        MutableChar ext = MutableChar.fromExternal(() -> value,
                                                   v -> this.value = v);
        ext.set(ten);
        assertThat(value,
                   equalTo((char) 10));
    ...
    public void externalMapInputObj() {
        value = 0;
        Mutable<Character> ext = MutableChar.fromExternal(() -> value,
                                                          v -> this.value = v)
    ...
    public void externalMapOutputToObj() {
        value = 200;
        Mutable<Integer> ext = MutableChar.fromExternal(() -> value,
                                                        v -> this.value = v)
