...
public class FlowableIOTest extends AbstractIOTestBase {
    ...
    @Test
    public void async() {
        assertThat(FlowableIO.fromPublisher(Future.of(() -> 10,
                                                      ex))
                             .map(i -> i + 1)
                             .run()
                             .orElse(-1),
    ...
    public void asyncError() {
        MatcherAssert.assertThat(FlowableIO.fromPublisher(Future.of(() -> 10,
                                                                    ex))
                                           .map(i -> {
                                               throw re;
                                           })
    ...
    public void onError() {
        ...
        Assert.assertThat(data.get(),
                          equalTo(false));
        Assert.assertThat(complete.get(),
                          equalTo(false));
        Assert.assertThat(error.get(),
                          instanceOf(RuntimeException.class));
        Assert.assertThat(result.get(),
                          equalTo(Vector.empty()));

        Assert.assertThat(count.get(),
                          equalTo(1));
