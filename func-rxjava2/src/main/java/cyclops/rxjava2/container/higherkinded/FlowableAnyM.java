...
import cyclops.rxjava2.container.higherkinded.Rx2Witness.flowable;
...
public interface FlowableAnyM {
        ...
        AnyM<W, ReactiveSeq<T>> flowableM = anyM.map(s -> {
            ...
            if (s instanceof ReactiveSeq) {
                return ((ReactiveSeq<T>) s).fold(sync -> new FlowableReactiveSeqImpl<T>(Flowable.fromIterable(sync)),
                                                 ...
                                                 async -> new FlowableReactiveSeqImpl<T>(Observables.fromStream(async)
    ...
    public static <W extends WitnessType<W>, T, R> R nestedFlowable(StreamT<W, T> nested,
                                                                    Function<? super AnyM<W, Flowable<T>>, ? extends R> mapper) {
    ...
    public static <W extends WitnessType<W>, T> AnyM<W, Flowable<T>> nestedFlowable(StreamT<W, T> nested) {
        ...
        return anyM.map(s -> {
            ...
            if (s instanceof ReactiveSeq) {
                ...
                return r.fold(sync -> Flowable.fromIterable(sync),
                              rs -> Flowable.fromPublisher((Publisher) s),
    ...
    /**
     * Construct an AnyM type from a Flowable. This allows the Flowable to be manipulated according to a standard interface along
     * with a vast array of other Java Monad implementations
     *
     * <pre>
     * {@code
     *
    ...
    public static <T> AnyMSeq<Rx2Witness.flowable, T> anyM(Flowable<T> flowable) {
        return AnyM.ofSeq(FlowableReactiveSeq.reactiveSeq(flowable),
                          Rx2Witness.flowable.INSTANCE);
    }
