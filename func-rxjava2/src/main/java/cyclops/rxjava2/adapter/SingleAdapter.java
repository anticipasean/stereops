...
import cyclops.rxjava2.container.higherkinded.SingleAnyM;
...
public class SingleAdapter extends AbstractMonadAdapter<single> {
    ...
    @Override
    public <T, R> AnyM<single, R> ap(AnyM<single, ? extends Function<? super T, ? extends R>> fn,
                                     AnyM<single, T> apply) {
        ...
        Single<R> res = Single.fromPublisher(crF1.zip(crFnF,
                                                      (a, b) -> b.apply(a)));
    ...
    @Override
    public <T> AnyM<single, T> filter(AnyM<single, T> t,
                                      Predicate<? super T> fn) {

        return SingleAnyM.anyM(future(t).filter(Functions.rxPredicate(fn))
                                        .toSingle());
    }
    ...
    @Override
    public <T, R> AnyM<single, R> map(AnyM<single, T> t,
