...
public class ObservableAdapter extends AbstractMonadAdapter<observable> {
    ...
    public <T> Iterable<T> toIterable(AnyM<observable, T> t) {
        return () -> observable(t).blockingIterable()
                                  .iterator();
    }

    @Override
    public <T, R> AnyM<observable, R> ap(AnyM<observable, ? extends Function<? super T, ? extends R>> fn,
                                         AnyM<observable, T> apply) {
        Observable<T> f = observable(apply);
        Observable<? extends Function<? super T, ? extends R>> fnF = observable(fn);
        Observable<R> res = fnF.zipWith(f,
                                        (a, b) -> a.apply(b));
    ...
    @Override
    public <T> AnyM<observable, T> filter(AnyM<observable, T> t,
    ...
    @Override
    public <T, R> AnyM<observable, R> map(AnyM<observable, T> t,
