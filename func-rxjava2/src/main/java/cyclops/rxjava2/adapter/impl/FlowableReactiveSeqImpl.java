...
public class FlowableReactiveSeqImpl<T> implements ReactiveSeq<T> {
    ...
    public <U> U foldRight(U identity,
        ...
        return flowable.reduce(identity,
    ...
    public <U, R> ReactiveSeq<R> zipWithStream(Stream<? extends U> other,
        ...
        return flux(flowable.zipWith((Iterable<U>) ReactiveSeq.fromStream((Stream<U>) other),
                                     Functions.rxBifunction(zipper)));
    }

    @Override
    public <U, R> ReactiveSeq<R> zipLatest(Publisher<? extends U> other,
                                           BiFunction<? super T, ? super U, ? extends R> zipper) {
        return flux(Flowable.combineLatest(flowable,
                                           other,
                                           (a, b) -> zipper.apply(a,
                                                                  b)));
    }
    ...
    public <U, R> ReactiveSeq<R> zip(BiFunction<? super T, ? super U, ? extends R> zipper,
        ...
        return flux(flowable.zipWith(other,
                                     Functions.rxBifunction(zipper)));
    }
    ...
    public Tuple2<ReactiveSeq<T>, ReactiveSeq<T>> duplicate() {
        return Spouts.from(flowable)
                     .duplicate()
                     .transform((s1, s2) -> Tuple.tuple(flux(s1),
                                                        flux(s2)));
    }
    ...
    public Tuple3<ReactiveSeq<T>, ReactiveSeq<T>, ReactiveSeq<T>> triplicate() {
        return Spouts.from(flowable)
                     .triplicate()
                     .transform((s1, s2, s3) -> Tuple.tuple(flux(s1),
                                                            flux(s2),
                                                            flux(s3)));
    }
    ...
    public ReactiveSeq<Seq<T>> sliding(int windowSize,
                                       int increment) {
        return flux(Spouts.from(flowable)
                          .sliding(windowSize,
                                   increment));
    }
    ...
    public <U> ReactiveSeq<U> scanLeft(U seed,
        ...
        return flux(flowable.scan(seed,
    ...
    public ReactiveSeq<T> drop(long time,
                               TimeUnit unit) {
        return flux(flowable.skip(time,
                                  unit));
    }
    ...
    public ReactiveSeq<T> take(long time,
                               TimeUnit unit) {
        return flux(flowable.take(time,
                                  unit));
    }
    ...
    public <R> R collect(Supplier<R> supplier,
        ...
        return flowable.collect(() -> supplier.get(),
                                (a, b) -> accumulator.accept(a,
                                                             b))
                       .blockingGet();
    }
    ...
    public <R> ReactiveSeq<R> reduceAll(R identity,
        ...
        Single<R> inter = flowable.reduce(identity,
    ...
    public <R, A> ReactiveSeq<R> collectAll(Collector<? super T, A, R> collector) {
        ...
        Single<A> inter = flowable.collect(() -> collector.supplier()
                                                          .get(),
    ...
    public <R, A> R collect(Collector<? super T, A, R> collector) {
        ...
        A inter = collect(collector.supplier(),
