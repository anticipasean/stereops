...
import cyclops.rxjava2.container.higherkinded.Rx2Witness;
...
public class FlowableAdapter extends AbstractMonadAdapter<flowable> {
    ...
    public <T> Iterable<T> toIterable(AnyM<flowable, T> t) {
        return () -> stream(t).blockingIterable()
                              .iterator();
    }

    @Override
    public <T, R> AnyM<flowable, R> ap(AnyM<flowable, ? extends Function<? super T, ? extends R>> fn,
                                       AnyM<flowable, T> apply) {
        Flowable<T> f = stream(apply);
        Flowable<? extends Function<? super T, ? extends R>> fnF = stream(fn);
        Flowable<R> res = fnF.zipWith(f,
                                      (a, b) -> a.apply(b));
    ...
    @Override
    public <T> AnyM<flowable, T> filter(AnyM<flowable, T> t,
    ...
    public <T, R> AnyM<flowable, R> flatMap(AnyM<flowable, T> t,
        ...
        return FlowableAnyM.anyM(stream(t).flatMap(i -> fn.andThen(a -> stream(a))
                                                          .apply(i)));
    ...
    @Override
    public <T, R> AnyM<flowable, R> map(AnyM<flowable, T> t,
