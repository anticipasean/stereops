...
public class Maybes {
    ...
    public static <T> Maybe<T> fromPublisher(Publisher<T> maybe) {
        return Single.fromPublisher(maybe)
                     .toMaybe();
    }
    ...
    public static <T> cyclops.container.control.lazy.maybe.Maybe<T> toMaybe(Maybe<T> future) {
        return cyclops.container.control.lazy.maybe.Maybe.fromPublisher(future.toFlowable());
    }

    public static <T> Maybe<T> fromMaybe(cyclops.container.control.lazy.maybe.Maybe<T> future) {
    ...
    public static <T> cyclops.container.control.lazy.maybe.Maybe<T> maybe(Maybe<T> opt) {
        return cyclops.container.control.lazy.maybe.Maybe.fromFuture(future(opt));
    }
     ...
     * @return Maybe that completes when all the provided Futures Complete. Empty Future result, or holds an Exception from a
     ...
     */
    @SafeVarargs
    public static <T> Maybe<Seq<T>> quorum(Predicate<Status<T>> breakout,
                                           Consumer<Throwable> errorHandler,
                                           Maybe<T>... fts) {

        return Single.fromPublisher(Futures.quorum(breakout,
                                                   errorHandler,
                                                   futures(fts)))
                     .toMaybe();
    ...
    /**
     * Perform a For Comprehension over a Maybe, accepting 3 generating functions. This results in a four level nested internal
     * iteration over the provided Maybes.
     *
     * <pre>
     * {@code
     *
     *   import static cyclops.async.reactive.futurestream.companion.reactor.Maybes.forEach4;
     *
    ...
    public static <T1, T2, T3, R1, R2, R3, R> Maybe<R> forEach4(Maybe<? extends T1> value1,
        ...
        Maybe<? extends R> res = value1.flatMap(in -> {
            ...
            return a.flatMap(ina -> {
                Maybe<R2> b = value3.apply(in,
                                           ina);
                return b.flatMap(inb -> {
                    Maybe<R3> c = value4.apply(in,
                                               ina,
                                               inb);
    ...
    /**
     * Perform a For Comprehension over a Maybe, accepting 2 generating functions. This results in a three level nested internal
     * iteration over the provided Maybes.
     *
     * <pre>
     * {@code
     *
     *   import static cyclops.async.reactive.futurestream.companion.reactor.Maybes.forEach3;
     *
    ...
    /**
     * Perform a For Comprehension over a Maybe, accepting a generating function. This results in a two level nested internal
     * iteration over the provided Maybes.
     *
     * <pre>
     * {@code
     *
     *   import static cyclops.async.reactive.futurestream.companion.reactor.Maybes.forEach;
     *
    ...
    public static <T, R1, R> Maybe<R> forEach(Maybe<? extends T> value1,
        ...
        Maybe<R> res = value1.flatMap(in -> {
            ...
            Maybe<R1> a = value2.apply(in);
            return a.map(ina -> yieldingFunction.apply(in,
                                                       ina));
     ...
     */
    public static <T1, T2, R> Maybe<R> combine(Maybe<? extends T1> maybe,
                                               Value<? extends T2> app,
     ...
     * @return Combined Maybe
     */
    public static <T1, T2, R> Maybe<R> zip(Maybe<? extends T1> maybe,
                                           Iterable<? extends T2> app,
                                           BiFunction<? super T1, ? super T2, ? extends R> fn) {
        return narrow(Single.fromPublisher(Future.fromPublisher(maybe.toFlowable())
                                                 .zip(app,
                                                      fn))
    ...
    public static <T1, T2, R> Maybe<R> zip(Maybe<? extends T1> maybe,
        ...
        Maybe<R> res = narrow(Single.fromPublisher(Future.fromPublisher(maybe.toFlowable())
                                                         .zip(fn,
                                                              app))
                                    .toMaybe());
        return res;
    ...
    public static <T> Maybe<T> fromIterable(Iterable<T> t) {
        return narrow(Single.fromPublisher(Future.fromIterable(t))
                            .toMaybe());
    }
