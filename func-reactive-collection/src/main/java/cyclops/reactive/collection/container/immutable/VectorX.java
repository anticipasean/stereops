...
import cyclops.reactive.collection.container.immutable.lazy.impl.LazyPVectorX;
import cyclops.reactive.collection.container.lazy.LazyCollectionX;
import cyclops.reactive.collection.container.mutable.ListX;
import cyclops.reactive.collection.function.higherkinded.ReactiveWitness.vectorX;
...
/**
 * An eXtended Persistent Vector type, that offers additional functional style operators such as bimap, filter and more Can
 * operate eagerly, lazily or reactively (async push)
 *
 * @param <T> the type of elements held in this collection
 * @author johnmcclean
 */
public interface VectorX<T> extends To<VectorX<T>>, PersistentList<T>, IndexedSequenceX<T>, LazyCollectionX<T>,
                                    OnEmptySwitch<T, PersistentList<T>>, Comparable<T>, Higher<vectorX, T> {

    static <T> VectorX<T> defer(Supplier<VectorX<T>> s) {
    ...
    static <T> Higher<vectorX, T> widen(VectorX<T> narrow) {
        return narrow;
    }
     ...
     */
    static <C2, T> Higher<C2, Higher<vectorX, T>> widen2(Higher<C2, VectorX<T>> list) {
     ...
     * @return PVectorType
     */
    static <T> VectorX<T> narrowK(final Higher<vectorX, T> list) {
     ...
     */
    static <T> VectorX<T> narrow(final VectorX<? extends T> vectorX) {
     ...
     */
    static VectorX<Integer> range(final int start,
                                  final int end) {
     ...
     * @return Range VectorX
     */
    static VectorX<Long> rangeLong(final long start,
                                   final long end) {
     ...
     */
    static <U, T> VectorX<T> unfold(final U seed,
     ...
     */
    static <T> VectorX<T> generate(final long limit,
                                   final Supplier<T> s) {
     ...
     * @return VectorX generated from the provided Supplier
     */
    static <T> VectorX<T> fill(final long limit,
                               final T s) {
     ...
     */
    static <T> VectorX<T> iterate(final long limit,
                                  final T seed,
     ...
     * @return new PVector
     */
    static <T> VectorX<T> of(final T... values) {
        return new LazyPVectorX<>(null,
                                  ReactiveSeq.of(values),
                                  Reducers.toPersistentVector(),
                                  LAZY);
    }
     ...
     */
    static <T> VectorX<T> fromIterator(final Iterator<T> it) {
     ...
     */
    static <T> VectorX<T> empty() {
        return new LazyPVectorX<T>(Vector.empty(),
                                   null,
                                   Reducers.toPersistentVector(),
                                   LAZY);
    }
     ...
     * </pre>
     * {@code List<String> single = PVectors.singleton("1");
     * <p>
     * //or
     * <p>
     ...
     */
    static <T> VectorX<T> singleton(final T value) {
        return new LazyPVectorX<>(Vector.of(value),
                                  null,
                                  Reducers.toPersistentVector(),
                                  LAZY);
    }
     ...
     *
     * @param publisher to construct VectorX from
     * @return VectorX
     */
    static <T> VectorX<T> fromPublisher(final Publisher<T> publisher) {
        return Spouts.from(publisher)
                     .to(ReactiveConvertableSequence::converter)
                     .vectorX(LAZY);
    }

    static <T> VectorX<T> fromIterable(final Iterable<T> iterable) {
        if (iterable instanceof VectorX) {
            return (VectorX) iterable;
        }
        if (iterable instanceof Vector) {
            return new LazyPVectorX<>((Vector) iterable,
                                      null,
                                      Reducers.toPersistentVector(),
                                      LAZY);
        }
     ...
     * @return
     */
    static <T> VectorX<T> vectorX(ReactiveSeq<T> stream) {

        return new LazyPVectorX<T>(null,
                                   stream,
                                   Reducers.toPersistentVector(),
                                   LAZY);
    }

    static <T, R> VectorX<R> tailRec(T initial,
                                     Function<? super T, ? extends VectorX<? extends Either<T, R>>> fn) {
    ...
    default <T> VectorX<T> fromStream(final ReactiveSeq<T> stream) {
        return ReactiveReducers.<T>toVectorX().foldMap(stream);
    }

    /**
     * Combine two adjacent elements in a VectorX using the supplied BinaryOperator This is a stateful grouping & reduction
     ...
     */
    @Override
    default VectorX<T> combine(final BiPredicate<? super T, ? super T> predicate,
                               final BinaryOperator<T> op) {
        return (VectorX<T>) LazyCollectionX.super.combine(predicate,
                                                          op);
    }

    @Override
    default VectorX<T> combine(final Monoid<T> op,
                               final BiPredicate<? super T, ? super T> predicate) {
        return (VectorX<T>) LazyCollectionX.super.combine(op,
                                                          predicate);
    }
    ...
    default <R1, R2, R3, R> VectorX<R> forEach4(Function<? super T, ? extends Iterable<R1>> stream1,
        ...
        return (VectorX) LazyCollectionX.super.forEach4(stream1,
                                                        stream2,
                                                        stream3,
                                                        yieldingFunction);
    }
    ...
    default <R1, R2, R> VectorX<R> forEach3(Function<? super T, ? extends Iterable<R1>> stream1,
        ...
        return (VectorX) LazyCollectionX.super.forEach3(stream1,
                                                        stream2,
                                                        yieldingFunction);
    }
    ...
    default <R1, R> VectorX<R> forEach2(Function<? super T, ? extends Iterable<R1>> stream1,
        ...
        return (VectorX) LazyCollectionX.super.forEach2(stream1,
                                                        yieldingFunction);
    }
     ...
     */
    @Override
    default VectorX<T> slice(final long from,
                             final long to) {
        return (VectorX<T>) LazyCollectionX.super.slice(from,
                                                        to);
    }
    ...
    @Override
    VectorX<T> plusAll(Iterable<? extends T> list);

    @Override
    VectorX<T> insertAt(int i,
                        T e);
    ...
    @Override
    VectorX<T> removeValue(T e);

    @Override
    VectorX<T> removeAll(Iterable<? extends T> list);

    @Override
    VectorX<T> removeAt(long i);
    ...
    @Override
    default VectorX<Vector<T>> grouped(final int groupSize) {
        return (VectorX<Vector<T>>) LazyCollectionX.super.grouped(groupSize);
    }
     ...
     */
    @Override
    default <U, R> VectorX<R> zip(final Iterable<? extends U> other,
                                  final BiFunction<? super T, ? super U, ? extends R> zipper) {

        return (VectorX<R>) LazyCollectionX.super.zip(other,
                                                      zipper);
    }
    ...
    @Override
    default VectorX<Seq<T>> sliding(final int windowSize) {
        return (VectorX<Seq<T>>) LazyCollectionX.super.sliding(windowSize);
    }
    ...
    @Override
    default <U> VectorX<U> scanLeft(final U seed,
                                    final BiFunction<? super U, ? super T, ? extends U> function) {
        return (VectorX<U>) LazyCollectionX.super.scanLeft(seed,
                                                           function);
    }
    ...
    @Override
    default <U> VectorX<U> scanRight(final U identity,
                                     final BiFunction<? super T, ? super U, ? extends U> combiner) {
        return (VectorX<U>) LazyCollectionX.super.scanRight(identity,
                                                            combiner);
    }
     ...
     */
    @Override
    default VectorX<T> cycle(final Monoid<T> m,
                             final long times) {

        return (VectorX<T>) LazyCollectionX.super.cycle(m,
                                                        times);
    }
     ...
     */
    @Override
    default <S, U> VectorX<Tuple3<T, S, U>> zip3(final Iterable<? extends S> second,
                                                 final Iterable<? extends U> third) {

        return (VectorX) LazyCollectionX.super.zip3(second,
                                                    third);
    }
     ...
     */
    @Override
    default <T2, T3, T4> VectorX<Tuple4<T, T2, T3, T4>> zip4(final Iterable<? extends T2> second,
                                                             final Iterable<? extends T3> third,
                                                             final Iterable<? extends T4> fourth) {

        return (VectorX) LazyCollectionX.super.zip4(second,
                                                    third,
                                                    fourth);
    }
    ...
    @Override
    default <C extends PersistentCollection<? super T>> VectorX<C> grouped(final int size,
                                                                           final Supplier<C> supplier) {

        return (VectorX<C>) LazyCollectionX.super.grouped(size,
                                                          supplier);
    }

    @Override
    default VectorX<Vector<T>> groupedUntil(final Predicate<? super T> predicate) {

        return (VectorX<Vector<T>>) LazyCollectionX.super.groupedUntil(predicate);
    }

    @Override
    default VectorX<Vector<T>> groupedUntil(final BiPredicate<Vector<? super T>, ? super T> predicate) {

        return (VectorX<Vector<T>>) LazyCollectionX.super.groupedUntil(predicate);
    }

    @Override
    default VectorX<Vector<T>> groupedWhile(final Predicate<? super T> predicate) {

        return (VectorX<Vector<T>>) LazyCollectionX.super.groupedWhile(predicate);
    }

    @Override
    default <C extends PersistentCollection<? super T>> VectorX<C> groupedWhile(final Predicate<? super T> predicate,
                                                                                final Supplier<C> factory) {

        return (VectorX<C>) LazyCollectionX.super.groupedWhile(predicate,
                                                               factory);
    }

    @Override
    default <C extends PersistentCollection<? super T>> VectorX<C> groupedUntil(final Predicate<? super T> predicate,
                                                                                final Supplier<C> factory) {

        return (VectorX<C>) LazyCollectionX.super.groupedUntil(predicate,
                                                               factory);
    }
    ...
    @Override
    default <R> VectorX<R> mergeMap(int maxConcurency,
                                    Function<? super T, ? extends Publisher<? extends R>> fn) {
        return (VectorX<R>) LazyCollectionX.super.mergeMap(maxConcurency,
                                                           fn);
    }
    ...
    @Override
    default VectorX<T> insertAt(int pos,
                                T... values) {
        return (VectorX<T>) LazyCollectionX.super.insertAt(pos,
                                                           values);
    }

    @Override
    default VectorX<T> deleteBetween(int start,
                                     int end) {
        return (VectorX<T>) LazyCollectionX.super.deleteBetween(start,
                                                                end);
    }

    @Override
    default VectorX<T> insertStreamAt(int pos,
                                      Stream<T> stream) {
        return (VectorX<T>) LazyCollectionX.super.insertStreamAt(pos,
                                                                 stream);
    }
    ...
    @Override
    default <EX extends Throwable> VectorX<T> recover(Class<EX> exceptionClass,
                                                      final Function<? super EX, ? extends T> fn) {
        return (VectorX<T>) LazyCollectionX.super.recover(exceptionClass,
                                                          fn);
    }

    @Override
    default VectorX<T> plusLoop(int max,
                                IntFunction<T> value) {
        return (VectorX<T>) LazyCollectionX.super.plusLoop(max,
                                                           value);
    }
    ...
    @Override
    default <T2, R> VectorX<R> zip(final BiFunction<? super T, ? super T2, ? extends R> fn,
                                   final Publisher<? extends T2> publisher) {
        return (VectorX<R>) LazyCollectionX.super.zip(fn,
                                                      publisher);
    }
    ...
    @Override
    default <S, U, R> VectorX<R> zip3(final Iterable<? extends S> second,
                                      final Iterable<? extends U> third,
                                      final Function3<? super T, ? super S, ? super U, ? extends R> fn3) {
        return (VectorX<R>) LazyCollectionX.super.zip3(second,
                                                       third,
                                                       fn3);
    }

    @Override
    default <T2, T3, T4, R> VectorX<R> zip4(final Iterable<? extends T2> second,
                                            final Iterable<? extends T3> third,
                                            final Iterable<? extends T4> fourth,
                                            final Function4<? super T, ? super T2, ? super T3, ? super T4, ? extends R> fn) {
        return (VectorX<R>) LazyCollectionX.super.zip4(second,
                                                       third,
                                                       fourth,
                                                       fn);
    }

    class CompletableVectorX<T> implements InvocationHandler {
        ...
        public Object invoke(Object proxy,
            ...
            VectorX<T> target = future.fold(l -> l,
                                            t -> {
