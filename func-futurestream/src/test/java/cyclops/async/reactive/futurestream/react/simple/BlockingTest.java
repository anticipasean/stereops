...
public class BlockingTest {
    ...
    public void testBlockToSet() throws InterruptedException, ExecutionException {
        ...
        Set<String> strings = new SimpleReact().<Integer>ofAsync(() -> 1,
                                                                 () -> 1,
                                                                 () -> 3).then(it -> it * 100)
                                                                         .then(it -> "*" + it)
    ...
    public void testBreakoutException() throws InterruptedException, ExecutionException {
        Throwable[] error = {null};
        List<Integer> results = new SimpleReact().<Integer>ofAsync(() -> 1,
                                                                   () -> 2,
                                                                   () -> 3).then(it -> it * 100).<Integer>then(it -> {
    ...
    public void testBreakoutAllCompletedAndTime() throws InterruptedException, ExecutionException {
        ...
        List<Integer> result = new SimpleReact().<Integer>ofAsync(() -> 1,
                                                                  ...
                                                                  () -> 3).then(it -> it * 100)
                                                                          ...
                                                                          .capture(e -> count.incrementAndGet())
                                                                          .block(status -> status.getAllCompleted() > 1
                                                                              && status.getElapsedMillis() > 20);
    ...
    public void testFirstSimple() throws InterruptedException, ExecutionException {
        ...
        SimpleReactStream<Integer> stage = new SimpleReact().<Integer>ofAsync(() -> 1,
                                                                              () -> 2,
                                                                              () -> 3,
                                                                              () -> 5).then(it -> it * 100)
                                                                                      .then(it -> sleep(it));

        int result = stage.block()
                          .firstValue(null);
