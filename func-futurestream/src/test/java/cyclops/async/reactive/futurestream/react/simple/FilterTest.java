...
public class FilterTest {
    ...
    public void testFilterBehavesAsStreamFilter() throws InterruptedException, ExecutionException {
        int expected = Arrays.asList("*1",
                             ...
                             .size();
        List<String> result = new SimpleReact().<Integer>ofAsync(() -> 1,
                                                                 () -> 2,
                                                                 () -> 3).then(it -> "*" + it)
                                                                         .filter(it -> it.startsWith("*"))
                                                                         .block();
    ...
    public void testFilterFirst() throws InterruptedException, ExecutionException {
        ...
        List<String> result = new SimpleReact().<Integer>ofAsync(() -> 1,
                                                                 () -> 2,
                                                                 () -> 3).filter(it -> 1 != it)
                                                                         .peek(it -> System.out.println(it)).<String>then(it ->
                                                                                                                              "*"
                                                                                                                                  + it).capture(e -> e.printStackTrace())
                                                                                                                                       .block();
    ...
    public void testFilterExceptions() throws InterruptedException, ExecutionException {
        ...
        List<String> result = new SimpleReact().<Integer>ofAsync(() -> 1,
                                                                 () -> 2,
                                                                 () -> 3).filter(it -> 1 != it).<String>then(it -> "*"
            + it).capture(e -> fail("No exception should be captured"))
                 .block();
