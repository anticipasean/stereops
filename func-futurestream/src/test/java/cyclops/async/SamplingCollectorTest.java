...
import cyclops.async.reactive.futurestream.pipeline.collector.SamplingCollector;
...
public class SamplingCollectorTest {
    ...
    public void setup() {
        recieved = 0;
        monitor = new SamplingCollector(3,
                                        new MyConsumer(it -> recieved++));
    }

    @Test
    public void testSamplingMonitorConsumerOfCompletableFuture() {
        monitor = new SamplingCollector(3,
                                        new MyConsumer(it -> recieved = recieved + 8));
        for (int i = 0; i < 100; i++) {
    ...
    public void testBuilder() {
        monitor = SamplingCollector.builder()
                                   .sampleRate(1)
                                   .consumer(new MyConsumer(it -> recieved++))
                                   .count(0)
                                   .build();
        for (int i = 0; i < 100; i++) {
            monitor.accept(FastFuture.completedFuture(10));
    ...
    public void testSamplingMonitorIntIntConsumerOfCompletableFuture() {
        monitor = new SamplingCollector(5,
                                        new MyConsumer(it -> recieved++));
        for (int i = 0; i < 100; i++) {
            monitor.accept(FastFuture.completedFuture(10));
        }
