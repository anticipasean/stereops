 ...
 * <p>
 * Connected Streams will not be able to complete collect or reduce style methods unless the underlying Adapter for data transfer
 * is closed. I.e. connected Streams remain connected until lazy the Adapter is closed, or they disconnect (due to a limit for
 * example).
 *
 * <pre>
 * {@close
...
public class Pipes<K, V> {
    ...
    /**
     * Extract the next x values from the Adapter identified by the provided Key If the Adapter doesn't exist an zero List is
     * returned
     *
     * <pre>
     * {@code
     ...
     */
    public ListX<V> xValues(final K key,
                            final long x) {
     ...
     *
     * @param key : Adapter identifier
     * @return Xor containing lazy a NoSuchElementException an Adapter with the specified key does not exist, or the next value
     * from that Adapter
     */
    public Either<Throwable, V> oneOrError(final K key) {
        ...
        return get(key).peek(a -> a.stream()
                                   .subscribe(sub))
                       .map(a -> sub.toEither())
                       .orElse(Either.left(new NoSuchElementException("no adapter for key " + key)));
    }

    /**
     * Extact one value from the selected pipe or an zero Maybe if it doesn't exist. Currently only Adapter's and not Publishers
     * are managed by Pipes so Publisher errors are not propagated (@see {@link Pipes#oneValue(Object)} or @see {@link
     * Pipes#oneOrError(Object)} is better at the moment.
     *
     * <pre>
     *  {@code
     *  Queue<String> q = new Queue<>();
    ...
    public <X extends Throwable> Option<Try<V, X>> oneValueOrError(final K key,
     ...
     */
    public Future<V> oneOrErrorAsync(final K key,
                                     final Executor ex) {
    ...
    /**
     * Return an Eval that allows retrieval of the next value from the attached pipe when getValue() is called, can be used as an
     * Iterator over the future & present values in the Adapter
     * <p>
    ...
    /**
     * Register a Queue, and getValue back a listening LazyFutureStream that runs on a single thread (not the calling thread)
     *
     * <pre>
     * {@code
     ...
     * @param adapter
     */
    public void register(final K key,
                         final Adapter<V> adapter) {
        registered.put(key,
                       adapter);
     ...
     */
    public void subscribeTo(final K key,
                            final Subscriber<V> subscriber) {
        registered.get(key)
                  .stream()
    ...
    public void subscribeTo(final K key,
                            final Subscriber<V> subscriber,
                            final Executor subscribeOn) {
        CompletableFuture.runAsync(() -> subscribeTo(key,
                                                     subscriber),
                                   subscribeOn);
     ...
     */
    public void publishTo(final K key,
                          final Publisher<V> publisher) {
        registered.get(key)
                  .fromStream(Spouts.from(publisher));
    }
