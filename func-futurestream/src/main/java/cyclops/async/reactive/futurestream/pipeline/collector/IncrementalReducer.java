...
import cyclops.async.reactive.futurestream.pipeline.stream.MissingValue;
...
public class IncrementalReducer<T> {
    ...
    public void forEach(final Consumer<? super T> c,
                        final Function<FastFuture, T> safeJoin) {

        forEachResults(consumer.getResults(),
                       c,
                       safeJoin);

    }

    public void forEachResults(final Collection<FastFuture<T>> results,
    ...
    public T reduce(final Function<FastFuture, T> safeJoin,
                    final T identity,
                    final BinaryOperator<T> accumulator) {
        return reduceResults(consumer.getResults(),
                             safeJoin,
                             identity,
                             accumulator);
    ...
    public T reduceResults(final Collection<FastFuture<T>> results,
                           final Function<FastFuture, T> safeJoin,
                           final T identity,
                           final BinaryOperator<T> accumulator) {
        final Stream<FastFuture<T>> streamToUse = results.stream();
    ...
    public Optional<T> reduce(final Function<FastFuture, T> safeJoin,
                              final BinaryOperator<T> accumulator) {
    ...
    public Optional<T> reduceResults(final Collection<FastFuture<T>> results,
                                     final Function<FastFuture, T> safeJoin,
                                     final BinaryOperator<T> accumulator) {
        final Stream<FastFuture<T>> streamToUse = results.stream();
    ...
    public <U> U reduce(final Function<FastFuture, T> safeJoin,
                        final U identity,
    ...
    public <U> U reduceResults(final Collection<FastFuture<T>> results,
                               final Function<FastFuture, T> safeJoin,
                               final U identity,
                               final BiFunction<U, ? super T, U> accumulator,
                               final BinaryOperator<U> combiner) {
