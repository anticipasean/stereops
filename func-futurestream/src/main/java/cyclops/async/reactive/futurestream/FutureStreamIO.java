...
public final class FutureStreamIO<T> implements IO<T> {
    ...
    public static <T> IO<T> of(T value) {
        return new FutureStreamIO<>(LazyReact.sequentialBuilder()
                                             .of(value));
    }

    public static <T> IO<T> of(Supplier<? extends T> value) {
        return new FutureStreamIO<T>(FutureStream.narrow(LazyReact.sequentialBuilder()
                                                                  .ofAsync(value)));
    }
    ...
    public static <T> IO<T> of(Executor executor,
                               Supplier<T>... values) {
        return new FutureStreamIO<T>(FutureStream.narrow(new LazyReact(values.length,
                                                                       executor).ofAsync(values)));
    }

    public static <T> IO<T> of(Future<T> flowable) {
        return new FutureStreamIO<>(FutureStream.builder()
                                                .from(flowable.getFuture()));
    }

    @Override
    public <B, R> IO<R> par(IO<B> that,
                            BiFunction<? super T, ? super B, ? extends R> fn) {
        return of(flowable.zip(fn,
                               that));
    }
    ...
    public <R> IO<R> flatMap(Function<? super T, IO<? extends R>> s) {
        return of(flowable.flatMap(in -> FutureStream.builder()
                                                     .fromPublisher(s.apply(in))));
    }

    @Override
    public <R> IO<R> mergeMap(int maxConcurrency,
                              Function<? super T, Publisher<? extends R>> s) {
        return of(flowable.mergeMap(maxConcurrency,
                                    s));
    }
    ...
    public <R extends AutoCloseable, R1> Managed.Tupled<R, R1> bracketWith(Function<? super T, ? extends R> fn,
        ...
        Managed.Tupled<? extends R, ? extends R1> x = FutureStreamManaged.of(map(fn))
                                                                         .with(with);
    ...
    public IO<T> ensuring(Consumer<T> action) {
        return FutureStreamManaged.of(this,
                                      action)
                                  .io();
    }
    ...
    public void forEach(Consumer<? super T> consumerElement,
                        ...
                        Runnable onComplete) {
        flowable.forEach(consumerElement,
                         consumerError,
                         onComplete);
    }
    ...
    public static abstract class FutureStreamManaged<T> extends Managed<T> {
        ...
        public static <T> Managed<T> managed(T acq,
                                             Consumer<T> cleanup) {
            return of(FutureStreamIO.of(() -> acq),
                      cleanup);
        }
            ...
            return new FutureStreamManaged<T>() {
                public <R> IO<R> apply(Function<? super T, ? extends IO<R>> fn) {
                    IO<R> y = IO.Comprehensions.forEach(acquire,
                                                        t1 -> {
                                                            IO<? extends Try<? extends IO<R>, Throwable>> res1 = FutureStreamIO.withCatch(() -> fn.apply(t1),
                                                                                                                                          Throwable.class);
                                                            return res1;
                                                        },
                                                        t2 -> {
                                                            ...
                                                            IO<R> res = tr.fold(r -> r,
                                                                                e -> FutureStreamIO.of(Future.ofError(e)));
        ...
        public <R> Managed<R> map(Function<? super T, ? extends R> mapper) {
            return of(apply(mapper.andThen(FutureStreamIO::of)),
                      __ -> {
                      });
        }
