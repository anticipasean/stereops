...
public class EagerFutureStreamFunctions {
     ...
     */
    static void closeOthers(final Queue active,
                            final List<Queue> all) {
        all.stream()
     ...
     */
    static void closeOthers(final SimpleReactStream active,
                            final List<SimpleReactStream> all) {
        all.stream()
           .filter(next -> next != active)
     ...
     */
    static <T1, T2> ReactiveSeq<Tuple2<T1, T2>> combineLatest(final SimpleReactStream<T1> left,
                                                              final SimpleReactStream<T2> right) {
        return combineLatest(left,
                             right,
                             Tuple::tuple);
    }
    ...
    public static <U> SimpleReactStream<U> firstOf(final SimpleReactStream<U>... futureStreams) {
        final List<Tuple2<SimpleReactStream<U>, QueueReader>> racers = Stream.of(futureStreams)
                                                                             .map(s -> Tuple.tuple(s,
                                                                                                   new Queue.QueueReader(s.toQueue(),
                                                                                                                         null)))
        ...
        while (true) {
            for (final Tuple2<SimpleReactStream<U>, Queue.QueueReader> q : racers) {
                if (q._2()
                     .notEmpty()) {
                    EagerFutureStreamFunctions.closeOthers(q._2()
                                                            .getQueue(),
                                                           racers.stream()
                                                                 .map(t -> t._2()
                                                                            .getQueue())
    ...
    /**
     * Zip two streams into one using a {@link BiFunction} to produce resulting. values. Uses the latest values from each rather
     * than waiting for both.
     */
    static <T1, T2, R> ReactiveSeq<R> combineLatest(final SimpleReactStream<T1> left,
                                                    final SimpleReactStream<T2> right,
                                                    final BiFunction<T1, T2, R> zipper) {

        final Queue q = left.then(it -> new Val(Val.Pos.left,
                                                it))
                            .merge(right.then(it -> new Val(Val.Pos.right,
                                                            it)))
                            .toQueue();
     ...
     * Zip two streams into one. Uses the latest values from each rather than waiting for both
     */
    static <T1, T2> ReactiveSeq<Tuple2<T1, T2>> withLatest(final SimpleReactStream<T1> left,
                                                           final SimpleReactStream<T2> right) {
        return withLatest(left,
                          right,
                          Tuple::tuple);
    }
     ...
     * than waiting for both.
     */
    static <T1, T2, R> ReactiveSeq<R> withLatest(final SimpleReactStream<T1> left,
                                                 final SimpleReactStream<T2> right,
                                                 final BiFunction<T1, T2, R> zipper) {
    ...
    static <T1, T2> ReactiveSeq<T1> dropUntil(final SimpleReactStream<T1> left,
                                              final SimpleReactStream<T2> right) {
    ...
    static <T1, T2> ReactiveSeq<T1> takeUntil(final SimpleReactStream<T1> left,
                                              final SimpleReactStream<T2> right) {
