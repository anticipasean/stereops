...
import cyclops.async.reactive.futurestream.pipeline.collector.IncrementalReducer;
...
public interface LazyStream<U> extends BlockingStream<U> {
    ...
    default void runThread(final Runnable r) {
        final Function<FastFuture, U> safeJoin = (final FastFuture cf) -> (U) BlockingStreamHelper.getSafe(cf,
                                                                                                           getErrorHandler());
        new Thread(() -> new Runner(r).run(getLastActive(),
                                           new EmptyCollector(getMaxActive(),
                                                              safeJoin))).start();
    ...
    default Continuation runContinuation(final Runnable r) {
        final Function<FastFuture, U> safeJoin = (final FastFuture cf) -> (U) BlockingStreamHelper.getSafe(cf,
                                                                                                           getErrorHandler());
        return new Runner(r).runContinuations(getLastActive(),
                                              new EmptyCollector(getMaxActive(),
                                                                 safeJoin),
                                              false);
     ...
     *
     * @param collector Supplier that creates a toX to store results in
    ...
    default <A, R> R run(final Collector<U, A, R> collector) {
        ...
        final Function<FastFuture<U>, U> safeJoin = (final FastFuture<U> cf) -> (U) BlockingStreamHelper.getSafe(cf,
                                                                                                                 getErrorHandler());
        final LazyResultConsumer<U> batcher = collector.supplier()
                                                       .get() != null ? getLazyCollector().get()
        ...
        return (R) batcher.getAllResults()
                          .stream()
                          .map(cf -> BlockingStreamHelper.extractNonFiltered(cf,
                                                                             getErrorHandler()))
    ...
    default void forEach(final Consumer<? super U> c) {
        ...
        final IncrementalReducer<U> collector = new IncrementalReducer(this.getLazyCollector()
                                                                           .get()
                                                                           .withResults(new ArrayList<>()),
                                                                       this);
        try {
        ...
        if (getLastActive().isSequential()) {
            ...
            forEach(r -> {
                if (result[0] == null) {
                    ...
                    result[0] = accumulator.apply((U) result[0],
                                                  r);
                }
            });
            ...
            this.getLastActive()
                ...
                .forEach(next -> {
                    ...
                    if (!result[0].isPresent()) {
                        result[0] = collector.reduce(safeJoin,
                                                     accumulator);
                    } else {
                        result[0] = result[0].map(v -> collector.reduce(safeJoin,
                                                                        (U) v,
                                                                        accumulator));
                    }
    ...
    default <R> R collect(final Supplier<R> supplier,
        ...
        return (R) this.run((Collector) Collector.of(supplier,
                                                     accumulator,
                                                     (a, b) -> {
                                                         combiner.accept(a,
                                                                         b);
