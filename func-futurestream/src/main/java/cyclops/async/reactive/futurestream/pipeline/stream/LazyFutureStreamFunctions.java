...
import static cyclops.async.reactive.futurestream.pipeline.stream.NullValue.NULL;
...
public class LazyFutureStreamFunctions {
     ...
     * </code>
     */
    public static <T1, T2> ReactiveSeq<Tuple2<T1, T2>> zip(final Stream<T1> left,
                                                           final Stream<? extends T2> right) {
        return zip(left,
                   right,
                   Tuple::tuple);
    }
     ...
     * </code>
     */
    public static <T1, T2, R> ReactiveSeq<R> zip(final Stream<T1> left,
                                                 final Stream<T2> right,
                                                 final BiFunction<? super T1, ? super T2, ? extends R> zipper) {
     ...
     * </code>
     */
    public static <T> ReactiveSeq<T> takeWhile(final Stream<T> stream,
    ...
    public static <T> ReactiveSeq<T> takeUntil(final Stream<T> stream,
                                               final Predicate<? super T> predicate) {
