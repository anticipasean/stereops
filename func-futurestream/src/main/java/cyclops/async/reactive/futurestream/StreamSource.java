...
import cyclops.async.reactive.futurestream.pushable.PushableReactiveSeq;
import cyclops.async.reactive.futurestream.pushable.PushableStream;
...
public class StreamSource {
    ...
    public static <T> MultipleStreamSource<T> ofMultiple() {
        return new MultipleStreamSource<T>(StreamSource.ofUnbounded()
                                                       .createQueue());
    }

    /**
     * Connect multiple Streams to a Pushable datasource, each Stream will recieve the same data. In this backpresure is applied
     * by using a LinkedBlockingQueue. @see cyclops2.stream.StreamSource#ofMultiple(QueueFactory) For more granular management of
     * Adapter based backpressure. Adapters can be backed by non-blocking data structures and different backpressure strategies
     * applied
     * <pre>
     * {@code
     *
     ...
     * </pre>
     *
     * @param backPressureAfter Excess number of emitted records over consumed (by all connected Streams after which backPressure
     *                          will be applied).
     * @return a builder that will use Topics to allow multiple Streams from the same data
     */
    public static <T> MultipleStreamSource<T> ofMultiple(final int backPressureAfter) {
        return new MultipleStreamSource<T>(StreamSource.of(backPressureAfter)
                                                       .createQueue());
    }

    /**
     * Construct a StreamSource that supports multiple readers of the same data backed by a Queue created from the supplied
     * QueueFactory
     *
    ...
    public static <T> MultipleStreamSource<T> ofMultiple(final QueueFactory<?> q) {
        ...
        return new MultipleStreamSource<T>(StreamSource.of(q)
                                                       .createQueue());
    }
    ...
    /**
     * Construct a Pushable StreamSource with no max size. Warning if data producers pushing data to this StreamSource are faster
     * than Data consumers the JVM will eventually run out of memory.
     * <pre>
     * {@code
     * PushableStream<Integer> pushable = StreamSource.ofUnbounded()
     * .stream();
     ...
     * For more granular management of Adapter based backpressure. Adapters can be backed by non-blocking data structures and
     * different backpressure strategies applied
     *
     * <pre>
     * {@code
    ...
    public static StreamSource of(final int backPressureAfter) {
        ...
        return new StreamSource(backPressureAfter,
                                true);
    }

    /**
     * Create a FutureStream. his will call FutureStream#futureStream(Stream) which creates a sequential LazyFutureStream
     *
     * <pre>
     * {@code
     *
     ...
     */
    public static <T> FutureStream<T> futureStream(final Adapter<T> adapter,
     ...
     * @return a Tuple2 with a Queue&lt;T&gt; and LazyFutureStream&lt;T&gt; - add data to the Queue to push it to the Stream
     */
    public <T> PushableFutureStream<T> futureStream(final LazyReact s) {

        final Queue<T> q = createQueue();
        return new PushableFutureStream<T>(q,
                                           s.fromStream(q.stream()));
     ...
     * </pre>
     *
     * @return PushableStream that can accept data to push into a Java 8 Stream to push it to the Stream
     */
    public <T> PushableStream<T> stream() {
        final Queue<T> q = createQueue();
        return new PushableStream<T>(q,
                                     q.jdkStream());
     ...
     * </pre>
     *
     * @return PushableStream that can accept data to push into a {@see cyclops2.stream.ReactiveSeq} to push it to the Stream
     */
    public <T> PushableReactiveSeq<T> reactiveSeq() {
        final Queue<T> q = createQueue();
        return new PushableReactiveSeq<T>(q,
                                          q.stream());
    }
