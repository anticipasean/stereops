...
import cyclops.pure.arrow.FunctionK;
...
public class Active<W, T> implements Filterable<T>, Transformable<T>, To<Active<W, T>> {
    ...
    public static <W, T> Active<W, T> of(Higher<W, T> single,
        ...
        return new Active<>(single,
                            def1);
    }

    public static <C, W, T> Active<W, T> of(Function<? super C, ? extends Higher<W, T>> single,
                                            C c,
                                            InstanceDefinitions<W> def1) {
        return of(single.apply(c),
                  def1);
    }

    public static <W, T> Active<W, T> of(InstanceDefinitions<W> def1,
                                         T value) {
        return new Active<>(def1.unit()
                                .unit(value),
                            def1);
    }
    ...
    public static <W, T, C2, R> Higher<C2, Active<W, R>> traverseA(Applicative<C2> applicative,
        ...
        Higher<C2, Higher<W, R>> r = traverse.traverseA(applicative,
                                                        fn,
                                                        n.single);
        Higher<C2, Active<W, R>> x = applicative.map(nr -> Active.of(nr,
                                                                     n.def1),
                                                     r);
        return x;
     ...
     */
    public <S, R> Active<W, R> custom(Function<? super Higher<W, T>, ? extends S> narrow,
                                      Function<? super S, ? extends Higher<W, R>> fn) {
        return Active.of(fn.apply(narrow.apply(single)),
                         def1);
     ...
     *     }
     * </pre>
     * Most cyclops-react types provide kindCokleisli implementations that convert a Higher Kinded encoding of the type back to
     * the concrete type
     *
    ...
    public <C, R> R fold(Function<? super Higher<W, T>, ? extends C> narrow,
                         Function<? super C, ? extends R> visitor) {
    ...
    public Active<W, T> filter(Predicate<? super T> predicate) {
        return of(def1.monadZero()
                      .fold(s -> s.filter(predicate,
                                          single),
                            () -> single),
                  def1);
    }
    ...
    public <W2, T2, R> Active<W, R> zipWith(Active<W2, T2> a,
                                            BiFunction<? super T, ? super Maybe<T2>, ? extends R> f) {
        return of(def1.traverse()
                      .zipWith(a.def1.foldable(),
                               f,
                               single,
                               a.single),
                  def1);
    ...
    public <C> Narrowed<C> concreteMonoid(Kleisli<W, C, T> widen,
    ...
    public <R> Active<W, R> zip(Active<W, T> p2,
        ...
        Function<T, Function<T, R>> fn = a -> b -> zipper.apply(a,
                                                                b);
        Higher<W, Function<T, Function<T, R>>> hfn = ap.unit(fn);
        return of(ap.ap(ap.ap(hfn,
                              single),
                        p2.getSingle()),
                  def1);
    }

    public Active<W, Tuple3<T, T, T>> zip(Active<W, T> p2,
                                          Active<W, T> p3) {
    ...
    public <R> Active<W, R> zip(Active<W, T> p2,
        ...
        Function<T, Function<T, Function<T, R>>> fn = a -> b -> c -> zipper.apply(a,
                                                                                  b,
                                                                                  c);
        Higher<W, Function<T, Function<T, Function<T, R>>>> hfn = ap.unit(fn);
        return of(ap.ap(ap.ap(ap.ap(hfn,
                                    single),
                              p2.getSingle()),
                        p3.getSingle()),
                  def1);
    ...
    public <R> Active<W, R> flatMapA(Function<? super T, ? extends Active<W, R>> fn) {
        return of(def1.monad()
                      .flatMap(fn.andThen(Active::getActive),
                               single),
                  def1);
    }
    ...
    public <C, R> Active<W, R> ap(C c,
                                  Function<? super C, ? extends Higher<W, ? extends Function<T, R>>> fn) {
        return ap(fn.apply(c));
    }

    public Active<W, T> plus(SemigroupK<W> semigroupK,
                             Higher<W, T> add) {
        return of(semigroupK.apply(single,
                                   add),
                  def1);
    }
    ...
    public Maybe<Unfolds> unfolds() {
        return def1.unfoldable()
                   .fold(e -> Maybe.just(new Unfolds(e)),
                         Maybe::nothing);
    }

    public Maybe<Plus> plus() {
        return def1.monadPlus()
                   .fold(e -> Maybe.just(new Plus(e)),
                         Maybe::nothing);
    }

    public <R> Active<W, R> tailRec(T initial,
                                    Function<? super T, ? extends Higher<W, ? extends Either<T, R>>> fn) {
        return of(def1.monadRec().tailRec(initial,
                                          fn),
                  def1);
    }

    public <R> Active<W, R> tailRecA(T initial,
                                     Function<? super T, ? extends Active<W, ? extends Either<T, R>>> fn) {
        return of(def1.monadRec().tailRec(initial,
                                          fn.andThen(Active::getActive)),
                  def1);
    }
    ...
    public <C2, T> Higher<C2, Active<W, T>> flatSequenceA(Applicative<C2> applicative,
        ...
        return applicative.map(i -> i.flatMapA(Function.identity()),
    ...
    public <U> Active<W, U> ofType(Class<? extends U> type) {
        return (Active<W, U>) Filterable.super.ofType(type);
    }

    @Override
    public Active<W, T> filterNot(Predicate<? super T> predicate) {
        return (Active<W, T>) Filterable.super.filterNot(predicate);
    }

    @Override
    public Active<W, T> notNull() {
        return (Active<W, T>) Filterable.super.notNull();
    }

    public <T2, R1, R2, R3, R> Active<W, R> forEach4(final Function<? super T, ? extends Higher<W, R1>> value1,
                                                     final BiFunction<? super T, ? super R1, ? extends Higher<W, R2>> value2,
                                                     final Function3<? super T, ? super R1, ? super R2, ? extends Higher<W, R3>> value3,
                                                     final Function4<? super T, ? super R1, ? super R2, ? super R3, ? extends R> yieldingFunction) {
        return of(Comprehensions.of(def1.monad())
                                .forEach4(this.single,
                                          value1,
                                          value2,
                                          value3,
                                          yieldingFunction),
    ...
    public <T2, R1, R2, R3, R> Maybe<Active<W, R>> forEach4(final Function<? super T, ? extends Higher<W, R1>> value1,
                                                            final BiFunction<? super T, ? super R1, ? extends Higher<W, R2>> value2,
                                                            final Function3<? super T, ? super R1, ? super R2, ? extends Higher<W, R3>> value3,
        ...
        MonadZero<W> mZero = def1.monadZero()
                                 .orElse(null);

        return Maybe.just(of(Comprehensions.of(mZero)
                                           .forEach4(this.single,
                                                     value1,
                                                     value2,
                                                     value3,
    ...
    public <T2, R1, R2, R> Active<W, R> forEach3(final Function<? super T, ? extends Higher<W, R1>> value1,
                                                 final BiFunction<? super T, ? super R1, ? extends Higher<W, R2>> value2,
                                                 final Function3<? super T, ? super R1, ? super R2, ? extends R> yieldingFunction) {
        return of(Comprehensions.of(def1.monad())
                                .forEach3(this.single,
                                          value1,
                                          value2,
                                          yieldingFunction),
                  def1);
    ...
    public <T2, R1, R2, R> Maybe<Active<W, R>> forEach3(final Function<? super T, ? extends Higher<W, R1>> value1,
                                                        final BiFunction<? super T, ? super R1, ? extends Higher<W, R2>> value2,
    ...
    public <R1, R> Active<W, R> forEach2(Function<? super T, ? extends Higher<W, R1>> value1,
                                         final BiFunction<? super T, ? super R1, ? extends R> yieldingFunction) {
        return of(Comprehensions.of(def1.monad())
                                .forEach2(this.single,
                                          value1,
                                          yieldingFunction),
                  def1);
    }

    public <R1, R> Maybe<Active<W, R>> forEach2(Function<? super T, ? extends Higher<W, R1>> value1,
    ...
    public class NarrowedFlatMap<C, R> {
        ...
        public <R2> Active<W, R2> zip(C fb,
    ...
    public class NarrowedTailRec<C, R> {
        ...
        public Active<W, R> tailRec(T initial,
                                    Function<? super T, ? extends C> fn) {
            return of(def1.monadRec().tailRec(initial,
                                              fn.andThen(r -> narrow.apply(r))),
                      def1);
        }
    }
    ...
    public class Narrowed<C> {
        ...
        public Active<W, T> plus(Monoid<C> m,
                                 C add) {
            return sum(m,
                       LazySeq.of(add));
        }

        public Active<W, T> sum(C seed,
                                BinaryOperator<C> op,
                                ImmutableList<C> list) {
            C res = list.plus(narrow.apply(single))
                        .foldLeft(seed,
                                  (a, b) -> op.apply(a,
                                                     b));
            return of(widen.apply(res),
                      def1);
        }

        public Active<W, T> sum(Monoid<C> s,
                                ImmutableList<C> list) {
            C res = list.plus(narrow.apply(single))
                        .foldLeft(s.zero(),
                                  (a, b) -> s.apply(a,
        ...
        public Active<W, T> sumInverted(Group<C> s,
                                        ImmutableList<C> list) {
            C res = s.invert(list.plus(narrow.apply(single))
                                 .foldLeft(s.zero(),
                                           (a, b) -> s.apply(a,
                                                             b)));
            return of(widen.apply(res),
    ...
    public class Plus {
        ...
        public Active<W, T> sum(ImmutableList<Higher<W, T>> list) {
            return of(monadPlus.sum(list.plus(single)),
                      def1);
        }
        ...
        public Active<W, T> plus(Higher<W, T> a) {
            return of(monadPlus.plus(single,
                                     a),
                      def1);
        }
    ...
    public class Unfolds {
        ...
        public <R, T> Active<W, R> unfold(T b,
                                          Function<? super T, Option<Tuple2<R, T>>> fn) {
            return of(unfoldable.unfold(b,
                                        fn),
                      def1);
        }

        public <T> Active<W, T> replicate(long n,
                                          T value) {
            return unfold(n,
                          i -> i > 0 ? Option.some(tuple(value,
                                                         i < Long.MAX_VALUE ? i - 1 : i)) : Option.none());
        }

        public <R> Nested<W, W, R> replicate(Function<? super T, Long> fn,
                                             Function<? super T, R> mapper) {
            return Nested.of(def1.functor()
                                 .map(value -> replicate(fn.apply(value),
                                                         mapper.apply(value)).getSingle(),
                                      single),
                             def1,
                             def1);
        }
        ...
        public <T> Active<W, T> cycle(T value) {
            return replicate(Long.MAX_VALUE,
                             value);
        }
