...
public class Coyoneda<F, T, R> implements Higher3<coyoneda, F, T, R> {
    ...
    public static <F, T, R> Coyoneda<F, T, R> of(Function<? super T, ? extends R> fn,
                                                 Higher<F, T> higher) {
        return new Coyoneda<>(fn,
                              higher);
    }
    ...
    public Yoneda<F, R> toYoneda(Functor<F> functor) {
        return new Yoneda<F, R>() {
            public <R2> Higher<F, R2> apply(Function<? super R, ? extends R2> f) {
                Higher<F, ? extends R2> local = Coyoneda.this.map(f)
                                                             .run(functor);
    ...
    public <R2> Coyoneda<F, T, R2> map(Function<? super R, ? extends R2> f) {
        return new Coyoneda<F, T, R2>(i -> f.apply(function.apply(i)),
                                      higher);
    }
    ...
    public static class Instances {
        ...
        public static <F, T1, R> Functor<Higher<Higher<coyoneda, F>, T1>> functor() {
            return new Functor<Higher<Higher<coyoneda, F>, T1>>() {
                @Override
                public <T, R> Higher<Higher<Higher<coyoneda, F>, T1>, R> map(Function<? super T, ? extends R> fn,
                                                                             Higher<Higher<Higher<coyoneda, F>, T1>, T> ds) {
                    return ds.convert(Coyoneda::narrowK)
                             .map(fn);
                }
