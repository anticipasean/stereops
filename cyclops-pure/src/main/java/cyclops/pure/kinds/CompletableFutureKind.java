...
public interface CompletableFutureKind<T> extends Higher<DataWitness.completableFuture, T>, CompletionStage<T> {
    ...
    static <W1, T> Nested<DataWitness.completableFuture, W1, T> nested(CompletableFuture<Higher<W1, T>> nested,
                                                                       InstanceDefinitions<W1> def2) {
        return Nested.of(CompletableFutureKind.widen(nested),
                         CompletableFutureInstances.definitions(),
                         def2);
    }

    static <W1, T> Product<DataWitness.completableFuture, W1, T> product(CompletableFuture<T> f,
                                                                         Active<W1, T> active) {
        return Product.of(allTypeclasses(f),
                          active);
    }

    static <W1, T> Coproduct<W1, DataWitness.completableFuture, T> coproduct(CompletableFuture<T> f,
                                                                             InstanceDefinitions<W1> def2) {
        return Coproduct.right(CompletableFutureKind.widen(f),
                               def2,
                               CompletableFutureInstances.definitions());
    }

    static <T> Active<DataWitness.completableFuture, T> allTypeclasses(CompletableFuture<T> f) {
        return Active.of(CompletableFutureKind.widen(f),
                         CompletableFutureInstances.definitions());
    }

    static <W2, T, R> Nested<DataWitness.completableFuture, W2, R> mapM(CompletableFuture<T> f,
    ...
    static <T> Kleisli<DataWitness.completableFuture, CompletableFuture<T>, T> kindKleisli() {
        return Kleisli.of(CompletableFutureInstances.monad(),
                          CompletableFutureKind::widen);
    }
     ...
     */
    static <T> CompletableFutureKind<T> completedFuture(T value) {
    ...
    /**
     * Convert a CompletableFuture to a simulated HigherKindedType that captures CompletableFuture nature and CompletableFuture
     * element data type separately. Recover via @see CompletableFutureKind#narrow
     * <p>
     * If the supplied CompletableFuture implements CompletableFutureKind it is returned already, otherwise it is wrapped into a
     * CompletableFuture implementation that does implement CompletableFutureKind
     *
     ...
     */
    static <T> CompletableFutureKind<T> widen(final CompletionStage<T> completableFuture) {
        if (completableFuture instanceof CompletableFutureKind) {
            return (CompletableFutureKind<T>) completableFuture;
        }
        return new Box<>(completableFuture);
    }

    static <T> CompletableFutureKind<T> fromFuture(final Future<T> completableFuture) {
        return widen(completableFuture.toCompletableFuture());
    }

    static <T> CompletableFutureKind<T> narrowFuture(final Higher<DataWitness.future, T> future) {
     ...
     */
    static <T> CompletableFutureKind<T> narrow(final Higher<DataWitness.completableFuture, T> future) {
     ...
     */
    static <T> CompletableFuture<T> narrowK(final Higher<DataWitness.completableFuture, T> completableFuture) {
    ...
    final class Box<T> implements CompletableFutureKind<T> {
        ...
        public <W2, R> Nested<DataWitness.completableFuture, W2, R> mapM(Function<? super T, ? extends Higher<W2, R>> fn,
                                                                         InstanceDefinitions<W2> defs) {
            return Nested.of(thenApply(fn),
                             CompletableFutureInstances.definitions(),
                             defs);
        }
        ...
        public <U, V> CompletableFutureKind<V> thenCombine(final CompletionStage<? extends U> other,
            ...
            return widen(boxed.thenCombine(other,
                                           fn));
        }
        ...
        public <U, V> CompletableFutureKind<V> thenCombineAsync(final CompletionStage<? extends U> other,
                                                                final BiFunction<? super T, ? super U, ? extends V> fn) {
            return widen(boxed.thenCombineAsync(other,
                                                fn));
        }
        ...
        public <U> CompletableFutureKind<Void> thenAcceptBoth(final CompletionStage<? extends U> other,
            ...
            return widen(boxed.thenAcceptBoth(other,
                                              action));
        }
        ...
        public <U> CompletableFutureKind<Void> thenAcceptBothAsync(final CompletionStage<? extends U> other,
                                                                   final BiConsumer<? super T, ? super U> action) {
            return widen(boxed.thenAcceptBothAsync(other,
                                                   action));
        }
        ...
        @Override
        public CompletableFutureKind<Void> runAfterBoth(final CompletionStage<?> other,
                                                        final Runnable action) {
            return widen(boxed.runAfterBoth(other,
                                            action));
        }

        @Override
        public CompletableFutureKind<Void> runAfterBothAsync(final CompletionStage<?> other,
                                                             final Runnable action) {
            return widen(boxed.runAfterBothAsync(other,
                                                 action));
        }
        ...
        public <U> CompletableFutureKind<U> applyToEither(final CompletionStage<? extends T> other,
            ...
            return widen(boxed.applyToEither(other,
                                             fn));
        }
        ...
        public <U> CompletableFutureKind<U> applyToEitherAsync(final CompletionStage<? extends T> other,
                                                               final Function<? super T, U> fn) {
            return widen(boxed.applyToEitherAsync(other,
                                                  fn));
        }
        ...
        public CompletableFutureKind<Void> acceptEither(final CompletionStage<? extends T> other,
            ...
            return widen(boxed.acceptEither(other,
                                            action));
        }
        ...
        public CompletableFutureKind<Void> acceptEitherAsync(final CompletionStage<? extends T> other,
                                                             final Consumer<? super T> action) {
            return widen(boxed.acceptEitherAsync(other,
                                                 action));
        }
        ...
        @Override
        public CompletableFutureKind<Void> runAfterEither(final CompletionStage<?> other,
                                                          final Runnable action) {
            return widen(boxed.runAfterEither(other,
                                              action));
        }

        @Override
        public CompletableFutureKind<Void> runAfterEitherAsync(final CompletionStage<?> other,
                                                               final Runnable action) {
            return widen(boxed.runAfterEitherAsync(other,
                                                   action));
        }
