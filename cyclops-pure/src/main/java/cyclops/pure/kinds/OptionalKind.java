...
public final class OptionalKind<T> implements Higher<optional, T> {
    ...
    public static <T> Kleisli<optional, Optional<T>, T> kindKleisli() {
        return Kleisli.of(OptionalInstances.monad(),
                          OptionalKind::widen);
    }
    ...
    public static <W1, T> Nested<optional, W1, T> nested(Optional<Higher<W1, T>> nested,
                                                         InstanceDefinitions<W1> def2) {
        return Nested.of(OptionalKind.widen(nested),
                         OptionalInstances.definitions(),
                         def2);
    }

    public static <W1, T> Coproduct<W1, optional, T> coproduct(Optional<T> f,
                                                               InstanceDefinitions<W1> def2) {
        return Coproduct.right(OptionalKind.widen(f),
                               def2,
                               OptionalInstances.definitions());
    }

    public static <T> Active<optional, T> allTypeclasses(Optional<T> f) {
        return Active.of(OptionalKind.widen(f),
                         OptionalInstances.definitions());
    }
     ...
     * Recover via @see OptionalKind#narrow
     * <p>
     * If the supplied Optional implements OptionalKind it is returned already, otherwise it is wrapped into a Optional
     * implementation that does implement OptionalKind
     *
    ...
    public <W1, T> Product<optional, W1, T> product(Optional<T> f,
                                                    Active<W1, T> active) {
        return Product.of(allTypeclasses(f),
                          active);
    }

    public <W2, T, R> Nested<optional, W2, R> mapM(Optional<T> f,
    ...
    public <W2, R> Nested<optional, W2, R> mapM(Function<? super T, ? extends Higher<W2, R>> fn,
                                                InstanceDefinitions<W2> defs) {
