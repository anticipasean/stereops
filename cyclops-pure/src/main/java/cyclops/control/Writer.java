...
public final class Writer<W, T> implements Transformable<T>, Iterable<T>, Higher2<writer, W, T>, Serializable {
    ...
    public static <W, T> Writer<W, T> writer(T value,
                                             Monoid<W> combiner) {
        return new Writer<W, T>(Tuple.tuple(value,
                                            combiner.zero()),
                                combiner);
    }
    ...
    public static <W, T> Writer<W, T> writer(Tuple2<T, W> values,
                                             Monoid<W> combiner) {
        return new Writer<W, T>(values,
                                combiner);
    }
    ...
    public <R> Writer<W, R> map(Function<? super T, ? extends R> mapper) {
        return writer(mapper.apply(value._1()),
                      value._2(),
                      monoid);
    }
    ...
    public <R> Writer<W, R> flatMap(Function<? super T, ? extends Writer<W, ? extends R>> fn) {
        ...
        return writer(writer.value._1(),
                      writer.monoid.apply(value._2(),
                                          writer.value._2()),
                      writer.monoid);
    }

    public Writer<W, T> tell(W write) {
        return writer(value._1(),
                      monoid.apply(write,
                                   value._2()),
                      monoid);
    }
    ...
    public <R1, R2, R3, R4> Writer<W, R4> forEach4(Function<? super T, ? extends Writer<W, R1>> value2,
        ...
        return this.flatMap(in -> {
            ...
            return a.flatMap(ina -> {
                Writer<W, R2> b = value3.apply(in,
                                               ina);
                return b.flatMap(inb -> {

                    Writer<W, R3> c = value4.apply(in,
                                                   ina,
                                                   inb);
    ...
    /**
     * Perform a For Comprehension over a Writer, accepting 2 generating function. This results in a three level nested internal
     * iteration over the provided Writers.
     *
     * <pre>
     * {@code
     *
    ...
    /**
     * Perform a For Comprehension over a Writer, accepting a generating function. This results in a two level nested internal
     * iteration over the provided Writers.
     *
     * <pre>
     * {@code
