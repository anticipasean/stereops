...
public interface StreamKind<T> extends Higher<DataWitness.stream, T>, Stream<T> {
    ...
    static <T> Kleisli<DataWitness.stream, Stream<T>, T> kindKleisli() {
        return Kleisli.of(StreamInstances.monad(),
                          StreamKind::widen);
    }
    ...
    static <W1, T> Nested<DataWitness.stream, W1, T> nested(Stream<Higher<W1, T>> nested,
                                                            InstanceDefinitions<W1> def2) {
        return Nested.of(StreamKind.widen(nested),
                         StreamInstances.definitions(),
                         def2);
    }

    static <W1, T> Product<DataWitness.stream, W1, T> product(Stream<T> f,
                                                              Active<W1, T> active) {
        return Product.of(allTypeclasses(f),
                          active);
    }

    static <W1, T> Coproduct<W1, DataWitness.stream, T> coproduct(Stream<T> f,
                                                                  InstanceDefinitions<W1> def2) {
        return Coproduct.right(StreamKind.widen(f),
                               def2,
                               StreamInstances.definitions());
    }

    static <T> Active<DataWitness.stream, T> allTypeclasses(Stream<T> f) {
        return Active.of(StreamKind.widen(f),
                         StreamInstances.definitions());
    }

    static <W2, T, R> Nested<DataWitness.stream, W2, R> mapM(Stream<T> f,
    ...
    static <T> StreamKind<T> of(T... elements) {
     ...
     * Recover via @see StreamKind#narrow
     * <p>
     * If the supplied Stream implements StreamKind it is returned already, otherwise it is wrapped into a Stream implementation
     * that does implement StreamKind
     *
     * @param stream Stream to widen to a StreamKind
     * @return StreamKind encoding HKT info about Streams
     */
    static <T> StreamKind<T> widen(final Stream<T> stream) {
     ...
     */
    static <C2, T> Higher<C2, Higher<DataWitness.stream, T>> widen2(Higher<C2, StreamKind<T>> stream) {
     ...
     * @return StreamKind
     */
    static <T> StreamKind<T> narrowK(final Higher<DataWitness.stream, T> stream) {
        return (StreamKind<T>) stream;
    }
     ...
     */
    static <T> Stream<T> narrow(final Higher<DataWitness.stream, T> stream) {
    ...
    final class Box<T> implements StreamKind<T> {
        ...
        public <W2, R> Nested<DataWitness.stream, W2, R> mapM(Function<? super T, ? extends Higher<W2, R>> fn,
                                                              InstanceDefinitions<W2> defs) {
            return Nested.of(map(fn),
                             StreamInstances.definitions(),
                             defs);
        }
    }
