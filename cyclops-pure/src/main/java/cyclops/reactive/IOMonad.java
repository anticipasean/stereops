...
public class IOMonad<W, T> implements IO<T> {
    ...
    public static <W, T> IOMonad<W, T> ioMonad(Monad<W> monad,
                                               Higher<W, T> hkt,
                                               ToPublsher<W> to,
                                               FromPublsher<W> from) {
        return new IOMonad<W, T>(monad,
                                 hkt,
                                 to,
                                 from);
    }

    public static <W, T> IOMonad<W, T> ioMonad(Converter<W> converter,
                                               Higher<W, T> hkt) {
        return new IOMonad<W, T>(converter.monad(),
                                 hkt,
                                 converter,
                                 converter);
    }
    ...
    public <R> IO<R> flatMap(Function<? super T, IO<? extends R>> s) {
        return new IOMonad<W, R>(monad,
                                 monad.flatMap(s.andThen(io -> fromPublsher.<R>fromPublisherFn().apply(io.publisher())),
                                               pub),
                                 toPublsher,
                                 fromPublsher);
    }
    ...
    public <R> IO<R> mergeMap(int maxConcurrency,
        ...
        ReactiveSeq<R> r = Spouts.from(this.toPublsher.<T>toPublisherFn().apply(pub))
                                 .mergeMap(maxConcurrency,
                                           s);
        return new IOMonad<W, R>(monad,
                                 fromPublsher.<R>fromPublisherFn().apply(r),
                                 toPublsher,
                                 fromPublsher);
