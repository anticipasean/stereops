...
public abstract class Free<F, T> implements Higher2<free, F, T> {
    ...
    public static <F, T> Free<F, T> liftF(final Higher<F, T> value,
                                          final Functor<F> functor) {

        return new Suspend<F, T>(functor.map(Free::done,
                                             value));
    }
    ...
    public static final <F, R1, R2, X1, X2> Tuple2<Either<X1, R1>, Either<X2, R2>> product(final Functor<F> functor,
                                                                                           Free<F, R1> free1,
                                                                                           Function<Higher<F, Free<F, R1>>, X1> decoder1,
                                                                                           Free<F, R2> free2,
                                                                                           Function<Higher<F, Free<F, R2>>, X2> decoder2) {

        return Tuple.tuple(free1.resume(functor,
                                        decoder1),
                           free2.resume(functor,
                                        decoder2));
    ...
    public <R1, R2, R3, R4, R5> Free<F, R5> forEach6(Function<? super T, ? extends Free<F, R1>> value2,
        ...
        return this.flatMap(in -> {
            ...
            return a.flatMap(ina -> {
                Free<F, R2> b = value3.apply(in,
                                             ina);
                return b.flatMap(inb -> {

                    Free<F, R3> c = value4.apply(in,
                                                 ina,
                                                 inb);

                    return c.flatMap(inc -> {
                        Free<F, R4> d = value5.apply(in,
                                                     ina,
                                                     inb,
                                                     inc);
                        return d.flatMap(ind -> {
                            Free<F, R5> e = value6.apply(in,
                                                         ina,
                                                         inb,
                                                         inc,
                                                         ind);
    ...
    public final T go(final Function<? super Higher<F, Free<F, T>>, ? extends Free<F, T>> fn,
                      final Functor<F> functor) {
        Free<F, T> toUse = this;
        for (; ; ) {
            Either<Higher<F, Free<F, T>>, T> xor = toUse.resume(functor);
            if (xor.isRight()) {
    ...
    public final <R> Either<R, T> resume(final Functor<F> functor,
                                         Function<Higher<F, Free<F, T>>, R> decoder) {
        return resume(functor).mapLeft(decoder);
    }

    public <B> Free<F, Tuple2<T, B>> zip(Functor<F> f,
                                         Free<F, B> b) {
        return zip(f,
                   b,
                   Tuple::tuple);
    ...
    public <B, R> Free<F, R> zip(Functor<F> f,
                                 Free<F, B> b,
        ...
        if (first.isLeft() && second.isLeft()) {
            return suspend(f.map_(first.leftOrElse(null),
                                  a1 -> {
                                      return suspend(f.map_(second.leftOrElse(null),
                                                            b1 -> a1.zip(f,
                                                                         b1,
                                                                         zipper)));
                                  }));
        ...
        if (first.isRight() && second.isRight()) {
            return done(zipper.apply(first.orElse(null),
                                     second.orElse(null)));
        }
        ...
        if (first.isRight() && second.isLeft()) {
            return suspend(f.map_(second.leftOrElse(null),
                                  a1 -> Free.<F, T>done(first.orElse(null)).zip(f,
                                                                                b,
                                                                                zipper)));
        }
        return null;
    ...
    public <B, C> Free<F, Tuple3<T, B, C>> zip(Functor<F> f,
                                               Free<F, B> b,
                                               Free<F, C> c) {
        return zip(f,
                   b,
    ...
    public <B, C, R> Free<F, R> zip(Functor<F> f,
                                    Free<F, B> b,
                                    Free<F, C> c,
                                    Function3<? super T, ? super B, ? super C, ? extends R> fn) {
        ...
        if (first.isLeft() && second.isLeft() && third.isLeft()) {
            return suspend(f.map_(first.leftOrElse(null),
                                  a1 -> {
                                      return suspend(f.map_(second.leftOrElse(null),
                                                            b1 -> {
                                                                return suspend(f.map_(third.leftOrElse(null),
                                                                                      c1 -> a1.zip(f,
                                                                                                   b1,
                                                                                                   c1,
                                                                                                   fn)));
        ...
        if (first.isRight() && second.isRight() && third.isRight()) {
            return done(fn.apply(first.orElse(null),
                                 second.orElse(null),
                                 third.orElse(null)));
        }
    ...
    public final Either<Higher<F, Free<F, T>>, T> resume(final Functor<F> functor) {
        return resumeInternal(functor).fold(Either::left,
                                            Either::right,
                                            t -> null);
    ...
    public interface Comprehensions {
        ...
        static <T, F, R1, R2, R3, R4, R5, R6, R7> Free<F, R7> forEach(Free<F, T> free,
            ...
            return free.flatMap(in -> {
                ...
                Free<F, R1> a = value2.apply(in);
                return a.flatMap(ina -> {
                    Free<F, R2> b = value3.apply(Tuple.tuple(in,
                                                             ina));
                    return b.flatMap(inb -> {

                        Free<F, R3> c = value4.apply(Tuple.tuple(in,
                                                                 ina,
                                                                 inb));

                        return c.flatMap(inc -> {
                            Free<F, R4> d = value5.apply(Tuple.tuple(in,
                                                                     ina,
                                                                     inb,
                                                                     inc));
                            return d.flatMap(ind -> {
                                Free<F, R5> e = value6.apply(Tuple.tuple(in,
                                                                         ina,
                                                                         inb,
                                                                         inc,
                                                                         ind));
                                return e.flatMap(ine -> {
                                    Free<F, R6> f = value7.apply(Tuple.tuple(in,
                                                                             ina,
                                                                             inb,
                                                                             inc,
                                                                             ind,
                                    ...
                                    return f.flatMap(inf -> {
                                        Free<F, R7> g = value8.apply(Tuple.tuple(in,
                                                                                 ina,
                                                                                 inb,
                                                                                 inc,
                                                                                 ind,
        ...
        static <T, F, R1, R2, R3, R4, R5, R6> Free<F, R6> forEach(Free<F, T> free,
                                                                  Function<? super T, ? extends Free<F, R1>> value2,
                                                                  Function<? super Tuple2<? super T, ? super R1>, ? extends Free<F, R2>> value3,
                                                                  Function<? super Tuple3<? super T, ? super R1, ? super R2>, ? extends Free<F, R3>> value4,
                                                                  Function<? super Tuple4<? super T, ? super R1, ? super R2, ? super R3>, ? extends Free<F, R4>> value5,
        ...
        static <T, F, R1, R2, R3, R4, R5> Free<F, R5> forEach(Free<F, T> free,
                                                              Function<? super T, ? extends Free<F, R1>> value2,
                                                              Function<? super Tuple2<? super T, ? super R1>, ? extends Free<F, R2>> value3,
                                                              Function<? super Tuple3<? super T, ? super R1, ? super R2>, ? extends Free<F, R3>> value4,
                                                              Function<? super Tuple4<? super T, ? super R1, ? super R2, ? super R3>, ? extends Free<F, R4>> value5,
        ...
        static <T, F, R1, R2, R3, R4> Free<F, R4> forEach(Free<F, T> free,
                                                          Function<? super T, ? extends Free<F, R1>> value2,
                                                          Function<? super Tuple2<? super T, ? super R1>, ? extends Free<F, R2>> value3,
                                                          Function<? super Tuple3<? super T, ? super R1, ? super R2>, ? extends Free<F, R3>> value4,
                                                          Function<? super Tuple4<? super T, ? super R1, ? super R2, ? super R3>, ? extends Free<F, R4>> value5
        ...
        static <T, F, R1, R2, R3> Free<F, R3> forEach(Free<F, T> free,
                                                      Function<? super T, ? extends Free<F, R1>> value2,
                                                      Function<? super Tuple2<? super T, ? super R1>, ? extends Free<F, R2>> value3,
                                                      Function<? super Tuple3<? super T, ? super R1, ? super R2>, ? extends Free<F, R3>> value4
        ...
        static <T, F, R1, R2> Free<F, R2> forEach(Free<F, T> free,
                                                  Function<? super T, ? extends Free<F, R1>> value2,
                                                  Function<? super Tuple2<? super T, ? super R1>, ? extends Free<F, R2>> value3
        ...
        static <T, F, R1> Free<F, R1> forEach(Free<F, T> free,
                                              Function<? super T, ? extends Free<F, R1>> value2
    ...
    static class FreeF<F, T> {
        ...
        public final <R1, R2, X1, X2> Tuple2<Either<X1, R1>, Either<X2, R2>> product(Free<F, R1> free1,
                                                                                     Free<F, R2> free2) {
    ...
    private static final class FlatMapped<F, IN, T> extends Free<F, T> {
        ...
        private FlatMapped(final Free<F, IN> free,
                           final Function<? super IN, ? extends Free<F, ? extends T>> fn) {
            this.free = free;
            this.fn = fn;
        }
        ...
        public <R> Free<F, R> flatMap(final Function<? super T, ? extends Free<F, ? extends R>> g) {
            return new FlatMapped<F, IN, R>(free,
                                            aa -> new FlatMapped<F, T, R>(narrowFn().apply(aa),
                                                                          g));
        }
        ...
        private <U> LazyEither3<Higher<F, Free<F, T>>, T, Free<F, T>> resumeNestedFree(Functor<F> functor) {
            ...
            return free.fold(pure -> LazyEither3.right(f.apply(pure.value)),
                             s -> LazyEither3.left1(functor.map(o -> o.flatMap(f),
                                                                s.suspended)),
                             fm -> {
                                 final FlatMapped<F, U, IN> flatMapped2 = (FlatMapped<F, U, IN>) fm;
                                 return LazyEither3.right(flatMapped2.free.flatMap(o -> flatMapped2.fn.apply(o)
                                                                                                      .flatMap(fn)));
                             });
