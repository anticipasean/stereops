...
import static cyclops.container.immutable.impl.LazySeq.narrowK;
...
public class LazySeqInstances {
    ...
    public static <T> Kleisli<lazySeq, LazySeq<T>, T> kindKleisli() {
        return Kleisli.of(LazySeqInstances.monad(),
                          LazySeq::widen);
    }
    ...
    public static <W1, T> Nested<lazySeq, W1, T> nested(LazySeq<Higher<W1, T>> nested,
    ...
    public static <W1, T> Product<lazySeq, W1, T> product(LazySeq<T> l,
                                                          Active<W1, T> active) {
        return Product.of(allTypeclasses(l),
                          active);
    }

    public static <W1, T> Coproduct<W1, lazySeq, T> coproduct(LazySeq<T> l,
                                                              InstanceDefinitions<W1> def2) {
    ...
    public static <W2, R, T> Nested<lazySeq, W2, R> mapM(LazySeq<T> l,
    ...
    public static class LazySeqTypeClasses implements MonadPlus<lazySeq>, MonadRec<lazySeq>, TraverseByTraverse<lazySeq>,
        ...
        @Override
        public <T> Higher<lazySeq, T> filter(Predicate<? super T> predicate,
                                             Higher<lazySeq, T> ds) {
            return narrowK(ds).filter(predicate);
        }

        @Override
        public <T, R> Higher<lazySeq, Tuple2<T, R>> zip(Higher<lazySeq, T> fa,
                                                        Higher<lazySeq, R> fb) {
            return narrowK(fa).zip(narrowK(fb));
        }

        @Override
        public <T1, T2, R> Higher<lazySeq, R> zip(Higher<lazySeq, T1> fa,
                                                  Higher<lazySeq, T2> fb,
        ...
        @Override
        public <T, R> Higher<lazySeq, R> flatMap(Function<? super T, ? extends Higher<lazySeq, R>> fn,
                                                 Higher<lazySeq, T> ds) {
            return narrowK(ds).flatMap(i -> narrowK(fn.apply(i)));
        }

        @Override
        public <T, R> Higher<lazySeq, R> ap(Higher<lazySeq, ? extends Function<T, R>> fn,
                                            Higher<lazySeq, T> apply) {
        ...
        @Override
        public <T, R> Higher<lazySeq, R> map(Function<? super T, ? extends R> fn,
                                             Higher<lazySeq, T> ds) {
            return narrowK(ds).map(fn);
        }

        @Override
        public <T, R> Higher<lazySeq, R> tailRec(T initial,
                                                 Function<? super T, ? extends Higher<lazySeq, ? extends Either<T, R>>> fn) {
            return LazySeq.tailRec(initial,
                                   i -> narrowK(fn.apply(i)));
        }

        @Override
        public <C2, T, R> Higher<C2, Higher<lazySeq, R>> traverseA(Applicative<C2> ap,
        ...
        @Override
        public <R, T> Higher<lazySeq, R> unfold(T b,
                                                Function<? super T, Option<Tuple2<R, T>>> fn) {
            return LazySeq.unfold(b,
                                  fn);
        }
    }
