...
public class SetXInstances {
    ...
    public static <W1, T> Nested<set, W1, T> nested(SetX<Higher<W1, T>> nested,
                                                    InstanceDefinitions<W1> def2) {
        return Nested.of(nested,
                         SetXInstances.definitions(),
                         def2);
    }

    public static <W1, T> Product<set, W1, T> product(SetX<T> s,
                                                      Active<W1, T> active) {
        return Product.of(allTypeclasses(s),
                          active);
    }

    public static <W1, T> Coproduct<W1, set, T> coproduct(SetX<T> s,
                                                          InstanceDefinitions<W1> def2) {
    ...
    public static <W2, R, T> Nested<set, W2, R> mapM(SetX<T> s,
    ...
    public static <T> Kleisli<set, SetX<T>, T> kindKleisli() {
        return Kleisli.of(SetXInstances.monad(),
                          SetX::widen);
    }
    ...
    public static class SetXTypeClasses implements MonadPlus<set>, MonadRec<set>, TraverseByTraverse<set>, Foldable<set>,
        ...
        @Override
        public <T> Higher<set, T> filter(Predicate<? super T> predicate,
                                         Higher<set, T> ds) {
            return narrowK(ds).filter(predicate);
        }

        @Override
        public <T, R> Higher<set, Tuple2<T, R>> zip(Higher<set, T> fa,
                                                    Higher<set, R> fb) {
            return narrowK(fa).zip(narrowK(fb));
        }

        @Override
        public <T1, T2, R> Higher<set, R> zip(Higher<set, T1> fa,
                                              Higher<set, T2> fb,
        ...
        @Override
        public <T, R> Higher<set, R> flatMap(Function<? super T, ? extends Higher<set, R>> fn,
                                             Higher<set, T> ds) {
            return narrowK(ds).concatMap(i -> narrowK(fn.apply(i)));
        }

        @Override
        public <T, R> Higher<set, R> ap(Higher<set, ? extends Function<T, R>> fn,
                                        Higher<set, T> apply) {
        ...
        @Override
        public <T, R> Higher<set, R> map(Function<? super T, ? extends R> fn,
                                         Higher<set, T> ds) {
        ...
        @Override
        public <T, R> Higher<set, R> tailRec(T initial,
                                             Function<? super T, ? extends Higher<set, ? extends Either<T, R>>> fn) {
            return SetX.tailRec(initial,
                                i -> narrowK(fn.apply(i)));
        }

        @Override
        public <C2, T, R> Higher<C2, Higher<set, R>> traverseA(Applicative<C2> ap,
        ...
        @Override
        public <R, T> Higher<set, R> unfold(T b,
                                            Function<? super T, Option<Tuple2<R, T>>> fn) {
            return SetX.unfold(b,
                               fn);
        }
