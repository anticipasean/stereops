...
public class ListXInstances {
    ...
    public static <T> Kleisli<list, ListX<T>, T> kindKleisli() {
        return Kleisli.of(ListXInstances.monad(),
                          ListXInstances::widen);
    }
    ...
    public static <W1, T> Nested<list, W1, T> nested(ListX<Higher<W1, T>> nested,
                                                     InstanceDefinitions<W1> def2) {
        return Nested.of(nested,
                         ListXInstances.definitions(),
                         def2);
    }

    public static <W1, T> Product<list, W1, T> product(ListX<T> l,
                                                       Active<W1, T> active) {
        return Product.of(allTypeclasses(l),
                          active);
    }

    public static <W1, T> Coproduct<W1, list, T> coproduct(ListX<T> l,
                                                           InstanceDefinitions<W1> def2) {
    ...
    public static <W2, R, T> Nested<list, W2, R> mapM(ListX<T> l,
    ...
    public static class ListXTypeClasses implements MonadPlus<list>, MonadRec<list>, TraverseByTraverse<list>, Foldable<list>,
        ...
        @Override
        public <T> Higher<list, T> filter(Predicate<? super T> predicate,
                                          Higher<list, T> ds) {
            return narrowK(ds).filter(predicate);
        }

        @Override
        public <T, R> Higher<list, Tuple2<T, R>> zip(Higher<list, T> fa,
                                                     Higher<list, R> fb) {
            return narrowK(fa).zip(narrowK(fb));
        }

        @Override
        public <T1, T2, R> Higher<list, R> zip(Higher<list, T1> fa,
                                               Higher<list, T2> fb,
        ...
        @Override
        public <T, R> Higher<list, R> flatMap(Function<? super T, ? extends Higher<list, R>> fn,
                                              Higher<list, T> ds) {
            return narrowK(ds).concatMap(i -> narrowK(fn.apply(i)));
        }

        @Override
        public <T, R> Higher<list, R> ap(Higher<list, ? extends Function<T, R>> fn,
                                         Higher<list, T> apply) {
        ...
        @Override
        public <T, R> Higher<list, R> map(Function<? super T, ? extends R> fn,
                                          Higher<list, T> ds) {
        ...
        @Override
        public <T, R> Higher<list, R> tailRec(T initial,
                                              Function<? super T, ? extends Higher<list, ? extends Either<T, R>>> fn) {
            return ListX.tailRec(initial,
                                 i -> narrowK(fn.apply(i)));
        }

        @Override
        public <C2, T, R> Higher<C2, Higher<list, R>> traverseA(Applicative<C2> ap,
        ...
        @Override
        public <R, T> Higher<list, R> unfold(T b,
                                             Function<? super T, Option<Tuple2<R, T>>> fn) {
            return ListX.unfold(b,
                                fn);
        }
