...
public class IOInstances {
    ...
    public static <W1, T> Nested<io, W1, T> nested(IO<Higher<W1, T>> nested,
                                                   InstanceDefinitions<W1> def2) {
        return Nested.of(nested,
                         IOInstances.definitions(),
                         def2);
    }

    public static <W1, T> Product<io, W1, T> product(IO<T> l,
                                                     Active<W1, T> active) {
        return Product.of(allTypeclasses(l),
                          active);
    }

    public static <W1, T> Coproduct<W1, io, T> coproduct(IO<T> l,
                                                         InstanceDefinitions<W1> def2) {
    ...
    public static <W2, R, T> Nested<io, W2, R> mapM(IO<T> l,
    ...
    public static class IOTypeClasses implements MonadPlus<io>, MonadRec<io>, TraverseByTraverse<io>, Foldable<io>,
        ...
        @Override
        public <T> Higher<io, T> filter(Predicate<? super T> predicate,
                                        Higher<io, T> ds) {
            return IO.fromPublisher(narrowK(ds).stream()
                                               .filter(predicate));
        }

        @Override
        public <T, R> Higher<io, Tuple2<T, R>> zip(Higher<io, T> fa,
                                                   Higher<io, R> fb) {
        ...
        @Override
        public <T1, T2, R> Higher<io, R> zip(Higher<io, T1> fa,
                                             Higher<io, T2> fb,
        ...
        @Override
        public <T, R> Higher<io, R> flatMap(Function<? super T, ? extends Higher<io, R>> fn,
                                            Higher<io, T> ds) {
            return narrowK(ds).flatMap(i -> narrowK(fn.apply(i)));
        }

        @Override
        public <T, R> Higher<io, R> ap(Higher<io, ? extends Function<T, R>> fn,
                                       Higher<io, T> apply) {
        ...
        @Override
        public <T, R> Higher<io, R> map(Function<? super T, ? extends R> fn,
                                        Higher<io, T> ds) {
            return narrowK(ds).map(fn);
        }//}, Traverse<io>{

        @Override
        public <T, R> Higher<io, R> tailRec(T initial,
                                            Function<? super T, ? extends Higher<io, ? extends Either<T, R>>> fn) {
            return IO.fromPublisher(ReactiveSeq.tailRec(initial,
                                                        i -> narrowK(fn.apply(i)).stream()));
        }

        @Override
        public <C2, T, R> Higher<C2, Higher<io, R>> traverseA(Applicative<C2> ap,
                                                              Function<? super T, ? extends Higher<C2, R>> fn,
                                                              Higher<io, T> ds) {
            IO<T> v = narrowK(ds);

            return v.foldLeft(ap.unit(IO.fromPublisher(Spouts.empty())),
                              (a, b) -> ap.zip(fn.apply(b),
                                                                          a,
                                                                          (sn, vec) -> IO.fromPublisher(narrowK(vec).stream()
                                                                                                                    .plus(sn))));
        ...
        public <T, R> Higher<io, Tuple2<T, Long>> zipWithIndex(Higher<io, T> ds) {
            return IO.fromPublisher(Spouts.from(narrowK(ds))
                                          .zipWithIndex());
        }
        ...
        public <T> T foldLeft(Monoid<T> monoid,
                              Higher<io, T> ds) {
            return narrowK(ds).foldLeft(monoid.zero(),
                                        monoid);
        }


        @Override
        public <R, T> Higher<io, R> unfold(T b,
                                           Function<? super T, Option<Tuple2<R, T>>> fn) {
            return IO.fromPublisher(Spouts.unfold(b,
                                                  fn));
        }
    }
