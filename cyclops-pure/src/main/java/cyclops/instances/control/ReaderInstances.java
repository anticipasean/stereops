...
import static cyclops.container.control.Reader.narrowK;

import cyclops.function.higherkinded.DataWitness.reader;
...
public class ReaderInstances {
    ...
    public static <IN, T, R> Reader<IN, R> tailRec(T initial,
                                                   Function<? super T, ? extends Reader<IN, ? extends Either<T, R>>> fn) {
        return narrowK(ReaderInstances.<IN, T, R>monadRec().tailRec(initial,
                                                                    fn));
    }

    public static <R, T> Kleisli<Higher<reader, T>, Reader<T, R>, R> kindKleisli() {
        return Kleisli.of(ReaderInstances.monad(),
                          Reader::widen);
    }
    ...
    public static <W1, T, R> Nested<Higher<reader, T>, W1, R> nested(Reader<T, Higher<W1, R>> nested,
                                                                     T defaultValue,
                                                                     InstanceDefinitions<W1> def2) {
        return Nested.of(nested,
                         ReaderInstances.definitions(defaultValue),
                         def2);
    }

    public static <W1, T, R> Product<Higher<reader, T>, W1, R> product(Reader<T, R> r,
                                                                       T defaultValue,
                                                                       Active<W1, R> active) {
        return Product.of(allTypeclasses(r,
                                         defaultValue),
                          active);
    }

    public static <W1, T, R> Coproduct<W1, Higher<reader, T>, R> coproduct(Reader<T, R> r,
                                                                           T defaultValue,
                                                                           InstanceDefinitions<W1> def2) {
    ...
    public static <T, R> Active<Higher<reader, T>, R> allTypeclasses(Reader<T, R> r,
                                                                     T defaultValue) {
        return Active.of(r,
                         ReaderInstances.definitions(defaultValue));
    }

    public static <W2, R2, T, R> Nested<Higher<reader, T>, W2, R2> mapM(Reader<T, R> r,
                                                                        T defaultValue,
                                                                        Function<? super R, ? extends Higher<W2, R2>> fn,
                                                                        InstanceDefinitions<W2> defs) {
        return Nested.of(r.mapFn(fn),
                         ReaderInstances.definitions(defaultValue),
                         defs);
    }
    ...
    public static <IN> Foldable<Higher<reader, IN>> foldable(IN t) {
        return new Foldable<Higher<reader, IN>>() {
            ...
            public <T, R> R foldMap(Monoid<R> mb,
                                    ...
                                    Higher<Higher<reader, IN>, T> nestedA) {
                return foldLeft(mb,
                                narrowK(nestedA).mapFn(fn));
            }
        };
    }

    public static <IN, C2, T, R> Higher<C2, Higher<Higher<reader, IN>, R>> traverseA(IN t,
                                                                                     Applicative<C2> applicative,
    ...
    public static <IN> Traverse<Higher<reader, IN>> traversable(IN t) {
        return new Traverse<Higher<reader, IN>>() {
            @Override
            public <C2, T, R> Higher<C2, Higher<Higher<reader, IN>, R>> traverseA(Applicative<C2> applicative,
                                                                                  Function<? super T, ? extends Higher<C2, R>> fn,
                                                                                  Higher<Higher<reader, IN>, T> ds) {
                Reader<IN, T> r = narrowK(ds);
            ...
            @Override
            public <C2, T> Higher<C2, Higher<Higher<reader, IN>, T>> sequenceA(Applicative<C2> applicative,
                                                                               Higher<Higher<reader, IN>, Higher<C2, T>> ds) {
            ...
            @Override
            public <T, R> Higher<Higher<reader, IN>, R> ap(Higher<Higher<reader, IN>, ? extends Function<T, R>> fn,
                                                           Higher<Higher<reader, IN>, T> apply) {
                return ReaderInstances.<IN>applicative().ap(fn,
                                                            apply);
            }
            ...
            @Override
            public <T, R> Higher<Higher<reader, IN>, R> map(Function<? super T, ? extends R> fn,
                                                            Higher<Higher<reader, IN>, T> ds) {
                return ReaderInstances.<IN>applicative().map(fn,
                                                             ds);
            }
        };
    ...
    public static class ReaderTypeclass<IN> implements Monad<Higher<reader, IN>>, MonadRec<Higher<reader, IN>>,
                                                       ProFunctor<reader> {


        @Override
        public <A, B, C, D> Higher<Higher<reader, C>, D> dimap(Function<? super C, ? extends A> f,
                                                               Function<? super B, ? extends D> g,
                                                               Higher<Higher<reader, A>, B> ds) {
            Reader<A, B> r = narrowK(ds);
            Function<? super C, ? extends D> f1 = g.compose(r)
                                                   .compose(f);
        ...
        @Override
        public <T, R> Higher<Higher<reader, IN>, R> flatMap(Function<? super T, ? extends Higher<Higher<reader, IN>, R>> fn,
                                                            Higher<Higher<reader, IN>, T> ds) {
        ...
        @Override
        public <T, R> Higher<Higher<reader, IN>, R> tailRec(T initial,
                                                            Function<? super T, ? extends Higher<Higher<reader, IN>, ? extends Either<T, R>>> fn) {
