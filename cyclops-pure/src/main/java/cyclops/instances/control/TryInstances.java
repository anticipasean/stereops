...
public class TryInstances {
    ...
    public static <W1, X extends Throwable, T> Nested<Higher<tryType, X>, W1, T> nested(Try<Higher<W1, T>, X> nested,
    ...
    public static <W1, T, X extends Throwable> Product<Higher<tryType, X>, W1, T> product(Try<T, X> t,
                                                                                          Active<W1, T> active) {
        return Product.of(allTypeclasses(t),
                          active);
    }

    public static <W1, T, X extends Throwable> Coproduct<W1, Higher<tryType, X>, T> coproduct(Try<T, X> t,
                                                                                              InstanceDefinitions<W1> def2) {
        return Coproduct.right(t,
                               def2,
                               TryInstances.definitions());
    }

    public static <X extends Throwable, T> Kleisli<Higher<tryType, X>, Try<T, X>, T> kindKleisli() {
        return Kleisli.of(TryInstances.monad(),
                          Try::widen);
    }
    ...
    public static <W2, R, T, X extends Throwable> Nested<Higher<tryType, X>, W2, R> mapM(Try<T, X> t,
                                                                                         Function<? super T, ? extends Higher<W2, R>> fn,
                                                                                         InstanceDefinitions<W2> defs) {
        return Nested.of(t.map(fn),
                         TryInstances.definitions(),
                         defs);
    }
    ...
    public static class TryTypeclasses<L extends Throwable, X> implements Monad<Higher<tryType, L>>, MonadRec<Higher<tryType, L>>,
                                                                          TraverseByTraverse<Higher<tryType, L>>,
                                                                          Foldable<Higher<tryType, L>> {

        @Override
        public <T> T foldRight(Monoid<T> monoid,
                               Higher<Higher<tryType, L>, T> ds) {
        ...
        public <T, R> R foldMap(Monoid<R> mb,
                                ...
                                Higher<Higher<tryType, L>, T> nestedA) {
            return foldLeft(mb,
                            narrowK(nestedA).map(fn));
        }

        @Override
        public <T, R> Higher<Higher<tryType, L>, R> flatMap(Function<? super T, ? extends Higher<Higher<tryType, L>, R>> fn,
                                                            Higher<Higher<tryType, L>, T> ds) {
            Try<T, L> tryType = narrowK(ds);
        ...
        @Override
        public <C2, T, R> Higher<C2, Higher<Higher<tryType, L>, R>> traverseA(Applicative<C2> applicative,
                                                                              Function<? super T, ? extends Higher<C2, R>> fn,
                                                                              Higher<Higher<tryType, L>, T> ds) {
            Try<T, L> maybe = narrowK(ds);
            Function<R, Try<R, L>> rightFn = r -> Try.success(r);

            return maybe.fold(r -> applicative.map(rightFn,
                                                   fn.apply(r)),
                              l -> applicative.unit(Try.failure(l)));
        }


        @Override
        public <T, R> Higher<Higher<tryType, L>, R> ap(Higher<Higher<tryType, L>, ? extends Function<T, R>> fn,
                                                       Higher<Higher<tryType, L>, T> apply) {
            Try<T, L> tryType = narrowK(apply);
            Try<? extends Function<T, R>, L> tryTypeFn = narrowK(fn);
            return tryTypeFn.zip(tryType,
                                 (a, b) -> a.apply(b));
        }
        ...
        @Override
        public <T, R> Higher<Higher<tryType, L>, R> map(Function<? super T, ? extends R> fn,
                                                        Higher<Higher<tryType, L>, T> ds) {
            Try<T, L> tryType = narrowK(ds);
        ...
        @Override
        public <T, R> Higher<Higher<tryType, L>, R> tailRec(T initial,
                                                            Function<? super T, ? extends Higher<Higher<tryType, L>, ? extends Either<T, R>>> fn) {
