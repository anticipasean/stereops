...
import static cyclops.container.control.State.narrowK;
...
public class StateInstances {
    ...
    public static <S, T> Kleisli<Higher<state, S>, State<S, T>, T> kindKleisli() {
        return Kleisli.of(StateInstances.monad(),
                          State::widen);
    }
    ...
    public static <W1, T, S> Nested<Higher<state, S>, W1, T> nested(State<S, Higher<W1, T>> nested,
                                                                    S value,
                                                                    InstanceDefinitions<W1> def2) {
        return Nested.of(nested,
                         StateInstances.definitions(value),
                         def2);
    }

    public static <W1, S, T> Product<Higher<state, S>, W1, T> product(State<S, T> s,
                                                                      S value,
                                                                      Active<W1, T> active) {
    ...
    public static <W1, S, T> Coproduct<W1, Higher<state, S>, T> coproduct(State<S, T> s,
                                                                          S value,
                                                                          InstanceDefinitions<W1> def2) {
    ...
    public <S, T> Active<Higher<state, S>, T> allTypeclasses(State<S, T> s,
                                                             S value) {
        return Active.of(s,
                         StateInstances.definitions(value));
    }

    public <W2, R, S, T> Nested<Higher<state, S>, W2, R> mapM(State<S, T> s,
                                                              S value,
    ...
    public static <S> Functor<Higher<state, S>> functor() {
        return new Functor<Higher<state, S>>() {
            @Override
            public <T, R> Higher<Higher<state, S>, R> map(Function<? super T, ? extends R> fn,
                                                          Higher<Higher<state, S>, T> ds) {
    ...
    public static <S> Traverse<Higher<state, S>> traverse(S defaultValue) {
        return new Traverse<Higher<state, S>>() {
            @Override
            public <C2, T, R> Higher<C2, Higher<Higher<state, S>, R>> traverseA(Applicative<C2> applicative,
                                                                                Function<? super T, ? extends Higher<C2, R>> fn,
                                                                                Higher<Higher<state, S>, T> ds) {
                State<S, T> s = narrowK(ds);
                Higher<C2, R> x = fn.apply(s.eval(defaultValue));
                return applicative.map(r -> State.constant(r),
                                       x);
            }

            @Override
            public <C2, T> Higher<C2, Higher<Higher<state, S>, T>> sequenceA(Applicative<C2> applicative,
                                                                             Higher<Higher<state, S>, Higher<C2, T>> ds) {
            ...
            @Override
            public <T, R> Higher<Higher<state, S>, R> ap(Higher<Higher<state, S>, ? extends Function<T, R>> fn,
                                                         Higher<Higher<state, S>, T> apply) {
                return StateInstances.<S>applicative().ap(fn,
                                                          apply);
            }
            ...
            public <T, R> Higher<Higher<state, S>, R> map(Function<? super T, ? extends R> fn,
                                                          Higher<Higher<state, S>, T> ds) {
                return StateInstances.<S>functor().map(fn,
                                                       ds);
            }
        };
    }
    ...
    public static <S> Foldable<Higher<state, S>> foldable(S val) {
        return new Foldable<Higher<state, S>>() {
            ...
            public <T, R> R foldMap(Monoid<R> mb,
                                    ...
                                    Higher<Higher<state, S>, T> nestedA) {
                return foldLeft(mb,
    ...
    public static class StateMonad<S> implements Monad<Higher<state, S>>, MonadRec<Higher<state, S>> {
        ...
        @Override
        public <T, R> Higher<Higher<state, S>, R> flatMap(Function<? super T, ? extends Higher<Higher<state, S>, R>> fn,
                                                          Higher<Higher<state, S>, T> ds) {
        ...
        @Override
        public <T, R> Higher<Higher<state, S>, R> tailRec(T initial,
                                                          Function<? super T, ? extends Higher<Higher<state, S>, ? extends Either<T, R>>> fn) {
