...
public final class Identity<T> implements Higher<identity, T>, Iterable<T>, Deconstructable1<T>, Serializable {

    private static final long serialVersionUID = 1L;
    public final T value;


    public static <T, R> Identity<R> tailRec(T initial,
                                             Function<? super T, ? extends Identity<? extends Either<T, R>>> fn) {
    ...
    public <T2, R> Identity<R> zip(Identity<? extends T2> id,
                                   BiFunction<? super T, ? super T2, ? extends R> fn) {
        return flatMap(a -> id.map(b -> fn.apply(a,
                                                 b)));
    }
        ...
        public static <T, F, R1, R2, R3, R4, R5, R6, R7> Identity<R7> forEach(Identity<T> id,
            ...
            return id.flatMap(in -> {
                ...
                return a.flatMap(ina -> {
                    Identity<R2> b = value3.apply(Tuple.tuple(in,
                                                              ina));
                    return b.flatMap(inb -> {

                        Identity<R3> c = value4.apply(Tuple.tuple(in,
                                                                  ina,
                                                                  inb));

                        return c.flatMap(inc -> {
                            Identity<R4> d = value5.apply(Tuple.tuple(in,
                                                                      ina,
                                                                      inb,
                                                                      inc));
                            return d.flatMap(ind -> {
                                Identity<R5> e = value6.apply(Tuple.tuple(in,
                                                                          ina,
                                                                          inb,
                                                                          inc,
                                                                          ind));
                                return e.flatMap(ine -> {
                                    Identity<R6> f = value7.apply(Tuple.tuple(in,
                                                                              ina,
                                                                              inb,
                                                                              inc,
                                                                              ind,
                                    ...
                                    return f.flatMap(inf -> {
                                        Identity<R7> g = value8.apply(Tuple.tuple(in,
                                                                                  ina,
                                                                                  inb,
                                                                                  inc,
                                                                                  ind,
