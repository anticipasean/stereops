...
public final class ReaderWriterState<R, W, S, T> implements Higher4<rws, R, W, S, T> {
    ...
    public static <T, R1, W, S, R> ReaderWriterState<R1, W, S, R> tailRec(Monoid<W> monoid,
                                                                          T initial,
                                                                          Function<? super T, ? extends ReaderWriterState<R1, W, S, ? extends Either<T, R>>> fn) {

        Higher<Higher<Higher<Higher<rws, R1>, W>, S>, R> x = ReaderWriterState.tailRecInternal(monoid,
                                                                                               initial,
                                                                                               fn);
        return narrowK(x);
    }

    private static <T, R1, W, S, R> Higher<Higher<Higher<Higher<rws, R1>, W>, S>, R> tailRecInternal(Monoid<W> monoid,
                                                                                                     T initial,
                                                                                                     Function<? super T, ? extends Higher<Higher<Higher<Higher<rws, R1>, W>, S>, ? extends Either<T, R>>> fn) {
        return narrowK(fn.apply(initial)).flatMap(eval -> eval.fold(s -> narrowK(tailRecInternal(monoid,
                                                                                                 s,
                                                                                                 fn)),
                                                                    p -> {
                                                                        ReaderWriterState<R1, W, S, R> k = narrowK(rws((a, s) -> tuple(monoid.zero(),
                                                                                                                                       s,
                                                                                                                                       p),
                                                                                                                       monoid));
                                                                        return k;
    ...
    private static <R, W, S, T> ReaderWriterState<R, W, S, T> suspended(BiFunction<? super R, ? super S, Free<supplier, Tuple3<W, S, T>>> runF,
        ...
        return new ReaderWriterState<R, W, S, T>(monoid,
                                                 (R r, S s) -> SupplierKind.suspend(SupplierKind.λK(() -> runF.apply(r,
                                                                                                                     s))));

    }

    public static <R, W, S, T> ReaderWriterState<R, W, S, T> rws(BiFunction<? super R, ? super S, ? extends Tuple3<W, S, T>> runF,
                                                                 Monoid<W> monoid) {

        return new ReaderWriterState<R, W, S, T>(monoid,
                                                 (r, s) -> Free.done(runF.apply(r,
                                                                                s)));
    }
    ...
    public Tuple3<W, S, T> run(R r,
                               S s) {
        return SupplierKind.run(runState.apply(r,
                                               s));
    }

    public ReaderWriterState<R, W, S, T> tell(W value) {
        BiFunction<? super R, ? super S, Free<supplier, Tuple3<W, S, T>>> fn = (r, s) -> runState.apply(r,
                                                                                                        s)
                                                                                                 .map(t3 -> tuple(monoid.apply(t3._1(),
                                                                                                                               value),
                                                                                                                  t3._2(),
                                                                                                                  t3._3()));
    ...
    public ReaderWriterState<R, W, S, T> ask() {
        return suspended((r, s) -> runState.apply(r,
                                                  s)
                                           .map(t3 -> Tuple.tuple(monoid.zero(),
                                                                  s,
                                                                  t3._3())),
                         monoid);
    }
    ...
    public ReaderWriterState<R, W, S, T> local(Function<? super R, ? extends R> fn) {
        BiFunction<? super R, ? super S, Free<supplier, Tuple3<W, S, T>>> runFn = (R r, S s) -> runState.apply(fn.apply(r),
                                                                                                               s);
        return suspended(runFn,
                         monoid);
    }

    public <R2> ReaderWriterState<R, W, S, R2> map(Function<? super T, ? extends R2> mapper) {

        return mapState(t -> tuple(t._1(),
                                   t._2(),
                                   mapper.apply(t._3())));
    }
    ...
    public <R2> ReaderWriterState<R, W, S, R2> flatMap(Function<? super T, ? extends ReaderWriterState<R, W, S, R2>> f) {
        ...
        return suspended((r, s) -> runState.apply(r,
                                           ...
                                           .flatMap(result -> Free.done(f.apply(result._3())
                                                                         .run(r,
                                                                              result._2())
                                                                         .transform((w2, s2, r2) -> tuple(monoid.apply(w2,
                                                                                                                       result._1()),
                                                                                                          s2,
                                                                                                          r2)
    ...
    public <R1, R2, R3, R4> ReaderWriterState<R, W, S, R4> forEach4(Function<? super T, ReaderWriterState<R, W, S, R1>> value2,
        ...
        return this.flatMap(in -> {
            ...
            return a.flatMap(ina -> {
                ReaderWriterState<R, W, S, R2> b = value3.apply(in,
                                                                ina);
                return b.flatMap(inb -> {

                    ReaderWriterState<R, W, S, R3> c = value4.apply(in,
                                                                    ina,
                                                                    inb);
