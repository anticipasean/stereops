...
public class PersistentMapDeserializer extends StdDeserializer<PersistentMap<?, ?>> {
    ...
    @Override
    public PersistentMap<?, ?> deserialize(JsonParser p,
                                           DeserializationContext ctxt) throws IOException, JsonProcessingException {
        JsonDeserializer deser = ctxt.findRootValueDeserializer(ctxt.getTypeFactory()
                                                                    .constructRawMapType(Map.class));

        Object o = deser.deserialize(p,
                                     ctxt);
        if (HashMap.class.isAssignableFrom(mapType)) {
            return HashMap.fromMap((Map) o);
        }
        if (TreeMap.class.isAssignableFrom(mapType)) {
            return TreeMap.fromMap((Comparator) Comparator.naturalOrder(),
                                   (Map) o);
        }
         ...
         **/
        Optional<Method> m = streamMethod.computeIfAbsent(mapType,
                                                          c -> Stream.of(c.getMethods())
                                                                     .filter(method -> "fromMap".equals(method.getName()))
                                                                     .filter(method -> method.getParameterCount() == 1)
                                                                     .filter(method -> method.getParameterTypes()[0].isAssignableFrom(Map.class))
                                                                     .findFirst()
                                                                     .map(m2 -> {
                                                                         m2.setAccessible(true);
                                                                         return m2;
                                                                     }));
        PersistentMap<?, ?> x = m.map(mt -> (PersistentMap) new Invoker().executeMethod(o,
                                                                                        mt,
                                                                                        mapType))
                                 .orElse(null);
    ...
    static class Invoker {
        ...
        private Object executeMethod(Object t,
                                     Method m,
                                     Class z) {
            try {
