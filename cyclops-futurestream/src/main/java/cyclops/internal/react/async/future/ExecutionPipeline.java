...
public class ExecutionPipeline {
    ...
    public <T, R> ExecutionPipeline thenApplyAsync(final Function<? super T, ? extends R> fn,
                                                   final Executor exec) {

        return new ExecutionPipeline(addFn(fn),
                                     addExec(exec),
                                     firstRecover,
                                     onFail);
    ...
    public <T, R> ExecutionPipeline thenComposeAsync(final Function<Object, CompletableFuture<?>> fn,
                                                     final Executor exec) {

        return new ExecutionPipeline(addFn(t -> fn.apply(t)
                                                  .join()),
                                     addExec(exec),
                                     firstRecover,
                                     onFail);
    ...
    public <T, R> ExecutionPipeline thenCompose(final Function<? super T, CompletableFuture<? extends R>> fn) {
        final Function<T, R> unpacked = t -> fn.apply(t)
                                               .join();
        return new ExecutionPipeline(swapFn(unpacked),
                                     execList.size() == 0 ? execList.plus(null) : execList,
                                     firstRecover,
                                     onFail);
    ...
    public <T, R> ExecutionPipeline thenApply(final Function<T, R> fn) {
        return new ExecutionPipeline(swapComposeFn(fn),
                                     execList.size() == 0 ? execList.plus(null) : execList,
                                     firstRecover,
                                     onFail);
    }
    ...
    public <X extends Throwable, T> ExecutionPipeline exceptionally(final Function<? super X, ? extends T> fn) {
        if (functionList.size() > 0) {
            final Function before = functionList.getOrElse(functionList.size() - 1,
                                                           null);
            ...
            return new ExecutionPipeline(swapFn(except),
                                         execList,
                                         firstRecover,
                                         onFail);
        }

        return new ExecutionPipeline(functionList,
                                     execList,
                                     addFirstRecovery(fn),
                                     onFail);
    ...
    public FinalPipeline toFinalPipeline() {
        ...
        return new FinalPipeline(functionList.stream()
                                             .toArray(i -> new Function[i]),
                                 execList.stream()
                                         .toArray(i -> new Executor[i]),
                                 firstRecover.stream()
                                             .toArray(i -> new Function[i]),
                                 onFail);
    }
    ...
    private Seq<Executor> addExec(final Executor exec) {
        ...
        return execList.insertAt(execList.size(),
                                 exec);
    }
    ...
    private Seq<Function> addFirstRecovery(final Function fn) {
        ...
        return firstRecover.insertAt(firstRecover.size(),
                                     fn);
    }
    ...
    private Seq<Function> addFn(final Function fn) {
        ...
        return functionList.insertAt(functionList.size(),
                                     fn);
    }
    ...
    private Seq<Function> swapFn(final Function fn) {
        ...
        return removed.insertAt(removed.size(),
                                fn);
