...
import cyclops.async.reactive.futurestream.pipeline.StageWithResults;
...
public interface SimpleReactStream<U> extends BaseSimpleReactStream<U>, BlockingStream<U>,
                                              ConfigurableStream<U, CompletableFuture<U>>, ToQueue<U> {
    ...
    static <U, R> Function<U, R> handleExceptions(final Function<? super U, ? extends R> fn) {
        return (input) -> {
            try {
                ...
                throw new SimpleReactFailedStageException(input,
                                                          t);
    ...
    /**
     * Merge this reactive dataflow with another - recommended for merging different types. To merge flows of the same type the
     * instance method merge is more appropriate.
     *
    ...
    public static <R> SimpleReactStream<R> merge(final SimpleReactStream s1,
                                                 final SimpleReactStream s2) {
        ...
        return s1.withLastActive(new EagerStreamWrapper(merged,
                                                        s1.getErrorHandler()));
    }
    ...
    static SimpleReactFailedStageException assureSimpleReactException(final Throwable throwable) {
        ...
        return new SimpleReactFailedStageException(null,
                                                   throwable);
    }
     ...
     * @return
     */
    static <U, R> SimpleReactStream<R> bind(final SimpleReactStream<U> stream,
                                            final Function<U, BaseSimpleReactStream<R>> flatFn) {
    ...
    default <T2> ReactiveSeq<Tuple2<U, T2>> combineLatest(final SimpleReactStream<T2> right) {
        return EagerFutureStreamFunctions.combineLatest(this,
                                                        right);
    }

    default <T2> ReactiveSeq<Tuple2<U, T2>> withLatest(final SimpleReactStream<T2> right) {
        return EagerFutureStreamFunctions.withLatest(this,
                                                     right);
    }
    ...
    default Tuple2<SimpleReactStream<U>, SimpleReactStream<U>> splitAt(final int position) {
        ...
        return Tuple.tuple(fromListCompletableFuture(split._1()
                                                          .collect(Collectors.toList())),
                           fromListCompletableFuture(split._2()
                                                          .collect(Collectors.toList())));
    }
    ...
    default Tuple2<SimpleReactStream<U>, SimpleReactStream<U>> duplicate() {
        ...
        final Tuple2<ReactiveSeq<CompletableFuture<U>>, ReactiveSeq<CompletableFuture<U>>> duplicated = ReactiveSeq.fromStream((Stream<CompletableFuture<U>>) stream)
                                                                                                                   .duplicate();
        final Tuple2 dup = Tuple.tuple(fromStreamOfFutures(duplicated._1()),
                                       fromStreamOfFutures(duplicated._2()));
    ...
    default <R> SimpleReactStream<Tuple2<U, R>> zip(final Stream<R> other) {
        ...
        final SimpleReactStream futureStream = fromStreamOfFutures((Stream) withType.map(t -> t._1()
                                                                                               .thenApply(v -> Tuple.tuple(t._1()
                                                                                                                            .join(),
                                                                                                                           t._2()))));
    ...
    /**
     * Zip two Streams, zipping against the underlying futures of both Streams Placeholders (Futures) will be populated
     * immediately in the new zipped Stream and results will be populated asyncrhonously
     *
    ...
    default <R> SimpleReactStream<Tuple2<U, R>> zip(final SimpleReactStream<R> other) {
        ...
        final SimpleReactStream futureStream = fromStreamOfFutures((Stream) withType.map(t -> CompletableFuture.allOf(t._1(),
                                                                                                                      t._2())
                                                                                                               .thenApply(v -> Tuple.tuple(t._1()
                                                                                                                                            .join(),
                                                                                                                                           t._2()
     ...
     * <p>
     * e.g. two function that return method name, but take varying lengths of time.
     * <pre>
     * <code>
    ...
    default SimpleReactStream<U> slice(final long from,
                                       final long to) {
    ...
    /**
     * Perform a limit operation on the underlying Stream of Futures In contrast to EagerFutureStream#limit this removes entries
     * basaed on their skip position
     *
     * <pre>
     * {@code
    ...
    /**
     * Return a Stream with the same values as this Stream, but with all values omitted until the provided stream starts emitting
     * values. Provided Stream ends the stream of values from this stream.
     *
    ...
    default <T> ReactiveSeq<U> dropUntil(final SimpleReactStream<T> s) {
        return EagerFutureStreamFunctions.dropUntil(this,
                                                    s);
    }

    /**
     * Return a Stream with the same values, but will stop emitting values once the provided Stream starts to emit values. e.g. if
     * the provided Stream is asynchronously refreshing state from some remote store, this stream can proceed until the provided
     * Stream succeeds in retrieving data.
     *
    ...
    default <T> ReactiveSeq<U> takeUntil(final SimpleReactStream<T> s) {
        return EagerFutureStreamFunctions.takeUntil(this,
                                                    s);
    }
    ...
    default ListX<SimpleReactStream<U>> copySimpleReactStream(final int times) {
        ...
        return (ListX) Streams.toBufferingCopier(getLastActive().stream()
                                                 ...
                                                 times)
                              .stream()
                              .map(it -> StreamSupport.stream(Spliterators.spliteratorUnknownSize(it,
                                                                                                  Spliterator.ORDERED),
                                                              false)).<BaseSimpleReactStream<U>>map(fs -> this.getSimpleReact()
                                                                                                              .construct(fs)).to(ReactiveConvertableSequence::converter)
                                                                                                                             .listX();
    }
    ...
    default <R> SimpleReactStream<R> then(final Function<? super U, ? extends R> fn,
        ...
        return (SimpleReactStream<R>) this.withLastActive(getLastActive().stream(s -> s.<CompletableFuture>map((ft) -> ft.thenApplyAsync(SimpleReactStream.<U, R>handleExceptions(fn),
                                                                                                                                         getTaskExecutor()))));
    }
    ...
    default <R> SimpleReactStream<R> thenSync(final Function<? super U, ? extends R> fn) {
        ...
        return (SimpleReactStream<R>) this.withLastActive(getLastActive().stream(s -> s.<CompletableFuture>map((ft) -> ft.thenApply(SimpleReactStream.<U, R>handleExceptions(fn)))));
    }

    /**
     * @param collector to perform aggregation / reduction operation on the results from active stage (e.g. to Collect into a List
     *                  or String)
    ...
    default <R1, R2> SimpleReactStream<R2> allOf(final Collector<? super U, ?, R1> collector,
        ...
        final Function<Exception, R2> f = (final Exception e) -> {
            BlockingStreamHelper.capture(e,
                                         getErrorHandler());
            return BlockingStreamHelper.block(this,
                                              Collectors.toList(),
        ...
        final CompletableFuture onSuccess = onFail.thenApplyAsync((result) -> {
                                                                      return new StageWithResults(getTaskExecutor(),
                                                                                                  null,
                                                                                                  result).submit(() -> fn.apply(BlockingStreamHelper.aggregateResultsCompletable(collector,
                                                                                                                                                                                 Stream.of(array)
                                                                                                                                                                                       .collect(Collectors.toList()),
                                                                                                                                                                                 getErrorHandler())));
                                                                  },
    ...
    /**
     * React to the completion of any of the events in the previous stage. Will not work reliably with Streams where filter has
     * been applied in earlier stages. (As Filter completes the Stream for events that are filtered out, they potentially
     * shortcircuit the completion of the stage).
     *
    ...
    default <R> SimpleReactStream<R> anyOf(final Function<? super U, ? extends R> fn) {
        ...
        final CompletableFuture onSuccess = cf.thenApplyAsync(fn,
                                                              getTaskExecutor());
    ...
    default <R> SimpleReactStream<R> fromStreamOfFutures(final Stream<CompletableFuture<R>> stream) {
        ...
        return (SimpleReactStream<R>) this.withLastActive(getLastActive().withNewStream(noType,
                                                                                        this.getSimpleReact()));
    }
     ...
     * <p>
     * <p>
     * Unlike 'with' this method is fluent, and returns another Stage Builder that can represent the next stage in the dataflow.
     *
     * <pre>
     * {@code
     ...
     * </pre>
     * <p>
     * React transform allows event reactors to be chained. Unlike React with, which returns a toX of Future references, React
     * transform is a fluent interface that returns the React builder - allowing further reactors to be added to the chain.
     * <p>
     * React transform does not block.
     * <p>
     * React with can be called after React transform which gives access to the full CompleteableFuture api. CompleteableFutures
     * can be passed back into SimpleReact via SimpleReact.react(streamOfCompleteableFutures);
     * <p>
     * See this blog post for examples of what can be achieved via CompleteableFuture :- <a href=
     ...
     * l </a>
     *
     * @param fn Function to be applied to the results of the currently active event tasks
     * @return A new builder object that can be used to define the next stage in the dataflow
     */
    @Override
    ...
    /**
     * Perform a flatMap operation where the CompletableFuture type returned is flattened from the resulting Stream This operation
    ...
    /**
     * Allows aggregate values in a Stream to be flatten into a single Stream. flatMap function turn each aggregate value into
     * it's own Stream, and SimpleReact aggregates those Streams into a single flattened stream
     *
     ...
     *
     * @see
     * com.aol.simple.react.stream.traits.FutureStream#zip(com.aol.simple.
     ...
     * React <b>with</b>
     * <p>
     * Asynchronously applyHKT the function supplied to the currently active event tasks in the dataflow.
     * <p>
     * While most methods in this class are fluent, and return a reference to a SimpleReact Stage builder, this method can be used
     * this method to access the underlying CompletableFutures.
     *
     * <pre>
     * {@code
     ...
     * </pre>
     * <p>
     * In this instance, 3 suppliers generate 3 numbers. These may be executed in parallel, when they complete each number will be
     * multiplied by 100 - as a separate parrellel task (handled by a ForkJoinPool or configurable task executor). A List of
     * Future objects will be returned immediately from Simple React and the tasks will be executed asynchronously.
     * <p>
     * React with does not block.
     *
     * @param fn Function to be applied to the results of the currently active event tasks
     * @return A list of CompletableFutures which will contain the result of the application of the supplied function
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Removes elements that do not fold the supplied predicate from the dataflow
     *
    ...
    default SimpleReactStream<U> merge(final SimpleReactStream<U>... s) {
        ...
        final List merged = Stream.concat(Stream.of(this),
                                          Stream.of(s))
                                  .map(stream -> ((SimpleReactStream) stream).getLastActive()
                                                                             .list())
                                  .flatMap(Collection::stream)
                                  .collect(Collectors.toList());
        return this.withLastActive(new EagerStreamWrapper(merged,
                                                          getErrorHandler()));
    }
     ...
     * <p>
     * Sometimes you may not need to block until all the work is complete, one result or a subset may be enough. To faciliate
     * this, block can accept a Predicate functional interface that will allow SimpleReact to stop blocking the current thread
     * when the Predicate has been fulfilled. E.g.
     *
     * <pre>
     * {@code
     ...
     * </pre>
     * <p>
     * In this example the current thread will unblock once more than one result has been returned.
     *
     * @param breakout Predicate that determines whether the block should be continued or removed
     * @return List of Completed results of currently active stage at full completion point or when breakout triggered (which ever
     * comes first). throws InterruptedException,ExecutionException
     */
    ...
    default ListX<U> block(final Predicate<Status<U>> breakout) {
        return new Blocker<U>((List) getLastActive().list(),
                              getErrorHandler()).block(breakout);
    }

    /**
     * @param collector to perform aggregation / reduction operation on the results (e.g. to Collect into a List or String)
     * @param breakout  Predicate that determines whether the block should be continued or removed
     * @return Completed results of currently active stage at full completion point or when breakout triggered (which ever comes
     * first), in aggregated in form determined by collector throws InterruptedException,ExecutionException
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
    @ThrowsSoftened({InterruptedException.class, ExecutionException.class})
     ...
     * <p>
     * <p>
     * Define a function that can be used to recover from exceptions during the preceeding stage of the dataflow. e.g.
     * <p>
     * <p>
     * <p>
     * onFail allows disaster recovery for each task (a separate onFail should be configured for each react phase that can fail).
     * E.g. if reading data from an external service fails, but default value is acceptable - onFail is a suitable mechanism to
     * set the default value. Asynchronously applyHKT the function supplied to the currently active event tasks in the dataflow.
     *
     * <pre>
     * {@code
     ...
     * <p>
     * <p>
     * In this example onFail recovers from the RuntimeException thrown when the input to the first 'transform' stage is 100.
     *
     * @param fn Recovery function, the exception is input, and the recovery value is emitted
     * @return A new builder object that can be used to define the next stage in the dataflow
     */
    @Override
    @SuppressWarnings({"unchecked", "rawtypes"})
    default SimpleReactStream<U> onFail(final Function<? super SimpleReactFailedStageException, ? extends U> fn) {
        return onFail(Throwable.class,
                      fn);
    }

    /**
     * Recover for a particular class of exceptions only. Chain onFail methods from specific Exception classes to general, as
     * Exceptions will be caught and handled in order. e.g.
     * <pre>
     * {@code
     ...
     * React <b>capture</b>
     * <p>
     * While onFail is used for disaster recovery (when it is possible to recover) - capture is used to capture those occasions
     * where the full pipeline has failed and is unrecoverable.
     *
     * <pre>
     * {@code
     * List<String> strings = new SimpleReact().<Integer, Integer> react(() -> 1, () -> 2, () -> 3)
     ...
     * <p>
     * <p>
     * In this case, strings will only contain the two successful results (for ()-&gt;1 and ()-&gt;3), an exception for the chain
     * starting from Supplier ()-&gt;2 will be logged by capture. Capture will not capture the exception thrown when an Integer
     * value of 100 is found, but will catch the exception when the String value "*200" is passed along the chain.
     *
     * @param errorHandler A consumer that recieves and deals with an unrecoverable error in the dataflow
     * @return A new builder object that can be used to define the next stage in the dataflow
     */
    @Override
    @SuppressWarnings("unchecked")
     ...
     * React and <b>allOf</b>
     * <p>
     * allOf is a non-blocking equivalent of block. The current thread is not impacted by the calculations, but the reactive chain
     * does not continue until all currently alloted tasks complete. The allOf task is transform provided with a list of the
     * results from the previous tasks in the chain.
     *
     * <pre>
     * {@code
     ...
     * </pre>
     * <p>
     * In this example, the current thread will continue and assert that it is not blocked, allOf could continue and be executed
     * in a separate thread.
     *
     * @param fn Function that recieves the results of all currently active tasks as input
     * @return A new builder object that can be used to define the next stage in the dataflow
     */
    @SuppressWarnings("unchecked")
    default <R> SimpleReactStream<R> allOf(final Function<? super List<U>, ? extends R> fn) {

        return (SimpleReactStream<R>) allOf(Collectors.<U>toList(),
                                            fn);
