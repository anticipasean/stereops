...
public class PipelineBuilder {
    ...
    public PipelineBuilder(final boolean autoOptimise,
                           final Executor optimisingExec,
                           final boolean autoMemoize,
                           final Cacheable memoizeFactory) {
        builder = new ExecutionPipeline();
    ...
    public <T, R> PipelineBuilder thenComposeAsync(final Function<? super T, CompletableFuture<? extends R>> fn,
                                                   final Executor exec) {
        ...
        return withBuilder(builder.thenComposeAsync((Function) memoize(fn),
                                                    exec));

    }

    public <T, R> PipelineBuilder thenApplyAsync(final Function<T, R> fn,
                                                 final Executor exec) {
        if (autoOptimise) {//if we already have a function present, compose with that
            if (builder.functionListSize() > 0) {
                return thenApply(fn);
        ...
        return withBuilder(builder.thenApplyAsync(memoize(fn),
                                                  exec));
    ...
    public <T> FastFuture<T> build() {
        ...
        return new FastFuture<T>(builder.toFinalPipeline(),
                                 0);
    }
