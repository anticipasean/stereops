...
import cyclops.internal.react.stream.StreamWrapper;
...
public interface BaseSimpleReactStream<U> extends BlockingStream<U> {
    ...
    /**
     * Create a 'free threaded' asynchronous stream that runs on a single thread (not current) The supplier will be executed
     * asyncrhonously, subsequent tasks will be executed synchronously unless the async() operator is invoked.
     */
    static <T> SimpleReactStream<T> react(final Supplier<T> value) {
        return new SimpleReact(ThreadPools.getStandard(),
                               false).ofAsync(value);
    }
    ...
    static <T> SimpleReactStream<T> freeThread(final T value) {
        return new SimpleReact(ThreadPools.getSequential(),
                               false).of(value);
    }
    ...
    /**
     * Create an zero sequential synchronous stream that runs on the current thread
     */
    ...
    /**
     * Construct an SimpleReactStream  from specified array, that will run in parallel on the common Parallel executor service (by
     * default the Common ForkJoinPool) see ThreadPools#setUseCommon to change to a different pool
     *
    ...
    /**
     * Create a 'free threaded' asynchronous stream that runs on the supplied CompletableFutures executor service (unless async
     * operator invoked , in which it will switch to the common 'free' thread executor) Subsequent tasks will be executed
     * synchronously unless the async() operator is invoked.
     *
     * @see Stream#of(Object)
     */
    ...
    static <T> SimpleReactStream<T> simpleReactStream(Stream<T> stream) {
        ...
        final SimpleReact sr = new SimpleReact(ThreadPools.getCurrentThreadExecutor(),
                                               false);
    ...
    static <T> SimpleReactStream<T> simpleReactStream(final Iterator<T> iterator) {
        return simpleReactStream(StreamSupport.stream(spliteratorUnknownSize(iterator,
                                                                             ORDERED),
                                                      false));
    }
