...
public interface BlockingStream<U> {
     ...
     * </pre>
     * <p>
     * In this example, once the current thread of execution meets the React block method, it will block until all tasks have been
     * completed. The result will be returned as a List. The Reactive tasks triggered by the Suppliers are non-blocking, and are
     * not impacted by the block method until they are complete. Block, only blocks the current thread.
     *
     * @return Results of currently active stage aggregated in a List throws InterruptedException,ExecutionException
     */
     ...
     * @return Results of currently active stage in aggregated in form determined by collector throws
    ...
    default <R, A> R block(final Collector<? super U, A, R> collector) {
        ...
        if (lastActive instanceof EagerStreamWrapper) {
            final EagerStreamWrapper last = (EagerStreamWrapper) lastActive;
            return (R) BlockingStreamHelper.block(this,
                                                  collector,
                                                  last);
        } else {
