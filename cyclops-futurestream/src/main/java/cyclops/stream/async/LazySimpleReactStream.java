...
public interface LazySimpleReactStream<U> extends BlockingStream<U>, ConfigurableStream<U, FastFuture<U>>, ToQueue<U>,
    ...
    default <R> LazySimpleReactStream<R> then(final Function<? super U, ? extends R> fn,
        ...
        return this.withLastActive(getLastActive().operation((ft) -> ft.thenApplyAsync(LazySimpleReactStream.<U, R>handleExceptions(fn),
                                                                                       service)));
    }
    ...
    default ListX<BaseSimpleReactStream<U>> copySimpleReactStream(final int times) {
        ...
        ReactiveSeq<BaseSimpleReactStream<U>> x = Streams.toBufferingCopier(iterator(),
                                                                            times)
                                                         .stream()
                                                         .map(it -> StreamSupport.stream(Spliterators.spliteratorUnknownSize((Iterator) it,
                                                                                                                             Spliterator.ORDERED),
                                                                                         false)).<BaseSimpleReactStream<U>>map(fs -> (BaseSimpleReactStream) this.getSimpleReact()
                                                                                                                                                                 .construct((Stream) fs));
        return x.to(ReactiveConvertableSequence::converter)
                .listX();
    }
