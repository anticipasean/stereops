...
public interface Futures {
    ...
    public static <T, R> Future<R> tailRec(T initial,
        ...
        return Future.of(() -> {
                         ...
                         sr.getExecutor())
                     .flatMap(i -> i)
                     .peek(e -> SequentialElasticPools.simpleReact.populate(sr))
                     .recover(t -> {
     ...
     */
    @SafeVarargs
    public static <T> Future<ListX<T>> quorum(Predicate<Status<T>> breakout,
                                              Future<T>... fts) {
        ...
        return Future.of(new Blocker<T>(list,
                                        Optional.empty()).nonBlocking(breakout));
    ...
    public static <T> Future<ListX<T>> quorum(Predicate<Status<T>> breakout,
        ...
        return Future.of(new Blocker<T>(list,
                                        Optional.of(errorHandler)).nonBlocking(breakout));
