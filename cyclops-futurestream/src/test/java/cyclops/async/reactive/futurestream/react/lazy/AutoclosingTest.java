...
public class AutoclosingTest {
    ...
    public void autoClosingLimit1() throws InterruptedException {
        ...
        List<String> results = new LazyReact().generateAsync(() -> nextValues())
                                              .withQueueFactory(() -> eventQueue())
        ...
        assertThat(close.get(),
                   greaterThan(0));
        assertThat(results.size(),
                   is(1));
        assertThat(localAdded,
                   is(added.get()));
    ...
    public void autoClosingZip() throws InterruptedException {
        ...
        added = new AtomicInteger();
        //subscription fills from outside in (lazyRight to lazyLeft), need to store open / closed for each queue
        List<Tuple2<List<List<String>>, Integer>> results = new LazyReact().generateAsync(() -> nextValues())
                                                                           .withQueueFactory(() -> eventQueue())
                                                                           .zip(new LazyReact().of(1,
                                                                                                   2,
                                                                                                   3))
                                                                           .collect(Collectors.toList());
        System.out.println("finished");
    ...
    public void autoClosingZipLots() throws InterruptedException {
        for (int i = 0; i < 1500; i++) {
            ...
            added = new AtomicInteger();
            //subscription fills from outside in (lazyRight to lazyLeft), need to store open / closed for each queue
            List<Tuple2<List<List<String>>, Integer>> results = new LazyReact().generate(() -> nextValues())
                                                                               .withQueueFactory(() -> eventQueue())
                                                                               .zip(new LazyReact().of(1,
                                                                                                       2,
                                                                                                       3))
    ...
    public void autoClosingIterate() throws InterruptedException {
        ...
        //subscription fills from outside in (lazyRight to lazyLeft), need to store open / closed for each queue
        List<Integer> results = new LazyReact().iterate(0,
                                                        val -> val + 1)
                                               .withQueueFactory(() -> eventQueueInts())
                                               .flatMap(val -> asList(asList(val,
                                                                             1,
                                                                             2,
                                                                             3)).stream())
                                               .peek(System.out::println)
