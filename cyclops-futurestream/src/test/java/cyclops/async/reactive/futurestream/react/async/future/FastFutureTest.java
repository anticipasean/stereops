package cyclops.async.reactive.futurestream.react.async.future;
...
import cyclops.async.reactive.futurestream.pipeline.FinalPipeline;
...
public class FastFutureTest {
    ...
    public void testThenApplyAsync() {
        try {
            future = future.<String, String>thenApplyAsync(String::toUpperCase,
                                                           ForkJoinPool.commonPool()).peek(System.out::println).<String, Integer>thenApply(s -> s.length()).thenApply(l -> {
                sizes.add((Integer) l);
                return l;
            })
                                                                                                                                                           .peek(System.out::println).<Integer, Integer>thenApplyAsync(l ->
                                                                                                                                                                                                                           l
                                                                                                                                                                                                                               + 2,
                                                                                                                                                                                                                       ForkJoinPool.commonPool()).peek(System.out::println).<Integer, Integer>thenApply(l -> {
                    sizes2.add(l);
                    return l;
                }).<Integer, Integer>thenApplyAsync(l -> l + 2,
                                                    ForkJoinPool.commonPool()).peek(System.out::println)
    ...
    public void testAllOf() throws InterruptedException {
        for (int i = 0; i < TIMES; i++) {
            complete = false;
            FastFuture f1 = new FastFuture(FinalPipeline.empty(),
                                           a -> {
                                           });

            FastFuture f2 = new FastFuture(FinalPipeline.empty(),
                                           a -> {
                                           });
            CountDownLatch race = new CountDownLatch(1);
            FastFuture allOf = FastFuture.allOf(() -> complete = true,
                                                f1,
                                                f2);
            new Thread(() -> {
                race.countDown();
    ...
    public void testAnyOf() {
        for (int i = 0; i < TIMES; i++) {
            ...
            CountDownLatch race = new CountDownLatch(1);
            FastFuture anyOf = FastFuture.anyOf(f1,
                                                f2);
            new Thread(() -> {
                race.countDown();
    ...
    public void testXOf() {
        for (int i = 0; i < TIMES; i++) {
            ...
            CountDownLatch race = new CountDownLatch(1);
            FastFuture xOf = FastFuture.xOf(2,
                                            () -> complete = true,
                                            f1,
                                            f2);
            new Thread(() -> {
