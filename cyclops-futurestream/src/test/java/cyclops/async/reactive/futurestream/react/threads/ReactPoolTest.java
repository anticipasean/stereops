package cyclops.async.reactive.futurestream.react.threads;

import static java.util.Arrays.asList;
import static org.hamcrest.Matchers.is;
import static org.junit.Assert.assertThat;

import cyclops.async.reactive.futurestream.threading.ReactPool;
...
public class ReactPoolTest {
    ...
    public void testReact() {
        ...
        ReactPool<SimpleReact> pool = ReactPool.boundedPool(asList(new SimpleReact(),
                                                                   new SimpleReact()));
        List<String> result = pool.react((er) -> er.ofAsync(() -> "hello",
                                                            () -> "world")
                                                   .block());
    ...
    public void testRoundRobin() {
        ...
        ReactPool<SimpleReact> pool = ReactPool.boundedPool(asList(react1,
                                                                   react2));
        List<Supplier<String>> suppliers = Arrays.asList(() -> "hello",
                                                         () -> "world");
        pool.react((er) -> er.fromIterableAsync(suppliers));
        pool.react((er) -> er.fromIterableAsync(suppliers));

        Mockito.verify(react1,
                       Mockito.times(1))
               .fromIterableAsync(suppliers);
        Mockito.verify(react2,
                       Mockito.times(1))
               .fromIterableAsync(suppliers);
    }
    ...
    public void testUnbounded() {
        ...
        ReactPool<LazyReact> pool = ReactPool.unboundedPool(asList(new LazyReact(),
                                                                   new LazyReact()));
        List<String> result = pool.react((er) -> er.ofAsync(() -> "hello",
                                                            () -> "world")
                                                   .block());
    ...
    public void testUnboundedRoundRobin() {
        ...
        ReactPool<SimpleReact> pool = ReactPool.unboundedPool(asList(react1,
                                                                     react2));
    ...
    public void testSyncrhonous() {
        ...
        List<String> result = pool.react((sr) -> sr.ofAsync(() -> "hello",
                                                            () -> "world")
