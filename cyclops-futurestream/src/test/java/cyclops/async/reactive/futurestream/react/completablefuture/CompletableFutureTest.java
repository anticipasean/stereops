...
public class CompletableFutureTest {
    ...
    public void asyncEventRecieverTest() throws InterruptedException, ExecutionException {
        ...
        convertedToStrings.streamCompletableFutures()
                          .forEach(f -> assertFalse(f.isDone()));

        new SimpleReact(new ForkJoinPool(3)).ofAsync(() -> 100,
                                                     () -> 200,
                                                     () -> 400)
                                            .then(it -> sleep(it))
                                            .then(it -> queue.poll()
    ...
    private Queue<CompletableFuture<Integer>> buildQueueOfAsyncEvents() {
        ...
        Queue<CompletableFuture<Integer>> queue = new ConcurrentLinkedQueue(Arrays.asList(future1,
                                                                                          future2,
                                                                                          future3));
        return queue;
    }
