...
public class BatchingCollectorTest {
    ...
    @Before
    public void setup() {
        collector = new BatchingCollector(MaxActive.IO,
                                          LazyReact.sequentialBuilder()
                                                   .of(1)).withResults(new ArrayList());
    }
    ...
    public void testAcceptMock495() {
        collector = new BatchingCollector(new MaxActive(500,
                                                        5),
                                          LazyReact.sequentialBuilder()
                                                   .of(1)).withResults(new ArrayList<>());
        FastFuture cf = Mockito.mock(FastFuture.class);
    ...
    public void testBuilder() {
        collector = BatchingCollector.builder()
                                     .blocking(LazyReact.sequentialBuilder()
                                                        .of(1))
                                     .maxActive(new MaxActive(2,
                                                              1))
    ...
    public void testWithMaxActive() {
        collector = collector.withMaxActive(new MaxActive(10000,
                                                          5));
        FastFuture cf = Mockito.mock(FastFuture.class);
        BDDMockito.given(cf.isDone())
                  .willReturn(true);
    ...
    public void testBatchingCollectorMaxActive() {
        collector = new BatchingCollector(new MaxActive(10,
                                                        5),
                                          LazyReact.sequentialBuilder()
                                                   .of(1)).withResults(new HashSet<>());
        FastFuture cf = Mockito.mock(FastFuture.class);
        BDDMockito.given(cf.isDone())
                  .willReturn(true);
        for (int i = 0; i < 1000; i++) {
