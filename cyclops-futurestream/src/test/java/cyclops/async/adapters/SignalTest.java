...
public class SignalTest {
    ...
    public void signalFromStream() {
        ...
        Stream<Integer> stage = q.getDiscrete()
                                 .stream()
                                 .limit(2);
        q.fromStream(Stream.of(1,
                               1,
                               1,
                               2,
                               2));

        int sum = stage.map(it -> it * 100)
                       .reduce(0,
                               (acc, n) -> acc + n);
    ...
    public void signalDiscrete3() {
        try {
            ...
            new SimpleReact().ofAsync(() -> q.set(1),
                                      () -> q.set(2),
                                      () -> {
                                          sleep(20);
                                          return q.set(4);
            ...
            parallel().fromStream(q.getDiscrete()
    ...
    public void signalDiscrete1() {
        for (int i = 0; i < 100; i++) {
            ...
            try {
                ...
                parallel().fromStreamOfFutures(q.getDiscrete()
                                                .streamCompletableFutures())
                          .then(it -> "*" + it)
                          .peek(it -> incrementFound())
                          .peek(it -> System.out.println(it))
    ...
    public void signalContinuous3() {
        for (int i = 0; i < 10; i++) {
            ...
            try {
                ...
                parallel().fromStream(q.getContinuous()
                                       .streamCompletableFutures())
                          .then(it -> "*" + it)
                          .peek(it -> incrementFound())
                          .peek(it -> System.out.println(it))
