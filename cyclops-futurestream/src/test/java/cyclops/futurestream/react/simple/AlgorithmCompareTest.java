...
public class AlgorithmCompareTest {
    ...
    public void testFastest() throws InterruptedException, ExecutionException {
        ...
        Result result = new SimpleReact().<Result>ofAsync(() -> Result.builder()
                                                                      .name("approach1")
                                                                      .result(retrieval(arrayList))
                                                                      .build(),
                                                          () -> Result.builder()
                                                                      .name("approach2")
                                                                      .result(retrieval(linkedList))
                                                                      .build()).then(it -> it.withTime(timer.getElapsedNanoseconds()))
    ...
    public void testFastestLessBlocking() throws InterruptedException, ExecutionException {
        ...
        Result result = new SimpleReact().<Result>ofAsync(() -> Result.builder()
                                                                      .name("approach1 : arrayList")
                                                                      .result(retrieval(arrayList))
                                                                      .build(),
                                                          () -> Result.builder()
                                                                      .name("approach2 : linkedList")
                                                                      .result(retrieval(linkedList))
                                                                      .build()).then(it -> it.withTime(timer.getElapsedNanoseconds()))
                                                                               .filter(it -> it.getResult() == 1000)
                                                                               .block()
    ...
    static class JavaResult {
        ...
        public JavaResult withName(String name) {
            return new JavaResult.Builder().name(name)
                                           .result(result)
                                           .time(time)
                                           .build();
        }
