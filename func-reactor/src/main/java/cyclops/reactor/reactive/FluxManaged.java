...
public abstract class FluxManaged<T> extends Managed<T> {

    public static <T> Managed<T> managed(T just,
                                         Consumer<T> cleanup) {
        return of(just(just),
                  cleanup);
    }

    public static <T> Managed<T> of(Supplier<? extends T> s,
                                    Consumer<T> cleanup) {
        return of(FluxIO.of(s),
                  cleanup);
    }
    ...
    public static <T extends AutoCloseable> Managed<T> of(Publisher<T> acq) {
        return of(FluxIO.fromPublisher(acq),
    ...
    public static <T> Managed<T> of(IO<T> acquire,
        ...
        return new FluxManaged<T>() {
            public <R> IO<R> apply(Function<? super T, ? extends IO<R>> fn) {
                IO<R> y = IO.Comprehensions.forEach(acquire,
                                                    t1 -> {
                                                        IO<? extends Try<? extends IO<R>, Throwable>> res1 = FluxIO.withCatch(() -> fn.apply(t1),
                                                                                                                              Throwable.class);
                                                        return res1;
                                                    },
                                                    t2 -> {
