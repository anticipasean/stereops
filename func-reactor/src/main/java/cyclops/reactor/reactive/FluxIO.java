...
public final class FluxIO<T> implements IO<T> {
    ...
    public IO<T> race(IO<T> that) {
        return fromPublisher(Flux.first(Seq.of(publisher(),
                                               that.publisher())));
    }
    ...
    public <R extends AutoCloseable, R1> Managed.Tupled<R, R1> bracketWith(Function<? super T, ? extends R> fn,
        ...
        Managed.Tupled<? extends R, ? extends R1> x = FluxManaged.of(map(fn))
                                                                 .with(with);
    ...
    public void forEach(Consumer<? super T> consumerElement,
                        ...
                        Runnable onComplete) {
        flowable.subscribe(consumerElement,
                           consumerError,
                           onComplete);
    }
    ...
    public <R> IO<R> unit(Publisher<R> pub) {
        return FluxIO.fromPublisher(pub);
    }
}
