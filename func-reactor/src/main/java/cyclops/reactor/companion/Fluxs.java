...
public class Fluxs {
    ...
    public static <T> Flux<Mono<T>> sequence(final Publisher<? extends Flux<T>> fts) {
        ...
        BiFunction<Flux<Mono<T>>, Flux<T>, Flux<Mono<T>>> combineToStream = (acc, next) -> Flux.merge(acc,
                                                                                                      next.map(Mono::just));

        Flux<Flux<Mono<T>>> x = Flux.from(fts)
                                    .reduce(identity,
                                            combineToStream)
    ...
    public static <T, R> Flux<Mono<R>> traverse(Function<? super T, ? extends R> fn,
                                                Publisher<Flux<T>> stream) {
        Mono<Flux<R>> s = Mono.from(stream)
                              .map(h -> h.map(fn));
        return sequence(s);
    }


    public static <T, R> Flux<R> tailRec(T initial,
                                         Function<? super T, ? extends Flux<? extends Either<T, R>>> fn) {
    ...
    public static <T> Flux<T> fluxFrom(ReactiveSeq<T> stream) {
        ...
        return stream.fold(sync -> Flux.fromStream(stream),
                           rs -> Flux.from(stream),
                           async -> Flux.from(stream));
    ...
    /**
     * Perform a For Comprehension over a Flux, accepting 3 generating functions. This results in a four level nested internal
     * iteration over the provided Publishers.
     *
     * <pre>
     * {@code
     *
     *   import static cyclops.reactor.companion.Fluxs.forEach4;
     *
    ...
    public static <T1, T2, T3, R1, R2, R3, R> Flux<R> forEach4(Flux<? extends T1> value1,
        ...
        return value1.flatMap(in -> {
            ...
            return a.flatMap(ina -> {
                Flux<R2> b = Flux.from(value3.apply(in,
                                                    ina));
                return b.flatMap(inb -> {
                    Flux<R3> c = Flux.from(value4.apply(in,
                                                        ina,
                                                        inb));
    ...
    /**
     * Perform a For Comprehension over a Flux, accepting 2 generating functions. This results in a three level nested internal
     * iteration over the provided Publishers.
     *
     * <pre>
     * {@code
     *
     * import static cyclops.reactor.companion.Fluxs.forEach;
     *
    ...
    /**
     * Perform a For Comprehension over a Flux, accepting an additonal generating function. This results in a two level nested
