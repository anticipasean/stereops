...
public class FlowablesTest {
    ...
    public void testSequenceError() throws InterruptedException {
        Flowable<Single<Integer>> maybes = Flowables.sequence(Flux.just(just,
                                                                        none));
    ...
    public void testSequenceErrorAsync() {
        ...
        Flowable<Single<Integer>> maybes = Flowables.sequence(Flowable.just(just.doOnNext(e -> done.set(true)),
                                                                            active));
    ...
    public void testSequenceTwo() {
        Flowable<Single<Integer>> maybes = Flowables.sequence(Flux.just(just,
                                                                        just2));
        assertThat(maybes.map(Single::blockingGet)
                         .toList()
                         .blockingGet(),
    ...
    public void testSequenceOneFlux() {
        Flowable<Single<Integer>> maybes = Flowables.sequence(Single.just(Flowable.just(10,
                                                                                        20))
                                                                    .toFlowable());
        assertThat(maybes.map(Single::blockingGet)
                         .toList()
