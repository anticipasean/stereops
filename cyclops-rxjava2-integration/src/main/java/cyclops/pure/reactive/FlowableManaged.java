...
import static cyclops.pure.reactive.FlowableIO.just;
...
public abstract class FlowableManaged<T> extends Managed<T> {
    ...
    public static <T extends AutoCloseable> Managed<T> of(Publisher<T> acq) {
        return Managed.of(FlowableIO.fromPublisher(acq),
        ...
        return new FlowableManaged<T>() {
            public <R> IO<R> apply(Function<? super T, ? extends IO<R>> fn) {
                IO<R> y = IO.Comprehensions.forEach(acquire,
                                                    t1 -> {
                                                        IO<? extends Try<? extends IO<R>, Throwable>> res1 = FlowableIO.withCatch(() -> fn.apply(t1),
                                                                                                                                  Throwable.class);
                                                        return res1;
                                                    },
                                                    t2 -> {
                                                        ...
                                                        IO<R> res = tr.fold(r -> r,
                                                                            e -> FlowableIO.of(Future.ofError(e)));
