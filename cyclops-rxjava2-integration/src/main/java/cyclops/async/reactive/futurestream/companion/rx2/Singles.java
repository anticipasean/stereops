package cyclops.async.reactive.futurestream.companion.rx2;
...
public class Singles {
    ...
    public static <T> Single<Flowable<T>> sequence(final Publisher<? extends Single<T>> fts) {
        ...
        Single<Single<Flowable<T>>> res = Flowable.fromPublisher(fts).<Single<Flowable<T>>>reduce(Single.just(Flowable.empty()),
                                                                                                  (acc, next) -> acc.zipWith(next,
                                                                                                                             (a, b) -> Flowable.concat(a,
                                                                                                                                                       Flowable.just(b))));
        return res.flatMap(i -> i);
    }

    public static <T, R> Single<Flowable<R>> traverse(Function<? super T, ? extends R> fn,
                                                      Publisher<Single<T>> stream) {
        Flowable<Single<R>> s = Flowable.fromPublisher(stream)
                                        .map(h -> h.map(i -> fn.apply(i)));
        return sequence(s);
    }
    ...
    public static <T, R> Single<R> tailRec(T initial,
                                           Function<? super T, ? extends Single<? extends Either<T, R>>> fn) {
     ...
     * @return Single that completes when all the provided Futures Complete. Empty Future result, or holds an Exception from a
     ...
     */
    @SafeVarargs
    public static <T> Single<Seq<T>> quorum(Predicate<Status<T>> breakout,
    ...
    /**
     * Perform a For Comprehension over a Single, accepting 3 generating functions. This results in a four level nested internal
     * iteration over the provided Singles.
     *
     * <pre>
     * {@code
     *
     *   import static cyclops.async.reactive.futurestream.companion.reactor.Singles.forEach4;
     *
    ...
    public static <T1, T2, T3, R1, R2, R3, R> Single<R> forEach4(Single<? extends T1> value1,
        ...
        Single<? extends R> res = value1.flatMap(in -> {
            ...
            return a.flatMap(ina -> {
                Single<R2> b = value3.apply(in,
                                            ina);
                return b.flatMap(inb -> {
                    Single<R3> c = value4.apply(in,
                                                ina,
                                                inb);
    ...
    /**
     * Perform a For Comprehension over a Single, accepting 2 generating functions. This results in a three level nested internal
     * iteration over the provided Singles.
     *
     * <pre>
     * {@code
     *
     *   import static cyclops.async.reactive.futurestream.companion.reactor.Singles.forEach3;
     *
    ...
    /**
     * Perform a For Comprehension over a Single, accepting a generating function. This results in a two level nested internal
     * iteration over the provided Singles.
     *
     * <pre>
     * {@code
     *
     *   import static cyclops.async.reactive.futurestream.companion.reactor.Singles.forEach;
     *
     ...
     */
    public static <T1, T2, R> Single<R> combine(Single<? extends T1> single,
     ...
     * @return Combined Single
     */
    public static <T1, T2, R> Single<R> zip(Single<? extends T1> single,
    ...
    public static <T1, T2, R> Single<R> zip(Single<? extends T1> single,
        ...
        Single<R> res = Single.fromPublisher(Future.fromPublisher(single.toFlowable())
                                                   .zip(fn,
                                                        app));
        return res;
    }
