...
public class ObservableReactiveSeqImpl<T> implements ReactiveSeq<T> {
    ...
    public <U> U foldRight(U identity,
                           BiFunction<? super T, ? super U, ? extends U> accumulator) {
        return observable.reduce(identity,
                                 (a, b) -> accumulator.apply(b,
                                                             a))
                         .blockingGet();
    }
    ...
    public <U, R> ReactiveSeq<R> zipWithStream(Stream<? extends U> other,
        ...
        if (other instanceof ReactiveSeq) {
            ...
            return o.fold(sync -> observable(observable.zipWith(ReactiveSeq.fromStream((Stream<U>) other),
                                                                (a, b) -> zipper.apply(a,
                                                                                       b))),
                          rs -> observable(observable.zipWith(ReactiveSeq.fromStream((Stream<U>) other),
                                                              (a, b) -> zipper.apply(a,
                                                                                     b))),
                          async -> observable(observable.zipWith(ReactiveSeq.fromStream((Stream<U>) other),
                                                                 (a, b) -> zipper.apply(a,
                                                                                        b))));
        ...
        return observable(observable.zipWith(ReactiveSeq.fromStream((Stream<U>) other),
                                             (a, b) -> zipper.apply(a,
                                                                    b)));
    }

    @Override
    public <U, R> ReactiveSeq<R> zipLatest(Publisher<? extends U> other,
                                           BiFunction<? super T, ? super U, ? extends R> zipper) {
        Observable<R> obs = Observable.combineLatest(observable,
                                                     Observables.observable(other),
                                                     (a, b) -> zipper.apply((T) a,
                                                                            (U) b));
        return observable(obs);
    }

    @Override
    public <U, R> ReactiveSeq<R> zip(BiFunction<? super T, ? super U, ? extends R> zipper,
                                     Publisher<? extends U> other) {
        return observable(observable.zipWith(Observables.observable(other),
                                             (a, b) -> zipper.apply(a,
                                                                    b)));
    }
    ...
    public Tuple2<ReactiveSeq<T>, ReactiveSeq<T>> duplicate() {
        return Observables.connectToReactiveSeq(observable)
                          .duplicate()
                          .transform((s1, s2) -> Tuple.tuple(observable(s1),
                                                             observable(s2)));
    }
    ...
    public Tuple3<ReactiveSeq<T>, ReactiveSeq<T>, ReactiveSeq<T>> triplicate() {
        return Observables.connectToReactiveSeq(observable)
                          .triplicate()
                          .transform((s1, s2, s3) -> Tuple.tuple(observable(s1),
                                                                 observable(s2),
                                                                 observable(s3)));
    }
    ...
    public Tuple4<ReactiveSeq<T>, ReactiveSeq<T>, ReactiveSeq<T>, ReactiveSeq<T>> quadruplicate() {
        return Observables.connectToReactiveSeq(observable)
                          .quadruplicate()
                          .to(t4 -> Tuple.tuple(observable(t4._1()),
                                                observable(t4._2()),
                                                observable(t4._3()),
                                                observable(t4._4())));
    }
    ...
    public Tuple2<Option<T>, ReactiveSeq<T>> splitAtHead() {
        return Observables.connectToReactiveSeq(observable)
                          .splitAtHead()
                          .transform((s1, s2) -> Tuple.tuple(s1,
                                                             observable(s2)));
    }
    ...
    @Override
    public ReactiveSeq<Seq<T>> sliding(int windowSize,
                                       int increment) {
        return observable(Observables.connectToReactiveSeq(observable)
                                     .sliding(windowSize,
                                              increment));
    }
    ...
    @Override
    public <C extends PersistentCollection<T>, R> ReactiveSeq<R> groupedUntil(BiPredicate<C, ? super T> predicate,
                                                                              Supplier<C> factory,
    ...
    @Override
    public <C extends PersistentCollection<T>, R> ReactiveSeq<R> groupedWhile(BiPredicate<C, ? super T> predicate,
                                                                              Supplier<C> factory,
                                                                              Function<? super C, ? extends R> finalizer) {
        return observable(Observables.connectToReactiveSeq(observable)
                                     .groupedWhile(predicate,
    ...
    @Override
    public ReactiveSeq<Vector<T>> groupedBySizeAndTime(int size,
                                                       long time,
                                                       TimeUnit t) {
        return observable(Observables.connectToReactiveSeq(observable)
    ...
    @Override
    public <C extends PersistentCollection<? super T>> ReactiveSeq<C> groupedBySizeAndTime(int size,
                                                                                           long time,
                                                                                           TimeUnit unit,
                                                                                           Supplier<C> factory) {
        return observable(Observables.connectToReactiveSeq(observable)
    ...
    @Override
    public <C extends PersistentCollection<? super T>, R> ReactiveSeq<R> groupedBySizeAndTime(int size,
                                                                                              long time,
                                                                                              TimeUnit unit,
                                                                                              Supplier<C> factory,
                                                                                              Function<? super C, ? extends R> finalizer) {
    ...
    @Override
    public <C extends PersistentCollection<? super T>, R> ReactiveSeq<R> groupedByTime(long time,
                                                                                       TimeUnit unit,
                                                                                       Supplier<C> factory,
                                                                                       Function<? super C, ? extends R> finalizer) {
        return groupedBySizeAndTime(Integer.MAX_VALUE,
                                    time,
                                    unit,
                                    factory,
                                    finalizer);
    ...
    @Override
    public ReactiveSeq<Vector<T>> groupedByTime(long time,
                                                TimeUnit t) {
        return observable(Observables.connectToReactiveSeq(observable)
                                     .groupedByTime(time,
                                                    t));
    ...
    @Override
    public <C extends PersistentCollection<? super T>> ReactiveSeq<C> groupedByTime(long time,
                                                                                    TimeUnit unit,
                                                                                    Supplier<C> factory) {
        return observable(Observables.connectToReactiveSeq(observable)
                                     .groupedByTime(time,
    ...
    @Override
    public <C extends PersistentCollection<? super T>> ReactiveSeq<C> grouped(int size,
                                                                              Supplier<C> supplier) {
        return observable(Observables.connectToReactiveSeq(observable)
                                     .grouped(size,
                                              supplier));
    ...
    @Override
    public <C extends PersistentCollection<? super T>> ReactiveSeq<C> groupedWhile(Predicate<? super T> predicate,
                                                                                   Supplier<C> factory) {
        return observable(Observables.connectToReactiveSeq(observable)
                                     .groupedWhile(predicate,
                                                   factory));
    ...
    @Override
    public <U> ReactiveSeq<U> scanLeft(U seed,
                                       BiFunction<? super U, ? super T, ? extends U> function) {
        return observable(observable.scan(seed,
                                          (a, b) -> function.apply(a,
                                                                   b)));
    }
    ...
    public void forEach(Consumer<? super T> action) {
        Observables.connectToReactiveSeq(observable)
                   .forEach(action);
    }
    ...
    @Override
    public <R> R foldMap(Function<? super T, ? extends R> mapper,
                         Monoid<R> reducer) {
        return Observables.connectToReactiveSeq(observable)
                          .foldMap(mapper,
                                   reducer);
    ...
    @Override
    public <R> ReactiveSeq<R> mergeMap(int maxConcurrency,
    ...
    public <R> ReactiveSeq<R> flatMapStream(Function<? super T, BaseStream<? extends R, ?>> fn) {
        ...
        return this.<R>observable((Observable) observable.flatMap(a -> fn.andThen(s -> {
                                                                                      ReactiveSeq<R> res =
                                                                                          s instanceof ReactiveSeq ? (ReactiveSeq) s : (ReactiveSeq) ReactiveSeq.fromSpliterator(s.spliterator());
    ...
    @Override
    public ReactiveSeq<T> drop(long time,
                               TimeUnit unit) {
        return observable(observable.skip(time,
                                          unit));
    }

    @Override
    public ReactiveSeq<T> take(long time,
                               TimeUnit unit) {
        return observable(observable.take(time,
                                          unit));
    }
    ...
    public <EX extends Throwable> ReactiveSeq<T> recover(Class<EX> exceptionClass,
                                                         Function<? super EX, ? extends T> fn) {
        return observable(Observables.connectToReactiveSeq(observable)
    ...
    public <R> ReactiveSeq<R> reduceAll(R identity,
        ...
        Single<R> inter = observable.reduce(identity,
                                            (a, b) -> accumulator.apply(a,
                                                                        b));
    ...
    public void subscribe(Subscriber<? super T> s) {
        Observables.publisher(observable)
                   .subscribe(s);
    }
