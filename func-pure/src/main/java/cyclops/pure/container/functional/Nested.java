...
import cyclops.function.higherkinded.DataWitness.nested;
...
import cyclops.pure.typeclasses.functor.Compose;
import cyclops.pure.typeclasses.functor.Functor;
import cyclops.pure.typeclasses.monad.Applicative;
import cyclops.pure.typeclasses.monad.ComposedTraverse;
...
@EqualsAndHashCode
public class Nested<W1, W2, T> implements Transformable<T>, Higher3<nested, W1, W2, T>, To<Nested<W1, W2, T>> {
    ...
    public static <W1, W2, T> Nested<W1, W2, T> of(Higher<W1, ? extends Higher<W2, ? extends T>> nested,
    ...
    public static <W, T> Active<W, T> flatten(Nested<W, W, T> nested) {
        return Active.of(nested.def1.monad()
                                    .flatMap(i -> i,
                                             nested.nested),
                         nested.def1);
    }
    ...
    public <R> Active<W1, R> pure1(R value) {
        return Active.of(def1.unit()
                             .unit(value),
                         def1);
    }

    public <R> Nested<W1, W2, R> pure2(R value) {
        return Nested.of(def1.unit()
                             .unit(def2.unit()
                                       .unit(value)),
                         def1,
                         def2);
    ...
    public Nested<W1, W2, T> reverse() {
        return Nested.of(def1.traverse()
                             .reverse(def1.functor()
                                          .map(i -> def2.traverse()
                                                        .reverse(i),
                                               nested)),
                         def1,
    ...
    public <R> Nested<W1, W2, R> ap(Higher<W2, ? extends Function<T, R>> fn) {
        Higher<W1, Higher<W2, R>> res = def1.functor()
                                            .map(a -> def2.applicative()
                                                          .ap(fn,
                                                              a),
                                                 nested);
        return of(res,
    ...
    public <R> Nested<W1, W2, R> flatMap(Function<? super T, ? extends Higher<W2, R>> fn) {
        Higher<W1, Higher<W2, R>> res = composedFunctor.map1(a -> def2.monad()
                                                                      .flatMap(fn,
                                                                               a),
                                                             nested);
        return new Nested<W1, W2, R>(res,
    ...
    public <T2, R> Nested<W1, W2, R> zip(Higher<W2, T2> fb,
                                         BiFunction<? super T, ? super T2, ? extends R> f) {
        return of(def1.functor()
                      .map_(nested,
                            i -> def2.applicative()
                                     .zip(i,
                                          fb,
    ...
    public <C> Narrowed<C> concreteMonoid(Kleisli<W2, C, T> widen,
                                          Cokleisli<W2, T, C> narrow) {
        return new Narrowed<C>(widen,
                               narrow);
    }
    ...
    public <S, R> Converter<W1, S> concreteConversion(Function<? super Higher<W2, T>, ? extends S> narrow2) {
        return new Converter<W1, S>() {
            ...
            public <R> Active<W1, R> to(Function<S, R> fn) {
                return Active.of(def1.functor()
                                     .map(f -> fn.apply(narrow2.apply(f)),
                                          nested),
                                 def1);
            }
        };
    ...
    public <R> Nested<W1, W2, R> flatMapA(Function<? super T, ? extends Active<W2, R>> fn) {
        Higher<W1, Higher<W2, R>> res = composedFunctor.map1(a -> def2.monad()
                                                                      .flatMap(fn.andThen(t -> t.getSingle()),
                                                                               a),
                                                             nested);
        return new Nested<W1, W2, R>(res,
                                     def1,
    ...
    public <R> Nested<W1, W2, R> tailRecN(T initial,
                                          Function<? super T, ? extends Higher<W2, ? extends Either<T, R>>> fn) {
        return flatMapA(in -> Active.of(def2.unit()
                                            .unit(in),
                                        def2)
                                    .tailRec(initial,
                                             fn));
    ...
    public <R> Nested<W1, W2, R> tailRec(T initial,
                                         Function<? super T, ? extends Nested<W1, W2, ? extends Either<T, R>>> fn) {
        return narrowK(Instances.monadRec(def1,
                                          def2)
                                .tailRec(initial,
                                         fn));
    }
    ...
    public Plus plus(MonadPlus<W1> plus1,
                     MonadPlus<W2> plus2) {
        return new Plus(plus1,
                        plus2);
    }

    public Unfolds unfoldsUnsafe() {
        return def2.unfoldable()
                   .fold(s -> new Unfolds(s),
                         () -> new Unfolds(new Unfoldable.UnsafeValueUnfoldable<>()));
    }

    private Plus plusUnsafe() {
        return new Plus(def1.monadPlus()
                            .orElse(null),
                        def2.monadPlus()
                            .orElse(null));
    }
    ...
    public Maybe<Unfolds> unfolds() {
        return def2.unfoldable()
                   .fold(s -> Maybe.just(new Unfolds(s)),
                         Maybe::nothing);
    }

    public Nested<W1, W2, T> plusNested(SemigroupK<W2> semigroupK,
                                        Higher<W2, T> add) {
        return of(def1.functor()
                      .map(i -> semigroupK.apply(i,
                                                 add),
                           nested),
                  def1,
                  def2);
    ...
    public <R> R foldMapBoth(final Monoid<R> mb,
                             final Function<? super T, ? extends R> fn) {
        return def1.foldable()
                   .foldRight(mb,
                              foldMap(mb,
                                      fn));
    }

    public T foldBothl(T identity,
                       BinaryOperator<T> semigroup) {
        return def1.foldable()
                   .foldLeft(identity,
                             semigroup,
                             foldLeft(Monoid.fromBiFunction(identity,
                                                            semigroup)));
    }

    public T foldBothr(T identity,
                       BinaryOperator<T> semigroup) {

        return def1.foldable()
                   .foldRight(identity,
                              semigroup,
                              foldRight(Monoid.fromBiFunction(identity,
                                                              semigroup)));
    }
    ...
    public <R> R foldRight(Monoid<T> monoid,
                           Function<? super Higher<W1, T>, ? extends R> narrowK) {
        return narrowK.apply(foldRight(monoid));
    }

    public <R> R foldLeft(Monoid<T> monoid,
                          Function<? super Higher<W1, T>, ? extends R> narrowK) {
        return narrowK.apply(foldLeft(monoid));
    }

    public Active<W1, T> foldl(T identity,
                               BinaryOperator<T> semigroup) {
        return foldl(Monoid.fromBiFunction(identity,
                                           semigroup));
    }

    public Active<W1, T> foldr(T identity,
                               BinaryOperator<T> semigroup) {
        return foldr(Monoid.fromBiFunction(identity,
                                           semigroup));
    }

    public Active<W1, T> foldl(Monoid<T> monoid) {
        return Active.of(foldLeft(monoid),
                         def1);
    }

    public Active<W1, T> foldr(Monoid<T> monoid) {
        return Active.of(foldRight(monoid),
                         def1);
    }

    public <R> Active<W1, R> foldMapA(final Monoid<R> mb,
                                      final Function<? super T, ? extends R> fn) {
        return Active.of(foldMap(mb,
                                 fn),
                         def1);
    }

    public <R> Higher<W1, R> foldMap(final Monoid<R> mb,
                                     final Function<? super T, ? extends R> fn) {
        return def1.functor()
                   .map(a -> def2.foldable()
                                 .foldMap(mb,
    ...
    public Higher<W1, T> foldLeft(T identity,
                                  BinaryOperator<T> semigroup) {
        return foldLeft(Monoid.fromBiFunction(identity,
                                              semigroup));
    }

    public Higher<W1, T> foldRight(T identity,
                                   BinaryOperator<T> semigroup) {
        return foldRight(Monoid.fromBiFunction(identity,
                                               semigroup));
    }

    public <C2, R> Higher<C2, Nested<W1, W2, R>> traverseA(Applicative<C2> applicative,
                                                           Function<? super T, ? extends Higher<C2, R>> fn) {
        Nested<W1, W2, T> n = this;
        ComposedTraverse<W1, W2> ct = ComposedTraverse.of(n.def1.traverse(),
                                                          n.def2.traverse(),
                                                          n.def2.applicative());
        Higher<C2, Higher<W1, Higher<W2, R>>> r = ct.traverse(applicative,
                                                              fn,
                                                              n.nested);
        Higher<C2, Nested<W1, W2, R>> x = applicative.map(nr -> Nested.of(nr,
                                                                          n.def1,
                                                                          n.def2),
                                                          r);
        return x;
    ...
    public <C2, T> Higher<C2, Nested<W1, W2, T>> sequenceA(Applicative<C2> applicative,
        ...
        Higher<C2, Higher<Higher<Higher<nested, W1>, W2>, T>> x = Instances.traverseA(applicative,
                                                                                      a -> a,
                                                                                      ds);
        return (Higher) x;
    ...
    public <C2, R> Higher<C2, Nested<W1, W2, R>> flatTraverse(Applicative<C2> applicative,
                                                              Function<? super T, ? extends Higher<C2, Nested<W1, W2, R>>> f,
                                                              TransformerFactory<W1, W2> factory) {
        return applicative.map_(traverseA(applicative,
                                          f),
                                it -> it.transformer(factory)
                                        .flatMap(a -> a));
    }

    public <C2, T> Higher<C2, Nested<W1, W2, T>> flatSequence(Applicative<C2> applicative,
                                                              Nested<W1, W2, Higher<C2, Nested<W1, W2, T>>> fgfa,
                                                              TransformerFactory<W1, W2> factory) {
        return applicative.map(i -> i.transformer(factory)
                                     .flatMap(Function.identity()),
                               sequenceA(applicative,
                                         fgfa));
    }

    public Nested<W2, W1, T> sequence() {
        Higher<W2, Higher<W1, T>> res = def1.traverse()
                                            .sequenceA(def2.applicative(),
                                                       nested);
        return of(res,
                  def2,
                  def1);
    ...
    public Option<NestedComprehensions.Guarded<W1, W2, T>> comprehensionsGuarded(TransformerFactory<W1, W2> factory) {
        InstanceDefinitions<Higher<Higher<nested, W1>, W2>> defs = definitions(def1,
                                                                               def2,
                                                                               factory);
        return defs.monadZero()
                   .map(z -> NestedComprehensions.of(this,
                                                     z));
    }
    ...
    public NestedComprehensions<W1, W2, T> comprehensions(TransformerFactory<W1, W2> factory) {
        InstanceDefinitions<Higher<Higher<nested, W1>, W2>> defs = definitions(def1,
                                                                               def2,
                                                                               factory);
        return NestedComprehensions.of(this,
                                       defs.monad());
    ...
    public InstanceDefinitions<Higher<Higher<nested, W1>, W2>> definitions(InstanceDefinitions<W1> def1,
                                                                           InstanceDefinitions<W2> def2,
                                                                           TransformerFactory<W1, W2> factory) {
        return definitions(def1,
                           def2,
                           factory,
                           Maybe.nothing());
    }
    ...
    public InstanceDefinitions<Higher<Higher<nested, W1>, W2>> definitions(InstanceDefinitions<W1> def1,
        ...
        return new InstanceDefinitions<Higher<Higher<nested, W1>, W2>>() {
            ...
            public <T, R> Option<MonadZero<Higher<Higher<DataWitness.nested, W1>, W2>>> monadZero() {
                return zero.map(z -> Instances.monadZero(def1,
                                                         def2,
                                                         factory,
                                                         z));
            }
            ...
            public <T> MonadRec<Higher<Higher<nested, W1>, W2>> monadRec() {
                return Instances.monadRec(def1,
                                          def2);
            }
            ...
            public <C2, T> Traverse<Higher<Higher<nested, W1>, W2>> traverse() {
                return Instances.traverse(def1,
                                          def2,
                                          factory);
            }
    ...
    public interface Converter<W, S> {
        ...
        <R> Active<W, R> to(Function<S, R> fn);
    }
    ...
    public static class Instances {
        ...
        public static <W1, W2> Functor<Higher<Higher<nested, W1>, W2>> functor() {
            return new Functor<Higher<Higher<nested, W1>, W2>>() {
                ...
                @Override
                public <T, R> Higher<Higher<Higher<nested, W1>, W2>, R> map(Function<? super T, ? extends R> fn,
                                                                            Higher<Higher<Higher<nested, W1>, W2>, T> ds) {
        ...
        public static <W1, W2> Pure<Higher<Higher<nested, W1>, W2>> unit(InstanceDefinitions<W1> def1,
                                                                         InstanceDefinitions<W2> def2) {
        ...
        public static <W1, W2> Applicative<Higher<Higher<nested, W1>, W2>> applicative(InstanceDefinitions<W1> def1,
                                                                                       InstanceDefinitions<W2> def2,
                                                                                       TransformerFactory<W1, W2> factory) {
            return new Applicative<Higher<Higher<nested, W1>, W2>>() {
                @Override
                public <T, R> Higher<Higher<Higher<nested, W1>, W2>, R> ap(Higher<Higher<Higher<nested, W1>, W2>, ? extends Function<T, R>> fn,
                                                                           Higher<Higher<Higher<nested, W1>, W2>, T> apply) {
                    Nested<W1, W2, ? extends Function<T, R>> fnA = narrowK(fn);
                    Nested<W1, W2, T> ap = narrowK(apply);
                    return factory.build(ap)
                                  .flatMap(t -> fnA.map(f -> f.apply(t)));
                }
                ...
                public <T, R> Higher<Higher<Higher<nested, W1>, W2>, R> map(Function<? super T, ? extends R> fn,
                                                                            Higher<Higher<Higher<nested, W1>, W2>, T> ds) {
                    return Instances.<W1, W2>functor().map(fn,
                                                           ds);
                }
            };
        }

        public static <W1, W2> MonadRec<Higher<Higher<nested, W1>, W2>> monadRec(InstanceDefinitions<W1> def1,
                                                                                 InstanceDefinitions<W2> def2) {
            return new MonadRec<Higher<Higher<nested, W1>, W2>>() {
                @Override
                public <T, R> Higher<Higher<Higher<nested, W1>, W2>, R> tailRec(T initial,
                                                                                Function<? super T, ? extends Higher<Higher<Higher<nested, W1>, W2>, ? extends Either<T, R>>> fn) {

                    Higher<Higher<Higher<nested, W1>, W2>, Either<T, R>>[] next = new Higher[1];
                    next[0] = Instances.unit(def1,
                                             def2)
                                       .unit(Either.left(initial));
                    Foldable<Higher<Higher<nested, W1>, W2>> foldable = Instances.foldable();
                    boolean[] cont = {true};
                    do {
                        BinaryOperator<Either<T, R>> bifn = (a, b) -> {
                            cont[0] = cont[0] && b.fold(s -> {
                        ...
                        foldable.foldLeft(Either.left(initial),
                                          bifn,
                                          next[0]);
        ...
        @Deprecated
        public static <W1, W2, C2, T, R> Higher<C2, Higher<Higher<Higher<nested, W1>, W2>, R>> traverseA(Applicative<C2> applicative,
                                                                                                         Function<? super T, ? extends Higher<C2, R>> fn,
                                                                                                         Higher<Higher<Higher<nested, W1>, W2>, T> ds) {
            ...
            Higher<C2, Higher<W1, Higher<W2, R>>> r = ct.traverse(applicative,
                                                                  fn,
                                                                  n.nested);
            Higher<C2, Higher<Higher<Higher<nested, W1>, W2>, R>> x = applicative.map(nr -> Nested.of(nr,
                                                                                                      n.def1,
                                                                                                      n.def2),
                                                                                      r);
            return x;
        ...
        public static <W1, W2, T> Traverse<Higher<Higher<nested, W1>, W2>> traverse(InstanceDefinitions<W1> def1,
                                                                                    InstanceDefinitions<W2> def2,
                                                                                    TransformerFactory<W1, W2> factory) {
            return new Traverse<Higher<Higher<DataWitness.nested, W1>, W2>>() {
                @Override
                public <C2, T, R> Higher<C2, Higher<Higher<Higher<DataWitness.nested, W1>, W2>, R>> traverseA(Applicative<C2> applicative,
                                                                                                              Function<? super T, ? extends Higher<C2, R>> fn,
                                                                                                              Higher<Higher<Higher<DataWitness.nested, W1>, W2>, T> ds) {
                    Nested<W1, W2, T> n = narrowK(ds);
                    ComposedTraverse<W1, W2> ct = ComposedTraverse.of(n.def1.traverse(),
                                                                      n.def2.traverse(),
                                                                      n.def2.applicative());
                ...
                @Override
                public <C2, T> Higher<C2, Higher<Higher<Higher<DataWitness.nested, W1>, W2>, T>> sequenceA(Applicative<C2> applicative,
                                                                                                           Higher<Higher<Higher<DataWitness.nested, W1>, W2>, Higher<C2, T>> ds) {
                ...
                @Override
                public <T, R> Higher<Higher<Higher<DataWitness.nested, W1>, W2>, R> ap(Higher<Higher<Higher<DataWitness.nested, W1>, W2>, ? extends Function<T, R>> fn,
                                                                                       Higher<Higher<Higher<DataWitness.nested, W1>, W2>, T> apply) {
                    return applicative(def1,
                                       def2,
                                       factory).ap(fn,
                                                   apply);
                }
                ...
                @Override
                public <T, R> Higher<Higher<Higher<DataWitness.nested, W1>, W2>, R> map(Function<? super T, ? extends R> fn,
                                                                                        Higher<Higher<Higher<DataWitness.nested, W1>, W2>, T> ds) {
                    return applicative(def1,
                                       def2,
                                       factory).map(fn,
        ...
        public static <W1, W2> Monad<Higher<Higher<nested, W1>, W2>> monad(InstanceDefinitions<W1> def1,
                                                                           InstanceDefinitions<W2> def2,
                                                                           TransformerFactory<W1, W2> factory) {
            return new Monad<Higher<Higher<nested, W1>, W2>>() {
                @Override
                public <T, R> Higher<Higher<Higher<nested, W1>, W2>, R> flatMap(Function<? super T, ? extends Higher<Higher<Higher<nested, W1>, W2>, R>> fn,
                                                                                Higher<Higher<Higher<nested, W1>, W2>, T> ds) {
        ...
        public static <W1, W2> MonadZero<Higher<Higher<nested, W1>, W2>> monadZero(InstanceDefinitions<W1> def1,
                                                                                   InstanceDefinitions<W2> def2,
                                                                                   TransformerFactory<W1, W2> factory,
                                                                                   Higher<W2, ?> zero) {
            return new MonadZero<Higher<Higher<nested, W1>, W2>>() {
                ...
                public <T> Higher<Higher<Higher<nested, W1>, W2>, T> zero() {
                    ...
                    Higher<W1, Higher<W2, T>> res = def1.unit()
                                                        .unit(identity);
        ...
        public static <W1, W2> Foldable<Higher<Higher<nested, W1>, W2>> foldable() {
            return new Foldable<Higher<Higher<nested, W1>, W2>>() {
                ...
                public <T, R> R foldMap(Monoid<R> mb,
                                        ...
                                        Higher<Higher<Higher<nested, W1>, W2>, T> nestedA) {
        ...
        public static <W1, W2> Unfoldable<Higher<Higher<nested, W1>, W2>> unfoldable(InstanceDefinitions<W1> def1,
                                                                                     InstanceDefinitions<W2> def2) {
            return new Unfoldable<Higher<Higher<nested, W1>, W2>>() {

                @Override
                public <R, T> Higher<Higher<Higher<nested, W1>, W2>, R> unfold(T b,
                                                                               Function<? super T, Option<Tuple2<R, T>>> fn) {
                    return narrowK(unit(def1,
                                        def2).unit(b)).unfoldsUnsafe()
                                                      .unfold(fn);
                }
            };
        }
    ...
    public static class NestedComprehensions<W1, W2, T> {
        ...
        public static <W1, W2, T> NestedComprehensions<W1, W2, T> of(Nested<W1, W2, T> value1,
                                                                     Monad<Higher<Higher<nested, W1>, W2>> monad) {
            return new NestedComprehensions<>(monad,
                                              value1);
        }

        public static <W1, W2, T> NestedComprehensions.Guarded<W1, W2, T> of(Nested<W1, W2, T> value1,
                                                                             MonadZero<Higher<Higher<nested, W1>, W2>> monad) {
            return new NestedComprehensions.Guarded<>(monad,
                                                      value1);
        }

        public <T2, T3, R1, R2, R3, R> Nested<W1, W2, R> forEach4(Function<? super T, ? extends Nested<W1, W2, R1>> value2,
            ...
            return narrowK(monad.flatMap_(value1,
                                          in -> {

                                              Nested<W1, W2, R1> a = value2.apply(in);
                                              return monad.flatMap_(a,
                                                                    ina -> {
                                                                        Nested<W1, W2, R2> b = value3.apply(in,
                                                                                                            ina);
                                                                        return monad.flatMap_(b,
                                                                                              inb -> {
                                                                                                  Nested<W1, W2, R3> c = value4.apply(in,
                                                                                                                                      ina,
                                                                                                                                      inb);
                                                                                                  return monad.map_(c,
                                                                                                                    in2 -> yieldingFunction.apply(in,
                                                                                                                                                  ina,
                                                                                                                                                  inb,
                                                                                                                                                  in2));
                                                                                              });
        ...
        public <T2, R1, R2, R> Nested<W1, W2, R> forEach3(Function<? super T, ? extends Nested<W1, W2, R1>> value2,
                                                          BiFunction<? super T, ? super R1, ? extends Nested<W1, W2, R2>> value3,
        ...
        public <R1, R> Nested<W1, W2, R> forEach2(Function<? super T, ? extends Nested<W1, W2, R1>> value2,
            ...
            Higher<Higher<Higher<nested, W1>, W2>, R> x = monad.flatMap_(value1,
                                                                         in -> {
        ...
        public static class Guarded<W1, W2, T> {
            ...
            public <T2, T3, R1, R2, R3, R> Nested<W1, W2, R> forEach4(Function<? super T, ? extends Nested<W1, W2, R1>> value2,
                ...
                return narrowK(monadZero.flatMap_(value1,
                                                  in -> {

                                                      Nested<W1, W2, R1> a = value2.apply(in);
                                                      return monadZero.flatMap_(a,
                                                                                ina -> {
                                                                                    Nested<W1, W2, R2> b = value3.apply(in,
                                                                                                                        ina);
                                                                                    return monadZero.flatMap_(b,
                                                                                                              inb -> {
                                                                                                                  Nested<W1, W2, R3> c = value4.apply(in,
                                                                                                                                                      ina,
                                                                                                                                                      inb);

                                                                                                                  Nested<W1, W2, R3> x = narrowK(monadZero.filter_(c,
                                                                                                                                                                   in2 -> filterFunction.apply(in,
                                                                                                                                                                                               ina,
                                                                                                                                                                                               inb,
                                                                                                                                                                                               in2)));
                                                                                                                  return monadZero.map_(x,
                                                                                                                                        in2 -> yieldingFunction.apply(in,
                                                                                                                                                                      ina,
                                                                                                                                                                      inb,
                                                                                                                                                                      in2));
                                                  ...
                                                  in -> {
                                                      ...
                                                      return monadZero.flatMap_(a,
                                                                                ina -> {
                                                                                    Nested<W1, W2, R2> b = value3.apply(in,
                                                                                                                        ina);
                                                                                    Nested<W1, W2, R2> x = narrowK(monadZero.filter_(b,
                                                                                                                                     in2 -> filterFunction.apply(in,
                                                                                                                                                                 ina,
                                                                                                                                                                 in2)));
                                                                                    return monadZero.map_(x,
            ...
            public <R1, R> Nested<W1, W2, R> forEach2(Function<? super T, ? extends Nested<W1, W2, R1>> value2,
                ...
                return narrowK(monadZero.flatMap_(value1,
                                                  in -> {
                                                      ...
                                                      Nested<W1, W2, R1> a = value2.apply(in);
                                                      Nested<W1, W2, R1> x = narrowK(monadZero.filter_(a,
                                                                                                       in2 -> filterFunction.apply(in,
                                                                                                                                   in2)));
                                                      return monadZero.map_(x,
                                                                            in2 -> yieldingFunction.apply(in,
    ...
    class NarrowedFlatMap<C, R> {
        ...
        public <R2> Nested<W1, W2, R2> zip(C fb,
                                           BiFunction<? super T, ? super R, ? extends R2> f) {
    ...
    class NarrowedTailRec<C, R> {
        ...
        public Nested<W1, W2, R> tailRecN(T initial,
                                          Function<? super T, ? extends C> fn) {
            return Nested.this.tailRecN(initial,
                                        fn.andThen(widen));
        }
    }
    ...
    class Narrowed<C> {
        ...
        public Nested<W1, W2, T> plus(Monoid<C> m,
                                      C add) {
            return sum(m,
                       LazySeq.of(add));
        }

        public Nested<W1, W2, T> sum(C seed,
                                     BinaryOperator<C> op,
                                     ImmutableList<C> list) {
            return of(def1.functor()
                          .map_(nested,
                                f -> {
                                    C res = list.plus(narrow.apply(f))
                                                .foldLeft(seed,
                                                          (a, b) -> op.apply(a,
                                                                             b));
        ...
        public Nested<W1, W2, T> sum(Monoid<C> s,
                                     ImmutableList<C> list) {
            return of(def1.functor()
                          .map_(nested,
                                f -> {
        ...
        public Nested<W1, W2, T> sumInverted(Group<C> s,
                                             ImmutableList<C> list) {
            return of(def1.functor()
                          .map_(nested,
                                f -> {
                                    C res = s.invert(list.plus(narrow.apply(f))
                                                         .foldLeft(s.zero(),
                                                                   (a, b) -> s.apply(a,
                                                                                     b)));
                                    return widen.apply(res);
        ...
        public Maybe<Nested<W1, W2, T>> sum(ImmutableList<C> list) {
            return Nested.this.plus()
                              .flatMap(s -> Maybe.just(sum(narrow.apply(s.monoid()
                                                                         .zero()),
                                                           (C a, C b) -> narrow.apply(s.monoid()
                                                                                       .apply(widen.apply(a),
                                                                                              widen.apply(b))),
                                                           list)));
        }
    ...
    public class Plus {
        ...
        public Nested<W1, W2, T> sum(ImmutableList<Nested<W1, W2, T>> list) {
            return of(plus1.sum(list.plus(Nested.this)
                                    .map(x -> x.nested)),
                      def1,
                      def2);
        }
        ...
        public Nested<W1, W2, T> plus(Higher<W2, T> b) {
            ...
            Higher<W1, Higher<W2, T>> x = f.map(a -> mp.plus(a,
                                                             b),
                                                nested);
            return of(x,
                      def1,
    ...
    public class Unfolds {
        ...
        public <R> Nested<W1, W2, R> unfold(Function<? super T, Option<Tuple2<R, T>>> fn) {
            ...
            Higher<W1, Higher<W2, R>> x = def1.functor()
                                              .map(a -> def2.monad()
                                                            .flatMap(c -> unf.unfold(c,
                                                                                     fn),
                                                                     a),
                                                   nested);
            return Nested.of(x,
        ...
        private <T2> Nested<W1, W2, T> unfoldPrivate(T2 b,
                                                     Function<T2, Option<Tuple2<T, T2>>> fn) {
            Unfoldable<W2> unf = unfold2;
            Higher<W1, Higher<W2, T>> x = def1.functor()
                                              .map(a -> def2.monad()
                                                            .flatMap(c -> unf.unfold(b,
                                                                                     fn.andThen(o -> o.map(t -> t.map1(v -> c)))),
                                                                     a),
                                                   nested);
            return Nested.of(x,
                             def1,
        ...
        private <T, R> Nested<W1, W2, R> unfoldIgnore(T b,
                                                      Function<T, Option<Tuple2<R, T>>> fn) {
            Unfoldable<W2> unf = unfold2;
            Higher<W1, Higher<W2, R>> x = def1.functor()
                                              .map(a -> def2.monad()
                                                            .flatMap(c -> unf.unfold(b,
        ...
        public <R> Nested<W1, W2, R> replaceWith(int n,
                                                 R value) {
            return this.unfoldIgnore(n,
                                     i -> Option.of(Tuple.tuple(value,
                                                                i - 1)));
        }

        public Nested<W1, W2, T> replicate(int n) {
            return this.unfoldPrivate(n,
                                      i -> Option.some(Tuple.tuple(null,
                                                                   i - 1)));
        }
