 ...
 * <p>
 * A technique for functional dependency injection. Functions rather than values are manipulated and dependencies injected into
 * Functions to execute them.
 * <p>
 * {@see <a href="https://medium.com/@johnmcclean/dependency-injection-using-the-reader-monad-in-java8-9056d9501c75">Dependency
 * injection using the Reader Monad in Java 8</a>}
 *
...
public interface Reader<T, R> extends Function1<T, R>, Transformable<R>, Higher<Higher<reader, T>, R> {
    ...
    static <T, R> Reader<T, R> of(Reader<T, R> i) {
        return i;
    }

    static <T, R> Reader<T, R> of(Function<T, R> i) {
        return in -> i.apply(in);
    }

    static <T, R> Higher<Higher<reader, T>, R> widen(Reader<T, R> narrow) {
        return narrow;
    }
    ...
    static <T, R> Reader<T, R> narrowK(Higher<Higher<reader, T>, R> hkt) {
    ...
    default <R2, B> Reader<T, B> zip(Reader<T, R2> o,
                                     BiFunction<? super R, ? super R2, ? extends B> fn) {
        return flatMap(a -> o.mapFn(b -> fn.apply(a,
                                                  b)));
    }
    ...
    default <R1, R2, R3, R4> Reader<T, R4> forEach4(Function<? super R, Function<? super T, ? extends R1>> value2,
        ...
        Reader<? super T, ? extends R4> res = this.flatMap(in -> {
            ...
            return a.flatMap(ina -> {
                Reader<T, R2> b = functionToReader(value3.apply(in,
                                                                ina));
                return b.flatMap(inb -> {

                    Reader<T, R3> c = functionToReader(value4.apply(in,
                                                                    ina,
                                                                    inb));
    ...
    default R foldLeft(T t,
                       Monoid<R> monoid) {
        Function1<T, R> x = this.andThen(v -> monoid.apply(monoid.zero(),
                                                           v));
        return x.apply(t);
    }
