...
public class MaybeInstances {
    ...
    public static <T> Kleisli<option, Maybe<T>, T> kindKleisli() {
        return Kleisli.of(MaybeInstances.monad(),
                          Maybe::widen);
    }
    ...
    public static <W1, T> Nested<option, W1, T> nested(Maybe<Higher<W1, T>> nested,
    ...
    public static <W1, T> Product<option, W1, T> product(Maybe<T> m,
                                                         Active<W1, T> active) {
        return Product.of(allTypeclasses(m),
                          active);
    }

    public static <W1, T> Coproduct<W1, option, T> coproduct(Maybe<T> m,
                                                             InstanceDefinitions<W1> def2) {
    ...
    public static <W2, R, T> Nested<option, W2, R> mapM(Maybe<T> m,
    ...
    public static class OptionTypeclasses implements MonadPlus<option>, MonadRec<option>, TraverseByTraverse<option>,
        ...
        public <R, T> Higher<option, R> unfold(T b,
            ...
            return Maybe.fromPublisher(fn.apply(b)
                                         .map(t -> t._1()));
        }
        ...
        public <T, R> Higher<option, R> flatMap(Function<? super T, ? extends Higher<option, R>> fn,
                                                Higher<option, T> ds) {
            return Maybe.narrowK(ds)
                        .flatMap(t -> Option.narrowK(fn.apply(t)));
        }
        ...
        public <T, R> Higher<option, R> ap(Higher<option, ? extends Function<T, R>> fn,
            ...
            return Maybe.narrowK(apply)
                        .zip(Option.narrowK(fn),
                             (a, b) -> b.apply(a));
        }
        ...
        public <T, R> Higher<option, R> tailRec(T initial,
            ...
            return Maybe.tailRec(initial,
                                 t -> Maybe.narrowK(fn.apply(t)));
        }
    }
