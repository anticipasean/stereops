...
public class Monos {
    ...
    public static <T> Mono<Flux<T>> sequence(final Publisher<? extends Mono<T>> fts) {
        ...
        BiFunction<Mono<Flux<T>>, Mono<T>, Mono<Flux<T>>> combineToStream = (acc, next) -> acc.zipWith(next,
                                                                                                       (a, b) -> Flux.concat(a,
                                                                                                                             Flux.just(b)));

        return Flux.from(fts)
                   .reduce(identity,
                           combineToStream)
                   .flatMap(i -> i);
    }

    public static <T, R> Mono<Flux<R>> traverse(Function<? super T, ? extends R> fn,
                                                Publisher<Mono<T>> stream) {
        Flux<Mono<R>> s = Flux.from(stream)
                              .map(h -> h.map(fn));
        return sequence(s);
    }

    public static <T, R> Mono<R> tailRec(T initial,
                                         Function<? super T, ? extends Mono<? extends Either<T, R>>> fn) {
     ...
     * @return Mono that completes when all the provided Futures Complete. Empty Future result, or holds an Exception from a
     ...
     */
    @SafeVarargs
    public static <T> Mono<ListX<T>> quorum(Predicate<Status<T>> breakout,
                                            Consumer<Throwable> errorHandler,
                                            Mono<T>... fts) {

        return Mono.from(Futures.quorum(breakout,
                                        errorHandler,
                                        futures(fts)));
    ...
    /**
     * Perform a For Comprehension over a Mono, accepting 3 generating functions. This results in a four level nested internal
     * iteration over the provided Monos.
     *
     * <pre>
     * {@code
     *
     *   import static cyclops.reactor.companion.Monos.forEach4;
     *
    ...
    public static <T1, T2, T3, R1, R2, R3, R> Mono<R> forEach4(Mono<? extends T1> value1,
        ...
        Future<? extends R> res = Future.fromPublisher(value1)
                                        .flatMap(in -> {

                                            Future<R1> a = Future.fromPublisher(value2.apply(in));
                                            return a.flatMap(ina -> {
                                                Future<R2> b = Future.fromPublisher(value3.apply(in,
                                                                                                 ina));
                                                return b.flatMap(inb -> {
                                                    Future<R3> c = Future.fromPublisher(value4.apply(in,
                                                                                                     ina,
                                                                                                     inb));
    ...
    /**
     * Perform a For Comprehension over a Mono, accepting 2 generating functions. This results in a three level nested internal
     * iteration over the provided Monos.
     *
     * <pre>
     * {@code
     *
     *   import static cyclops.reactor.companion.Monos.forEach3;
     *
    ...
    /**
     * Perform a For Comprehension over a Mono, accepting a generating function. This results in a two level nested internal
     * iteration over the provided Monos.
     *
     * <pre>
     * {@code
     *
     *   import static cyclops.reactor.companion.Monos.forEach;
     *
     * forEach(Mono.just(1),
     * a-> Mono.just(a+1),
     * Tuple::tuple)
    ...
    public static <T, R1, R> Mono<R> forEach(Mono<? extends T> value1,
        ...
        Future<R> res = Future.fromPublisher(value1)
                              .flatMap(in -> {
     ...
     */
    public static <T1, T2, R> Mono<R> combine(Mono<? extends T1> mono,
     ...
     * @return Combined Mono
     */
    public static <T1, T2, R> Mono<R> zip(Mono<? extends T1> mono,
    ...
    public static <T1, T2, R> Mono<R> zip(Mono<? extends T1> mono,
        ...
        Mono<R> res = Mono.from(Future.of(mono.toFuture())
                                      .zip(fn,
                                           app));
        return res;
    }
    ...
    public static <T> Iterator<T> iterator(Mono<T> pub) {
        return Future.fromPublisher(pub)
                     .iterator();
