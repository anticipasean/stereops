...
public class ReactiveConvertableSequence<T> extends ConvertableSequence<T> {
    ...
    public <K, V> MapX<K, V> mapX(final Function<? super T, ? extends K> keyMapper,
                                  final Function<? super T, ? extends V> valueMapper) {
        return MapX.fromMap(stream().collect(Collectors.toMap(keyMapper,
                                                              valueMapper)));
    }
    ...
    public Maybe<T> lazyFirstValue() {
        return Eval.later(() -> listX(Evaluation.LAZY))
                   .toMaybe()
    ...
    public <C extends Collection<T>> C collection(final Supplier<C> factory) {
        return ReactiveSeq.fromIterable(getIterable())
