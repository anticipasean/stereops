...
public interface Function0<R> extends Supplier<R> {
    ...
    default Future<R> future(Executor ex) {
        return Future.of(CompletableFuture.supplyAsync(this,
                                                       ex));
    }
    ...
    default Function0<Future<R>> lift(Executor ex) {
        return () -> Future.of(() -> apply(),
                               ex);
    }

    default Function0<Try<R, Throwable>> liftTry() {
        return () -> Try.withCatch(() -> apply(),
                                   Throwable.class);
    }
    ...
    default Function0<R> memoizeAsync(ScheduledExecutorService ex,
                                      String cron) {
        return Memoize.memoizeSupplierAsync(this,
                                            ex,
                                            cron);
    }
    ...
    default ReactiveSeq<R> iterate(final UnaryOperator<R> fn) {
        return ReactiveSeq.iterate(get(),
                                   fn);
    }
