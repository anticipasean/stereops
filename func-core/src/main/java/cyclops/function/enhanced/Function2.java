...
public interface Function2<T1, T2, R> extends BiFunction<T1, T2, R>, To<Function2<T1, T2, R>> {
    ...
    default Function2<T1, T2, Future<R>> lift(Executor ex) {
        Function2<T1, T2, R> host = this;
        return (T1, T2) -> Future.of(() -> host.apply(T1,
                                                      T2),
                                     ex);
    }
    ...
    default Function2<T1, T2, R> memoizeAsync(ScheduledExecutorService ex,
                                              String cron) {
        return Memoize.memoizeBiFunctionAsync(this,
                                              ex,
                                              cron);
    }
