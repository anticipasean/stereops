...
public class QueueFactories {
    ...
    /**
     * Create a QueueFactory for boundedQueues where bound is determined by the provided queueSize parameter Generated Queues will
     * be backed by a LinkedBlockingQueue
     *
     * <pre>
     * {@code
     *   Queue<String> transferQueue = QueueFactories.<String>boundedQueue(4)
    ...
    /**
     * Creates an async.Queue backed by a JDK Wait Free unbounded ConcurrentLinkedQueue Wait strategy used is NoWaitRetry by
     * default for both Consumers and Producers (both Consumers and Producers will repeatedly retry until successful). Use
     * withConsumerWaitStrategy &amp; withProducerWaitStrategy methods on the returned queue to change the wait strategy
     * <pre>
     * {@code
    ...
    public static <T> QueueFactory<T> unboundedNonBlockingQueue() {
        return () -> new Queue<T>(new ConcurrentLinkedQueue<>(),
                                  new NoWaitRetry<>(),
                                  new NoWaitRetry<>());
    ...
    /**
     * Creates an async.Queue backed by a JDK Wait Free unbounded ConcurrentLinkedQueue The provided WaitStrategy is used to
     * determine behaviour of both producers and consumers when the Queue is full (producer) or zero (consumer). {@see
     * WaitStrategy#spinWait() , @see WaitStrategy#exponentialBackOff() , @see WaitStrategy#noWaitRetry() }
     *
    ...
    /**
     * Creates an async.Queue backed by an Agrona ManyToOneConcurrentArrayQueue bounded by specified queueSize Wait strategy used
     * is NoWaitRetry by default for both Consumers and Producers (both Consumers and Producers will repeatedly retry until
     * successful). Use withConsumerWaitStrategy  &amp; withProducerWaitStrategy methods on the returned queue to change the wait
     * strategy
     * <pre>
     * {@code
     *    queue.withConsumerWaitStrategy(new DirectWaitStrategy())
    ...
    public static <T> QueueFactory<T> boundedNonBlockingQueue(final int queueSize) {
        return () -> new Queue<T>(new ManyToOneConcurrentArrayQueue<>(queueSize),
                                  new NoWaitRetry<>(),
                                  new NoWaitRetry<>());
    }

    /**
     * Generate QueueFactory for bounded non blocking queues. Max queue size is determined by the input parameter. The provided
     * WaitStrategy is used to determine behaviour of both producers and consumers when the Queue is full (producer) or zero
     * (consumer). {@see WaitStrategy#spinWait() , @see WaitStrategy#exponentialBackOff() , @see WaitStrategy#noWaitRetry() }
     *
     * @param queueSize Max Queue size
     * @param strategy  Strategy to be employed by producers when Queue is full, or consumers when Queue is zero
     * @return bounded wait free Queue Factory backed by an Agrona ManyToOneConcurrentArrayQueue
    ...
    /**
     * Creates an async.Queue backed by an Agrona OneToOneConcurrentArrayQueue bounded by specified queueSize Wait strategy used
     * is NoWaitRetry by default for both Consumers and Producers (both Consumers and Producers will repeatedly retry until
     * successful). Use withConsumerWaitStrategy  &amp; withProducerWaitStrategy methods on the returned queue to change the wait
     * strategy
     * <pre>
    ...
    public static <T> QueueFactory<T> singleWriterboundedNonBlockingQueue(final int queueSize) {
        return () -> new Queue<T>(new OneToOneConcurrentArrayQueue<>(queueSize),
                                  new NoWaitRetry<>(),
                                  new NoWaitRetry<>());
