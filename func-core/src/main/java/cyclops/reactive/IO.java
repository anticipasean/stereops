...
public interface IO<T> extends To<IO<T>>, Higher<io, T>, ReactiveTransformable<T>, Publisher<T> {
    ...
    default <X extends Throwable> IO<T> recoverWith(Class<X> type,
                                                    final BiFunction<Integer, X, ? extends IO<? extends T>> fn) {
        AtomicInteger count = new AtomicInteger(0);
        return recoverWith(t -> {
    ...
    default <R> IO<R> flatMap(Function<? super T, IO<? extends R>> s) {
        return unit(stream().flatMap(t -> s.apply(t)
                                           .stream()));
    }
    ...
    default <R> IO<R> mergeMap(int maxConcurrency,
                               Function<? super T, Publisher<? extends R>> s) {
        return unit(stream().mergeMap(maxConcurrency,
                                      t -> s.apply(t)));
    }
    ...
    default <R extends AutoCloseable, R1> Managed.Tupled<R, R1> bracketWith(Function<? super T, ? extends R> fn,
                                                                            Function<? super R, ? extends R1> with) {
        Managed.Tupled<? extends R, ? extends R1> x = Managed.of(map(fn))
                                                             .with(with);
    ...
    default <R> IO<R> bracket(Function<? super T, ? extends R> fn,
                              Consumer<R> consumer) {
        Managed<R> m = Managed.of(map(fn),
                                  consumer);
        return m.io();
    ...
    default IO<T> ensuring(Consumer<T> action) {
        return Managed.of(this,
                          action)
                      .io(in -> unit(Spouts.of(in)));
    }

    default <B, R> IO<R> zip(IO<B> that,
                             BiFunction<? super T, ? super B, ? extends R> fn) {
        return flatMap(a -> that.map(t -> fn.apply(a,
                                                   t)));
    }

    default <B, R> IO<R> par(IO<B> that,
                             BiFunction<? super T, ? super B, ? extends R> fn) {
        ReactiveSeq<? extends R> x = stream().zip(fn,
                                                  that);
        return unit(ReactiveSeq.narrow(x));
    }

    default IO<T> race(IO<T> that) {
        return unit(Spouts.amb(Seq.of(publisher(),
                                      that.publisher())));
    }
    ...
    default <R, X extends Throwable> IO<Try<R, X>> mapTry(Function<? super T, ? extends R> s,
                                                          Class<? extends X>... classes) {
        return map(t -> Try.withCatch(() -> s.apply(t),
                                      classes));
    }

    default void forEach(Consumer<? super T> consumerElement,
    ...
    default Subscription forEach(int n,
                                 Consumer<? super T> consumerElement,
                                 Consumer<? super Throwable> consumerError,
                                 Runnable onComplete) {
        return stream().forEach(n,
                                consumerElement,
                                consumerError,
                                onComplete);
    }
    ...
    default <R> R foldLeft(final R identity,
    ...
    default Try<T, Throwable> runAsync(Executor e) {
        return Try.fromPublisher(Future.of(() -> run(),
                                           e))
        ...
        public static <T, F, R1, R2, R3, R4, R5, R6, R7> IO<R7> forEach(IO<T> io,
            ...
            return io.flatMap(in -> {
                ...
                return a.flatMap(ina -> {
                    IO<R2> b = value3.apply(Tuple.tuple(in,
                                                        ina));
                    return b.flatMap(inb -> {

                        IO<R3> c = value4.apply(Tuple.tuple(in,
                                                            ina,
                                                            inb));

                        return c.flatMap(inc -> {
                            IO<R4> d = value5.apply(Tuple.tuple(in,
                                                                ina,
                                                                inb,
                                                                inc));
                            return d.flatMap(ind -> {
                                IO<R5> e = value6.apply(Tuple.tuple(in,
                                                                    ina,
                                                                    inb,
                                                                    inc,
                                                                    ind));
                                return e.flatMap(ine -> {
                                    IO<R6> f = value7.apply(Tuple.tuple(in,
                                                                        ina,
                                                                        inb,
                                                                        inc,
                                                                        ind,
                                    ...
                                    return f.flatMap(inf -> {
                                        IO<R7> g = value8.apply(Tuple.tuple(in,
                                                                            ina,
                                                                            inb,
                                                                            inc,
                                                                            ind,
    ...
    final class ReactiveSeqIO<T> implements IO<T> {
        ...
        public static <T> IO<T> of(Supplier<? extends T> s) {
            return new ReactiveSeqIO<T>(ReactiveSeq.narrow(Spouts.generate(Memoize.memoizeSupplier(s))
                                                                 .take(1)));
        }

        public static <T> IO<T> of(Supplier<? extends T> s,
                                   Executor ex) {
            return new ReactiveSeqIO<T>(Spouts.from(Future.narrow(Future.of(s,
                                                                            ex))));
        }
        ...
        public static <T1, T2, R> IO<R> merge(Publisher<T1> p1,
            ...
            return fromPublisher(s1.zip(fn2,
                                        s2));
        }

        public static <T1, T2, T3, R> IO<R> merge(Publisher<T1> p1,
                                                  Publisher<T2> p2,
                                                  Publisher<T3> p3,
                                                  Function3<? super T1, ? super T2, ? super T3, ? extends R> fn3) {
            return merge(merge(p1,
                               p2,
                               Tuple::tuple),
                         p3,
                         (t2, c) -> fn3.apply(t2._1(),
                                              t2._2(),
                                              c));
        }
        ...
        public <R> IO<R> flatMap(Function<? super T, IO<? extends R>> s) {
            return fromPublisher(fn.mergeMap(t -> s.apply(t)
                                                   .publisher()));
        }
        ...
        public <R> IO<R> mergeMap(int maxConcurrency,
                                  Function<? super T, Publisher<? extends R>> s) {
            return fromPublisher(fn.mergeMap(maxConcurrency,
                                             t -> s.apply(t)));
        }
        ...
        @Override
        public <R, X extends Throwable> IO<Try<R, X>> mapTry(Function<? super T, ? extends R> s,
    ...
    final class SyncIO<T> implements IO<T> {
        ...
        public static <T> IO<T> of(Supplier<? extends T> s) {
            return new SyncIO<T>(ReactiveSeq.generate(Memoize.memoizeSupplier((Supplier<T>) s))
                                            .take(1));
        }
        ...
        public <R> IO<R> mergeMap(int maxConcurrency,
                                  Function<? super T, Publisher<? extends R>> s) {
            return new SyncIO<R>(fn.mergeMap(maxConcurrency,
                                             s));
        }
        ...
        public <R extends AutoCloseable, R1> Managed.Tupled<R, R1> bracketWith(Function<? super T, ? extends R> fn,
                                                                               Function<? super R, ? extends R1> with) {
            Managed.Tupled<? extends R, ? extends R1> x = SyncManaged.of(map(fn))
                                                                     .with(with);
            return (Managed.Tupled<R, R1>) x;
        }
        ...
        public static abstract class SyncManaged<T> extends Managed<T> {
            ...
            public static <T> Managed<T> of(IO<T> acquire,
                                            Consumer<T> cleanup) {

                return new SyncManaged<T>() {
                    public <R> IO<R> apply(Function<? super T, ? extends IO<R>> fn) {
                        IO<R> y = IO.Comprehensions.forEach(acquire,
                                                            t1 -> {
                                                                IO<? extends Try<? extends IO<R>, Throwable>> res1 = SyncIO.withCatch(() -> fn.apply(t1),
                                                                                                                                      Throwable.class);
                                                                return res1;
                                                            },
                                                            t2 -> {
                                                                ...
                                                                IO<R> res = tr.fold(r -> r,
                                                                                    e -> SyncIO.of(Future.ofError(e)));
            ...
            public <R> Managed<R> map(Function<? super T, ? extends R> mapper) {
                return of(apply(mapper.andThen(IO::of)),
                          __ -> {
                          });
            }
