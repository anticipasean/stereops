...
/**
 * A totally Lazy Either implementation with tail call optimization for transform and flatMap operators. LazyEither can operate
 * reactively (i.e. suppports data arriving asynchronsouly the monadic chain of computations will only be executed once data
 * arrives).
 * <p>
...
public interface LazyEither<LT, RT> extends Either<LT, RT> {
    ...
    static <ST, T> LazyEither<ST, T> narrowK2(final Higher2<either, ST, T> xor) {
        return fromEither(Either.narrowK2(xor));

    }

    static <ST, T> LazyEither<ST, T> narrowK(final Higher<Higher<either, ST>, T> xor) {
        return fromEither(Either.narrowK(xor));
    }

    static <L, T, R> LazyEither<L, R> tailRec(T initial,
                                              Function<? super T, ? extends LazyEither<L, ? extends Either<T, R>>> fn) {
    ...
    static <RT> LazyEither<Throwable, RT> async(final Executor ex,
                                                final Supplier<RT> s) {
        return fromFuture(Future.of(s,
                                    ex));
    }
    ...
    static <RT> CompletableEither<RT, RT> either() {
        ...
        return new LazyEither.CompletableEither<RT, RT>(c,
    ...
    static <ST, PT> LazyEither<ST, PT> fromEither(Either<ST, PT> xor) {
        ...
        return xor.fold(LazyEither::left,
                        LazyEither::right);
    }
    ...
    static <LT, RT> LazyEither<LT, RT> later(Supplier<Either<LT, RT>> lazy) {
        return new LazyEither.Lazy<>(Eval.later(lazy)
                                         .map(LazyEither::fromEither));
    }

    static <LT, RT> LazyEither<LT, RT> always(Supplier<Either<LT, RT>> lazy) {
        return new LazyEither.Lazy<>(Eval.always(lazy)
                                         .map(LazyEither::fromEither));
    }

    static <T> LazyEither<Throwable, T> fromFuture(Future<T> future) {
        return fromLazy(Eval.fromFuture(future.<LazyEither<Throwable, T>>map(LazyEither::right).recover(e -> LazyEither.left(e.getCause()))));
     ...
     */
    static <LT1, PT> LazyEither<LT1, ReactiveSeq<PT>> sequenceRight(final Iterable<LazyEither<LT1, PT>> xors) {
        Objects.requireNonNull(xors);
        return sequence(ReactiveSeq.fromIterable(xors)
                                   .filter(LazyEither::isRight));
    }

    static <LT1, PT> LazyEither<ReactiveSeq<LT1>, PT> sequenceLeft(final Iterable<LazyEither<LT1, PT>> xors) {
        Objects.requireNonNull(xors);
    ...
    static <L, T> LazyEither<L, ReactiveSeq<T>> sequence(ReactiveSeq<? extends LazyEither<L, T>> stream) {
        ...
        BiFunction<LazyEither<L, ReactiveSeq<T>>, LazyEither<L, T>, LazyEither<L, ReactiveSeq<T>>> combineToStream = (acc, next) -> acc.zip(next,
                                                                                                                                            (a, b) -> a.append(b));

        BinaryOperator<LazyEither<L, ReactiveSeq<T>>> combineStreams = (a, b) -> a.zip(b,
    ...
    static <L, T, R> LazyEither<L, ReactiveSeq<R>> traverse(Function<? super T, ? extends R> fn,
                                                            ReactiveSeq<LazyEither<L, T>> stream) {
        return sequence(stream.map(h -> h.map(fn)));
    }

    /**
     * TraverseOps a Collection of Either producting an Either3 with a Seq, applying the transformation function to every element
     * in the list
     *
     ...
     */
    static <LT1, PT, R> LazyEither<LT1, ReactiveSeq<R>> traverseRight(final Iterable<LazyEither<LT1, PT>> xors,
                                                                      Function<? super PT, ? extends R> fn) {
        return sequenceRight(xors).map(l -> l.map(fn));
    }

    static <LT1, PT, R> LazyEither<ReactiveSeq<R>, PT> traverseLeft(final Iterable<LazyEither<LT1, PT>> xors,
                                                                    Function<? super LT1, ? extends R> fn) {
     ...
     */
    static <LT1, RT> LazyEither<LT1, RT> accumulate(final Monoid<RT> reducer,
                                                    final Iterable<LazyEither<LT1, RT>> xors) {
        return sequenceRight(xors).map(s -> s.reduce(reducer));
    }

    static <LT, B, RT> LazyEither<LT, RT> rightEval(final Eval<RT> right) {
        return new Right(right);
    }

    static <LT, B, RT> LazyEither<LT, RT> leftEval(final Eval<LT> left) {
    ...
    static <X, PT extends X, ST extends X, R> R visitAny(LazyEither<ST, PT> either,
                                                         Function<? super X, ? extends R> fn) {
        return foldAny(either,
                       fn);
    }

    static <X, PT extends X, ST extends X, R> R foldAny(LazyEither<ST, PT> either,
                                                        Function<? super X, ? extends R> fn) {
     ...
     */
    static <T> LazyEither<Throwable, T> fromPublisher(final Publisher<T> pub) {
     ...
     */
    static <ST, T> LazyEither<ST, T> fromIterable(final Iterable<T> iterable,
                                                  T alt) {
        return later(() -> {
    ...
    /**
     * Create an instance of the left type. Most methods are biased to the right type, so you will need to use swap() or
     * secondaryXXXX to manipulate the wrapped value
     *
     * <pre>
     * {@code
     ...
     */
    static <ST, PT> LazyEither<ST, PT> left(final ST value) {
     ...
     */
    static <ST, PT> LazyEither<ST, PT> right(final PT value) {

        return new Right(Eval.later(() -> value));
    }
    ...
    @Override
    default <T2, R1, R2, R3, R> LazyEither<LT, R> forEach4(Function<? super RT, ? extends Either<LT, R1>> value1,
        ...
        return (LazyEither<LT, R>) Either.super.forEach4(value1,
                                                         value2,
                                                         value3,
                                                         yieldingFunction);
    }
    ...
    default <T2, R1, R2, R> Either<LT, R> forEach3(Function<? super RT, ? extends Either<LT, R1>> value1,
        ...
        return (LazyEither<LT, R>) Either.super.forEach3(value1,
                                                         value2,
                                                         yieldingFunction);
    }
    ...
    default <R1, R> Either<LT, R> forEach2(Function<? super RT, ? extends Either<LT, R1>> value1,
        ...
        return (LazyEither<LT, R>) Either.super.forEach2(value1,
                                                         yieldingFunction);
    }

    /* (non-Javadoc)
     * @see com.oath.cyclops.container.control.Xor#combineToList(com.oath.cyclops.container.control.Xor, java.util.function.BiFunction)
     */
    @Override
    default <T2, R> LazyEither<LazySeq<LT>, R> combineToLazySeq(Either<LT, ? extends T2> app,
                                                                BiFunction<? super RT, ? super T2, ? extends R> fn) {

        return (LazyEither<LazySeq<LT>, R>) Either.super.combineToLazySeq(app,
                                                                          fn);
    }

    /* (non-Javadoc)
     * @see com.oath.cyclops.container.control.Xor#combine(com.oath.cyclops.container.control.Xor, java.util.function.BinaryOperator, java.util.function.BiFunction)
     */
    @Override
    default <T2, R> LazyEither<LT, R> combine(Either<? extends LT, ? extends T2> app,
                                              BinaryOperator<LT> semigroup,
                                              BiFunction<? super RT, ? super T2, ? extends R> fn) {

        return (LazyEither<LT, R>) Either.super.combine(app,
                                                        semigroup,
                                                        fn);
    }
    ...
    default <T2, R> LazyEither<LT, R> zip(final Ior<LT, ? extends T2> app,
    ...
    default <T2, R> LazyEither<LT, R> zip(final Either<LT, ? extends T2> app,
                                          final BiFunction<? super RT, ? super T2, ? extends R> fn) {
        return flatMap(t -> app.map(t2 -> fn.apply(t,
                                                   t2)));
    }
    ...
    default LazyEither<LT, RT> filter(Predicate<? super RT> test,
    ...
    default <ST2> Either<ST2, RT> toEither(final ST2 secondary) {
        return fold(s -> Either.left(secondary),
                    p -> Either.right(p));
    }
     ...
     */
    <R> R fold(Function<? super LT, ? extends R> secondary,
               Function<? super RT, ? extends R> primary);
     ...
     */
    @Override
    default LazyEither<LT, RT> bipeek(Consumer<? super LT> c1,
                                      Consumer<? super RT> c2) {

        return (LazyEither<LT, RT>) Either.super.bipeek(c1,
                                                        c2);
    }
    ...
    <RT1> LazyEither<LT, RT1> flatMap(Function<? super RT, ? extends Either<? extends LT, ? extends RT1>> mapper);
    ...
    /**
     * Accumulate secondarys into a LazySeq (extended Persistent List) and Right with the supplied combiner function Right
     * accumulation only occurs if all phases are right
     *
    ...
    default <T2, R> LazyEither<LT, R> combine(final LazyEither<? extends LT, ? extends T2> app,
                                              ...
                                              final BiFunction<? super RT, ? super T2, ? extends R> fn) {
        return this.fold(secondary -> app.fold(s2 -> LazyEither.left(semigroup.apply(s2,
                                                                                     secondary)),
                                               p2 -> LazyEither.left(secondary)),
                         primary -> app.fold(s2 -> LazyEither.left(s2),
                                             p2 -> LazyEither.right(fn.apply(primary,
                                                                             p2))));
    }

    @AllArgsConstructor
    class CompletableEither<ORG, RT> implements LazyEither<Throwable, RT>, Completable<ORG> {
        ...
        @Override
        public <R> R fold(Function<? super java.lang.Throwable, ? extends R> secondary,
                          Function<? super RT, ? extends R> primary) {
            return either.fold(secondary,
                               primary);
        }
        ...
        public <R> R fold(Function<? super RT, ? extends R> present,
            ...
            return either.fold(i -> absent.get(),
                               present);
        }
    ...
    final class Lazy<ST, PT> implements LazyEither<ST, PT> {
        ...
        public LazyEither<ST, PT> recover(Supplier<? extends PT> value) {
            return new Lazy<ST, PT>(lazy.map(m -> m.recover(value)));
        }
        ...
        public LazyEither<ST, PT> recoverWith(Supplier<? extends Either<ST, PT>> fn) {
            return new Lazy<ST, PT>(lazy.map(m -> m.recoverWith(fn)));
        ...
        private <PT> LazyEither<ST, PT> toEither(MonadicValue<? extends PT> value) {
            return value.fold(p -> LazyEither.right(p),
                              () -> LazyEither.left(null));
        }
        ...
        @Override
        public LazyEither<ST, PT> filter(Predicate<? super PT> test,
                                         Function<? super PT, ? extends ST> rightToLeft) {
            return LazyEither.fromLazy(lazy.map(m -> m.filter(test,
                                                              rightToLeft)));
        }
        ...
        @Override
        public <R> R fold(Function<? super ST, ? extends R> secondary,
                          Function<? super PT, ? extends R> primary) {
            return trampoline().fold(secondary,
                                     primary);
        }
        ...
        public ReactiveSeq<ST> leftToStream() {
            return ReactiveSeq.generate(() -> trampoline().leftToStream())
        ...
        @Override
        public <R> R fold(Function<? super PT, ? extends R> present,
                          Supplier<? extends R> absent) {
    ...
    @AllArgsConstructor(access = AccessLevel.PRIVATE)
    class Right<ST, PT> implements LazyEither<ST, PT> {
        ...
        public <R> LazyEither<ST, R> map(final Function<? super PT, ? extends R> fn) {
            return new LazyEither.Right(value.map(fn));
        }
        ...
        @Override
        public LazyEither<PT, ST> swap() {
            return new LazyEither.Left(value);
        }
        ...
        @Override
        public <LT1> LazyEither<LT1, PT> flatMapLeft(final Function<? super ST, ? extends Either<LT1, PT>> mapper) {
        ...
        public boolean equals(Object obj) {
            ...
            if (obj instanceof Either.Right) {
                return Objects.equals(value.get(),
                                      ((Either.Right) obj).orElse(null));
    ...
    @AllArgsConstructor(access = AccessLevel.PRIVATE)
    class Left<ST, PT> implements LazyEither<ST, PT> {
        ...
        @Override
        public <R> LazyEither<R, PT> mapLeft(final Function<? super ST, ? extends R> fn) {
            return new LazyEither.Left(value.map(fn));
        }
        ...
        @Override
        public LazyEither<PT, ST> swap() {
            return new LazyEither.Right(value);
        }
        ...
        @Override
        public LazyEither<ST, PT> flatMapLeftToRight(final Function<? super ST, ? extends Either<ST, PT>> fn) {
            return new Lazy<ST, PT>(Eval.now(this)).flatMapLeftToRight(fn);
        }
        ...
        public boolean equals(Object obj) {
            ...
            if (obj instanceof Either.Left) {
                return Objects.equals(value.get(),
                                      ((Either.Left) obj).leftOrElse(null));
            }
            if (getClass() != obj.getClass()) {
                return false;
            }
