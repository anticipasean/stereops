 ...
 * <p>
 * An Either or Union type, but right biased. Primary and Secondary are used instead of Right & Left. 'Right' (or right type)
 * biased disjunct union. No 'projections' are provided, swap() and secondaryXXXX alternative methods can be used instead.
 * <p>
 * <p>
...
public interface Ior<LT, RT> extends To<Ior<LT, RT>>, Value<RT>, OrElseValue<RT, Ior<LT, RT>>, Unit<RT>, Transformable<RT>,
     ...
     * transform method operates on the right type but does nothing on left Iors
     *
     * <pre>
     * {@code
    ...
    /**
     * Accumulate the result of the Secondary types in the Collection of Iors provided using the supplied Reducer  {@see
     * cyclops2.Reducers}.
     *
     * <pre>
     * {@code
     ...
     * convert the data from each Ior before reducing them using the supplied Monoid (a combining BiFunction/BinaryOperator and
    ...
    /**
     * Accumulate the results only from those Iors which have a Secondary type present, using the supplied Monoid (a combining
    ...
    static <L, T> Ior<L, ReactiveSeq<T>> sequence(ReactiveSeq<? extends Ior<L, T>> stream) {
        ...
        BiFunction<Ior<L, ReactiveSeq<T>>, Ior<L, T>, Ior<L, ReactiveSeq<T>>> combineToStream = (acc, next) -> acc.zip(next,
                                                                                                                       (a, b) -> a.append(b));

        BinaryOperator<Ior<L, ReactiveSeq<T>>> combineStreams = (a, b) -> a.zip(b,
    ...
    static <L, T> Ior<ReactiveSeq<L>, ReactiveSeq<T>> sequenceBoth(ReactiveSeq<? extends Ior<L, T>> stream) {
        ...
        BinaryOperator<Ior<ReactiveSeq<L>, ReactiveSeq<T>>> combineStreams = (a, b) -> a.zip(b,
                                                                                             (z1, z2) -> z1.appendStream(z2));
    ...
    /**
     * Accumulate the result of the Primary types in the Collection of Iors provided using the supplied Reducer  {@see
     * cyclops2.Reducers}.
     *
     * <pre>
     * {@code
     ...
     * convert the data from each Ior before reducing them using the supplied Semgigroup (a combining BiFunction/BinaryOperator
     * that takes two input values of the same type and returns the combined result) {@see cyclops2.SemigroupK }.
     *
     * <pre>
     * {@code
     *  Ior<String,Integer> just  = Ior.right(10);
    ...
    /**
     * Accumulate the results only from those Iors which have a Primary type present, using the supplied  Semgigroup (a combining
     * BiFunction/BinaryOperator that takes two input values of the same type and returns the combined result) {@see
     * cyclops2.SemigroupK }.
     *
     * <pre>
     * {@code
    ...
    default <T2, R1, R2, R3, R> Ior<LT, R> forEach4(Function<? super RT, ? extends Ior<LT, R1>> value1,
        ...
        return this.flatMap(in -> {
            ...
            return a.flatMap(ina -> {
                Ior<LT, R2> b = value2.apply(in,
                                             ina);
                return b.flatMap(inb -> {
                    Ior<LT, R3> c = value3.apply(in,
                                                 ina,
                                                 inb);
    ...
    /**
     * @return An zero Option if this Ior only has lazy the Secondary or Primary type. Or an Optional containing a Tuple2 with
     * both the Secondary and Primary types if they are both present.
     */
    ...
    <R1, R2> Ior<R1, R2> bimap(final Function<? super LT, ? extends R1> fn1,
                               final Function<? super RT, ? extends R2> fn2);

    /**
     * Visitor pattern for this Ior. Execute the left function if this Ior contains an element of the left type only Execute the
     * right function if this Ior contains an element of the right type only Execute the both function if this Ior contains an
     * element of both type
     *
     * <pre>
     * {@code
     *  Ior.right(10)
     ...
     */
    <R> R fold(final Function<? super LT, ? extends R> secondary,
     ...
     * @return Ior containing the value inside the result of the transformation function as the Secondary value, if the Secondary
     * type was present
     */
    ...
    default Option<RT> filterNot(final Predicate<? super RT> fn) {
        ...
        return (Option<RT>) Filterable.super.filterNot(fn);
    }

    @Override
    default Option<RT> notNull() {

        return (Option<RT>) Filterable.super.notNull();
    }
     ...
     */
    @Override
    default Ior<LT, RT> bipeek(final Consumer<? super LT> c1,
                               final Consumer<? super RT> c2) {

        return (Ior<LT, RT>) BiTransformable.super.bipeek(c1,
                                                          c2);
    }

    default <T2, R> Ior<LT, R> zip(final Ior<LT, ? extends T2> app,
                                   final BiFunction<? super RT, ? super T2, ? extends R> fn) {
        return flatMap(t -> app.map(t2 -> fn.apply(t,
                                                   t2)));
    }

    default <T2, R> Ior<LT, R> zip(final Either<LT, ? extends T2> app,
                                   final BiFunction<? super RT, ? super T2, ? extends R> fn) {
        return flatMap(t -> app.map(t2 -> fn.apply(t,
                                                   t2))
                               .toIor());
    ...
    class Primary<ST, PT> implements Ior<ST, PT> {
        ...
        @Override
        public Ior<ST, PT> bipeek(final Consumer<? super ST> stAction,
                                  final Consumer<? super PT> ptAction) {
    ...
    class Both<ST, PT> implements Ior<ST, PT> {
        ...
        private static <ST, PT> Ior<ST, PT> both(final ST secondary,
                                                 final PT primary) {
            return new Both<ST, PT>(secondary,
                                    primary);
        }
        ...
        @Override
        public <R> Ior<R, PT> mapLeft(final Function<? super ST, ? extends R> fn) {
            return Both.both(fn.apply(secondary),
                             primary);
        }
        ...
        public <R> R fold(final Function<? super ST, ? extends R> secondary,
                          final Function<? super PT, ? extends R> primary,
                          final BiFunction<? super ST, ? super PT, ? extends R> both) {
            return both.apply(this.secondary,
                              this.primary);
        }

        @Override
        public Option<Tuple2<ST, PT>> both() {
            return Option.some(Tuple.tuple(secondary,
                                           primary));
        }
        ...
        @Override
        public Ior<ST, PT> bipeek(final Consumer<? super ST> actionA,
                                  final Consumer<? super PT> actionB) {
        ...
        public <R1, R2> Ior<R1, R2> bimap(final Function<? super ST, ? extends R1> fn1,
                                          final Function<? super PT, ? extends R2> fn2) {
            return Both.both(fn1.apply(secondary),
                             fn2.apply(primary));
        }
