...
public interface Maybe<T> extends Option<T> {
    ...
    static <T, R> Maybe<R> tailRec(T initial,
                                   Function<? super T, ? extends Maybe<? extends Either<T, R>>> fn) {
        return narrowK(fn.apply(initial)).flatMap(eval -> eval.fold(s -> tailRec(s,
                                                                                 fn),
                                                                    p -> Maybe.just(p)));
    }

    static <T> Higher<option, T> widen(Maybe<T> narrow) {
        return narrow;
    }

    static <T> Maybe<T> async(final Executor ex,
                              final Supplier<T> s) {
    ...
    static <T> CompletableMaybe<T, T> maybe() {
        ...
        return new Maybe.CompletableMaybe<T, T>(c,
    ...
    static <T, R> Function<? super T, ? extends Maybe<R>> arrow(Function<? super T, ? extends R> fn) {
        return in -> Maybe.ofNullable(fn.apply(in));
    }

    static <C2, T> Higher<C2, Higher<option, T>> widen2(Higher<C2, Maybe<T>> nestedMaybe) {
     ...
     * @return MaybeType
     */
    static <T> Maybe<T> narrowK(final Higher<option, T> future) {
        return Maybe.fromOption(Option.narrowK(future));
    }

    static <T> Maybe<T> fromOption(Option<T> opt) {
     ...
     */
    static <T> Optional<T> narrowOptional(final Higher<option, T> maybe) {
     ...
     */
    static <T> Maybe<T> fromPublisher(final Publisher<T> pub) {
    ...
    static <T> Maybe<T> fromEvalNullable(final Eval<T> eval) {
        return new Lazy<T>(eval.map(u -> Maybe.ofNullable(u)));
    }

    static <T> Maybe<T> fromEvalOptional(final Eval<Optional<T>> value) {
        return new Lazy<T>(value.map(in -> Maybe.fromOptional(in)));
    }

    static <T> Maybe<T> fromLazyOption(final Supplier<Option<T>> value) {
        return new Lazy<T>(Eval.later(value)
                               .map(in -> Maybe.fromOption(in)));
    }

    /**
     * Construct an Maybe which contains the provided (non-null) value. Alias for @see {@link Maybe#of(Object)}
     *
     * <pre>
     * {@code
     *
    ...
    /**
     * Construct an Maybe which contains the provided (non-null) value Equivalent to @see {@link Maybe#just(Object)}
     * <pre>
     * {@code
     *
    ...
    /**
     * Sequence operation, take a Collection of Maybes and turn it into a Maybe with a Collection Only successes are retained. By
     * constrast with {@link Maybe#sequence(Iterable)} Maybe#zero/ None types are tolerated and ignored.
     *
     * <pre>
     * {@code
     ...
     */
    static <T> Maybe<ReactiveSeq<T>> sequenceJust(final Iterable<? extends Maybe<T>> maybes) {
        return sequence(ReactiveSeq.fromIterable(maybes)
                                   .filter(Maybe::isPresent)
                                   .stream());
    }

    static <T> Maybe<ReactiveSeq<T>> sequence(ReactiveSeq<? extends Maybe<T>> stream) {

        Maybe<ReactiveSeq<T>> identity = Maybe.just(ReactiveSeq.empty());

        BiFunction<Maybe<ReactiveSeq<T>>, Maybe<T>, Maybe<ReactiveSeq<T>>> combineToStream = (acc, next) -> acc.zip(next,
                                                                                                                    (a, b) -> a.append(b));

        BinaryOperator<Maybe<ReactiveSeq<T>>> combineStreams = (a, b) -> a.zip(b,
    ...
    static <T, R> Maybe<ReactiveSeq<R>> traverse(Function<? super T, ? extends R> fn,
    ...
    /**
     * Sequence operation, take a Collection of Maybes and turn it into a Maybe with a Collection By constrast with {@link
     * Maybe#sequenceJust(Iterable)} if any Maybe types are None / zero the return type will be an zero Maybe / None
     *
     * <pre>
     * {@code
     *
     ...
     * @return Maybe with a List of values
     */
    static <T> Maybe<ReactiveSeq<T>> sequence(final Iterable<? extends Maybe<T>> maybes) {
        return sequence(ReactiveSeq.fromIterable(maybes));

    }

    /**
     * Sequence operation, take a Stream of Maybes and turn it into a Maybe with a Stream By constrast with {@link
     * Maybe#sequenceJust(Iterable)} Maybe#zero/ None types are result in the returned Maybe being Maybe.zero / None
     *
     *
     * <pre>
     * {@code
     ...
     */
    static <T> Maybe<ReactiveSeq<T>> sequence(final Stream<? extends Maybe<T>> maybes) {
    ...
    /**
     * Accummulating operation using the supplied Reducer (@see cyclops.Reducers). A typical use case is to accumulate into a
     ...
     */
    static <T, R> Maybe<R> accumulateJust(final Iterable<Maybe<T>> maybes,
     ...
     * @return Maybe with reduced value
     */
    static <T> Maybe<T> accumulateJust(final Monoid<T> reducer,
                                       final Iterable<Maybe<T>> maybes) {
    ...
    default Option<T> eager() {
        return fold(s -> Option.some(s),
                    () -> Option.none());
    }

    @Override
    default <U> Maybe<Tuple2<T, U>> zipWithPublisher(final Publisher<? extends U> other) {
        return mergeMap(a -> Maybe.fromPublisher(other)
                                  .map(b -> Tuple.tuple(a,
                                                        b)));
    }

    @Override
    default <S, U> Maybe<Tuple3<T, S, U>> zip3(final Iterable<? extends S> second,
                                               final Iterable<? extends U> third) {
        return (Maybe) Option.super.zip3(second,
                                         third);
    }

    @Override
    default <S, U, R> Maybe<R> zip3(final Iterable<? extends S> second,
                                    final Iterable<? extends U> third,
                                    final Function3<? super T, ? super S, ? super U, ? extends R> fn3) {
        return (Maybe<R>) Option.super.zip3(second,
                                            third,
                                            fn3);
    }

    @Override
    default <T2, T3, T4> Maybe<Tuple4<T, T2, T3, T4>> zip4(final Iterable<? extends T2> second,
                                                           final Iterable<? extends T3> third,
                                                           final Iterable<? extends T4> fourth) {
        return (Maybe) Option.super.zip4(second,
                                         third,
                                         fourth);
    }

    @Override
    default <T2, T3, T4, R> Maybe<R> zip4(final Iterable<? extends T2> second,
                                          final Iterable<? extends T3> third,
                                          final Iterable<? extends T4> fourth,
                                          final Function4<? super T, ? super T2, ? super T3, ? super T4, ? extends R> fn) {
        return (Maybe<R>) Option.super.zip4(second,
                                            third,
                                            fourth,
                                            fn);
    }
    ...
    default <T2, R1, R2, R3, R> Maybe<R> forEach4(Function<? super T, ? extends MonadicValue<R1>> value1,
        ...
        return (Maybe<R>) Option.super.forEach4(value1,
                                                value2,
                                                value3,
                                                yieldingFunction);
    }
    ...
    default <T2, R1, R2, R> Maybe<R> forEach3(Function<? super T, ? extends MonadicValue<R1>> value1,
        ...
        return (Maybe<R>) Option.super.forEach3(value1,
                                                value2,
                                                yieldingFunction);
    }
    ...
    default <R1, R> Maybe<R> forEach2(Function<? super T, ? extends MonadicValue<R1>> value1,
        ...
        return (Maybe<R>) Option.super.forEach2(value1,
                                                yieldingFunction);
    }
    ...
    @Override
    default <T2, R> Maybe<R> zip(final Iterable<? extends T2> app,
                                 final BiFunction<? super T, ? super T2, ? extends R> fn) {

        return flatMap(a -> Maybe.fromIterable(app)
                                 .map(b -> fn.apply(a,
                                                    b)));
    }

    @Override
    default <T2, R> Maybe<R> zip(final BiFunction<? super T, ? super T2, ? extends R> fn,
                                 final Publisher<? extends T2> app) {
        return mergeMap(a -> Maybe.fromPublisher(app)
                                  .map(b -> fn.apply(a,
                                                     b)));
    }
    ...
    @AllArgsConstructor
    class CompletableMaybe<ORG, T2> implements Maybe<T2>, Completable<ORG> {
        ...
        public static <T, F, R1, R2, R3, R4, R5, R6, R7> Maybe<R7> forEach(Maybe<T> free,
            ...
            return free.flatMap(in -> {
                ...
                return a.flatMap(ina -> {
                    Maybe<R2> b = value3.apply(Tuple.tuple(in,
                                                           ina));
                    return b.flatMap(inb -> {

                        Maybe<R3> c = value4.apply(Tuple.tuple(in,
                                                               ina,
                                                               inb));

                        return c.flatMap(inc -> {
                            Maybe<R4> d = value5.apply(Tuple.tuple(in,
                                                                   ina,
                                                                   inb,
                                                                   inc));
                            return d.flatMap(ind -> {
                                Maybe<R5> e = value6.apply(Tuple.tuple(in,
                                                                       ina,
                                                                       inb,
                                                                       inc,
                                                                       ind));
                                return e.flatMap(ine -> {
                                    Maybe<R6> f = value7.apply(Tuple.tuple(in,
                                                                           ina,
                                                                           inb,
                                                                           inc,
                                                                           ind,
                                    ...
                                    return f.flatMap(inf -> {
                                        Maybe<R7> g = value8.apply(Tuple.tuple(in,
                                                                               ina,
                                                                               inb,
                                                                               inc,
                                                                               ind,
