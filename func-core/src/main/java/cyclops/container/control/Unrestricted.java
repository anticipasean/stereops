...
public abstract class Unrestricted<T> {
    ...
    public static final <F, R1, R2, X1, X2> Tuple2<Either<X1, R1>, Either<X2, R2>> product(Unrestricted<R1> free1,
                                                                                           Function<Transformable<Unrestricted<R1>>, X1> decoder1,
                                                                                           Unrestricted<R2> free2,
                                                                                           Function<Transformable<Unrestricted<R2>>, X2> decoder2) {

        return Tuple.tuple(free1.resume(decoder1),
                           free2.resume(decoder2));

    }

    /**
     * Perform a For Comprehension over a Unrestricted, accepting 3 generating function. This results in a four level nested
     * internal iteration over the provided Computationss.
     *
    ...
    public <T2, T3, R1, R2, R3> Unrestricted<R3> forEach4(Function<? super T, ? extends Unrestricted<R1>> value2,
        ...
        return this.flatMap(in -> {
            ...
            return a.flatMap(ina -> {
                Unrestricted<R2> b = value3.apply(in,
                                                  ina);
                return b.flatMap(inb -> {
                    Unrestricted<R3> c = value4.apply(in,
                                                      ina,
                                                      inb);
                    return c;
                });
    ...
    /**
     * Perform a For Comprehension over a Unrestricted, accepting 2 generating function. This results in a three level nested
     * internal iteration over the provided Computationss.
     *
     * @param value1 top level Unrestricted
     * @param value2 Nested Unrestricted
    ...
    /**
     * Perform a For Comprehension over a Unrestricted, accepting a generating function. This results in a two level nested
     * internal iteration over the provided Computationss.
     *
     * @param value2 Nested Unrestricted
     * @return Resulting Unrestricted
    ...
    public <B, R> Unrestricted<R> zip(Unrestricted<B> b,
        ...
        if (first.isLeft() && second.isLeft()) {
            return suspend(first.leftOrElse(null)
                                .map(a1 -> {
                                    return suspend(second.leftOrElse(null)
                                                         .map(b1 -> a1.zip(b1,
                                                                           zipper)));
                                }));
        }
    ...
    public <B, C> Unrestricted<Tuple3<T, B, C>> zip(Unrestricted<B> b,
                                                    Unrestricted<C> c) {
        return zip(b,
                   c,
                   (x, y, z) -> Tuple.tuple(x,
    ...
    public <B, C, R> Unrestricted<R> zip(Unrestricted<B> b,
                                         Unrestricted<C> c,
        ...
        if (first.isLeft() && second.isLeft() && third.isLeft()) {
            return suspend(first.leftOrElse(null)
                                .map(a1 -> {
                                    return suspend(second.leftOrElse(null)
                                                         .map(b1 -> {
                                                             return suspend(third.leftOrElse(null)
                                                                                 .map(c1 -> a1.zip(b1,
                                                                                                   c1,
                                                                                                   fn)));
                                                         }));
    ...
    public final Either<Transformable<Unrestricted<T>>, T> resume() {
        return resumeInternal().fold(Either::left,
                                     Either::right,
                                     t -> null);
    ...
    public static class Comprehensions {
        ...
        public static <T, F, R1, R2, R3, R4, R5, R6, R7> Unrestricted<R7> forEach(Unrestricted<T> free,
            ...
            return free.flatMap(in -> {
                ...
                Unrestricted<R1> a = value2.apply(in);
                return a.flatMap(ina -> {
                    Unrestricted<R2> b = value3.apply(Tuple.tuple(in,
                                                                  ina));
                    return b.flatMap(inb -> {

                        Unrestricted<R3> c = value4.apply(Tuple.tuple(in,
                                                                      ina,
                                                                      inb));

                        return c.flatMap(inc -> {
                            Unrestricted<R4> d = value5.apply(Tuple.tuple(in,
                                                                          ina,
                                                                          inb,
                                                                          inc));
                            return d.flatMap(ind -> {
                                Unrestricted<R5> e = value6.apply(Tuple.tuple(in,
                                                                              ina,
                                                                              inb,
                                                                              inc,
                                                                              ind));
                                return e.flatMap(ine -> {
                                    Unrestricted<R6> f = value7.apply(Tuple.tuple(in,
                                                                                  ina,
                                                                                  inb,
                                                                                  inc,
                                                                                  ind,
                                    ...
                                    return f.flatMap(inf -> {
                                        Unrestricted<R7> g = value8.apply(Tuple.tuple(in,
                                                                                      ina,
                                                                                      inb,
                                                                                      inc,
                                                                                      ind,
    ...
    static class FreeF<T> {
        ...
        public final <R1, R2, X1, X2> Tuple2<Either<X1, R1>, Either<X2, R2>> product(Unrestricted<R1> free1,
                                                                                     Unrestricted<R2> free2) {

            return Tuple.tuple(free1.resume(decoder()),
                               free2.resume(decoder()));
    ...
    private static final class FlatMapped<IN, T> extends Unrestricted<T> {
        ...
        private FlatMapped(final Unrestricted<IN> free,
                           final Function<? super IN, ? extends Unrestricted<? extends T>> fn) {
            this.free = free;
            this.fn = fn;
        }
        ...
        public <R> Unrestricted<R> flatMap(final Function<? super T, ? extends Unrestricted<? extends R>> g) {
            return new FlatMapped<IN, R>(free,
                                         aa -> new FlatMapped<T, R>(narrowFn().apply(aa),
                                                                    g));
        }
