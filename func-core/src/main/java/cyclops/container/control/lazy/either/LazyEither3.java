 ...
 * @author johnmcclean
 */
public interface LazyEither3<LT1, LT2, RT> extends Value<RT>, OrElseValue<RT, LazyEither3<LT1, LT2, RT>>, Unit<RT>,
                                                   Transformable<RT>, Filterable<RT>, BiTransformable<LT2, RT>,
                                                   To<LazyEither3<LT1, LT2, RT>>, Sealed3<LT1, LT2, RT>,
                                                   Higher3<lazyEither3, LT1, LT2, RT> {


    static <LT1, LT2, T> Higher<Higher<Higher<lazyEither3, LT1>, LT2>, T> widen(LazyEither3<LT1, LT2, T> narrow) {
        return narrow;
    }
    ...
    static <LT2, RT> LazyEither3.CompletableEither3<RT, LT2, RT> either3() {
        ...
        return new LazyEither3.CompletableEither3<RT, LT2, RT>(c,
    ...
    static <LT2, RT> LazyEither3<Throwable, LT2, RT> fromFuture(Future<RT> future) {
        return fromLazy(Eval.fromFuture(future.<LazyEither3<Throwable, LT2, RT>>map(LazyEither3::right).recover(e -> LazyEither3.left1(e.getCause()))));
     ...
     */
    static <LT1, LT2, PT> LazyEither3<LT1, LT2, ReactiveSeq<PT>> sequence(final Iterable<? extends LazyEither3<LT1, LT2, PT>> xors) {
        Objects.requireNonNull(xors);
        return sequence(ReactiveSeq.fromIterable(xors)
                                   .filter(LazyEither3::isRight));

    }

    static <L1, L2, T> LazyEither3<L1, L2, ReactiveSeq<T>> sequence(ReactiveSeq<? extends LazyEither3<L1, L2, T>> stream) {

        LazyEither3<L1, L2, ReactiveSeq<T>> identity = right(ReactiveSeq.empty());

        BiFunction<LazyEither3<L1, L2, ReactiveSeq<T>>, LazyEither3<L1, L2, T>, LazyEither3<L1, L2, ReactiveSeq<T>>> combineToStream = (acc, next) -> acc.zip(next,
                                                                                                                                                              (a, b) -> a.append(b));

        BinaryOperator<LazyEither3<L1, L2, ReactiveSeq<T>>> combineStreams = (a, b) -> a.zip(b,
    ...
    static <L1, L2, T, R> LazyEither3<L1, L2, ReactiveSeq<R>> traverse(Function<? super T, ? extends R> fn,
                                                                       ReactiveSeq<LazyEither3<L1, L2, T>> stream) {
        return sequence(stream.map(h -> h.map(fn)));
    }

    /**
     * Traverse a Collection of Either3 producing an Either3 with a Seq, applying the transformation function to every element in
     * the list
     *
     ...
     */
    static <LT1, LT2, PT, R> LazyEither3<LT1, LT2, ReactiveSeq<R>> traverse(final Iterable<LazyEither3<LT1, LT2, PT>> xors,
     ...
     */
    static <LT1, LT2, RT> LazyEither3<LT1, LT2, RT> accumulate(final Monoid<RT> reducer,
                                                               final Iterable<LazyEither3<LT1, LT2, RT>> xors) {
     ...
     */
    static <T1, T> LazyEither3<Throwable, T1, T> fromPublisher(final Publisher<T> pub) {
     ...
     */
    static <ST, T, RT> LazyEither3<ST, T, RT> fromIterable(final Iterable<RT> iterable) {
    ...
    static <X, LT extends X, M extends X, RT extends X, R> R visitAny(LazyEither3<LT, M, RT> either,
                                                                      Function<? super X, ? extends R> fn) {
        return foldAny(either,
                       fn);
    }

    static <X, LT extends X, M extends X, RT extends X, R> R foldAny(LazyEither3<LT, M, RT> either,
                                                                     Function<? super X, ? extends R> fn) {
    ...
    static <X, LT extends X, M extends X, RT extends X> X visitAny(Consumer<? super X> c,
     ...
     */
    static <LT, B, RT> LazyEither3<LT, B, RT> rightEval(final Eval<RT> right) {
     ...
     */
    static <LT, B, RT> LazyEither3<LT, B, RT> left1Eval(final Eval<LT> left) {
     ...
     * @return Either3 Right instance
     */
    static <LT, B, RT> LazyEither3<LT, B, RT> right(final RT right) {
     ...
     */
    static <LT, B, RT> LazyEither3<LT, B, RT> left1(final LT left) {
     ...
     */
    static <LT, B, RT> LazyEither3<LT, B, RT> left2(final B middle) {
     ...
     */
    static <LT, B, RT> LazyEither3<LT, B, RT> left2Eval(final Eval<B> middle) {
        return new Left2<>(middle);
    }

    static <L1, L2, T> LazyEither3<L1, L2, T> narrowK3(final Higher3<lazyEither3, L1, L2, T> xor) {
        return (LazyEither3<L1, L2, T>) xor;
    }

    static <L1, L2, T> LazyEither3<L1, L2, T> narrowK(final Higher<Higher<Higher<lazyEither3, L1>, L2>, T> xor) {
        return (LazyEither3<L1, L2, T>) xor;
    }

    default LazyEither3<LT1, LT2, RT> filter(Predicate<? super RT> test,
    ...
    default <T2, R> LazyEither3<LT1, LT2, R> zip(final LazyEither3<LT1, LT2, ? extends T2> app,
                                                 final BiFunction<? super RT, ? super T2, ? extends R> fn) {
        return flatMap(t -> app.map(t2 -> fn.apply(t,
                                                   t2)));
    }

    default <T2, R> LazyEither3<LT1, LT2, R> zip(final Publisher<? extends T2> app,
                                                 final BiFunction<? super RT, ? super T2, ? extends R> fn) {
        return zip(LazyEither.fromPublisher(app),
                   fn);
    }
     ...
     */
    <R> R fold(final Function<? super LT1, ? extends R> left1,
               final Function<? super LT2, ? extends R> mid,
               final Function<? super RT, ? extends R> right);

    /**
     * Filter this Either3 resulting in a Maybe#none if it is not a Right instance or if the predicate does not hold. Otherwise
    ...
    /**
     * Flattening transformation on this Either3. Contains an internal trampoline so will convert tail-recursive calls to
     * iteration.
     *
    ...
    default <T2, R1, R2, R3, R> LazyEither3<LT1, LT2, R> forEach4(Function<? super RT, ? extends LazyEither3<LT1, LT2, R1>> value1,
        ...
        return this.flatMap(in -> {
            ...
            return a.flatMap(ina -> {
                LazyEither3<LT1, LT2, R2> b = value2.apply(in,
                                                           ina);
                return b.flatMap(inb -> {
                    LazyEither3<LT1, LT2, R3> c = value3.apply(in,
                                                               ina,
                                                               inb);
     ...
     */
    @Override
    <R1, R2> LazyEither3<LT1, R1, R2> bimap(Function<? super LT2, ? extends R1> fn1,
     ...
     */
    @Override
    default LazyEither3<LT1, LT2, RT> bipeek(final Consumer<? super LT2> c1,
                                             final Consumer<? super RT> c2) {

        return (LazyEither3<LT1, LT2, RT>) BiTransformable.super.bipeek(c1,
                                                                        c2);
    }
    ...
    @AllArgsConstructor
    class CompletableEither3<ORG, LT1, RT> implements LazyEither3<Throwable, LT1, RT>, Completable<ORG> {
        ...
        public <R> R fold(Function<? super Throwable, ? extends R> left1,
                          Function<? super LT1, ? extends R> mid,
        ...
        @Override
        public <R1, R2> LazyEither3<Throwable, R1, R2> bimap(Function<? super LT1, ? extends R1> fn1,
                                                             Function<? super RT, ? extends R2> fn2) {
    ...
    final class Lazy<ST, M, PT> implements LazyEither3<ST, M, PT> {
        ...
        @Override
        public <RT1> LazyEither3<ST, M, RT1> flatMap(final Function<? super PT, ? extends LazyEither3<ST, M, ? extends RT1>> mapper) {
        ...
        public LazyEither3<ST, M, PT> recoverWith(Supplier<? extends LazyEither3<ST, M, PT>> fn) {
            return new Lazy(lazy.map(m -> m.recoverWith(fn)));
        }
        ...
        public <R1, R2> LazyEither3<ST, R1, R2> bimap(final Function<? super M, ? extends R1> fn1,
            ...
            return LazyEither3.fromLazy(lazy.map(m -> m.bimap(fn1,
                                                              fn2)));
        }
        ...
        @Override
        public int hashCode() {
            return this.fold(LazyEither3::left1,
                             LazyEither3::left2,
                             LazyEither3::right)
                       .hashCode();
        }
    ...
    @AllArgsConstructor(access = AccessLevel.PRIVATE)
    class Left1<ST, M, PT> implements LazyEither3<ST, M, PT> {
    ...
    @AllArgsConstructor(access = AccessLevel.PRIVATE)
    class Left2<ST, M, PT> implements LazyEither3<ST, M, PT> {
