 ...
 */
public class LazyMappingSpliterator<T, R> extends Spliterators.AbstractSpliterator<R> implements CopyableSpliterator<R>,
    ...
    public LazyMappingSpliterator(final Spliterator<T> source,
                                  Supplier<Function<? super T, ? extends R>> mapper) {
    ...
    public <R2> LazyMappingSpliterator<T, ?> compose(Function<? super R, ? extends R2> fn) {
        return new LazyMappingSpliterator<T, R2>(CopyableSpliterator.copy(source),
                                                 () -> mapperSupplier.get()
                                                                     .andThen(fn));
    }
    ...
    public Spliterator<R> copy() {
        return new LazyMappingSpliterator<T, R>(CopyableSpliterator.copy(source),
                                                mapperSupplier);
    }
