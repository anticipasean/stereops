 ...
 */
public class ArrayConcatonatingSpliterator<IN, T> extends
                                                  BaseComposableSpliterator<IN, T, ArrayConcatonatingSpliterator<IN, ?>> implements
    ...
    public ArrayConcatonatingSpliterator(Spliterator<IN>... spliterators) {
        super(size(spliterators),
              calcCharacteristics(spliterators),
              null);
        this.spliterators = spliterators;
    }

    public ArrayConcatonatingSpliterator(Function<? super IN, ? extends T> fn,
                                         Spliterator<IN>... spliterators) {
        super(size(spliterators),
              calcCharacteristics(spliterators),
              fn);
        this.spliterators = spliterators;
    ...
    public Spliterator<T> copy() {
        return new ArrayConcatonatingSpliterator<IN, T>(fn,
                                                        CopyableSpliterator.copy(spliterators));
    }
    ...
    <R2> ArrayConcatonatingSpliterator<IN, ?> create(Function<? super IN, ? extends R2> after) {
        return new ArrayConcatonatingSpliterator(after,
                                                 spliterators);
    }
}
