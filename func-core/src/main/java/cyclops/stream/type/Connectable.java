...
public interface Connectable<T> {
    ...
    /**
     * Connect to this Connectable using the provided transfer async.Queue. The transfer Queue can be used to applyHKT
     * backpressure to the Connectable if it produces data faster than the connected Stream can consume it {@see
     * cyclops2.async.wait.WaitStrategy}
     *
     * <pre>
     * {@code
     ...
     * to the Connectable if it produces data faster than the connected Stream can consume it {@see
     * cyclops2.async.wait.WaitStrategy} Convert the emitted Stream to the required type with the provided function
     *
     * @param queue Transfer Queue between the Streams
