...
public class TrieSetTest extends BaseImmutableSetTest {
    ...
    @Override
    public ImmutableSet<Integer> range(int start,
                                       int end) {
        return TrieSet.range(start,
                             end);
    }

    @Override
    public ImmutableSet<Long> rangeLong(long start,
                                        long end) {
        return TrieSet.rangeLong(start,
                                 end);
    }

    @Override
    public <T> ImmutableSet<T> iterate(int times,
                                       T seed,
                                       UnaryOperator<T> fn) {
        return TrieSet.iterate(seed,
                               fn,
                               times);
    }
    ...
    @Override
    public <T> ImmutableSet<T> generate(int times,
                                        Supplier<T> fn) {
        return TrieSet.generate(fn,
                                times);
    }

    @Override
    public <U, T> ImmutableSet<T> unfold(U seed,
                                         Function<? super U, Option<Tuple2<T, U>>> unfolder) {
        return TrieSet.unfold(seed,
                              unfolder);
    }
