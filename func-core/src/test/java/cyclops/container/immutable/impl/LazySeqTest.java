...
public class LazySeqTest extends BaseImmutableListTest {
    ...
    public void testLazy() {
        ...
        of(1,
           2,
           3,
           4).concatMap(i -> LazySeq.of(count++));
        assertThat(count,
                   equalTo(0));
        count = 0;
        of(1,
    ...
    public void testNoMapCalledBeforeFetchingAndOnlyOnceForEachInput() throws Exception {
        @SuppressWarnings("unchecked") Function<Integer, Integer> mockFunction = Mockito.mock(Function.class);
        ...
        LazySeq<Integer> result = LazySeq.of(1,
                                             2,
                                             3,
                                             4,
                                             5)
    ...
    public void splitLarge() {
        fromStream(ReactiveSeq.range(0,
                                     100_000)).split(i -> i == 2)
                                              .printOut();
        fromStream(ReactiveSeq.range(0,
                                     15_000)
                              .intersperse(0)).split(i -> i == 0)
                                              .printOut();
    }
    ...
    public void inifiniteFoldRight() {
        ...
        assertTrue(LazySeq.generate(() -> true)
                          .lazyFoldRight(false,
                                         (a, b) -> a ? true : b.get()));
    ...
    @Override
    public LazySeq<Integer> range(int start,
                                  int end) {
        return LazySeq.range(start,
                             end);
    }

    @Override
    public LazySeq<Long> rangeLong(long start,
                                   long end) {
        return LazySeq.rangeLong(start,
                                 end);
    }
    ...
    public <T> ImmutableList<T> iterate(int times,
                                        T seed,
                                        UnaryOperator<T> fn) {
        return LazySeq.iterate(seed,
                               fn,
                               times);
    }

    @Override
    public <T> LazySeq<T> generate(int times,
                                   Supplier<T> fn) {
        return LazySeq.generate(fn,
                                times);
    }
    ...
    public void prependAllTest() {
        assertThat(LazySeq.of(1,
                              2,
                              3)
                          .prependAll(LazySeq.of(4,
                                                 5,
                                                 6)),
                   equalTo(LazySeq.of(4,
                                      5,
                                      6,
                                      1,
                                      2,
    ...
    public void fromIterableSize() {
        assertThat(LazySeq.fromIterable(Seq.of(1,
                                               2,
                                               3))
                          .size(),
                   equalTo(3));
    ...
    public void scanRightLazy() {
        ...
        called = 0;
        LazySeq<Integer> ls = LazySeq.of(1,
                                         2,
                                         3)
                                     .peek(a -> {
                                         called++;
