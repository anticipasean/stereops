...
import static cyclops.container.control.computations.ToyLanguage.bell;
import static cyclops.container.control.computations.ToyLanguage.done;
import static cyclops.container.control.computations.ToyLanguage.output;
...
import cyclops.container.control.computations.ToyLanguage.Output;
import cyclops.container.immutable.tuple.Tuple2;
import org.junit.Test;

public final class UnrestrictedTest {

    static <R> String interleaveProgram(Unrestricted<R> program1,
                                        Unrestricted<R> program2) {

        Tuple2<Either<ToyLanguage<Unrestricted<R>>, R>, Either<ToyLanguage<Unrestricted<R>>, R>> tuple = Unrestricted.product(program1,
                                                                                                                              ToyLanguage.decoder(),
                                                                                                                              program2,
                                                                                                                              ToyLanguage.decoder());
        ...
        String one = a.fold(r -> r.match()
                                  .fold(o -> interleaveOutput(o,
                                                              program2),
                                        UnrestrictedTest::handleBell,
                                        UnrestrictedTest::handleDone),
                            UnrestrictedTest::handleReturn);
        String two = b.fold(r -> r.match()
                                  .fold(o -> interleaveOutput1(o,
                                                               program1),
                                        UnrestrictedTest::handleBell,
                                        UnrestrictedTest::handleDone),
                            UnrestrictedTest::handleReturn);
    ...
    static <R> String interleaveOutput(Output<Unrestricted<R>> output,
                                       Unrestricted<R> program2) {
        System.out.println("Running interA");
        return output.visit((a, next) -> "emitted " + a + "\n" + interleaveProgram(next,
                                                                                   program2));
    }

    static <R> String interleaveOutput1(Output<Unrestricted<R>> output,
                                        Unrestricted<R> program1) {
        System.out.println("Running interB");
        return output.visit((a, next) -> "emitted " + a + "\n" + interleaveProgram(program1,
                                                                                   next));
    }
    ...
    public void interpreter() {
        ...
        assertThat(expected,
                   equalTo(showProgram(output('A').forEach4(unit1 -> bell(),
    ...
    public void interpreterInterleave() {
        ...
        Unrestricted<Void> two = output('C').forEach4(__ -> bell(),
                                                      (__, ___) -> output('D'),
                                                      (__, ___, ____) -> done());

        assertThat(expected,
                   equalTo(interleaveProgram(one,
                                             two)));
    }
