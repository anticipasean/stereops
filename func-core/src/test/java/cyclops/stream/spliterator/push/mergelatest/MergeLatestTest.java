package cyclops.stream.spliterator.push.mergelatest;
...
public class MergeLatestTest {
    ...
    protected <U> ReactiveSeq<U> flux(U... array) {
        ...
        return Spouts.from(Flux.just(array)
                               .subscribeOn(Schedulers.fromExecutor(ex)))
                     .peek(s -> System.out.println("next value " + s));
    ...
    public void mergeLongStreamSimple() {
        ...
        long count = Spouts.mergeLatest(range(size1))
                           .count();
        assertThat(count,
                   equalTo((long) size1));
    }
    ...
    public void mergeStreamSimple3() {
        for (int i = 0; i < 10; i++) {
            ...
            long count = Spouts.mergeLatest(range(size1),
                               ...
                               .count();
            assertThat(count,
                       equalTo((long) (size1 + size2 + size3)));
        }
    }
    ...
    public void mergeLong() {
        ...
        int size2 = 4000;
        int size3 = 1000;
        long size = Spouts.mergeLatest(range(size1),
                                       range(size2),
                                       range(size3))
                          .count();
    ...
    public void mergeSimple() {
        for (int i = 0; i < ITERATIONS; i++) {
            assertThat(Spouts.mergeLatest(nextAsync())
                             .toList(),
                       equalTo(Arrays.asList(1,
                                             2)));
        }
    ...
    public void mergeSimpleList() {
        for (int i = 0; i < ITERATIONS; i++) {
            assertThat(Spouts.mergeLatest(ReactiveSeq.of(nextAsync()))
                             .toList(),
                       equalTo(Arrays.asList(1,
                                             2)));
        }
    ...
    public void mergePAsync2() {
        for (int k = 0; k < ITERATIONS; k++) {
            ...
            System.out.println("****************************NEXT ITERATION " + k);
            System.out.println("****************************NEXT ITERATION " + k + "**************************");
            List<Integer> res = Spouts.mergeLatest(nextAsync(),
                                                   nextAsync(),
                                                   nextAsync())
                                      .toList();
            System.out.println("Result is " + res);
    ...
    public void mergePAsyncSynchronous2() {
        for (int k = 0; k < ITERATIONS; k++) {
            ...
            System.out.println("****************************NEXT ITERATION " + k);
            System.out.println("****************************NEXT ITERATION " + k + "**************************");
            List<Integer> res = ReactiveSeq.fromPublisher(Spouts.mergeLatest(nextAsync(),
                                                                             nextAsync(),
                                                                             nextAsync()))
                                           .toList();
            System.out.println("Result is " + res);
    ...
    public void fluxMerge() {
        ...
        assertThat("Values " + data,
                   ...
                   hasItems(1,
                            ...
                            5,
                            6));
        Assert.assertThat(complete.get(),
    ...
    public void fluxStepverifier() {
        StepVerifier.create(Spouts.mergeLatest(Future.ofResult(1),
                                               Flux.just(2,
                                                         3)))
                    .expectNext(1,
                                2,
    ...
    public void mergePublisherPublisher() {
        ...
        Spouts.mergeLatest(Flux.just(Flux.just(1,
                                               2),
                                     Flux.just(3,
                                               4)))
              .forEach(z -> {
